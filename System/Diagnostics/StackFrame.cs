...
public class StackFrame
{
	...
	[DllImport("QCall", EntryPoint = "StackFrame_GetMethodDescFromNativeIP", ExactSpelling = true)]
	[LibraryImport("QCall", EntryPoint = "StackFrame_GetMethodDescFromNativeIP")]
	private static extern RuntimeMethodHandleInternal GetMethodDescFromNativeIP(nint ip);

	internal static MethodBase GetMethodFromNativeIP(nint ip)
	{
		RuntimeMethodHandleInternal methodDescFromNativeIP = GetMethodDescFromNativeIP(ip);
		if (methodDescFromNativeIP.Value == IntPtr.Zero)
		{
			return null;
		}
		return RuntimeType.GetMethodBase(null, methodDescFromNativeIP);
	}
