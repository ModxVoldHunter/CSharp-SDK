...
internal sealed class ActivityTracker
{
	private sealed class ActivityInfo
	{
		...
		private unsafe void CreateActivityPathGuid(out Guid idRet, out int activityPathGuidOffset)
		{
			fixed (Guid* outPtr = &idRet)
			{
				...
				{
					int id = 1;
					whereToAddId = AddIdToGuid(outPtr, whereToAddId, (uint)id);
				}
	...
	private static string NormalizeActivityName(string providerName, string activityName, int task)
	{
		...
		{
			ReadOnlySpan<char> readOnlySpan = providerName;
			ReadOnlySpan<char> readOnlySpan2 = activityName.AsSpan();
			ref ReadOnlySpan<char> reference = ref readOnlySpan2;
			int length = "Start".Length;
			return string.Concat(readOnlySpan, reference.Slice(0, reference.Length - length));
		}
		if (activityName.EndsWith("Stop", StringComparison.Ordinal))
		{
			ReadOnlySpan<char> readOnlySpan3 = providerName;
			ReadOnlySpan<char> readOnlySpan2 = activityName.AsSpan();
			ref ReadOnlySpan<char> reference = ref readOnlySpan2;
			int length = "Stop".Length;
			return string.Concat(readOnlySpan3, reference.Slice(0, reference.Length - length));
		}
		if (task != 0)
		{
