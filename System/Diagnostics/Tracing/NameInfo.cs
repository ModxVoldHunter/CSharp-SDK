...
internal sealed class NameInfo : ConcurrentSetItem<KeyValuePair<string, EventTags>, NameInfo>
{
	...
	public unsafe nint GetOrCreateEventHandle(EventProvider provider, TraceLoggingEventHandleTable eventHandleTable, EventDescriptor descriptor, TraceLoggingEventTypes eventTypes)
	{
		nint num;
		if ((num = eventHandleTable[descriptor.EventId]) == IntPtr.Zero)
		{
			lock (eventHandleTable)
			{
				if ((num = eventHandleTable[descriptor.EventId]) == IntPtr.Zero)
				{
					...
					{
						num = provider._eventProvider.DefineEventHandle((uint)descriptor.EventId, name, descriptor.Keywords, descriptor.Version, descriptor.Level, pMetadata, metadataLength);
					}
					eventHandleTable.SetEventHandle(descriptor.EventId, num);
				}
			}
		}
		return num;
