...
internal sealed class ManifestBuilder
{
		...
		: this(resources, flags)
	{
		...
		if (dllName != null)
		{
			...
			handler.AppendLiteral(" resourceFileName=\"");
			handler.AppendFormatted(dllName);
			handler.AppendLiteral("\" messageFileName=\"");
			handler.AppendFormatted(dllName);
			handler.AppendLiteral("\"");
	...
	public void AddKeyword(string name, ulong value)
	{
		if ((value & (value - 1)) != 0L)
		{
			ManifestError(SR.Format(SR.EventSource_KeywordNeedPowerOfTwo, $"0x{value:x}", name), runtimeCritical: true);
		}
		if ((flags & EventManifestOptions.Strict) != 0)
		{
			...
			{
				ManifestError(SR.Format(SR.EventSource_KeywordCollision, name, value2, $"0x{value:x}"));
			}
		}
		if (keywordTab == null)
		{
	...
	public void StartEvent(string eventName, EventAttribute eventAttribute)
	{
		...
		{
			string opcodeName = GetOpcodeName(eventAttribute.Opcode, eventName);
			events?.Append(" opcode=\"").Append(opcodeName).Append('"');
		}
		if (eventAttribute.Task != 0)
		{
			string taskName = GetTaskName(eventAttribute.Task, eventName);
			events?.Append(" task=\"").Append(taskName).Append('"');
		}
		if (eventAttribute.Channel != 0)
		{
			string channelName = GetChannelName(eventAttribute.Channel, eventName, eventAttribute.Message);
			events?.Append(" channel=\"").Append(channelName).Append('"');
		}
	}
	...
	{
		string text = CreateManifestString();
		if (!(text != ""))
		{
	...
	private string CreateManifestString()
	{
		...
		{
			sb?.AppendLine(" <maps>");
			foreach (Type value4 in mapsTab.Values)
			{
				bool flag2 = EventSource.IsCustomAttributeDefinedHelper(value4, typeof(FlagsAttribute), flags);
				string value2 = (flag2 ? "bitMap" : "valueMap");
				sb?.Append("  <").Append(value2).Append(" name=\"")
					.Append(value4.Name)
					.AppendLine("\">");
				FieldInfo[] array = GetEnumFields(value4);
				bool flag3 = false;
				...
				foreach (FieldInfo fieldInfo in array2)
				{
					...
					{
						ulong value3 = ((!(rawConstantValue is ulong)) ? ((ulong)Convert.ToInt64(rawConstantValue)) : ((ulong)rawConstantValue));
						if (!flag2 || BitOperations.IsPow2(value3))
						{
							value3.TryFormat(destination, out var charsWritten, "x");
							Span<char> span = destination.Slice(0, charsWritten);
							sb?.Append("   <map value=\"0x").Append(span).Append('"');
							WriteMessageAttrib(sb, "map", value4.Name + "." + fieldInfo.Name, fieldInfo.Name);
							sb?.AppendLine("/>");
							flag3 = true;
						}
					}
				...
				if (!flag3)
				{
					...
					WriteMessageAttrib(sb, "map", value4.Name + ".None", "None");
					sb?.AppendLine("/>");
				}
		...
		StringBuilder stringBuilder = templates;
		...
		Array.Sort(array3, (IComparer<string>?)StringComparer.Ordinal);
