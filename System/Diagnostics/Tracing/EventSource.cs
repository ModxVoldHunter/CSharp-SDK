...
public class EventSource : IDisposable
{
	...
	private sealed class OverrideEventProvider : EventProvider
	{
		...
		internal override void OnControllerCommand(ControllerCommand command, IDictionary<string, string> arguments, int perEventSourceSessionId)
		{
			...
			m_eventSource.SendCommand(listener, m_eventProviderType, perEventSourceSessionId, (EventCommand)command, IsEnabled(), base.Level, base.MatchAnyKeyword, arguments);
		}
	}
	...
	internal struct EventMetadata
	{
		...
		public TraceLoggingEventTypes TraceLoggingEventTypes
		{
			...
			[RequiresUnreferencedCode("EventSource will serialize the whole object graph. Trimmer will not safely handle this case because properties may be trimmed.")]
			get
			{
	...
	protected unsafe void WriteEvent(int eventId, string? arg1, string? arg2)
	{
		...
		fixed (char* dataPointer2 = arg1)
		{
			...
			if (arg2 == null)
			{
				...
				EventData* ptr = stackalloc EventData[2];
				ptr->DataPointer = (nint)dataPointer2;
				ptr->Size = (arg1.Length + 1) * 2;
				ptr->Reserved = 0;
		...
		fixed (char* dataPointer3 = arg1)
		{
			...
			if (arg2 == null)
			{
				...
				fixed (char* ptr = arg3)
				{
					...
					ptr2->DataPointer = (nint)dataPointer3;
					ptr2->Size = (arg1.Length + 1) * 2;
					ptr2->Reserved = 0;
					ptr2[1].DataPointer = (nint)dataPointer;
					ptr2[1].Size = (arg2.Length + 1) * 2;
					ptr2[1].Reserved = 0;
					ptr2[2].DataPointer = (nint)dataPointer2;
	...
	protected unsafe void WriteEventWithRelatedActivityIdCore(int eventId, Guid* relatedActivityId, int eventDataCount, EventData* data)
	{
		...
		try
		{
			...
			if (!SelfDescribingEvents)
			{
				if (reference.EnabledForETW && !m_etwProvider.WriteEvent(ref reference.Descriptor, reference.EventHandle, ptr, relatedActivityId, eventDataCount, (nint)data))
				{
					ThrowEventSourceException(reference.Name);
				}
				if (reference.EnabledForEventPipe && !m_eventPipeProvider.WriteEvent(ref reference.Descriptor, reference.EventHandle, ptr, relatedActivityId, eventDataCount, (nint)data))
				{
					ThrowEventSourceException(reference.Name);
				}
			}
	...
	private unsafe void WriteEventString(string msgString)
	{
		...
		[UnconditionalSuppressMessage("ReflectionAnalysis", "IL2119", Justification = "DAM on EventSource references this compiler-generated local function which calls a constructor that requires unreferenced code. EventSource will not access this local function.")]
	...
	private void WriteStringToAllListeners(string eventName, string msg)
	{
		...
		eventWrittenEventArgs.PayloadNames = s_errorPayloadNames ?? (s_errorPayloadNames = new ReadOnlyCollection<string>(new string[1] { "message" }));
	...
	internal void SendCommand(EventListener listener, EventProviderType eventProviderType, int perEventSourceSessionId, EventCommand command, bool enable, EventLevel level, EventKeywords matchAnyKeyword, IDictionary<string, string> commandArguments)
	{
		...
		EventCommandEventArgs eventCommandEventArgs = new EventCommandEventArgs(command, commandArguments, this, listener, eventProviderType, perEventSourceSessionId, enable, level, matchAnyKeyword);
		lock (EventListener.EventListenersLock)
		{
		...
		fixed (byte* ptr2 = rawManifest)
		{
			...
			while (true)
			{
				...
				while (num > 0)
				{
					...
					if (m_etwProvider != null && !m_etwProvider.WriteEvent(ref eventDescriptor, IntPtr.Zero, null, null, 2, (nint)ptr))
					{
	...
	private static byte[] CreateManifestAndDescriptors([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type eventSourceType, string eventSourceDllName, EventSource source, EventManifestOptions flags = EventManifestOptions.None)
	{
		...
		try
		{
			...
			string[] array2 = new string[3] { "Keywords", "Tasks", "Opcodes" };
						...
						if (flag2)
						{
							if (eventAttribute.Opcode == EventOpcode.Start)
							{
								if (name.EndsWith("Start", StringComparison.Ordinal))
								{
									...
									string name2 = text2.Substring(0, text2.Length - length);
							...
							else if (eventAttribute.Opcode == EventOpcode.Stop)
							{
								...
								if (eventData != null && num2 < eventData.Length)
								{
									...
									if (eventMetadata.Descriptor.Opcode == 1 && eventMetadata.Name.EndsWith("Start", StringComparison.Ordinal) && name.EndsWith("Stop", StringComparison.Ordinal))
									{
										ReadOnlySpan<char> readOnlySpan = eventMetadata.Name.AsSpan();
										ref ReadOnlySpan<char> reference = ref readOnlySpan;
										int length = "Start".Length;
										ReadOnlySpan<char> span = reference.Slice(0, reference.Length - length);
										ReadOnlySpan<char> readOnlySpan2 = name.AsSpan();
										reference = ref readOnlySpan2;
										length = "Stop".Length;
										if (span.SequenceEqual(reference.Slice(0, reference.Length - length)))
										{
			...
			fixed (byte* pointer = providerMetadata)
			{
				fixed (byte* pointer2 = nameInfo.nameMetadata)
				{
					fixed (byte* pointer3 = eventTypes.typeMetadata)
					{
						...
						WriteEventRaw(eventName, ref descriptor, orCreateEventHandle, activityID, childActivityID, num2, (nint)ptr);
					}
				}
			}
		}
					...
					fixed (byte* pointer2 = nameInfo.nameMetadata)
					{
						fixed (byte* pointer3 = eventTypes.typeMetadata)
						{
							...
							try
							{
								...
								WriteEventRaw(eventName, ref descriptor, orCreateEventHandle, pActivityId, pRelatedActivityId, (int)(DataCollector.ThreadInstance.Finish() - ptr), (nint)ptr);
