	...
	internal bool IsLocalDomain(string host)
	{
		...
		default:
		{
			...
			if (destination.Length == 4)
			{
				...
				if (source[range.Start..range.End].SequenceEqual("127".AsSpan()))
				{
					int i;
					for (i = 1; i < destination.Length; i++)
					{
						...
						{
						case 3:
							if (!char.IsAsciiDigit(readOnlySpan[2]))
							{
								break;
							}
							goto case 2;
						case 2:
							if (!char.IsAsciiDigit(readOnlySpan[1]))
							{
								break;
							}
							goto case 1;
						case 1:
							if (char.IsAsciiDigit(readOnlySpan[0]))
							{
								continue;
							}
							break;
