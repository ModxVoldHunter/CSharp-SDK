...
public class HttpMethod : IEquatable<HttpMethod>
{
	...
	internal static HttpMethod Normalize(HttpMethod method)
	{
		...
		if (!http3Index.HasValue)
		{
			HttpMethod knownMethod = GetKnownMethod(method._method);
	...
	internal static HttpMethod GetKnownMethod(ReadOnlySpan<char> method)
	{
		if (method.Length >= 3)
		{
			...
			if ((object)httpMethod != null && MemoryExtensions.Equals(method, httpMethod._method, StringComparison.OrdinalIgnoreCase))
			{
				return httpMethod;
			}
		}
