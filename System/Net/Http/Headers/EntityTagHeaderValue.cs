...
public class EntityTagHeaderValue : ICloneable
{
	...
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(tag, "tag");
		if (HttpRuleParser.GetQuotedStringLength(tag, 0, out var length) != 0 || length != tag.Length)
		{
	...
	internal static int GetEntityTagLength(string input, int startIndex, out EntityTagHeaderValue parsedValue)
	{
		...
		else
		{
			...
			int startIndex2 = num;
			if (num == input.Length || HttpRuleParser.GetQuotedStringLength(input, num, out var length) != 0)
			{
				return 0;
			}
