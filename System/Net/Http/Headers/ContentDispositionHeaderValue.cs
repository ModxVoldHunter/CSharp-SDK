...
public class ContentDispositionHeaderValue : ICloneable
{
	...
	private static void CheckDispositionTypeFormat(string dispositionType, [CallerArgumentExpression("dispositionType")] string parameterName = null)
	{
	...
	private static bool TryDecodeMime(string input, [NotNullWhen(true)] out string output)
	{
		...
		if (source.Split(destination, '?') == 5)
		{
			...
			if (source[range.Start..range.End].SequenceEqual("\"=".AsSpan()))
			{
				...
				{
					range = destination[2];
					if (MemoryExtensions.Equals(source[range.Start..range.End], "b", StringComparison.OrdinalIgnoreCase))
					{
						try
						{
							range = destination[1];
