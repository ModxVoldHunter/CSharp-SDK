...
internal static class SocksHelper
{
	public static async ValueTask EstablishSocksTunnelAsync(Stream stream, string host, int port, Uri proxyUri, ICredentials proxyCredentials, bool async, CancellationToken cancellationToken)
	{
		...
		}, stream))
		{
			...
			try
			{
				...
				{
					await EstablishSocks5TunnelAsync(stream, host, port, credentials, async).ConfigureAwait(continueOnCapturedContext: false);
				}
				else if (string.Equals(proxyUri.Scheme, "socks4a", StringComparison.OrdinalIgnoreCase))
				{
					await EstablishSocks4TunnelAsync(stream, isVersion4a: true, host, port, credentials, async, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				}
				else if (string.Equals(proxyUri.Scheme, "socks4", StringComparison.OrdinalIgnoreCase))
				{
					await EstablishSocks4TunnelAsync(stream, isVersion4a: false, host, port, credentials, async, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				}
			}
			catch
			{
	...
	{
		int num = ((!async) ? stream.ReadAtLeast(buffer.Span, buffer.Length, throwOnEndOfStream: false) : (await stream.ReadAtLeastAsync(buffer, buffer.Length, throwOnEndOfStream: false).ConfigureAwait(continueOnCapturedContext: false)));
		int num2 = num;
