...
internal sealed class WebSocketHttpListenerDuplexStream : Stream, WebSocketBase.IWebSocketStream
{
	internal sealed class HttpListenerAsyncEventArgs : EventArgs, IDisposable
	{
		...
		private unsafe void UpdateDataChunk(int index, byte[] buffer, int offset, int count)
		{
			...
			{
				_dataChunks[index].pBuffer = (byte*)_webSocket.InternalBuffer.ConvertPinnedSendPayloadToNative(buffer, offset, count);
			}
	...
	private async Task<int> ReadAsyncCore(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			if (!_inOpaqueMode)
			{
				result = await _inputStream.ReadAsync(buffer.AsMemory(offset, count), cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			}
			else
			{
				...
				{
					result = await _readTaskCompletionSource.Task.ConfigureAwait(continueOnCapturedContext: false);
				}
			}
		}
		catch (Exception arg)
	...
	private unsafe bool ReadAsyncFast(HttpListenerAsyncEventArgs eventArgs)
	{
		...
		try
		{
			...
			{
				num = _inputStream.GetChunks(eventArgs.Buffer, eventArgs.Offset, eventArgs.Count);
				if (_inputStream.BufferedDataChunksAvailable && num == eventArgs.Count)
				{
			...
			uint num2 = global::Interop.HttpApi.HttpReceiveRequestEntityBody(_inputStream.InternalHttpContext.RequestQueueHandle, _inputStream.InternalHttpContext.RequestId, flags, (void*)_webSocket.InternalBuffer.ToIntPtr(eventArgs.Offset), (uint)eventArgs.Count, out bytesReturned, eventArgs.NativeOverlapped);
			if (num2 != 0 && num2 != 997 && num2 != 38)
			{
				throw new HttpListenerException((int)num2);
			}
			if (num2 == 0 && HttpListener.SkipIOCPCallbackOnSuccess)
			{
	...
	private async Task MultipleWriteAsyncCore(IList<ArraySegment<byte>> sendBuffers, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			{
				await _writeTaskCompletionSource.Task.ConfigureAwait(continueOnCapturedContext: false);
			}
		}
		catch (Exception arg)
		{
	...
	private async Task WriteAsyncCore(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			if (!_inOpaqueMode)
			{
				await _outputStream.WriteAsync(buffer.AsMemory(offset, count), cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				return;
			}
			_writeTaskCompletionSource = new TaskCompletionSource();
