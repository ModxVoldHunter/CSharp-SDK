...
internal sealed class ManagedWebSocket : WebSocket
{
	...
	[StructLayout(LayoutKind.Auto)]
	[CompilerGenerated]
	private struct _003CEnsureBufferContainsAsync_003Ed__75 : IAsyncStateMachine
	{
		...
		private void MoveNext()
		{
			...
			try
			{
				...
				if (num2 < _003CbytesToRead_003E5__2)
				{
	...
	[StructLayout(LayoutKind.Auto)]
	[CompilerGenerated]
	private struct _003CReceiveAsyncPrivate_003Ed__64<TResult> : IAsyncStateMachine
	{
		public int _003C_003E1__state;
		...
		private void MoveNext()
		{
			...
			try
			{
				...
				try
				{
					...
					awaiter.GetResult();
					ObjectDisposedException.ThrowIf(managedWebSocket._disposed, typeof(WebSocket));
					goto IL_00d6;
					IL_00d6:
					try
					{
						ConfiguredValueTaskAwaitable<int>.ConfiguredValueTaskAwaiter awaiter9;
							...
							if (_003Cheader_003E5__3.Processed)
							{
								if (managedWebSocket._receiveBufferCount < (managedWebSocket._isServer ? 14 : 10))
								{
									if (managedWebSocket._receiveBufferCount < 2)
									{
										...
										{
											awaiter9 = managedWebSocket._stream.ReadAsync(Memory<byte>.Empty, cancellationToken).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter();
											if (!awaiter9.IsCompleted)
											{
												num = (_003C_003E1__state = 1);
												_003C_003Eu__2 = awaiter9;
												_003C_003Et__builder.AwaitUnsafeOnCompleted(ref awaiter9, ref this);
												return;
											}
											goto IL_01c1;
										}
						...
						case 8:
							...
							num4 = result;
							if (num4 < _003CbytesToRead_003E5__5)
							{
								managedWebSocket.ThrowEOFUnexpected();
							}
							_003CtotalBytesReceived_003E5__4 += num4;
							goto IL_0791;
							IL_0392:
							...
							IL_023c:
							span = managedWebSocket._receiveBuffer.Span;
							num5 = span[managedWebSocket._receiveBufferOffset + 1] & 0x7F;
							if (managedWebSocket._isServer || num5 > 125)
							{
								int minimumRequiredBytes = 2 + (managedWebSocket._isServer ? 4 : 0) + ((num5 > 125) ? ((num5 == 126) ? 2 : 8) : 0);
	...
	private ValueTask SendFrameLockAcquiredNonCancelableAsync(MessageOpcode opcode, bool endOfMessage, bool disableCompression, ReadOnlyMemory<byte> payloadBuffer)
	{
		...
		{
			return new ValueTask(Task.FromException((ex is OperationCanceledException) ? ex : ((_state == WebSocketState.Aborted) ? ((SystemException)CreateOperationCanceledException(ex)) : ((SystemException)new WebSocketException(WebSocketError.ConnectionClosedPrematurely, ex)))));
		}
		finally
		{
			if (flag)
	...
	[AsyncStateMachine(typeof(_003CReceiveAsyncPrivate_003Ed__64<>))]
	...
	{
		Unsafe.SkipInit(out _003CReceiveAsyncPrivate_003Ed__64<TResult> stateMachine);
	...
	[AsyncStateMachine(typeof(_003CEnsureBufferContainsAsync_003Ed__75))]
	[AsyncMethodBuilder(typeof(PoolingAsyncValueTaskMethodBuilder))]
	private ValueTask EnsureBufferContainsAsync(int minimumRequiredBytes, CancellationToken cancellationToken)
	{
		Unsafe.SkipInit(out _003CEnsureBufferContainsAsync_003Ed__75 stateMachine);
	...
	private unsafe static int ApplyMask(Span<byte> toMask, int mask, int maskIndex)
	{
		fixed (byte* ptr = &MemoryMarshal.GetReference(toMask))
		{
			...
			if (ptr3 - ptr2 >= 4)
			{
				...
				if (Vector.IsHardwareAccelerated && ptr3 - ptr2 >= Vector<byte>.Count)
				{
