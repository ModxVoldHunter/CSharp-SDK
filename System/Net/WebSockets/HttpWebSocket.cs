...
internal static class HttpWebSocket
{
	...
	internal static void ValidateOptions(string subProtocol, int receiveBufferSize, int sendBufferSize, TimeSpan keepAliveInterval)
	{
		...
		ArgumentOutOfRangeException.ThrowIfLessThan(receiveBufferSize, 256, "receiveBufferSize");
		ArgumentOutOfRangeException.ThrowIfLessThan(sendBufferSize, 16, "sendBufferSize");
		ArgumentOutOfRangeException.ThrowIfGreaterThan(receiveBufferSize, 65536, "receiveBufferSize");
		ArgumentOutOfRangeException.ThrowIfGreaterThan(sendBufferSize, 65536, "sendBufferSize");
		ArgumentOutOfRangeException.ThrowIfLessThan(keepAliveInterval, Timeout.InfiniteTimeSpan, "keepAliveInterval");
	}
	...
	private static async Task<HttpListenerWebSocketContext> AcceptWebSocketAsyncCore(HttpListenerContext context, string subProtocol, int receiveBufferSize, TimeSpan keepAliveInterval, ArraySegment<byte> internalBuffer)
	{
		try
		{
			...
			await response.OutputStream.FlushAsync().ConfigureAwait(continueOnCapturedContext: false);
