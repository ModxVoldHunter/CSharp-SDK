...
public class FileWebRequest : WebRequest, ISerializable
{
	...
	public override IAsyncResult BeginGetRequestStream(AsyncCallback? callback, object? state)
	{
		...
		Task<Stream> task = Task.Factory.StartNew((Func<object?, Stream>)((object s) => ((FileWebRequest)s).CreateWriteStream()), (object?)this, CancellationToken.None, TaskCreationOptions.DenyChildAttach, TaskScheduler.Default);
	...
	{
		Stream result = TaskToAsyncResult.End<Stream>(asyncResult);
		_writePending = false;
		return result;
	}
	...
	{
		WebResponse result = TaskToAsyncResult.End<WebResponse>(asyncResult);
		_readPending = false;
		return result;
	}
