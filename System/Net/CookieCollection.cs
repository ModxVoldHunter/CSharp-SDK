...
public class CookieCollection : ICollection<Cookie>, IEnumerable<Cookie>, IEnumerable, IReadOnlyCollection<Cookie>, ICollection
{
	...
	public Cookie this[int index]
	{
		get
		{
			...
			ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(index, m_list.Count, "index");
	...
	internal int InternalAdd(Cookie cookie, bool isStrict)
	{
		...
		if (isStrict)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (CookieComparer.Equals(cookie, cookie2))
				{
					result = 0;
	...
	internal int IndexOf(Cookie cookie)
	{
		...
		foreach (Cookie item in m_list)
		{
			if (CookieComparer.Equals(cookie, item))
			{
				return num;
			}
			num++;
