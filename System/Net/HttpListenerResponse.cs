...
public sealed class HttpListenerResponse : IDisposable
{
	...
	public string StatusDescription
	{
		get
		{
			string text = _statusDescription;
			if (text == null)
			{
				string obj = HttpStatusDescription.Get(StatusCode) ?? string.Empty;
				string text2 = obj;
				_statusDescription = obj;
				text = text2;
			}
			return text;
		}
	...
	{
		bool flag = (((uint)(responseCode - 100) <= 1u || (uint)(responseCode - 204) <= 1u || responseCode == 304) ? true : false);
		return !flag;
	}
	...
	{
		CheckDisposed();
		ArgumentNullException.ThrowIfNull(responseEntity, "responseEntity");
		if (System.Net.NetEventSource.Log.IsEnabled())
		{
	...
	internal unsafe uint SendHeaders(global::Interop.HttpApi.HTTP_DATA_CHUNK* pDataChunk, HttpResponseStreamAsyncResult asyncResult, global::Interop.HttpApi.HTTP_FLAGS flags, bool isWebSocketHandshake)
	{
		if (System.Net.NetEventSource.Log.IsEnabled())
		{
			System.Net.NetEventSource.Info(this, $"pDataChunk: {(nint)pDataChunk}, asyncResult: {asyncResult}", "SendHeaders");
		}
		if (StatusCode == 401)
		{
		...
		try
		{
			...
			if (StatusDescription.Length > 0)
			{
				...
				fixed (byte* pReason = &array[0])
				{
					...
					{
						num = global::Interop.HttpApi.HttpSendHttpResponse(HttpListenerContext.RequestQueueHandle, HttpListenerRequest.RequestId, (uint)flags, pHttpResponse, null, &numBytes, null, 0u, (asyncResult == null) ? null : asyncResult._pOverlapped, null);
			...
			else
			{
				...
				{
					num = global::Interop.HttpApi.HttpSendHttpResponse(HttpListenerContext.RequestQueueHandle, HttpListenerRequest.RequestId, (uint)flags, pHttpResponse2, null, &numBytes, null, 0u, (asyncResult == null) ? null : asyncResult._pOverlapped, null);
					if (asyncResult != null && num == 0 && HttpListener.SkipIOCPCallbackOnSuccess)
					{
						asyncResult.IOCompleted(num, numBytes);
					}
				...
				for (int j = 0; j < Headers.Count; j++)
				{
					...
					if (num2 == -1)
					{
						if (array == null)
						{
							...
							list.Add(item);
							headers.pUnknownHeaders = (global::Interop.HttpApi.HTTP_UNKNOWN_HEADER*)item.AddrOfPinnedObject();
						}
						...
						for (int k = 0; k < values2.Length; k++)
						{
							...
							list.Add(item);
							array[headers.UnknownHeaderCount].pName = (sbyte*)item.AddrOfPinnedObject();
							text = values2[k];
							...
							list.Add(item);
							array[headers.UnknownHeaderCount].pRawValue = (sbyte*)item.AddrOfPinnedObject();
					...
					if (text != null)
					{
						...
						GCHandle item = GCHandle.Alloc(array2, GCHandleType.Pinned);
						list.Add(item);
						ptr[num2].pRawValue = (sbyte*)item.AddrOfPinnedObject();
						if (System.Net.NetEventSource.Log.IsEnabled())
						{
							System.Net.NetEventSource.Info(this, $"pRawValue:{(nint)ptr[num2].pRawValue} RawValueLength:{ptr[num2].RawValueLength} lookup: {num2}", "SerializeHeaders");
						}
					}
				}
				return list;
