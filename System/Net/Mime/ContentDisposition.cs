...
public class ContentDisposition
{
	...
	{
		ArgumentNullException.ThrowIfNull(disposition, "disposition");
		_isChanged = true;
	...
	private void ParseValue()
	{
		...
		try
		{
			...
			while (MailBnfHelper.SkipCFWS(_disposition, ref offset))
			{
				...
				{
					string text = MailBnfHelper.ReadParameterAttribute(_disposition, ref offset);
					if (_disposition[offset++] != '=')
					{
						throw new FormatException(System.SR.MailHeaderFieldMalformedHeader);
					}
					...
					string value = ((_disposition[offset] == '"') ? MailBnfHelper.ReadQuotedString(_disposition, ref offset, null) : MailBnfHelper.ReadToken(_disposition, ref offset));
