...
public static class WebUtility
{
	...
	public static string? HtmlDecode(string? value)
	{
		...
		ValueStringBuilder output = valueStringBuilder;
		output.Append(span.Slice(0, num));
		HtmlDecode(span.Slice(num), ref output);
		return output.ToString();
	}
	...
	public static void HtmlDecode(string? value, TextWriter output)
	{
		...
		ValueStringBuilder output2 = valueStringBuilder;
		output2.Append(span.Slice(0, num));
		HtmlDecode(span.Slice(num), ref output2);
		output.Write(output2.AsSpan());
		output2.Dispose();
	}
	...
	{
		if (!char.IsAsciiLetter(ch) && ((uint)(ch - 32) > 25u || ((1 << ch - 32) & 0x3FF6702) == 0))
		{
			return ch == '_';
		}
		return true;
