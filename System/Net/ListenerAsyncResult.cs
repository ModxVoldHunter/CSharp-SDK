...
internal sealed class ListenerAsyncResult : System.Net.LazyAsyncResult
{
	...
	internal unsafe uint QueueBeginGetContext()
	{
		...
		while (true)
		{
			if (System.Net.NetEventSource.Log.IsEnabled())
			{
				System.Net.NetEventSource.Info(this, $"Calling Interop.HttpApi.HttpReceiveHttpRequest RequestId: {_requestContext.RequestBlob->RequestId} Buffer: 0x{(nint)_requestContext.RequestBlob:x} Size: {_requestContext.Size}", "QueueBeginGetContext");
			}
			uint num2 = 0u;
