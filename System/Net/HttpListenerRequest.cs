...
public sealed class HttpListenerRequest
{
	...
	internal nint OriginalBlobAddress
	{
		get
		{
			CheckDisposed();
	...
	public Stream InputStream => _requestStream ?? (_requestStream = (HasEntityBody ? new HttpRequestStream(HttpListenerContext) : Stream.Null));
	...
	internal unsafe HttpListenerRequest(HttpListenerContext httpContext, RequestContextBase memoryBlob)
	{
		if (System.Net.NetEventSource.Log.IsEnabled())
		{
			System.Net.NetEventSource.Info(this, $"httpContext:${httpContext} memoryBlob {(nint)memoryBlob.RequestBlob}", ".ctor");
		...
		{
			_rawUrl = Marshal.PtrToStringAnsi((nint)memoryBlob.RequestBlob->pRawUrl, memoryBlob.RequestBlob->RawUrlLength);
		}
		...
		{
			_cookedUrlHost = Marshal.PtrToStringUni((nint)cookedUrl.pHost, cookedUrl.HostLength / 2);
		}
		if (cookedUrl.pAbsPath != null && cookedUrl.AbsPathLength > 0)
		{
			_cookedUrlPath = Marshal.PtrToStringUni((nint)cookedUrl.pAbsPath, cookedUrl.AbsPathLength / 2);
		}
		if (cookedUrl.pQueryString != null && cookedUrl.QueryStringLength > 0)
		{
			_cookedUrlQuery = Marshal.PtrToStringUni((nint)cookedUrl.pQueryString, cookedUrl.QueryStringLength / 2);
		}
		...
		while (true)
		{
			...
			fixed (byte* ptr = &array[0])
			{
				...
				case 0u:
					...
					if (System.Net.NetEventSource.Log.IsEnabled())
					{
						System.Net.NetEventSource.Info(this, $"pClientCertInfo:{(nint)ptr2} pClientCertInfo->CertFlags: {ptr2->CertFlags} pClientCertInfo->CertEncodedSize: {ptr2->CertEncodedSize} pClientCertInfo->pCertEncoded: {(nint)ptr2->pCertEncoded} pClientCertInfo->Token: {(nint)ptr2->Token} pClientCertInfo->CertDeniedByMapper: {ptr2->CertDeniedByMapper}", "GetClientCertificateCore");
					}
					if (ptr2->pCertEncoded != null)
					{
						try
						{
							...
							Marshal.Copy((nint)ptr2->pCertEncoded, array2, 0, array2.Length);
