...
internal sealed class HeaderInfoTable
{
	...
	private static string[] ParseValueHelper(string value, bool isSetCookie)
	{
		...
		for (int i = 0; i < value.Length; i++)
		{
			...
			{
				ReadOnlySpan<char> singleValue = value.AsSpan(num, num2).Trim();
		...
		{
			list.Add(value.AsSpan(num, num2).Trim().ToString());
		}
		return list.ToArray();
	}
	...
	{
		int num = singleValue.LastIndexOf(';');
		if (num >= 0)
		{
			ReadOnlySpan<char> span = singleValue.Slice(num + 1);
			if (!span.Contains(','))
			{
				...
				return MemoryExtensions.Equals(span.Slice(0, num2).Trim(), "Expires", StringComparison.OrdinalIgnoreCase);
			}
		}
		return false;
	}
