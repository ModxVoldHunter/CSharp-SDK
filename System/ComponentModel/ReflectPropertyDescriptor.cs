...
internal sealed class ReflectPropertyDescriptor : PropertyDescriptor
{
	...
	private static readonly int s_bitDefaultValueQueried = 1;
	...
	protected override void FillAttributes(IList attributes)
	{
		...
		if (num > 0)
		{
			...
			while (type != null && type != typeof(object))
			{
				...
				MemberInfo memberInfo = ((!IsExtender) ? ((MemberInfo)type.GetProperty(Name, bindingAttr, null, PropertyType, Type.EmptyTypes, Array.Empty<ParameterModifier>())) : ((MemberInfo)type.GetMethod("Get" + Name, bindingAttr, null, new Type[1] { _receiverType }, null)));
				if (memberInfo != null)
				{
