...
public class MaskedTextProvider : ICloneable
{
	...
	private sealed class CharDescriptor
	{
		...
		public override string ToString()
		{
			...
			DefaultInterpolatedStringHandler handler = new DefaultInterpolatedStringHandler(56, 4, invariantCulture, initialBuffer);
			handler.AppendLiteral("MaskPosition[");
			handler.AppendFormatted(MaskPosition);
			handler.AppendLiteral("] <CaseConversion.");
			handler.AppendFormatted(CaseConversion);
			handler.AppendLiteral("><CharType.");
			handler.AppendFormatted(CharType);
			handler.AppendLiteral("><IsAssigned: ");
			handler.AppendFormatted(IsAssigned);
	...
	{
		ArgumentException.ThrowIfNullOrEmpty(mask, "mask");
	...
	public object Clone()
	{
		...
		else
		{
			...
			maskedTextProvider = Activator.CreateInstance(GetType(), args) as MaskedTextProvider;
		}
	...
	private bool TestString(string input, int position, out int testPosition, out MaskedTextResultHint resultHint)
	{
		...
		foreach (char input2 in input)
		{
			...
			if (!TestChar(input2, testPosition, out var resultHint2))
			{
				resultHint = resultHint2;
				return false;
			}
	...
	public string ToString(bool ignorePasswordChar, bool includePrompt, bool includeLiterals, int startPosition, int length)
	{
		...
		if (!includePrompt)
		{
			int num3 = (includeLiterals ? FindNonEditPositionInRange(startPosition, num2, direction: false) : (-1));
			int num4 = FindAssignedEditPositionInRange((num3 == -1) ? startPosition : num3, num2, direction: false);
			num2 = ((num4 != -1) ? num4 : num3);
			if (num2 == -1)
			{
				return string.Empty;
			}
