...
internal sealed class AssociatedMetadataTypeTypeDescriptor : CustomTypeDescriptor
{
	private static class TypeDescriptorCache
	{
		...
		{
			if (TryGetAssociatedMetadataTypeFromCache(type, out var associatedMetadataType2))
			{
				return associatedMetadataType2;
			}
			...
			{
				associatedMetadataType2 = metadataTypeAttribute.MetadataClassType;
			}
			s_metadataTypeCache.TryAdd(type, associatedMetadataType2);
			return associatedMetadataType2;
			[UnconditionalSuppressMessage("ReflectionAnalysis", "IL2067:ParameterDoesntMeetParameterRequirements", Justification = "The cache is a dictionary which is hard to annotate. All values in the cachehave annotation All (since we only ever add attribute.MetadataClassType which has All).But the call to TryGetValue doesn't carry the annotation so this warns when tryingto assign to the out parameter.")]
