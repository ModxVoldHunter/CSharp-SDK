...
internal static class ILGen
{
	...
	private static void EmitNumericConversion(this ILGenerator il, Type typeFrom, Type typeTo, bool isChecked)
	{
		...
		case TypeCode.SByte:
			opcode = ((!isChecked) ? OpCodes.Conv_I1 : (flag ? OpCodes.Conv_Ovf_I1_Un : OpCodes.Conv_Ovf_I1));
			break;
		case TypeCode.Byte:
			opcode = ((!isChecked) ? OpCodes.Conv_U1 : (flag ? OpCodes.Conv_Ovf_U1_Un : OpCodes.Conv_Ovf_U1));
			break;
		case TypeCode.Int16:
			if ((uint)(typeCode2 - 5) <= 1u)
			{
				return;
			}
