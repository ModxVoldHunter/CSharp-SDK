...
public abstract class EastAsianLunisolarCalendar : Calendar
{
	...
	public override int TwoDigitYearMax
	{
		get
		{
			...
			{
				_twoDigitYearMax = Calendar.GetSystemTwoDigitYearSetting(BaseCalendarID, GetYear(new DateTime(2049, 1, 1)));
			}
	...
	{
		int num = (GregorianIsLeapYear(solarYear) ? DaysToMonth366[solarMonth - 1] : DaysToMonth365[solarMonth - 1]);
		num += solarDate;
		int num2 = num;
		lunarYear = solarYear;
		int yearInfo;
		...
		num2 -= DaysToMonth365[yearInfo - 1];
		num2 -= yearInfo2 - 1;
		int num3 = 32768;
	...
	private bool LunarToGregorian(int lunarYear, int lunarMonth, int lunarDate, out int solarYear, out int solarMonth, out int solarDay)
	{
		...
		ReadOnlySpan<int> readOnlySpan = (flag ? DaysToMonth366 : DaysToMonth365);
		solarDay = yearInfo2;
		if (yearInfo > 1)
		{
			solarDay += readOnlySpan[yearInfo - 1];
		}
		solarDay += num;
		...
		solarMonth = 1;
		while (solarMonth < 12 && readOnlySpan[solarMonth] < solarDay)
		{
			solarMonth++;
		}
		solarDay -= readOnlySpan[solarMonth - 1];
		return true;
	}
