...
public class GregorianCalendar : Calendar
{
	...
	internal static ReadOnlySpan<int> DaysToMonth365 => RuntimeHelpers.CreateSpan<int>((RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);

	internal static ReadOnlySpan<int> DaysToMonth366 => RuntimeHelpers.CreateSpan<int>((RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);
	...
	public override int TwoDigitYearMax
	{
		get
		{
			...
			{
				_twoDigitYearMax = Calendar.GetSystemTwoDigitYearSetting(ID, 2049);
			}
	...
	internal static long GetAbsoluteDate(int year, int month, int day)
	{
		...
		{
			ReadOnlySpan<int> readOnlySpan = ((year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)) ? DaysToMonth366 : DaysToMonth365);
			if (day >= 1 && day <= readOnlySpan[month] - readOnlySpan[month - 1])
			{
				int num = year - 1;
				return num * 365 + num / 4 - num / 100 + num / 400 + readOnlySpan[month - 1] + day - 1;
			}
		}
	...
	internal override bool IsValidDay(int year, int month, int day, int era)
	{
		...
		ReadOnlySpan<int> readOnlySpan = ((year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)) ? DaysToMonth366 : DaysToMonth365);
		return day <= readOnlySpan[month] - readOnlySpan[month - 1];
	}
	...
	public override int ToFourDigitYear(int year)
	{
		...
		ArgumentOutOfRangeException.ThrowIfGreaterThan(year, 9999, "year");
