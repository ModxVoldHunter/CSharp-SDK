...
public class CultureInfo : IFormatProvider, ICloneable
{
		...
		get
		{
			if (_parent == null)
			{
				...
				if (text == "zh")
				{
					...
					else if (_name.Length > 8 && _name.AsSpan(2, 4).SequenceEqual("-Han".AsSpan()) && _name[7] == '-')
					{
						if (_name[6] == 't')
						{
							text = "zh-Hant";
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegative(culture, "culture");
		switch (culture)
		{
		case 0:
		case 1024:
	...
	{
		ArgumentNullException.ThrowIfNull(textAndCompareCultureName, "textAndCompareCultureName");
	...
	{
		ArgumentNullException.ThrowIfNull(ci, "ci");
		if (ci.IsReadOnly)
		{
			return ci;
		}
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegativeOrZero(culture, "culture");
	...
	{
		ArgumentNullException.ThrowIfNull(name, "name");
		ArgumentNullException.ThrowIfNull(altName, "altName");
		name = CultureData.AnsiToLower(name);
