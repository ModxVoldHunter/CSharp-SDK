...
public class JapaneseCalendar : Calendar
{
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegativeOrZero(year, "year");
		ArgumentOutOfRangeException.ThrowIfGreaterThan(year, _helper.MaxYear, "year");
		return year;
	}
	...
	private static EraInfo GetEraFromValue(string value, string data)
	{
		...
		if (source.Split(destination, '_') == 4)
		{
			Range range = destination[0];
			ReadOnlySpan<char> readOnlySpan2 = source[range.Start..range.End];
			range = destination[1];
			ReadOnlySpan<char> readOnlySpan3 = source[range.Start..range.End];
			range = destination[2];
			ReadOnlySpan<char> readOnlySpan4 = source[range.Start..range.End];
			range = destination[3];
			ReadOnlySpan<char> readOnlySpan5 = source[range.Start..range.End];
			if (!readOnlySpan2.IsEmpty && !readOnlySpan3.IsEmpty && !readOnlySpan4.IsEmpty && !readOnlySpan5.IsEmpty)
			{
				return new EraInfo(0, result, result2, result3, result - 1, 1, 0, readOnlySpan2.ToString(), readOnlySpan3.ToString(), readOnlySpan5.ToString());
			}
		}
		return null;
	}
