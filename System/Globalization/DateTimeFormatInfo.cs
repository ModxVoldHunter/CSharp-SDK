...
public sealed class DateTimeFormatInfo : IFormatProvider, ICloneable
{
	...
	internal string DateTimeOffsetPattern
	{
		get
		{
			if (dateTimeOffsetPattern == null)
			{
				...
				bool flag2 = false;
				char c = '\'';
				string text = LongTimePattern;
				int num = 0;
				while (!flag && num < text.Length)
				{
					switch (text[num])
					{
					case 'z':
				...
				dateTimeOffsetPattern = (flag ? (ShortDatePattern + " " + text) : (ShortDatePattern + " " + text + " zzz"));
			}
	...
	internal ReadOnlySpan<TChar> AMDesignatorTChar<TChar>() where TChar : unmanaged, IUtfChar<TChar>
	{
		...
		{
			return MemoryMarshal.Cast<byte, TChar>(amDesignatorUtf8 ?? (amDesignatorUtf8 = Encoding.UTF8.GetBytes(AMDesignator)));
		}
		return MemoryMarshal.Cast<char, TChar>(AMDesignator);
	}

	public int GetEra(string eraName)
	{
		ArgumentNullException.ThrowIfNull(eraName, "eraName");
		if (eraName.Length == 0)
		{
			return -1;
		}
	...
	public string GetEraName(int era)
	{
		...
		string[] eraNames = EraNames;
		era--;
		if ((uint)era >= eraNames.Length)
		{
			throw new ArgumentOutOfRangeException("era", era + 1, SR.ArgumentOutOfRange_InvalidEraValue);
		}
		return eraNames[era];
	}
	...
	public string GetAbbreviatedEraName(int era)
	{
		...
		string[] abbrevEraNames = m_abbrevEraNames;
		era--;
		if ((uint)era >= (uint)abbrevEraNames.Length)
		{
			throw new ArgumentOutOfRangeException("era", era + 1, SR.ArgumentOutOfRange_InvalidEraValue);
		}
		return abbrevEraNames[era];
	}
	...
	internal ReadOnlySpan<TChar> DateSeparatorTChar<TChar>() where TChar : unmanaged, IUtfChar<TChar>
	{
		if (!(typeof(TChar) == typeof(char)))
		{
			return MemoryMarshal.Cast<byte, TChar>(dateSeparatorUtf8 ?? (dateSeparatorUtf8 = Encoding.UTF8.GetBytes(DateSeparator)));
		}
		return MemoryMarshal.Cast<char, TChar>(DateSeparator);
	}
	...
	internal ReadOnlySpan<TChar> PMDesignatorTChar<TChar>() where TChar : unmanaged, IUtfChar<TChar>
	{
		if (!(typeof(TChar) == typeof(char)))
		{
			return MemoryMarshal.Cast<byte, TChar>(pmDesignatorUtf8 ?? (pmDesignatorUtf8 = Encoding.UTF8.GetBytes(PMDesignator)));
		}
		return MemoryMarshal.Cast<char, TChar>(PMDesignator);
	}
	...
	internal ReadOnlySpan<TChar> TimeSeparatorTChar<TChar>() where TChar : unmanaged, IUtfChar<TChar>
	{
		if (!(typeof(TChar) == typeof(char)))
		{
			return MemoryMarshal.Cast<byte, TChar>(timeSeparatorUtf8 ?? (timeSeparatorUtf8 = Encoding.UTF8.GetBytes(TimeSeparator)));
		}
		return MemoryMarshal.Cast<char, TChar>(TimeSeparator);
	}
	...
	internal string InternalGetMonthName(int month, MonthNameStyles style, bool abbreviated)
	{
		...
		month--;
		if ((uint)month >= (uint)array.Length)
		{
			ThrowHelper.ThrowArgumentOutOfRange_Range("month", month + 1, 1, array.Length);
		}
		return array[month];
	}
	...
	{
		string[] array = InternalGetAbbreviatedDayOfWeekNames();
		if ((uint)dayofweek >= (uint)array.Length)
		{
			ThrowHelper.ThrowArgumentOutOfRange_Range("dayofweek", dayofweek, DayOfWeek.Sunday, DayOfWeek.Saturday);
		}
	...
	{
		string[] array = InternalGetSuperShortDayNames();
		if ((uint)dayOfWeek >= (uint)array.Length)
		{
			ThrowHelper.ThrowArgumentOutOfRange_Range("dayOfWeek", dayOfWeek, DayOfWeek.Sunday, DayOfWeek.Saturday);
		}
	...
	public string[] GetAllDateTimePatterns()
	{
		...
		{
			string[] allDateTimePatterns = GetAllDateTimePatterns(format);
			foreach (string item in allDateTimePatterns)
			{
				list.Add(item);
			}
		}
	...
	public string[] GetAllDateTimePatterns(char format)
	{
		...
		case 'T':
			return AllLongTimePatterns;
		case 'u':
			return new string[1] { "yyyy'-'MM'-'dd HH':'mm':'ss'Z'" };
		case 'Y':
		case 'y':
			return AllYearMonthPatterns;
		default:
	...
	{
		string[] array = InternalGetDayOfWeekNames();
		if ((uint)dayofweek >= (uint)array.Length)
		{
			ThrowHelper.ThrowArgumentOutOfRange_Range("dayofweek", dayofweek, DayOfWeek.Sunday, DayOfWeek.Saturday);
		}
	...
	{
		string[] array = InternalGetAbbreviatedMonthNames();
		month--;
		if ((uint)month >= (uint)array.Length)
		{
			ThrowHelper.ThrowArgumentOutOfRange_Range("month", month + 1, 1, 13);
		}
		return array[month];
	}
	...
	{
		string[] array = InternalGetMonthNames();
		month--;
		if ((uint)month >= (uint)array.Length)
		{
			ThrowHelper.ThrowArgumentOutOfRange_Range("month", month + 1, 1, 13);
	...
	{
		ArgumentNullException.ThrowIfNull(dtfi, "dtfi");
		if (dtfi.IsReadOnly)
		{
			return dtfi;
		}
	...
	public void SetAllDateTimePatterns(string[] patterns, char format)
	{
		...
		ArgumentNullException.ThrowIfNull(patterns, "patterns");
	...
	internal ReadOnlySpan<TChar> DecimalSeparatorTChar<TChar>() where TChar : unmanaged, IUtfChar<TChar>
	{
		if (!(typeof(TChar) == typeof(char)))
		{
			return MemoryMarshal.Cast<byte, TChar>(_decimalSeparatorUtf8 ?? (_decimalSeparatorUtf8 = Encoding.UTF8.GetBytes(DecimalSeparator)));
		}
		return MemoryMarshal.Cast<char, TChar>(DecimalSeparator);
	}
