...
internal static class CapiHelper
{
	...
	internal static string GetDefaultProvider(int dwType)
	{
		...
		string text = new string(array.AsSpan(0, array.AsSpan().IndexOf('\0')));
		string text2 = null;
		switch (dwType)
		{
		case 1:
	...
	public static object GetProviderParameter(SafeProvHandle safeProvHandle, int keyNumber, ClrPropertyId keyParam)
	{
		...
		try
		{
			...
			{
				int providerParameterWorker = GetProviderParameterWorker(safeProvHandle, impType, ref cb, global::Interop.Advapi32.CryptProvParam.PP_IMPTYPE);
				if (!IsFlagBitSet((uint)providerParameterWorker, 1u))
				{
					...
					flag = IsFlagBitSet((uint)num2, 4u);
				}
				else
				{
					flag = false;
			...
			{
				int providerParameterWorker = GetProviderParameterWorker(safeProvHandle, impType, ref cb, global::Interop.Advapi32.CryptProvParam.PP_IMPTYPE);
				flag = IsFlagBitSet((uint)providerParameterWorker, 8u);
				break;
			}
			...
			{
				int providerParameterWorker = GetProviderParameterWorker(safeProvHandle, impType, ref cb, global::Interop.Advapi32.CryptProvParam.PP_IMPTYPE);
				flag = IsFlagBitSet((uint)providerParameterWorker, 1u);
				break;
			}
			...
			case ClrPropertyId.CLR_UNIQUE_CONTAINER:
			{
				num = 1;
				byte[] impType2 = null;
				int providerParameterWorker = GetProviderParameterWorker(safeProvHandle, impType2, ref cb, global::Interop.Advapi32.CryptProvParam.PP_UNIQUE_CONTAINER);
				impType2 = new byte[cb];
				providerParameterWorker = GetProviderParameterWorker(safeProvHandle, impType2, ref cb, global::Interop.Advapi32.CryptProvParam.PP_UNIQUE_CONTAINER);
	...
	internal static SafeCapiKeyHandle GetKeyPairHelper(CspAlgorithmType keyType, CspParameters parameters, int keySize, SafeProvHandle safeProvHandle)
	{
		...
		byte[] keyParameter = GetKeyParameter(safeKeyHandle, ClrPropertyId.CLR_ALGID);
	...
	public static int NameOrOidToHashAlgId(string nameOrOid, OidGroup oidGroup)
	{
		...
		string oid = CryptoConfig.MapNameToOID(nameOrOid) ?? nameOrOid;
		int algIdFromOid = GetAlgIdFromOid(oid, oidGroup);
