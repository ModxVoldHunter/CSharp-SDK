...
internal sealed class RSAPssX509SignatureGenerator : X509SignatureGenerator
{
	...
	public override byte[] GetSignatureAlgorithmIdentifier(HashAlgorithmName hashAlgorithm)
	{
		...
		pssParamsAsn.HashAlgorithm = new AlgorithmIdentifierAsn
		{
			Algorithm = text
		};
		pssParamsAsn.MaskGenAlgorithm = new AlgorithmIdentifierAsn
		{
		...
		AlgorithmIdentifierAsn algorithmIdentifierAsn = default(AlgorithmIdentifierAsn);
		algorithmIdentifierAsn.Algorithm = "1.2.840.113549.1.1.10";
		algorithmIdentifierAsn.Parameters = asnWriter.Encode();
		AlgorithmIdentifierAsn algorithmIdentifierAsn2 = algorithmIdentifierAsn;
		asnWriter.Reset();
