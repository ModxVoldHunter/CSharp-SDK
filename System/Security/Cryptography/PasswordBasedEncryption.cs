...
internal static class PasswordBasedEncryption
{
	...
	internal unsafe static int Decrypt(in AlgorithmIdentifierAsn algorithmIdentifier, ReadOnlySpan<char> password, ReadOnlySpan<byte> passwordBytes, ReadOnlySpan<byte> encryptedData, Span<byte> destination)
	{
		...
		using (symmetricAlgorithm)
		{
			if (flag)
			{
				...
				result = Pkcs12PbeDecrypt(algorithmIdentifier, password, hashAlgorithm, symmetricAlgorithm, encryptedData, destination);
			}
			else
			{
				...
				fixed (byte* ptr = &MemoryMarshal.GetReference(span))
				{
					...
					try
					{
						result = Pbes1Decrypt(algorithmIdentifier.Parameters, password2, hasher, symmetricAlgorithm, encryptedData, destination);
					}
					finally
					{
	...
	internal static void InitiateEncryption(PbeParameters pbeParameters, out SymmetricAlgorithm cipher, out string hmacOid, out string encryptionAlgorithmOid, out bool isPkcs12)
	{
		if (!Helpers.HasSymmetricEncryption)
		{
			throw new CryptographicException(System.SR.Format(System.SR.Cryptography_UnknownAlgorithmIdentifier, pbeParameters.EncryptionAlgorithm));
		}
		isPkcs12 = false;
	...
	private static SymmetricAlgorithm OpenCipher(AlgorithmIdentifierAsn encryptionScheme, int? requestedKeyLength, ref Span<byte> iv)
	{
		...
		if (!Helpers.HasSymmetricEncryption)
		{
			throw new CryptographicException(System.SR.Format(System.SR.Cryptography_AlgorithmNotSupported, algorithm));
		}
		switch (algorithm)
		{
