...
public abstract class SHA3_512 : HashAlgorithm
{
	private sealed class Implementation : SHA3_512
	{
		...
		{
			_hashProvider = HashProviderDispenser.CreateHashProvider("SHA3-512");
	...
	public static bool IsSupported { get; } = HashProviderDispenser.HashSupported("SHA3-512");
	...
	public static bool TryHashData(ReadOnlySpan<byte> source, Span<byte> destination, out int bytesWritten)
	{
		...
		bytesWritten = HashProviderDispenser.OneShotHashProvider.HashData("SHA3-512", source, destination);
		return true;
	}
	...
	public static int HashData(Stream source, Span<byte> destination)
	{
		...
		CheckSha3Support();
		return LiteHashProvider.HashStream("SHA3-512", source, destination);
	}
	...
	public static byte[] HashData(Stream source)
	{
		...
		CheckSha3Support();
		return LiteHashProvider.HashStream("SHA3-512", 64, source);
	}
	...
	public static ValueTask<byte[]> HashDataAsync(Stream source, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		CheckSha3Support();
		return LiteHashProvider.HashStreamAsync("SHA3-512", source, cancellationToken);
	}
	...
	public static ValueTask<int> HashDataAsync(Stream source, Memory<byte> destination, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		CheckSha3Support();
		return LiteHashProvider.HashStreamAsync("SHA3-512", source, destination, cancellationToken);
	}
