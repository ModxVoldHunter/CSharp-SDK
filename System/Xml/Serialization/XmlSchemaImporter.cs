...
public class XmlSchemaImporter : SchemaImporter
{
	...
	internal static TypeMapping GetDefaultMapping(TypeFlags flags)
	{
		...
		primitiveMapping.TypeDesc = TypeScope.GetTypeDesc("string", "http://www.w3.org/2001/XMLSchema", flags);
	...
	private TypeMapping ImportElementType(XmlSchemaElement element, string identifier, Type desiredMappingType, Type baseType, string ns)
	{
		...
		{
			typeMapping = ((!(element.SchemaType is XmlSchemaComplexType)) ? ImportDataType((XmlSchemaSimpleType)element.SchemaType, ns, identifier, baseType, (TypeFlags)56, isList: false) : ImportType((XmlSchemaComplexType)element.SchemaType, ns, identifier, desiredMappingType, baseType));
	...
	internal override void ImportDerivedTypes(XmlQualifiedName baseName)
	{
		foreach (XmlSchema schema in base.Schemas)
		{
			...
			{
				if (value is XmlSchemaType xmlSchemaType && xmlSchemaType.DerivedFrom == baseName && base.TypesInUse[xmlSchemaType.Name, schema.TargetNamespace] == null)
				{
	...
	private TypeMapping ImportType(XmlQualifiedName name, Type desiredMappingType, Type baseType, TypeFlags flags, bool addref)
	{
		...
		if (addref)
		{
			SchemaImporter.AddReference(name, base.TypesInUse, System.SR.XmlCircularTypeReference);
		}
		if (obj is XmlSchemaComplexType)
		{
			typeMapping = ImportType((XmlSchemaComplexType)obj, name.Namespace, name.Name, desiredMappingType, baseType);
		}
		else
		{
		...
		{
			SchemaImporter.RemoveReference(name, base.TypesInUse);
		}
		return typeMapping;
	}
	...
	private TypeMapping ImportType(XmlSchemaComplexType type, string typeNs, string identifier, Type desiredMappingType, Type baseType)
	{
		...
		if (desiredMappingType == typeof(TypeMapping))
		{
			...
			if (baseType == null && (typeMapping = ImportArrayMapping(type, identifier, typeNs)) == null)
			{
				typeMapping = ImportAnyMapping(type, repeats: false);
			}
			if (typeMapping == null)
			{
				typeMapping = ImportStructType(type, typeNs, identifier, baseType, arrayLike: false);
				if (typeMapping != null && !string.IsNullOrEmpty(type.Name))
				{
	...
	private StructMapping ImportStructType(XmlSchemaType type, string typeNs, string identifier, Type baseType, bool arrayLike)
	{
		...
		string text = (string.IsNullOrEmpty(type.Name) ? GenerateUniqueTypeName(identifier, typeNs) : GenerateUniqueTypeName(identifier));
	...
	private TypeItems GetTypeItems(XmlSchemaType type)
	{
		...
		if (type is XmlSchemaComplexType)
		{
			...
			if (xmlSchemaComplexType.ContentModel != null)
			{
				...
				if (content is XmlSchemaComplexContentExtension xmlSchemaComplexContentExtension)
				{
				...
				else if (content is XmlSchemaSimpleContentExtension xmlSchemaSimpleContentExtension)
				{
	...
	private MemberMapping ImportChoiceGroup(XmlSchemaGroupBase group, string identifier, CodeIdentifiers members, CodeIdentifiers membersScope, INameScope elementsScope, string ns, bool groupRepeats, ref bool needExplicitOrder, bool allowDuplicates)
	{
		...
		else
		{
			...
			memberMapping.TypeDesc = TypeDesc.FindCommonBaseTypeDesc(array) ?? base.Scope.GetTypeDesc(typeof(object));
		}
		if (groupRepeats)
		{
	...
	private bool GatherGroupChoices(XmlSchemaParticle particle, NameTable choiceElements, string identifier, string ns, ref bool needExplicitOrder, bool allowDuplicates)
	{
		if (particle is XmlSchemaGroupRef xmlSchemaGroupRef)
		{
			...
			{
				SchemaImporter.AddReference(xmlSchemaGroupRef.RefName, base.GroupsInUse, System.SR.XmlCircularGroupReference);
				...
				{
					SchemaImporter.RemoveReference(xmlSchemaGroupRef.RefName, base.GroupsInUse);
					return true;
				}
				SchemaImporter.RemoveReference(xmlSchemaGroupRef.RefName, base.GroupsInUse);
			}
		}
		else if (particle is XmlSchemaGroupBase { IsMultipleOccurrence: var flag } xmlSchemaGroupBase)
		{
			...
			for (int i = 0; i < xmlSchemaGroupBase.Items.Count; i++)
			{
				...
				{
					if (!(obj is XmlSchemaElement xmlSchemaElement))
					{
						continue;
					}
	...
	private void ImportGroupMembers(XmlSchemaParticle particle, string identifier, CodeIdentifiers members, CodeIdentifiers membersScope, INameScope elementsScope, string ns, bool groupRepeats, ref bool mixed, ref bool needExplicitOrder, bool allowDuplicates, bool allowUnboundedElements)
	{
		...
		else
		{
			...
			for (int i = 0; i < xmlSchemaGroupBase.Items.Count; i++)
			{
				...
				{
					ImportAnyMember((XmlSchemaAny)obj, members, membersScope, elementsScope, ns, ref mixed, ref needExplicitOrder, allowDuplicates);
				}
	...
	private XmlSchemaElement[] GetEquivalentElements(XmlSchemaElement element)
	{
		...
		foreach (XmlSchema item in base.Schemas.SchemaSet.Schemas())
		{
			for (int i = 0; i < item.Items.Count; i++)
			{
				...
				if (obj is XmlSchemaElement { IsAbstract: false } xmlSchemaElement && xmlSchemaElement.SubstitutionGroup.Namespace == item.TargetNamespace && xmlSchemaElement.SubstitutionGroup.Name == element.Name)
				{
	...
	private ElementAccessor ImportArray(XmlSchemaElement element, string identifier, string ns, bool repeats)
	{
		...
		ArrayMapping arrayMapping = ImportArrayMapping(schemaType, identifier, ns);
	...
	private ArrayMapping ImportArrayMapping(XmlSchemaType type, string identifier, string ns)
	{
		...
		else
		{
			...
			arrayMapping.TypeName = (string.IsNullOrEmpty(type.Name) ? ("ArrayOf" + CodeIdentifier.MakePascal(elementAccessor.Mapping.TypeDesc.Name)) : type.Name);
		}
	...
	private static bool KeepXmlnsDeclarations(XmlSchemaType type, out string xmlnsMemberName)
	{
		...
		foreach (XmlSchemaObject item in type.Annotation.Items)
		{
			...
			{
				if (xmlNode is XmlElement { Name: "keepNamespaceDeclarations" } xmlElement)
				{
	...
	private void ImportXmlnsDeclarationsMember(XmlSchemaType type, CodeIdentifiers members, CodeIdentifiers membersScope)
	{
		if (KeepXmlnsDeclarations(type, out var xmlnsMemberName))
		{
			...
			MemberMapping memberMapping = new MemberMapping();
			memberMapping.Elements = new ElementAccessor[1] { elementAccessor };
			memberMapping.Name = CodeIdentifier.MakeValid(xmlnsMemberName ?? "Namespaces");
			memberMapping.Name = membersScope.AddUnique(memberMapping.Name, memberMapping);
			members.Add(memberMapping.Name, memberMapping);
	...
	private TypeMapping ImportDataType(XmlSchemaSimpleType dataType, string typeNs, string identifier, Type baseType, TypeFlags flags, bool isList)
	{
		...
		if (dataType.Content is XmlSchemaSimpleTypeRestriction xmlSchemaSimpleTypeRestriction)
		{
			...
			SchemaImporter.AddReference(xmlSchemaSimpleTypeRestriction.BaseTypeName, base.TypesInUse, System.SR.XmlCircularTypeReference);
			...
			{
				SchemaImporter.RemoveReference(xmlSchemaSimpleTypeRestriction.BaseTypeName, base.TypesInUse);
			}
			return typeMapping;
		}
		if (dataType.Content is XmlSchemaSimpleTypeList || dataType.Content is XmlSchemaSimpleTypeUnion)
		{
			if (dataType.Content is XmlSchemaSimpleTypeList xmlSchemaSimpleTypeList)
			{
	...
	private TypeMapping ImportEnumeratedDataType(XmlSchemaSimpleType dataType, string typeNs, string identifier, TypeFlags flags, bool isList)
	{
		...
		TypeDesc typeDesc = TypeScope.GetTypeDesc((XmlSchemaSimpleType)xmlSchemaType);
	...
	private static PrimitiveMapping ImportNonXsdPrimitiveDataType(XmlSchemaSimpleType dataType, string ns, TypeFlags flags)
	{
		...
		{
			TypeDesc typeDesc = TypeScope.GetTypeDesc(dataType.Name, ns, flags);
			if (typeDesc != null)
			{
	...
	internal static XmlQualifiedName BaseTypeName(XmlSchemaSimpleType dataType)
	{
		...
		if (content is XmlSchemaSimpleTypeList xmlSchemaSimpleTypeList)
		{
	...
	private TypeDesc GetDataTypeSource(XmlSchemaSimpleType dataType, TypeFlags flags)
	{
		...
		SchemaImporter.AddReference(xmlQualifiedName, base.TypesInUse, System.SR.XmlCircularTypeReference);
		...
		{
			SchemaImporter.RemoveReference(xmlQualifiedName, base.TypesInUse);
		}
		return typeDesc;
	}
	...
	private XmlSchemaSimpleType FindDataType(XmlQualifiedName name, TypeFlags flags)
	{
		...
		{
			return (XmlSchemaSimpleType)TypeScope.GetTypeDesc("string", "http://www.w3.org/2001/XMLSchema", flags).DataType;
		}
