...
public class XmlReflectionImporter
{
	...
	public XmlMembersMapping ImportMembersMapping(string? elementName, string? ns, XmlReflectionMember[] members, bool hasWrapperElement, bool rpc, bool openModel, XmlMappingAccess access)
	{
		...
		elementAccessor.Name = (string.IsNullOrEmpty(elementName) ? elementName : XmlConvert.EncodeLocalName(elementName));
	...
	private ElementAccessor ImportElement(TypeModel model, XmlRootAttribute root, string defaultNamespace, RecursionLimiter limiter)
	{
		...
		_savedArrayItemAttributes = null;
		_savedArrayNamespace = null;
		ElementAccessor elementAccessor = CreateElementAccessor(ImportTypeMapping(model, ns, ImportContext.Element, string.Empty, attributes, limiter), ns);
		if (root != null)
		{
		...
		try
		{
			...
			case TypeKind.Class:
				...
				if (model.TypeDesc.IsOptionalValue)
				{
					...
					TypeMapping baseMapping = GetTypeMapping(typeName, ns, typeDesc3, _types, null) ?? ImportTypeMapping(_modelScope.GetTypeModel(model.TypeDesc.BaseTypeDesc.Type), ns, context, dataType, null, repeats, openModel, limiter);
	...
	internal static MethodInfo GetMethodFromSchemaProvider(XmlSchemaProviderAttribute provider, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] Type type)
	{
		...
		ArgumentNullException.ThrowIfNull(provider.MethodName, "MethodName");
	...
	internal void SetBase(SerializableMapping mapping, XmlQualifiedName baseQname)
	{
		if (!baseQname.IsEmpty && !(baseQname.Namespace == "http://www.w3.org/2001/XMLSchema"))
		{
			...
			xmlSchemaType = xmlSchemaType.Redefined ?? xmlSchemaType;
	...
	{
		TypeMapping typeMapping = ((!string.IsNullOrEmpty(typeName)) ? ((TypeMapping)typeLib[typeName, ns]) : ((type == null) ? null : ((TypeMapping)_anonymous[type])));
		if (typeMapping == null)
		{
			return null;
		}
	...
	private bool InitializeStructMembers(StructMapping mapping, StructModel model, bool openModel, string typeName, RecursionLimiter limiter)
	{
		...
		{
			StructMapping structMapping2 = (mapping.BaseMapping = GetRootMapping());
		}
	...
	private void SetArrayMappingType(ArrayMapping mapping, string defaultNs, Type type)
	{
		...
		while (typeMapping2 != null && (!(typeMapping2 is ArrayMapping arrayMapping) || !AccessorMapping.ElementsMatch(arrayMapping.Elements, mapping.Elements)))
		{
	...
	private ArrayMapping ImportArrayLikeMapping(ArrayModel model, string ns, RecursionLimiter limiter)
	{
		...
		CreateArrayElementsFromAttributes(arrayMapping, _savedArrayItemAttributes, model.Element.Type, _savedArrayNamespace ?? ns, limiter);
	...
	private ConstantMapping ImportConstantMapping(ConstantModel model)
	{
		...
		{
			XmlEnumAttribute xmlEnumAttribute2 = (xmlAttributes.XmlEnum = new XmlEnumAttribute());
		}
		ConstantMapping constantMapping = new ConstantMapping();
		constantMapping.XmlName = attributes.XmlEnum.Name ?? model.Name;
	...
	private MemberMapping ImportMemberMapping(XmlReflectionMember xmlReflectionMember, string ns, XmlReflectionMember[] xmlReflectionMembers, bool rpc, bool openModel, RecursionLimiter limiter)
	{
		...
		if (xmlAttributes.XmlFlags == (XmlAttributeFlags)0)
		{
			...
			{
				XmlArrayAttribute xmlArrayAttribute = CreateArrayAttribute();
	...
	private static Type GetChoiceIdentifierType(XmlChoiceIdentifierAttribute choice, XmlReflectionMember[] xmlReflectionMembers, bool isArrayLike, string accessorName)
	{
		...
		throw new InvalidOperationException(System.SR.Format(System.SR.XmlChoiceIdentifierMemberMissing, choice.MemberName, accessorName));
	}
	...
	private static Type GetChoiceIdentifierType(XmlChoiceIdentifierAttribute choice, StructModel structModel, bool isArrayLike, string accessorName)
	{
		...
		{
			throw new InvalidOperationException(System.SR.Format(System.SR.XmlChoiceIdentifierAmbiguous, choice.MemberName));
		}
	...
	private void CreateArrayElementsFromAttributes(ArrayMapping arrayMapping, XmlArrayItemAttributes attributes, Type arrayElementType, string arrayElementNs, RecursionLimiter limiter)
	{
		...
		while (attributes != null && num < attributes.Count)
		{
			...
			{
				Type type = xmlArrayItemAttribute.Type ?? arrayElementType;
				TypeDesc typeDesc = _typeScope.GetTypeDesc(type);
				ElementAccessor elementAccessor = new ElementAccessor();
				elementAccessor.Namespace = xmlArrayItemAttribute.Namespace ?? arrayElementNs;
	...
	private void ImportAccessorMapping(MemberMapping accessor, FieldModel model, XmlAttributes a, string ns, Type choiceIdentifierType, bool rpc, bool openModel, RecursionLimiter limiter)
	{
		...
		if (accessor.TypeDesc.IsArrayLike)
		{
			...
			if ((xmlFlags & xmlAttributeFlags2) != 0)
			{
				...
				attributeAccessor.Namespace = a.XmlAttribute.Namespace ?? ns;
			...
			else if ((xmlFlags & xmlAttributeFlags) != 0)
			{
				...
				if (a.XmlText != null)
				{
					...
					Type type2 = a.XmlText.Type ?? arrayElementType;
				...
				for (int i = 0; i < a.XmlElements.Count; i++)
				{
					...
					Type type3 = xmlElementAttribute.Type ?? arrayElementType;
					TypeDesc typeDesc3 = _typeScope.GetTypeDesc(type3);
					TypeModel typeModel = _modelScope.GetTypeModel(type3);
					ElementAccessor elementAccessor = new ElementAccessor();
					elementAccessor.Namespace = (rpc ? null : (xmlElementAttribute.Namespace ?? ns));
				...
				for (int j = 0; j < a.XmlAnyElements.Count; j++)
				{
					...
					if (nameTable[name2, text ?? ns] != null)
					{
						throw new InvalidOperationException(System.SR.Format(System.SR.XmlAnyElementDuplicate, name, xmlAnyElementAttribute.Name, xmlAnyElementAttribute.Namespace ?? "null"));
					}
					ElementAccessor elementAccessor2 = new ElementAccessor();
					elementAccessor2.Name = name2;
					elementAccessor2.Namespace = text ?? ns;
			...
			else
			{
				...
				if (a.XmlArray == null)
				{
					XmlArrayAttribute xmlArrayAttribute2 = (a.XmlArray = CreateArrayAttribute());
				}
				...
				elementAccessor3.Namespace = (rpc ? null : (a.XmlArray.Namespace ?? ns));
		...
		else if (!accessor.TypeDesc.IsVoid)
		{
			...
			if (accessor.TypeDesc.IsPrimitive || accessor.TypeDesc.IsEnum)
			{
				...
				if (a.XmlAttribute != null)
				{
					...
					attributeAccessor2.Namespace = a.XmlAttribute.Namespace ?? ns;
				...
				else
				{
					...
					for (int k = 0; k < a.XmlElements.Count; k++)
					{
						...
						elementAccessor4.Namespace = (rpc ? null : (xmlElementAttribute2.Namespace ?? ns));
			...
			else
			{
				...
				for (int l = 0; l < a.XmlElements.Count; l++)
				{
					...
					Type type5 = xmlElementAttribute3.Type ?? fieldType;
					...
					elementAccessor5.Namespace = (rpc ? null : (xmlElementAttribute3.Namespace ?? ns));
				...
				for (int m = 0; m < a.XmlAnyElements.Count; m++)
				{
					...
					if (nameTable[name3, text2 ?? ns] != null)
					{
						throw new InvalidOperationException(System.SR.Format(System.SR.XmlAnyElementDuplicate, name, xmlAnyElementAttribute2.Name, xmlAnyElementAttribute2.Namespace ?? "null"));
					}
					ElementAccessor elementAccessor6 = new ElementAccessor();
					elementAccessor6.Name = name3;
					elementAccessor6.Namespace = text2 ?? ns;
		...
		if (accessor.ChoiceIdentifier != null)
		{
			...
			for (int n = 0; n < accessor.Elements.Length; n++)
			{
				...
				for (int num2 = 0; num2 < enumMapping.Constants.Length; num2++)
				{
					...
					{
						string text3 = elementAccessor7.AnyNamespaces ?? "##any";
	...
	private void CheckAmbiguousChoice(XmlAttributes a, Type accessorType, string accessorName)
	{
		...
		if (xmlElements != null && xmlElements.Count >= 2 && a.XmlChoiceIdentifier == null)
		{
			for (int i = 0; i < xmlElements.Count; i++)
			{
				...
				{
					throw new InvalidOperationException(System.SR.Format(System.SR.XmlChoiceIdentifierMissing, "XmlChoiceIdentifierAttribute", accessorName));
				}
