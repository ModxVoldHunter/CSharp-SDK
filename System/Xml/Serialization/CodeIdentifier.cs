...
{
	private static readonly char[] s_identifierSeparators = new char[4] { '.', ',', '<', '>' };
	...
	public static string MakeValid(string identifier)
	{
		...
		for (int i = 0; i < identifier.Length; i++)
		{
			...
			if (IsValid(c))
			{
				if (valueStringBuilder.Length == 0 && !IsValidStart(c))
				{
	...
	private static int GetCSharpName(Type t, Type[] parameters, int index, StringBuilder sb)
	{
		...
		if (num > 0)
		{
			...
			sb.Append('<');
			int num2 = int.Parse(name.AsSpan(num + 1), CultureInfo.InvariantCulture) + index;
			while (index < num2)
			{
	...
	private static string EscapeKeywords(string identifier)
	{
		...
		string[] array = identifier.Split(s_identifierSeparators);
		StringBuilder stringBuilder = new StringBuilder();
		int num = -1;
		for (int i = 0; i < array.Length; i++)
		{
