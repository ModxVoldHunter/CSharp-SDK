...
internal sealed class StructMapping : TypeMapping, INameScope
{
	...
	internal NameTable LocalElements => _elements ?? (_elements = new NameTable());

	internal NameTable LocalAttributes => _attributes ?? (_attributes = new NameTable());
	...
	internal CodeIdentifiers Scope
	{
		get
		{
			return _scope ?? (_scope = new CodeIdentifiers());
		}
		set
		{
			_scope = value;
