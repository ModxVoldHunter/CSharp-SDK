...
public class XmlSchemas : CollectionBase, IEnumerable<XmlSchema>, IEnumerable
{
	...
	internal Hashtable MergedSchemas => _mergedSchemas ?? (_mergedSchemas = new Hashtable());

	internal Hashtable References => _references ?? (_references = new Hashtable());
	...
	internal static XmlSchema XsdSchema => s_xsd ?? (s_xsd = CreateFakeXsdSchema("http://www.w3.org/2001/XMLSchema", "schema"));

	internal static XmlSchema XmlSchema => s_xml ?? (s_xml = System.Xml.Schema.XmlSchema.Read(new StringReader("<?xml version='1.0' encoding='UTF-8' ?>\r\n<xs:schema targetNamespace='http://www.w3.org/XML/1998/namespace' xmlns:xs='http://www.w3.org/2001/XMLSchema' xml:lang='en'>\r\n <xs:attribute name='lang' type='xs:language'/>\r\n <xs:attribute name='space'>\r\n  <xs:simpleType>\r\n   <xs:restriction base='xs:NCName'>\r\n    <xs:enumeration value='default'/>\r\n    <xs:enumeration value='preserve'/>\r\n   </xs:restriction>\r\n  </xs:simpleType>\r\n </xs:attribute>\r\n <xs:attribute name='base' type='xs:anyURI'/>\r\n <xs:attribute name='id' type='xs:ID' />\r\n <xs:attributeGroup name='specialAttrs'>\r\n  <xs:attribute ref='xml:base'/>\r\n  <xs:attribute ref='xml:lang'/>\r\n  <xs:attribute ref='xml:space'/>\r\n </xs:attributeGroup>\r\n</xs:schema>"), null));
	...
	internal object Find(XmlQualifiedName name, Type type, bool checkCache)
	{
		...
		foreach (XmlSchema item2 in list)
		{
			...
			{
				xmlSchemaObject = Cache.AddItem(xmlSchemaObject, name);
			}
	...
	public static bool IsDataSet(XmlSchema schema)
	{
		...
		{
			if (!(item is XmlSchemaElement { UnhandledAttributes: not null } xmlSchemaElement))
			{
				continue;
			}
	...
	internal static XmlQualifiedName GetParentName(XmlSchemaObject item)
	{
		...
		{
			if (item.Parent is XmlSchemaType xmlSchemaType && !string.IsNullOrEmpty(xmlSchemaType.Name))
			{
	...
	private static string GetSchemaItem(XmlSchemaObject o, string ns, string details)
	{
		...
		if (o is XmlSchemaElement xmlSchemaElement)
		{
		...
		{
			if (string.IsNullOrEmpty(xmlSchemaAttribute.Name))
			{
	...
	private static string MergeFailedMessage(XmlSchemaObject src, XmlSchemaObject dest, string ns)
	{
		...
		return text + Environment.NewLine + Dump(src) + Environment.NewLine + Dump(dest);
	}
