...
internal sealed class CodeGenerator
{
	...
	internal LocalBuilder ReturnLocal => retLocal ?? (retLocal = DeclareLocal(_methodBuilder.ReturnType, "_ret"));
	...
	internal MethodBuilder EndMethod()
	{
		...
		Ret();
		MethodBuilder methodBuilder = _methodBuilder;
		_methodBuilder = null;
		_ilGen = null;
		_freeLocals = null;
		_blockStack = null;
	...
	internal unsafe void StackallocSpan(Type elementType, int len)
	{
		Ldc(len);
		_ilGen.Emit(OpCodes.Conv_U);
		_ilGen.Emit(OpCodes.Localloc);
		Ldc(len);
		_ilGen.Emit(OpCodes.Newobj, typeof(Span<>).MakeGenericType(elementType).GetConstructor(new Type[2]
		{
			typeof(void*),
			typeof(int)
		}));
	...
	internal Type LoadMember(MemberInfo memberInfo)
	{
		...
		else
		{
			...
			{
				MethodInfo methodInfo = propertyInfo.GetMethod ?? GetPropertyMethodFromBaseType(propertyInfo, isGetter: true);
				Call(methodInfo);
			}
		}
		return result;
	...
	internal void StoreMember(MemberInfo memberInfo)
	{
		...
		if (propertyInfo != null)
		{
			MethodInfo methodInfo = propertyInfo.SetMethod ?? GetPropertyMethodFromBaseType(propertyInfo, isGetter: false);
			Call(methodInfo);
		}
	}
