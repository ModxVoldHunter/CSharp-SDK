...
internal sealed class ReflectionAwareCodeGen
{
	...
	private string WriteTypeInfo(TypeScope scope, TypeDesc typeDesc, Type type)
	{
		...
		if (type.IsArray)
		{
			...
			{
				string value = WriteTypeInfo(scope, arrayElementTypeDesc, scope.GetTypeFromTypeDesc(arrayElementTypeDesc));
				_writer.WriteLine($"static {typeof(Type).FullName} {text} = {value}.MakeArrayType();");
			}
			else
			{
				string value2 = WriteAssemblyInfo(type);
				_writer.Write($"static {typeof(Type).FullName} {text} = {value2}.GetType(");
		...
		else
		{
			...
			{
				string value3 = WriteTypeInfo(scope, scope.GetTypeDesc(underlyingType), underlyingType);
				_writer.WriteLine($"static {typeof(Type).FullName} {text} = typeof(System.Nullable<>).MakeGenericType(new {typeof(Type).FullName}[] {{{value3}}});");
			}
			else
			{
				string value4 = WriteAssemblyInfo(type);
				_writer.Write($"static {typeof(Type).FullName} {text} = {value4}.GetType(");
				WriteQuotedCSharpString(type.FullName);
				_writer.WriteLine(");");
			}
		}
