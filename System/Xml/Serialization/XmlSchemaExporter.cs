...
public class XmlSchemaExporter
{
	...
	private static bool IsAnyType(XmlSchemaType schemaType, bool mixed, bool unbounded)
	{
		if (schemaType is XmlSchemaComplexType xmlSchemaComplexType)
		{
			...
			if (xmlSchemaComplexType.Particle is XmlSchemaSequence xmlSchemaSequence && xmlSchemaSequence.Items.Count == 1 && xmlSchemaSequence.Items[0] is XmlSchemaAny)
			{
	...
	public string ExportAnyType(string? ns)
	{
		...
		if (xmlSchema != null)
		{
			while (true)
			{
				...
				DefaultInterpolatedStringHandler handler = new DefaultInterpolatedStringHandler(3, 1, invariantCulture);
				handler.AppendLiteral("any");
	...
	{
		XmlSchema xmlSchema = _schemas[ns] ?? AddSchema(ns);
	...
	private void AddSchemaImport(string ns, string referencingNs)
	{
		...
		XmlSchema xmlSchema = _schemas[referencingNs] ?? AddSchema(referencingNs);
	...
	private static XmlSchemaImport FindImport(XmlSchema schema, string ns)
	{
		...
		{
			if (include is XmlSchemaImport xmlSchemaImport && NamespacesEqual(xmlSchemaImport.Namespace, ns))
			{
	...
	private void ExportElementMapping(XmlSchemaElement element, Mapping mapping, string ns, bool isAny)
	{
		...
		if (mapping is PrimitiveMapping primitiveMapping)
		{
	...
	private XmlSchemaType ExportSpecialMapping(SpecialMapping mapping, string ns, bool isAny, XmlSchemaElement element)
	{
		...
		case TypeKind.Serializable:
		{
			...
			if (serializableMapping.Schema != null)
			{
				...
				xmlSchemaAny2.Namespace = targetNamespace ?? "";
	...
	private XmlSchemaSimpleType ExportAnonymousPrimitiveMapping(PrimitiveMapping mapping)
	{
		...
		throw new InvalidOperationException(System.SR.Format(System.SR.XmlInternalErrorDetails, $"Unsupported anonymous mapping type: {mapping}"));
	}
	...
	private void ExportAttributeAccessor(XmlSchemaComplexType type, AttributeAccessor accessor, bool valueTypeOptional, string ns)
	{
		...
		if (accessor.Any)
		{
			...
			else if (content is XmlSchemaComplexContentRestriction xmlSchemaComplexContentRestriction)
			{
		...
		if (accessor.Mapping is PrimitiveMapping primitiveMapping)
		{
			if (primitiveMapping.IsList)
			{
				...
				if (primitiveMapping.IsAnonymousType)
				{
					xmlSchemaSimpleTypeList.ItemType = ExportAnonymousPrimitiveMapping(primitiveMapping);
				}
				else
				{
					xmlSchemaSimpleTypeList.ItemTypeName = ExportPrimitiveMapping(primitiveMapping, accessor.Namespace ?? ns);
				}
			...
			{
				xmlSchemaAttribute2.SchemaType = ExportAnonymousPrimitiveMapping(primitiveMapping);
			}
			else
			{
				xmlSchemaAttribute2.SchemaTypeName = ExportPrimitiveMapping(primitiveMapping, accessor.Namespace ?? ns);
			}
		}
	...
	internal static string ExportDefaultValue(TypeMapping mapping, object value)
	{
		...
		if (mapping is EnumMapping { Constants: var constants } enumMapping)
		{
