...
internal class XmlSerializationILGen
{
	...
	private static readonly string[] s_typeString = new string[1] { "type" };

	private static readonly string[] s_xmlReaderString = new string[1] { "xmlReader" };

	private static readonly string[] s_objectToSerializeWriterString = new string[2] { "objectToSerialize", "writer" };

	private static readonly string[] s_readerString = new string[1] { "reader" };

	private static readonly Type[] s_typeType = new Type[1] { typeof(Type) };

	private static readonly Type[] s_xmlReaderType = new Type[1] { typeof(XmlReader) };

	private static readonly Type[] s_objectXmlSerializationWriterType = new Type[2]
	{
		typeof(object),
		typeof(XmlSerializationWriter)
	};

	private static readonly Type[] s_xmlSerializationReaderType = new Type[1] { typeof(XmlSerializationReader) };
	...
	{
		ilg = new CodeGenerator(serializerContractTypeBuilder);
		ilg.BeginMethod(typeof(bool), "CanSerialize", s_typeType, s_typeString, MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.HideBySig);
	...
	internal string GenerateTypedSerializer(string readMethod, string writeMethod, XmlMapping mapping, CodeIdentifiers classes, string baseSerializer, string readerClass, string writerClass)
	{
		...
		ilg = new CodeGenerator(typeBuilder);
		ilg.BeginMethod(typeof(bool), "CanDeserialize", s_xmlReaderType, s_xmlReaderString, MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.HideBySig);
		...
		{
			ilg = new CodeGenerator(typeBuilder);
			ilg.BeginMethod(typeof(void), "Serialize", s_objectXmlSerializationWriterType, s_objectToSerializeWriterString, MethodAttributes.Family | MethodAttributes.Virtual | MethodAttributes.HideBySig);
		...
		{
			ilg = new CodeGenerator(typeBuilder);
			ilg.BeginMethod(typeof(object), "Deserialize", s_xmlSerializationReaderType, s_readerString, MethodAttributes.Family | MethodAttributes.Virtual | MethodAttributes.HideBySig);
	...
	{
		ilg = new CodeGenerator(serializerContractTypeBuilder);
		ilg.BeginMethod(typeof(XmlSerializer), "GetSerializer", s_typeType, s_typeString, MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.HideBySig);
