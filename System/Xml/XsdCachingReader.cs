...
internal sealed class XsdCachingReader : XmlReader, IXmlLineInfo
{
	...
	private ValidatingReaderNodeData AddAttribute(int attIndex)
	{
		...
		ref ValidatingReaderNodeData reference = ref _attributeEvents[attIndex];
		return reference ?? (reference = new ValidatingReaderNodeData(XmlNodeType.Attribute));
	}
	...
	private ValidatingReaderNodeData AddContent(XmlNodeType nodeType)
	{
		...
		ref ValidatingReaderNodeData reference = ref _contentEvents[_contentIndex];
		validatingReaderNodeData = reference ?? (reference = new ValidatingReaderNodeData(nodeType));
		_contentIndex++;
		return validatingReaderNodeData;
	}
