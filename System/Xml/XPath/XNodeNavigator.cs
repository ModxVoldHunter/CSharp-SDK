...
internal sealed class XNodeNavigator : XPathNavigator, IXmlLineInfo
{
	...
	private static ReadOnlySpan<int> ElementContentMasks => RuntimeHelpers.CreateSpan<int>((RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);
	...
	public override bool MoveToFirstNamespace(XPathNamespaceScope scope)
	{
		if (_source is XElement xElement)
		{
			...
			case XPathNamespaceScope.All:
				xAttribute = GetFirstNamespaceDeclarationGlobal(xElement) ?? GetXmlNamespaceDeclaration();
				break;
			}
			if (xAttribute != null)
			{
	...
	public override bool MoveToNext()
	{
		if (_source is XNode xNode)
		{
			...
			if (parent != null)
			{
				...
				while (xNode2 != null)
				{
					XNode nextNode = xNode2.NextNode;
					if (nextNode == null)
					{
						break;
					}
					if (IsContent(parent, nextNode) && (!(xNode2 is XText) || !(nextNode is XText)))
					{
						_source = nextNode;
						return true;
					}
