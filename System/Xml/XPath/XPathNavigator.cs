...
public abstract class XPathNavigator : XPathItem, ICloneable, IXPathNavigable, IXmlNamespaceResolver
{
	...
	internal virtual string UniqueId
	{
		get
		{
			...
			stringBuilder.Append("REANTSWPCX"[(int)NodeType]);
			while (true)
			{
				...
				if (num <= 31)
				{
					stringBuilder.Append("ABCDEFGHIJKLMNOPQRSTUVWXYZ123456"[(int)num]);
					continue;
				}
				stringBuilder.Append('0');
				do
				{
					stringBuilder.Append("ABCDEFGHIJKLMNOPQRSTUVWXYZ123456"[(int)(num & 0x1F)]);
					num >>= 5;
				}
				while (num != 0);
				stringBuilder.Append('0');
	...
	public virtual bool CheckValidity(XmlSchemaSet schemas, ValidationEventHandler validationEventHandler)
	{
		...
		XPathNavigatorReader reader = CreateReader();
	...
	{
		XmlTextReader newNode2 = CreateContextReader(newNode, fromCurrentNode: false);
		ReplaceSelf(newNode2);
	}

	public virtual void ReplaceSelf(XmlReader newNode)
	{
		ArgumentNullException.ThrowIfNull(newNode, "newNode");
		XPathNodeType nodeType = NodeType;
	...
	{
		ArgumentNullException.ThrowIfNull(newChild, "newChild");
	...
	{
		ArgumentNullException.ThrowIfNull(newSibling, "newSibling");
	...
	{
		ArgumentNullException.ThrowIfNull(xml, "xml");
		XPathNavigator xPathNavigator = CreateNavigator();
