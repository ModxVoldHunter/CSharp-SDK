...
public class XmlDataDocument : XmlDocument
{
	...
	{
		DataSetMapper.GetRegion(node, out var rowElem);
	...
	private XmlElement EnsureDocumentElement()
	{
		...
		if (xmlElement == null)
		{
			...
			xmlElement = new XmlBoundElement(string.Empty, text, DataSet.Namespace ?? string.Empty, this);
	...
	private XmlElement GetRowInsertBeforeLocation(DataRow row, XmlNode parentElement)
	{
		DataRow dataRow = row;
		int i;
		for (i = 0; i < row.Table.Rows.Count && row != row.Table.Rows[i]; i++)
		{
		}
		int num = i;
			...
			else
			{
				...
				else
				{
					...
					while (true)
					{
						...
						if (!flag && !Convert.IsDBNull(value))
						{
							XmlBoundElement xmlBoundElement2 = new XmlBoundElement(string.Empty, col.EncodedColumnName, col.Namespace, this);
							xmlBoundElement2.AppendChild(CreateTextNode(col.ConvertObjectToXml(value)));
							XmlNode columnInsertAfterLocation = GetColumnInsertAfterLocation(col, rowElement);
							if (columnInsertAfterLocation != null)
							{
								rowElement.InsertAfter(xmlBoundElement2, columnInsertAfterLocation);
							}
							else if (rowElement.FirstChild != null)
							{
								rowElement.InsertBefore(xmlBoundElement2, rowElement.FirstChild);
							}
							else
							{
								rowElement.AppendChild(xmlBoundElement2);
							}
						}
						break;
					}
	...
	private void OnNodeChanged(object sender, XmlNodeChangedEventArgs args)
	{
		...
		try
		{
			...
			if (DataSetMapper.GetRegion(args.Node, out rowElem))
			{
				SynchronizeRowFromRowElement(rowElem);
			}
		}
	...
	{
		if (DataSetMapper.GetRegion(oldParent, out var rowElem))
		{
			SynchronizeRowFromRowElement(rowElem);
		}
				...
				case DataRowAction.Change:
					...
					num2 = row.GetRecordFromVersion(DataRowVersion.Current);
					{
						foreach (DataColumn column2 in row.Table.Columns)
						{
							...
							{
								FoliateIfDataPointers(elementFromRow);
							}
	...
	private void OnUndeleteRow(DataRow row, XmlElement rowElement)
	{
		...
		if ((rowInsertBeforeLocation = GetRowInsertBeforeLocation(row, xmlElement)) != null)
		{
	...
	{
		DataSetMapper.GetRegion(parent.ParentNode, out var rowElem);
	...
	private void SynchronizeRowFromRowElementEx(XmlBoundElement rowElement, ArrayList rowElemList)
	{
		...
		if (textOnlyColumn != null)
		{
			...
			string attribute;
			if (value.Length == 0 && ((attribute = rowElement.GetAttribute("xsi:nil")) == "1" || attribute == "true"))
			{
		...
		while (flag)
		{
			...
			if (columnSchemaForNode != null && hashtable[columnSchemaForNode] == null)
			{
				...
				string attribute;
				if (value2.Length == 0 && ((attribute = xmlElement.GetAttribute("xsi:nil")) == "1" || attribute == "true"))
				{
		...
		foreach (XmlAttribute attribute2 in rowElement.Attributes)
		{
			DataColumn columnSchemaForNode2 = _mapper.GetColumnSchemaForNode(rowElement, attribute2);
			if (columnSchemaForNode2 != null && hashtable[columnSchemaForNode2] == null)
			{
				hashtable[columnSchemaForNode2] = columnSchemaForNode2;
				SetRowValueFromXmlText(row, columnSchemaForNode2, attribute2.Value);
			}
		}
		foreach (DataColumn column in row.Table.Columns)
		{
	...
	{
		ArrayList arrayList = new ArrayList();
		if (DataSetMapper.GetRegion(node, out var rowElem))
		{
			if (rowElem == node)
			{
	...
	private void OnNodeInsertedInFragment(XmlNode node)
	{
		...
		ArrayList arrayList = new ArrayList();
		OnNonRowElementInsertedInFragment(rowElem, arrayList);
		while (arrayList.Count > 0)
		{
	...
	{
		DataSetMapper.GetRegion(childRowElem.ParentNode, out var rowElem);
		SetNestedParentRegion(childRowElem, rowElem);
	}
