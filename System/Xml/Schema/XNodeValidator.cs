...
internal sealed class XNodeValidator
{
	...
	private void ReplaceSchemaInfo(XObject o, XmlSchemaInfo schemaInfo)
	{
		...
		XmlSchemaInfo actualValue = o.Annotation<XmlSchemaInfo>();
		if (actualValue != null)
		{
			schemaInfos.Add(actualValue);
			o.RemoveAnnotations<XmlSchemaInfo>();
		}
		if (!schemaInfos.TryGetValue(schemaInfo, out actualValue))
		{
			actualValue = schemaInfo;
			schemaInfos.Add(actualValue);
		}
		o.AddAnnotation(actualValue);
	}
