...
public sealed class XmlSchemaValidator
{
	...
	public void AddSchema(XmlSchema schema)
	{
		...
		string text = schema.TargetNamespace ?? string.Empty;
		...
		for (int i = 0; i < schema.ImportedSchemas.Count; i++)
		{
			...
			text = xmlSchema.TargetNamespace ?? string.Empty;
	...
	{
		ArgumentNullException.ThrowIfNull(attributeValue, "attributeValue");
	...
	{
		ArgumentNullException.ThrowIfNull(defaultAttributes, "defaultAttributes");
	...
	{
		ArgumentNullException.ThrowIfNull(elementValue, "elementValue");
	...
	private void ValidateText(string elementStringValue, XmlValueGetter elementValueGetter)
	{
		...
		case XmlSchemaContentType.ElementOnly:
		{
			...
			if (!XmlCharType.IsOnlyWhitespace(text))
			{
	...
	{
		ArgumentNullException.ThrowIfNull(typedValue, "typedValue");
	...
	private object InternalValidateEndElement(XmlSchemaInfo schemaInfo, object typedValue)
	{
		...
		if (elementDecl != null)
		{
			...
			if (HasIdentityConstraints)
			{
				XmlSchemaType xmlSchemaType = memberType ?? elementDecl.SchemaType;
	...
	private object ValidateElementContext(XmlQualifiedName elementName, out bool invalidElementInContext)
	{
		...
		if (_context.NeedValidateChildren)
		{
			...
			while (true)
			{
				obj = _context.ElementDecl.ContentValidator.ValidateElement(xmlQualifiedName, _context, out var errorCode);
				if (obj != null)
				{
					break;
				}
	...
	private object ValidateAtomicValue(object parsedValue, out XmlSchemaSimpleType memberType)
	{
		...
		if (!_context.IsNill)
		{
			...
			if (ex != null)
			{
				string text = (parsedValue as string) ?? XmlSchemaDatatype.ConcatenatedToString(parsedValue);
	...
	private SchemaElementDecl FastGetElementDecl(XmlQualifiedName elementName, object particle)
	{
		...
		if (schemaElementDecl == null && _processContents != XmlSchemaContentProcessing.Skip)
		{
			...
			{
				if (_partialValidationType is XmlSchemaElement xmlSchemaElement2)
				{
				...
				else if (_partialValidationType is XmlSchemaType xmlSchemaType)
				{
	...
	internal static string PrintExpectedElements(ArrayList expected, bool getParticles)
	{
		if (getParticles)
		{
			string value = System.SR.Format(System.SR.Sch_ContinuationString, " ");
