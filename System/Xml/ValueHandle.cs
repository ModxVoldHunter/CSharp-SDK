...
internal sealed class ValueHandle
{
	...
	public byte[] ToByteArray()
	{
		...
		if (_type == ValueHandleType.UTF8 && _length % 4 == 0)
		{
			try
			{
				...
				int bytes = DataContractSerializer.Base64Encoding.GetBytes(_bufferReader.Buffer, _offset, _length, array2, 0);
		...
		{
			throw new XmlException(ex2.Message, ex2.InnerException);
		}
	}
	...
	public string GetString()
	{
		...
		case ValueHandleType.Base64:
		{
			...
			return DataContractSerializer.Base64Encoding.GetString(array, 0, array.Length);
		}
		case ValueHandleType.List:
	...
	public bool TryReadBase64(byte[] buffer, int offset, int count, out int actual)
	{
		...
		if (_type == ValueHandleType.UTF8 && count >= 3 && _length % 4 == 0)
		{
			try
			{
				...
				actual = DataContractSerializer.Base64Encoding.GetBytes(_bufferReader.Buffer, _offset, num, buffer, offset);
				_offset += num;
				_length -= num;
				return true;
			}
	...
	public bool TryReadChars(char[] chars, int offset, int count, out int actual)
	{
		...
		int num4 = _length;
		bool flag = false;
		UTF8Encoding validatingUTF = DataContractSerializer.ValidatingUTF8;
		while (true)
		{
			if (num2 > 0 && num4 > 0)
			{
			...
			try
			{
				if (validatingUTF.TryGetChars(buffer.AsSpan(num3, num4), chars.AsSpan(num, num2), out charsWritten))
				{
					num5 = num4;
				}
				else
				{
					Decoder decoder = validatingUTF.GetDecoder();
					num5 = Math.Min(num2, num4);
					charsWritten = decoder.GetChars(buffer, num3, num5, chars, num);
					while (charsWritten == 0)
					{
						...
						charsWritten = decoder.GetChars(buffer, num3 + num5, 1, chars, num);
						num5++;
					}
					num5 = validatingUTF.GetByteCount(chars, num, charsWritten);
				}
			}
			catch (FormatException exception)
			{
				throw XmlExceptionHelper.CreateEncodingException(buffer, num3, num4, exception);
			}
			num3 += num5;
			num4 -= num5;
			num += charsWritten;
