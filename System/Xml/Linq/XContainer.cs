...
public abstract class XContainer : XNode
{
	private sealed class ContentReader
	{
		...
		public bool ReadContentFrom(XContainer rootContainer, XmlReader r)
		{
			...
			case XmlNodeType.EndElement:
			{
				...
				if (currentContainer.content == null)
				{
					currentContainer.content = string.Empty;
				}
		...
		public async ValueTask<bool> ReadContentFromAsync(XContainer rootContainer, XmlReader r)
		{
			...
			case XmlNodeType.SignificantWhitespace:
			{
				XContainer currentContainer2 = _currentContainer;
				currentContainer2.AddStringSkipNotify(await r.GetValueAsync().ConfigureAwait(continueOnCapturedContext: false));
				break;
			}
			case XmlNodeType.CDATA:
			{
				XContainer currentContainer2 = _currentContainer;
				currentContainer2.AddNodeSkipNotify(new XCData(await r.GetValueAsync().ConfigureAwait(continueOnCapturedContext: false)));
				break;
			}
			case XmlNodeType.Comment:
			{
				XContainer currentContainer2 = _currentContainer;
				currentContainer2.AddNodeSkipNotify(new XComment(await r.GetValueAsync().ConfigureAwait(continueOnCapturedContext: false)));
				break;
			}
			case XmlNodeType.ProcessingInstruction:
			{
				...
				currentContainer2.AddNodeSkipNotify(new XProcessingInstruction(localName, await r.GetValueAsync().ConfigureAwait(continueOnCapturedContext: false)));
				break;
			}
			case XmlNodeType.DocumentType:
			{
				...
				currentContainer2.AddNodeSkipNotify(new XDocumentType(localName, attribute, attribute2, await r.GetValueAsync().ConfigureAwait(continueOnCapturedContext: false)));
				break;
			}
			case XmlNodeType.EntityReference:
				if (!r.CanResolveEntity)
	...
	internal void ReadContentFrom(XmlReader r, LoadOptions o)
	{
		...
		while (contentReader.ReadContentFromContainer(this, r) && r.Read())
		{
		}
	}
	...
	internal async Task ReadContentFromAsync(XmlReader r, LoadOptions o, CancellationToken cancellationToken)
	{
		...
		{
			cancellationToken.ThrowIfCancellationRequested();
			flag = await cr.ReadContentFromContainerAsync(this, r).ConfigureAwait(continueOnCapturedContext: false);
			if (flag)
			{
				flag = await r.ReadAsync().ConfigureAwait(continueOnCapturedContext: false);
			}
