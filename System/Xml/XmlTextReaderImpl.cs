...
internal sealed class XmlTextReaderImpl : XmlReader, IXmlLineInfo, IXmlNamespaceResolver
{
	...
	internal DtdProcessing DtdProcessing
	{
		...
		set
		{
			ArgumentOutOfRangeException.ThrowIfGreaterThan((uint)value, 2u, "value");
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegative(i, "i");
		ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(i, _attrCount, "i");
	...
	private void InitStreamInput(Uri baseUri, string baseUriStr, Stream stream, byte[] bytes, int byteCount, Encoding encoding)
	{
		...
		{
			num = ((_laterInitParam == null || !_laterInitParam.useAsync) ? XmlReader.CalcBufferSize(stream) : 32768);
		...
		if (_ps.bytesUsed < 4 && _ps.bytes.Length - _ps.bytesUsed > 0)
		{
			...
			int num3 = stream.ReadAtLeast(_ps.bytes.AsSpan(_ps.bytesUsed), num2, throwOnEndOfStream: false);
			if (num3 < num2)
			{
	...
	private bool ParseXmlDeclaration(bool isTextDecl)
	{
		do
		{
			...
			if (!_ps.chars.AsSpan(_ps.charPos).StartsWith("<?xml") || XmlCharType.IsNameSingleChar(_ps.chars[_ps.charPos + 5]))
			{
				break;
			}
			if (!isTextDecl)
			...
			while (true)
			{
				...
				switch (_ps.chars.AsSpan(_ps.charPos, num3 - _ps.charPos))
				{
				case "version":
					if (num == 0)
					{
				...
				case 2:
				{
					Span<char> span = _ps.chars.AsSpan(_ps.charPos, i - _ps.charPos);
					if (!span.SequenceEqual("yes".AsSpan()))
					{
						if (span.SequenceEqual("no".AsSpan()))
						{
							_standalone = false;
						}
						else
					...
					case '!':
						...
						if (chars[charPos] == '[')
						{
							if (_fragmentType != XmlNodeType.Document)
							{
								...
								if (chars.AsSpan(charPos).StartsWith("CDATA["))
								{
	...
	private void ParseElement()
	{
		...
		else
		{
			...
			if (num2 == _lastPrefix.Length && chars.AsSpan(charPos).StartsWith(_lastPrefix))
			{
	...
	private void AddDefaultAttributesAndNormalize()
	{
		...
		{
			if (AddDefaultAttributeDtd(item, array) && DtdValidation && _standalone && item.IsDeclaredInExternal)
			{
	...
	private void ParseEndElement()
	{
		...
		{
			if (!chars.AsSpan(_ps.charPos).StartsWith(nodeData.localName))
			{
		...
		else
		{
			...
			if (!chars.AsSpan(_ps.charPos).StartsWith(nodeData.prefix) || chars[num2] != ':' || !chars.AsSpan(num2 + 1).StartsWith(nodeData.localName))
			{
				ThrowTagMismatch(nodeData);
			}
	...
	private void OnXmlReservedAttribute(NodeData attr)
	{
		...
		ReadOnlySpan<char> span = attr.StringValue.AsSpan().Trim(XmlConvert.WhitespaceChars);
		if (!span.SequenceEqual("preserve".AsSpan()))
		{
	...
	private bool ParseText()
	{
		...
		if (_parsingMode != 0)
		{
			while (!ParseText(out endPos, out startPos, ref outOrChars))
			{
			}
		}
		else
		{
			_curNode.SetLineInfo(_ps.LineNo, _ps.LinePos);
			if (ParseText(out var startPos2, out var endPos2, ref outOrChars))
			{
				if (endPos2 - startPos2 != 0)
				{
					XmlNodeType textNodeType = GetTextNodeType(outOrChars);
					if (textNodeType != 0)
					{
						_curNode.SetValueNode(textNodeType, _ps.chars, startPos2, endPos2 - startPos2);
						return true;
					}
				}
			}
			else if (_v1Compat)
			{
				do
				{
					if (endPos2 - startPos2 > 0)
					{
						_stringBuilder.Append(_ps.chars, startPos2, endPos2 - startPos2);
					}
				}
				while (!ParseText(out startPos2, out endPos2, ref outOrChars));
				if (endPos2 - startPos2 > 0)
				{
					_stringBuilder.Append(_ps.chars, startPos2, endPos2 - startPos2);
				}
			...
			else
			{
				if (outOrChars > 32)
				{
					_curNode.SetValueNode(XmlNodeType.Text, _ps.chars, startPos2, endPos2 - startPos2);
					_nextParsingFunction = _parsingFunction;
				...
				do
				{
					flag = ParseText(out startPos2, out endPos2, ref outOrChars);
					if (endPos2 - startPos2 > 0)
					{
						_stringBuilder.Append(_ps.chars, startPos2, endPos2 - startPos2);
					}
		...
		else
		{
			...
			if (piInDtdStringBuilder == null)
			{
				if (_ignorePIs || _parsingMode != 0)
				{
					int outStartPos2;
					int outEndPos2;
					while (!ParsePIValue(out outStartPos2, out outEndPos2))
					{
					}
					return false;
				}
	...
	private void ParseCDataOrComment(XmlNodeType type)
	{
		...
		{
			_curNode.SetLineInfo(_ps.LineNo, _ps.LinePos);
			if (ParseCDataOrComment(type, out var outStartPos, out var outEndPos))
			{
		...
		else
		{
			int outStartPos2;
			int outEndPos2;
			while (!ParseCDataOrComment(type, out outStartPos2, out outEndPos2))
			{
			}
		}
	}
	...
	private NodeData AllocNode(int nodeIndex, int nodeDepth)
	{
		...
		ref NodeData reference = ref _nodes[nodeIndex];
	...
	private NodeData AddAttribute(int endNamePos, int colonPos)
	{
		...
		if (num != _lastPrefix.Length || !_ps.chars.AsSpan(_ps.charPos).StartsWith(_lastPrefix))
		{
		...
		while (true)
		{
			...
			while (true)
			{
				...
				case IncrementalReadState.PI:
					if (ParsePIValue(out outStartPos, out i))
					{
						_ps.charPos -= 2;
						_incReadState = IncrementalReadState.Text;
					}
				...
				case IncrementalReadState.Comment:
					if (ParseCDataOrComment(XmlNodeType.Comment, out outStartPos, out i))
					{
						_ps.charPos -= 3;
						_incReadState = IncrementalReadState.Text;
					}
				...
				case IncrementalReadState.CDATA:
					if (ParseCDataOrComment(XmlNodeType.CDATA, out outStartPos, out i))
					{
						_ps.charPos -= 3;
						_incReadState = IncrementalReadState.Text;
					}
							...
							else
							{
								if (c2 == '<')
								{
									...
									if (c3 != '!')
									{
										...
										case '/':
										{
											int num5 = ParseQName(isQName: true, 2, out colonPos);
											string nameWPrefix = _curNode.GetNameWPrefix(_nameTable);
											if (num5 - _ps.charPos - 2 != nameWPrefix.Length || !chars.AsSpan(_ps.charPos + 2).StartsWith(nameWPrefix) || (_ps.chars[num5] != '>' && !XmlCharType.IsWhiteSpace(_ps.chars[num5])))
											{
												i = num5;
												outStartPos = _ps.charPos;
												chars = _ps.chars;
										...
										default:
										{
											...
											if (num4 - _ps.charPos - 1 == _curNode.localName.Length && _ps.chars.AsSpan(_ps.charPos + 1).StartsWith(_curNode.localName) && (_ps.chars[num4] == '>' || _ps.chars[num4] == '/' || XmlCharType.IsWhiteSpace(_ps.chars[num4])))
											{
												_incReadDepth++;
									...
									else
									{
										...
										else
										{
											...
											if (!chars.AsSpan(i + 2).StartsWith("[CDATA["))
											{
												continue;
											}
											i += 9;
	...
	public override async Task<int> ReadValueChunkAsync(char[] buffer, int index, int count)
	{
		...
		int readCount = 0;
		int num = _curNode.CopyTo(_readValueOffset, buffer, index + readCount, count - readCount);
		readCount += num;
		_readValueOffset += num;
		if (readCount == count)
		{
		...
		if (_incReadState == IncrementalReadState.ReadValueChunk_OnPartialValue)
		{
			...
			while (readCount < count && !flag)
			{
				...
				BlockCopyChars(_ps.chars, num2, buffer, index + readCount, num4);
				readCount += num4;
				num2 += num4;
			}
			_incReadState = (flag ? IncrementalReadState.ReadValueChunk_OnCachedValue : IncrementalReadState.ReadValueChunk_OnPartialValue);
	...
	private async Task InitStreamInputAsync(Uri baseUri, string baseUriStr, Stream stream, byte[] bytes, int byteCount, Encoding encoding)
	{
		...
		if (_ps.bytesUsed < 4 && _ps.bytes.Length - _ps.bytesUsed > 0)
		{
			...
			int num2 = await stream.ReadAtLeastAsync(_ps.bytes.AsMemory(_ps.bytesUsed), bytesToRead, throwOnEndOfStream: false).ConfigureAwait(continueOnCapturedContext: false);
			if (num2 < bytesToRead)
			{
				_ps.isStreamEof = true;
			}
	...
	private Task<int> InitTextReaderInputAsync(string baseUriStr, Uri baseUri, TextReader input)
	{
		...
		if (_ps.chars == null)
		{
			int num = ((_laterInitParam == null || !_laterInitParam.useAsync) ? 4096 : 32768);
			_ps.chars = new char[num + 1];
		}
		_ps.encoding = Encoding.Unicode;
		_ps.eolNormalized = !_normalize;
	...
	private async Task<bool> ParseXmlDeclarationAsync(bool isTextDecl)
	{
		do
		{
			...
			while (true)
			{
				...
				NodeData attr = null;
				switch (_ps.chars.AsSpan(_ps.charPos, num2 - _ps.charPos))
				{
				case "version":
					if (xmlDeclState == 0)
					{
				...
				case 2:
				{
					Span<char> span = _ps.chars.AsSpan(_ps.charPos, pos - _ps.charPos);
					if (!span.SequenceEqual("yes".AsSpan()))
					{
						if (span.SequenceEqual("no".AsSpan()))
						{
	...
	private Task ParseElementAsync_SetElement(int colonPos, int pos)
	{
		...
		else
		{
			int charPos = _ps.charPos;
			int num = colonPos - charPos;
			if (num == _lastPrefix.Length && chars.AsSpan(charPos).StartsWith(_lastPrefix))
			{
	...
	private Task ParseEndElementAsync_CheckNameAndParse()
	{
		...
		else
		{
			...
			if (!chars.AsSpan(_ps.charPos).StartsWith(nodeData.prefix) || chars[num] != ':' || !chars.AsSpan(num + 1).StartsWith(nodeData.localName))
			{
				return ThrowTagMismatchAsync(nodeData);
			}
	...
	private async Task ThrowTagMismatchAsync(NodeData startTag)
	{
		if (startTag.type == XmlNodeType.Element)
		{
			int item = (await ParseQNameAsync().ConfigureAwait(continueOnCapturedContext: false)).Item2;
			Throw(args: new string[4]
			{
				startTag.GetNameWPrefix(_nameTable),
				startTag.lineInfo.lineNo.ToString(CultureInfo.InvariantCulture),
	...
	private async Task<bool> ParsePIAsync(StringBuilder piInDtdStringBuilder)
	{
		...
		else
		{
			...
			if (piInDtdStringBuilder == null)
			{
				if (_ignorePIs || _parsingMode != 0)
				{
					while (!(await ParsePIValueAsync().ConfigureAwait(continueOnCapturedContext: false)).Item3)
					{
					}
					return false;
				}
	...
	private async Task ParseCDataOrCommentAsync(XmlNodeType type)
	{
		...
		{
			while (!(await ParseCDataOrCommentTupleAsync(type).ConfigureAwait(continueOnCapturedContext: false)).Item3)
			{
			}
		}
	}
		...
		while (true)
		{
			...
			if (_incReadState == IncrementalReadState.ReadContentAsBinary_OnPartialValue)
			{
				...
				while (!_incReadDecoder.IsFull && !flag)
				{
					int outOrChars = 0;
					_incReadLineInfo.Set(_ps.LineNo, _ps.LinePos);
					(num, num2, _, flag) = await ParseTextAsync(outOrChars).ConfigureAwait(continueOnCapturedContext: false);
					try
					{
