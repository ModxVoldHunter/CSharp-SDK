...
internal sealed class XmlBinaryReader : XmlBaseReader, IXmlBinaryReaderInitializer
{
	...
	private int ReadArray(bool[] array, int offset, int count)
	{
		...
		base.BufferReader.ReadRawArrayBytes(array.AsSpan(offset, num));
	...
	private int ReadArray(DateTime[] array, int offset, int count)
	{
		CheckArray(array, offset, count);
		int num = Math.Min(count, _arrayCount);
		base.BufferReader.GetBuffer(num * 8, out var _, out var _);
		Span<DateTime> span = array.AsSpan(offset, num);
		for (int i = 0; i < span.Length; i++)
		{
			span[i] = base.BufferReader.ReadDateTime();
		}
		SkipArrayElements(num);
		return num;
	}
	...
	private int ReadArray(Guid[] array, int offset, int count)
	{
		...
		{
			base.BufferReader.GetBuffer(num * 16, out var _, out var _);
			Span<Guid> span = array.AsSpan(offset, num);
			for (int i = 0; i < span.Length; i++)
			{
				span[i] = base.BufferReader.ReadGuid();
			}
		}
		SkipArrayElements(num);
		return num;
	...
	private int ReadArray(TimeSpan[] array, int offset, int count)
	{
		...
		int num = Math.Min(count, _arrayCount);
		base.BufferReader.GetBuffer(num * 8, out var _, out var _);
		Span<TimeSpan> span = array.AsSpan(offset, num);
		for (int i = 0; i < span.Length; i++)
		{
			span[i] = base.BufferReader.ReadTimeSpan();
		}
		SkipArrayElements(num);
		return num;
	}
