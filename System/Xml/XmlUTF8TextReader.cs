...
internal sealed class XmlUTF8TextReader : XmlBaseReader, IXmlLineInfo, IXmlTextReaderInitializer
{
	...
	{
		while (!base.BufferReader.EndOfFile && (CharTypeMap[base.BufferReader.GetByte()] & 4u) != 0)
		{
	...
	private static int ReadAttributeText(byte[] buffer, int offset, int offsetMax)
	{
		...
		int num = offset;
		while (offset < offsetMax && (charTypeMap[buffer[offset]] & 0x10u) != 0)
		{
			offset++;
		}
		return offset - num;
	...
	private static int ReadWhitespace(byte[] buffer, int offset, int offsetMax)
	{
		ReadOnlySpan<byte> charTypeMap = CharTypeMap;
		int num = offset;
		while (offset < offsetMax && (charTypeMap[buffer[offset]] & 0x20u) != 0)
		{
			offset++;
		}
		return offset - num;
	...
	private static int ReadText(byte[] buffer, int offset, int offsetMax)
	{
		ReadOnlySpan<byte> charTypeMap = CharTypeMap;
		int num = offset;
		while (offset < offsetMax && (charTypeMap[buffer[offset]] & 8u) != 0)
		{
			offset++;
		}
		return offset - num;
	...
	private int ReadTextAndWatchForInvalidCharacters(byte[] buffer, int offset, int offsetMax)
	{
		ReadOnlySpan<byte> charTypeMap = CharTypeMap;
		int num = offset;
		while (offset < offsetMax && ((charTypeMap[buffer[offset]] & 8u) != 0 || buffer[offset] == 239))
		{
	...
	{
		int num = ReadCharRef();
		if (num < 256 && (CharTypeMap[num] & 4u) != 0)
		{
	...
	public override bool Read()
	{
		...
		else if ((CharTypeMap[@byte] & 0x20u) != 0)
		{
			ReadWhitespace();
		}
