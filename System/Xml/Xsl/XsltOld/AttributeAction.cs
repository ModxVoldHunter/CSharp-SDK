...
internal sealed class AttributeAction : ContainerAction
{
	...
	private static PrefixQName CreateAttributeQName(string name, string nsUri, InputScopeManager manager)
	{
		...
		prefixQName.Namespace = nsUri ?? manager.ResolveXPathNamespace(prefixQName.Prefix);
	...
	internal override void Compile(Compiler compiler)
	{
		...
		CompiledAction.CheckRequiredAttribute(_nameAvt, "name");
	...
	internal override void Execute(Processor processor, ActionFrame frame)
	{
		...
		case 0:
			...
			{
				frame.CalculatedName = CreateAttributeQName((_nameAvt == null) ? _name : _nameAvt.Evaluate(processor, frame), (_nsAvt == null) ? _nsUri : _nsAvt.Evaluate(processor, frame), _manager);
		...
		case 2:
		{
			PrefixQName calculatedName = frame.CalculatedName;
			if (!processor.BeginEvent(XPathNodeType.Attribute, calculatedName.Prefix, calculatedName.Name, calculatedName.Namespace, empty: false))
			{
				frame.State = 2;
				break;
			}
