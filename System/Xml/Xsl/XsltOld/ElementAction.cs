...
internal sealed class ElementAction : ContainerAction
{
	...
	internal override void Execute(Processor processor, ActionFrame frame)
	{
		...
		case 0:
			...
			{
				frame.CalculatedName = CreateElementQName((_nameAvt == null) ? _name : _nameAvt.Evaluate(processor, frame), (_nsAvt == null) ? _nsUri : _nsAvt.Evaluate(processor, frame), _manager);
			}
			goto case 2;
		case 2:
		{
			PrefixQName calculatedName = frame.CalculatedName;
			if (!processor.BeginEvent(XPathNodeType.Element, calculatedName.Prefix, calculatedName.Name, calculatedName.Namespace, _empty))
			{
				frame.State = 2;
				return;
			}
