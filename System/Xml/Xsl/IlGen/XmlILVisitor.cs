...
internal sealed class XmlILVisitor : QilVisitor
{
	...
	protected override QilNode VisitAnd(QilBinary ndAnd)
	{
		...
		EndConjunctiveTests(iterCurr.CurrentBranchingContext, lblOnFalse);
		return ndAnd;
	}
	...
	private bool MatchesNodeKinds(QilTargetType ndIsType, XmlQueryType typDerived, XmlQueryType typBase)
	{
		...
		if (!BitOperations.IsPow2((uint)xmlNodeKindFlags))
		{
	...
	[UnconditionalSuppressMessage("ReflectionAnalysis", "IL2072:RequiresUnreferencedCode", Justification = "Suppressing warning about not having the RequiresUnreferencedCode attribute since we added the attribute to this subclass' constructor. This allows us to not have to annotate the whole QilNode hirerarchy.")]
