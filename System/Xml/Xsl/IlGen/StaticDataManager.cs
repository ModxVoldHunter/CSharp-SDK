...
internal sealed class StaticDataManager
{
	...
	public string[] Names => _uniqueNames?.ToArray();

	public Int32Pair[] NameFilters => _uniqueFilters?.ToArray();

	public StringPair[][] PrefixMappingsList => _prefixMappingsList?.ToArray();

	public string[] GlobalNames => _globalNames?.ToArray();

	public EarlyBoundInfo[] EarlyBound => _earlyInfo?.ToArray();

	public XmlQueryType[] XmlTypes => _uniqueXmlTypes?.ToArray();

	public XmlCollation[] Collations => _uniqueCollations?.ToArray();
