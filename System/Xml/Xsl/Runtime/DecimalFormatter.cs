	...
	public DecimalFormatter(string formatPicture, DecimalFormat decimalFormat)
	{
		...
		for (int i = 0; i < formatPicture.Length; i++)
		{
			...
			else if (c5 == decimalFormat.patternSeparator)
			{
				...
				num4 = RemoveTrailingComma(stringBuilder, num, num2);
				if (num4 > 9)
				{
					num4 = 0;
				}
				_posFormatInfo.NumberGroupSizes = new int[1] { num4 };
				if (!flag5)
				{
			...
			{
				if ((char.IsAsciiDigit(c5) || c5 == '\a') && decimalFormat.zeroDigit != '0')
				{
		...
		numberFormatInfo.NumberGroupSizes = new int[1] { num4 };
		if (!flag5)
		{
