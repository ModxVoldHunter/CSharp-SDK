...
internal sealed class XmlEncodedRawTextWriterIndent : XmlEncodedRawTextWriter
{
	...
	[MemberNotNull("_mixedContentStack")]
	private void Init(XmlWriterSettings settings)
	{
		...
		if (_newLineOnAttributes)
		{
			XmlEncodedRawTextWriter.ValidateContentChars(_indentChars, "IndentChars", allowOnlyWhitespace: true);
			XmlEncodedRawTextWriter.ValidateContentChars(_newLineChars, "NewLineChars", allowOnlyWhitespace: true);
			return;
		}
		XmlEncodedRawTextWriter.ValidateContentChars(_indentChars, "IndentChars", allowOnlyWhitespace: false);
		if (_newLineHandling != 0)
		{
			XmlEncodedRawTextWriter.ValidateContentChars(_newLineChars, "NewLineChars", allowOnlyWhitespace: false);
		}
	}
	...
	internal override async Task WriteStartNamespaceDeclarationAsync(string prefix)
	{
		...
		await base.WriteStartNamespaceDeclarationAsync(prefix).ConfigureAwait(continueOnCapturedContext: false);
	}
