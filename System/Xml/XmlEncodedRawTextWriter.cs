...
internal class XmlEncodedRawTextWriter : XmlRawWriter
{
	...
	public override Task WriteStartElementAsync(string prefix, string localName, string ns)
	{
		...
		_bufChars[_bufPos++] = '<';
		Task task = (string.IsNullOrEmpty(prefix) ? RawTextAsync(localName) : RawTextAsync(prefix, ":", localName));
	...
	protected Task WriteAttributeTextBlockAsync(string text)
	{
		...
		int num2 = WriteAttributeTextBlockNoFlush(text, num, length);
		num += num2;
		length -= num2;
		if (num2 >= 0)
		{
			return _WriteAttributeTextBlockAsync(text, num, length);
		}
		return Task.CompletedTask;
	}
	...
	protected Task WriteElementTextBlockAsync(string text)
	{
		...
		int length = text.Length;
		bool needWriteNewLine;
		int num2 = WriteElementTextBlockNoFlush(text, num, length, out needWriteNewLine);
		num += num2;
		length -= num2;
		if (needWriteNewLine)
		{
			return _WriteElementTextBlockAsync(newLine: true, text, num, length);
		}
		if (num2 >= 0)
		{
			return _WriteElementTextBlockAsync(newLine: false, text, num, length);
		}
		return Task.CompletedTask;
	}
