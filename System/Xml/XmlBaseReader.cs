...
internal abstract class XmlBaseReader : XmlDictionaryReader
{
	...
	private sealed class NamespaceManager
	{
		...
		public static Namespace XmlNamespace
		{
			get
			{
				...
				{
					byte[] array = "xmlhttp://www.w3.org/XML/1998/namespace"u8.ToArray();
	...
	private sealed class QuotaNameTable : XmlNameTable
	{
		...
		{
			string text = _nameTable.Get(chars, offset, count);
			if (text != null)
			{
				return text;
			}
		...
		{
			string text = _nameTable.Get(value);
			if (text != null)
			{
				return text;
			}
	...
	public override char QuoteChar => _node.QuoteChar;

	public override ReadState ReadState => _node.ReadState;

	public override string Value => _value ?? (_value = _node.ValueAsString);
	...
	{
		ArgumentNullException.ThrowIfNull(quotas, "quotas");
	...
	private XmlAttributeNode GetAttributeNode(int index)
	{
		if (!_node.CanGetAttribute)
		{
			throw new ArgumentOutOfRangeException("index", System.SR.XmlElementAttributes);
		}
		ArgumentOutOfRangeException.ThrowIfNegative(index, "index");
		if (index >= _attributeCount)
		{
			throw new ArgumentOutOfRangeException("index", System.SR.Format(System.SR.OffsetExceedsBufferSize, _attributeCount));
		}
	...
	private void CheckAttributes(XmlAttributeNode[] attributeNodes, int attributeCount)
	{
		...
		{
			_attributeSorter.GetIndices(out var attributeIndex, out var attributeIndex2);
		...
		if (_node.IsNamespaceUri(namespaceUri))
		{
			...
			else
			{
				...
				for (int j = 0; j < localNames.Length; j++)
				{
					...
					if (text2 == null)
					{
						throw new ArgumentNullException($"localNames[{j}]");
					}
					if (prefix == text2)
					{
						return j;
	...
	public override int ReadValueAsBase64(byte[] buffer, int offset, int count)
	{
		...
		return ReadBytes(DataContractSerializer.Base64Encoding, 3, 4, buffer, offset, Math.Min(count, 512), readContent: false);
	}
	...
	public override void ReadEndElement()
	{
		if (_node.NodeType != XmlNodeType.EndElement && MoveToContent() != XmlNodeType.EndElement)
		{
			...
			if (num == 0)
			{
				throw new InvalidOperationException(System.SR.XmlEndElementNoOpenNodes);
			}
	...
	public override int ReadContentAsBase64(byte[] buffer, int offset, int count)
	{
		...
		return ReadBytes(DataContractSerializer.Base64Encoding, 3, 4, buffer, offset, Math.Min(count, 512), readContent: true);
	}
	...
	public override int ReadContentAsBinHex(byte[] buffer, int offset, int count)
	{
		...
		return ReadBytes(DataContractSerializer.BinHexEncoding, 1, 2, buffer, offset, Math.Min(count, 512), readContent: true);
	}
	...
	private int ReadBytes(Encoding encoding, int byteBlock, int charBlock, byte[] buffer, int offset, int byteCount, bool readContent)
	{
		...
		while (true)
		{
			...
			{
				int num6 = XmlConverter.StripWhitespace(charBuffer.AsSpan(0, num3));
				if (num6 == num3)
				{
