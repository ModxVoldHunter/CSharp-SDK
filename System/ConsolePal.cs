...
internal static class ConsolePal
{
	...
	private sealed class WindowsConsoleStream : ConsoleStream
	{
		...
		private unsafe static int WriteFileNative(nint hFile, ReadOnlySpan<byte> bytes, bool useFileAPIs)
		{
			if (bytes.IsEmpty)
			{
				return 0;
	...
	private static bool IsKeyDownEvent(global::Interop.INPUT_RECORD ir)
	{
		if (ir.EventType == 1)
		{
			return ir.keyEvent.bKeyDown != global::Interop.BOOL.FALSE;
		}
		return false;
	}

	private static bool IsModKey(global::Interop.INPUT_RECORD ir)
	{
		ushort wVirtualKeyCode = ir.keyEvent.wVirtualKeyCode;
		if ((wVirtualKeyCode < 16 || wVirtualKeyCode > 18) && wVirtualKeyCode != 20 && wVirtualKeyCode != 144)
		{
			return wVirtualKeyCode == 145;
		}
		return true;
	}

	private static bool IsAltKeyDown(global::Interop.INPUT_RECORD ir)
	{
		return (ir.keyEvent.dwControlKeyState & 3) != 0;
	}
	...
	public static ConsoleKeyInfo ReadKey(bool intercept)
	{
		...
		lock (s_readKeySyncObject)
		{
			...
			else
			{
				while (true)
				{
					if (!global::Interop.Kernel32.ReadConsoleInput(InputHandle, out buffer, 1, out var numEventsRead) || numEventsRead == 0)
					{
						throw new InvalidOperationException(System.SR.InvalidOperation_ConsoleReadKeyOnFile);
					}
					ushort wVirtualKeyCode = buffer.keyEvent.wVirtualKeyCode;
					if ((!IsKeyDownEvent(buffer) && wVirtualKeyCode != 18) || (buffer.keyEvent.uChar == '\0' && IsModKey(buffer)))
					{
						continue;
					}
		...
		ControlKeyState dwControlKeyState = (ControlKeyState)buffer.keyEvent.dwControlKeyState;
		bool shift = (dwControlKeyState & ControlKeyState.ShiftPressed) != 0;
		bool alt = (dwControlKeyState & (ControlKeyState.RightAltPressed | ControlKeyState.LeftAltPressed)) != 0;
		bool control = (dwControlKeyState & (ControlKeyState.RightCtrlPressed | ControlKeyState.LeftCtrlPressed)) != 0;
