...
public sealed class MetadataReaderProvider : IDisposable
{
	...
	public static MetadataReaderProvider FromMetadataStream(Stream stream, MetadataStreamOptions options = MetadataStreamOptions.Default, int size = 0)
	{
		...
		try
		{
			...
			{
				result = new MetadataReaderProvider(new StreamMemoryBlockProvider(stream, position, andValidateSize, (options & MetadataStreamOptions.LeaveOpen) != 0));
				flag = false;
			}
			else
			{
				result = new MetadataReaderProvider(StreamMemoryBlockProvider.ReadMemoryBlockNoLock(stream, position, andValidateSize));
			}
		}
		finally
		{
