...
public class CustomAttributeBuilder
{
	...
	public CustomAttributeBuilder(ConstructorInfo con, object?[] constructorArgs, PropertyInfo[] namedProperties, object?[] propertyValues, FieldInfo[] namedFields, object?[] fieldValues)
	{
		...
		ArgumentNullException.ThrowIfNull(constructorArgs, "constructorArgs");
		ArgumentNullException.ThrowIfNull(namedProperties, "namedProperties");
		ArgumentNullException.ThrowIfNull(propertyValues, "propertyValues");
		ArgumentNullException.ThrowIfNull(namedFields, "namedFields");
		ArgumentNullException.ThrowIfNull(fieldValues, "fieldValues");
		...
		for (int i = 0; i < namedProperties.Length; i++)
		{
			...
			if (propertyInfo.DeclaringType != con.DeclaringType && !(con.DeclaringType is TypeBuilderInstantiation) && !con.DeclaringType.IsSubclassOf(propertyInfo.DeclaringType) && !RuntimeTypeBuilder.IsTypeEqual(propertyInfo.DeclaringType, con.DeclaringType) && (!(propertyInfo.DeclaringType is TypeBuilder) || !con.DeclaringType.IsSubclassOf(((RuntimeTypeBuilder)propertyInfo.DeclaringType).BakedRuntimeType)))
			{
