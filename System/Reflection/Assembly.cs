...
public abstract class Assembly : ICustomAttributeProvider, ISerializable
{
	...
	{
		ArgumentNullException.ThrowIfNull(partialName, "partialName");
	...
	{
		ArgumentNullException.ThrowIfNull(assemblyRef, "assemblyRef");
		StackCrawlMark stackMark = StackCrawlMark.LookForMyCaller;
		return RuntimeAssembly.InternalLoad(assemblyRef, ref stackMark, AssemblyLoadContext.CurrentContextualReflectionContext);
	}

	[DllImport("QCall", EntryPoint = "AssemblyNative_GetExecutingAssembly", ExactSpelling = true)]
	[LibraryImport("QCall", EntryPoint = "AssemblyNative_GetExecutingAssembly")]
	...
	[DllImport("QCall", EntryPoint = "AssemblyNative_GetEntryAssembly", ExactSpelling = true)]
	[LibraryImport("QCall", EntryPoint = "AssemblyNative_GetEntryAssembly")]
	...
	[DllImport("QCall", EntryPoint = "AssemblyNative_GetAssemblyCount", ExactSpelling = true)]
	[LibraryImport("QCall", EntryPoint = "AssemblyNative_GetAssemblyCount")]
	...
	{
		ArgumentNullException.ThrowIfNull(rawAssembly, "rawAssembly");
	...
	public static Assembly LoadFile(string path)
	{
		...
		lock (s_loadfile)
		{
			...
			{
				throw new FileNotFoundException(SR.Format(SR.FileNotFound_LoadFile, fullPath), fullPath);
			}
	...
	private static Assembly LoadFromResolveHandler(object sender, ResolveEventArgs args)
	{
		...
		{
			if (!s_loadFromAssemblyList.Contains(location))
			{
				if (AssemblyLoadContext.IsTracingEnabled())
				{
					AssemblyLoadContext.TraceAssemblyLoadFromResolveHandlerInvoked(args.Name, isTrackedAssembly: false, location, null);
				}
				return null;
			}
		}
		AssemblyName assemblyName = new AssemblyName(args.Name);
		string text = Path.Combine(Path.GetDirectoryName(location), assemblyName.Name + ".dll");
		if (AssemblyLoadContext.IsTracingEnabled())
		{
			AssemblyLoadContext.TraceAssemblyLoadFromResolveHandlerInvoked(args.Name, isTrackedAssembly: true, location, text);
		}
		try
		{
