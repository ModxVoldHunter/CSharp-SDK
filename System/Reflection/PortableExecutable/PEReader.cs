...
public sealed class PEReader : IDisposable
{
	...
	public unsafe PEReader(Stream peStream, PEStreamOptions options, int size)
	{
		...
		try
		{
			...
			{
				_peImage = new StreamMemoryBlockProvider(peStream, position, andValidateSize, (options & PEStreamOptions.LeaveOpen) != 0);
				flag = false;
			}
			else if ((options & PEStreamOptions.PrefetchEntireImage) != 0)
			{
				NativeHeapMemoryBlock nativeHeapMemoryBlock = (NativeHeapMemoryBlock)(_lazyImageBlock = StreamMemoryBlockProvider.ReadMemoryBlockNoLock(peStream, position, andValidateSize));
	...
	private AbstractMemoryBlock GetPESectionBlock(int index)
	{
		...
		AbstractMemoryBlock abstractMemoryBlock = Volatile.Read(ref _lazyPESectionBlocks[index]);
