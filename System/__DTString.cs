...
internal ref struct __DTString
{
	...
	internal bool MatchSpecifiedWords(string target, bool checkWordBoundary, scoped ref int matchLength)
	{
		...
		if (matchLength > num || m_info.Compare(Value.Slice(Index, matchLength), target, CompareOptions.IgnoreCase) != 0)
		{
			int num2 = 0;
			int num3 = Index;
			int num4 = target.AsSpan(num2).IndexOfAny(" \u00a0\u202f");
			if (num4 < 0)
			{
				return false;
			}
			...
			while (true)
			{
				...
				num4 = target.AsSpan(num2).IndexOfAny(" \u00a0\u202f");
				if (num4 < 0)
				{
					break;
				}
	...
	internal bool GetNextDigit()
	{
		if (++Index < Length)
		{
			return char.IsAsciiDigit(Value[Index]);
		}
		return false;
	}
	...
	internal bool SkipWhiteSpaceAndRtlMarkCurrent()
	{
		...
		if (!char.IsWhiteSpace(m_current) && m_current != '\u200f')
		{
			return true;
		}
		while (++Index < Length)
	...
	internal DTSubString GetSubString()
	{
		...
		while (Index + result.length < Length)
		{
			...
			DTSubStringType dTSubStringType = (char.IsAsciiDigit(c) ? DTSubStringType.Number : DTSubStringType.Other);
			if (result.length == 0)
			{
