...
public sealed class Version : ICloneable, IComparable, IComparable<Version?>, IEquatable<Version?>, ISpanFormattable, IFormattable, IUtf8SpanFormattable
{
	...
	private bool TryFormatCore<TChar>(Span<TChar> destination, int fieldCount, out int charsWritten) where TChar : unmanaged, IUtfChar<TChar>
	{
		...
		for (int i = 0; i < fieldCount; i++)
		{
			...
			{
				uint num3 = (uint)num2;
				num4 = num3.TryFormat(MemoryMarshal.Cast<TChar, byte>(destination), out charsWritten2, default(ReadOnlySpan<char>), CultureInfo.InvariantCulture);
			}
			else
			{
				uint num3 = (uint)num2;
				num4 = num3.TryFormat(MemoryMarshal.Cast<TChar, char>(destination), out charsWritten2);
			}
			if (!num4)
			{
				charsWritten = 0;
