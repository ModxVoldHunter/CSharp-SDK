...
public sealed class CommittableTransaction : Transaction, IAsyncResult
{
		...
		: base(isoLevel, (InternalTransaction)null)
	{
		...
		if (log.IsEnabled())
		{
			log.TransactionCreated(TraceSourceType.TraceSourceLtm, base.TransactionTraceId, "CommittableTransaction");
		}
	}
	...
	public IAsyncResult BeginCommit(AsyncCallback? asyncCallback, object? asyncState)
	{
		...
		if (log.IsEnabled())
		{
			...
			log.TransactionCommit(TraceSourceType.TraceSourceLtm, base.TransactionTraceId, "CommittableTransaction");
		}
		ObjectDisposedException.ThrowIf(base.Disposed, this);
		lock (_internalTransaction)
		{
			if (_complete)
			{
