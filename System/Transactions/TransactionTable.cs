...
internal sealed class TransactionTable
{
	...
	{
		int result = _rwLock.EnterReadLock();
		try
		{
	...
	private void ThreadTimer(object state)
	{
		...
		if (weakReference != null)
		{
			bucketSet2 = (BucketSet)weakReference.Target;
		}
		if (bucketSet2 == null)
		{
			...
			try
			{
				weakReference = (WeakReference)bucketSet.nextSetWeak;
				if (weakReference != null)
				{
					bucketSet2 = (BucketSet)weakReference.Target;
				}
		...
		while (true)
		{
			...
			BucketSet bucketSet3 = bucketSet;
			bucketSet = bucketSet2;
			if (bucketSet.AbsoluteTimeout <= _ticks)
			{
				weakReference2 = (WeakReference)Interlocked.CompareExchange(ref bucketSet3.nextSetWeak, null, weakReference);
				if (weakReference2 == weakReference)
				{
					break;
				}
