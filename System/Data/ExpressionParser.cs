		...
		while (_token != Tokens.EOS)
		{
			while (true)
			{
				...
				case Tokens.Parent:
				{
					...
					case Tokens.Numeric:
					{
						string constant = new string(_text, _start, _pos - _start);
						node = new ConstNode(_table, ValueType.Numeric, constant);
						break;
					}
					case Tokens.Decimal:
					{
						string constant = new string(_text, _start, _pos - _start);
						node = new ConstNode(_table, ValueType.Decimal, constant);
						break;
					}
					case Tokens.Float:
					{
						string constant = new string(_text, _start, _pos - _start);
						node = new ConstNode(_table, ValueType.Float, constant);
						break;
					}
					case Tokens.StringConst:
					{
						string constant = new string(_text, _start + 1, _pos - _start - 2);
						node = new ConstNode(_table, ValueType.Str, constant);
						break;
					}
					case Tokens.Date:
					{
						string constant = new string(_text, _start + 1, _pos - _start - 2);
						node = new ConstNode(_table, ValueType.Date, constant);
						break;
					}
					}
					NodePush(node);
	...
	private void BuildExpression(int pri)
	{
		while (true)
		{
			...
			case Nodes.Binop:
			{
				...
				ExpressionNode expressionNode = NodePop();
				...
				default:
					node = ((operatorInfo._op != 14) ? new BinaryNode(_table, operatorInfo._op, expressionNode, right) : new LikeNode(_table, operatorInfo._op, expressionNode, right));
					break;
				}
				break;
			}
			case Nodes.Unop:
			{
				...
				node = new UnaryNode(_table, operatorInfo._op, right);
				break;
			}
			case Nodes.Zop:
