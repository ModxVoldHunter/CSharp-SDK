...
public sealed class ConstraintCollection : InternalDataCollectionBase
{
	...
	internal int InternalIndexOf(string constraintName)
	{
		...
		if (constraintName != null && 0 < constraintName.Length)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				switch (InternalDataCollectionBase.NamesEqual(constraint.ConstraintName, constraintName, fCaseSensitive: false, _table.Locale))
				{
				case 1:
					return i;
				case -1:
	...
	internal void RegisterName(string name)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (InternalDataCollectionBase.NamesEqual(name, ((Constraint)List[i]).ConstraintName, fCaseSensitive: true, _table.Locale) != 0)
			{
				throw ExceptionBuilder.DuplicateConstraintName(((Constraint)List[i]).ConstraintName);
			}
		}
		if (InternalDataCollectionBase.NamesEqual(name, MakeName(_defaultNameIndex), fCaseSensitive: true, _table.Locale) != 0)
		{
			_defaultNameIndex++;
		}
	}
