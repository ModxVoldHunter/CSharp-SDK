...
internal class BinaryNode : ExpressionNode
{
	...
	internal override ExpressionNode Optimize()
	{
		...
		if (_op == 13)
		{
			if (_right is UnaryNode unaryNode)
			{
				if (unaryNode._op != 3)
				{
	...
	private object EvalBinaryOp(int op, ExpressionNode left, ExpressionNode right, DataRow row, DataRowVersion version, int[] recordNos)
	{
		...
		if (op != 27 && op != 26 && op != 5 && op != 13 && op != 39)
		{
			...
			storageType3 = ((!(flag || flag2)) ? ResultType(storageType, storageType2, left is ConstNode, right is ConstNode, op) : ResultSqlType(storageType, storageType2, op));
	...
	internal static StorageType ResultSqlType(StorageType left, StorageType right, int op)
	{
		...
		DataTypePrecedence dataTypePrecedence = (DataTypePrecedence)Math.Max(precedence, precedence2);
		StorageType storageType = GetPrecedenceType((DataTypePrecedence)SqlResultType((int)dataTypePrecedence));
		...
		if (IsMixedSql(left, right) && ExpressionNode.IsUnsignedSql(storageType))
		{
			if (dataTypePrecedence >= DataTypePrecedence.UInt64)
			{
				throw ExprException.AmbiguousBinop(op, DataStorage.GetTypeStorage(left), DataStorage.GetTypeStorage(right));
			}
			storageType = GetPrecedenceType(dataTypePrecedence + 1);
		}
		return storageType;
	}
