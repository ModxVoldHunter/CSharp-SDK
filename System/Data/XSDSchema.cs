...
internal sealed class XSDSchema : XMLSchema
{
		...
		if (elements.Count == 1 || (FromInference && elements.Count > 0))
		{
			...
			while (xmlSchemaComplexType != null)
			{
				...
				if (xmlSchemaComplexType.ContentModel is XmlSchemaSimpleContent)
				{
					...
					if (content is XmlSchemaSimpleContentExtension xmlSchemaSimpleContentExtension)
					{
	...
	public void LoadSchema(XmlSchemaSet schemaSet, DataSet ds)
	{
		...
		try
		{
			if (enumerator.MoveNext())
			{
				...
				{
					ds._namespaceURI = targetNamespace ?? string.Empty;
				}
			}
		}
		finally
	...
	internal void HandleComplexType(XmlSchemaComplexType ct, DataTable table, ArrayList tableChildren, bool isNillable)
	{
		...
		if (ct.ContentModel != null)
		{
			...
			else
			{
				...
				if (content2 is XmlSchemaSimpleContentExtension xmlSchemaSimpleContentExtension)
				{
	...
	internal static bool GetBooleanAttribute(XmlSchemaAnnotated element, string attrName, bool defVal)
	{
		...
		if (string.IsNullOrEmpty(msdataAttribute))
		{
			return defVal;
		}
	...
	internal void HandleKeyref(XmlSchemaKeyref keyref)
	{
		...
		else
		{
			...
			if (num2 < 0)
			{
				...
				if (FromInference && dataRelation.Nested && _tableDictionary.TryGetValue(dataRelation.ParentTable, out var value))
				{
					value.Add(dataRelation.ChildTable);
				}
	...
	{
		string result = null;
		if (node is XmlSchemaElement xmlSchemaElement)
		{
			result = xmlSchemaElement.Name ?? xmlSchemaElement.RefName.Name;
		}
		else if (node is XmlSchemaAttribute xmlSchemaAttribute)
		{
			result = xmlSchemaAttribute.Name ?? xmlSchemaAttribute.RefName.Name;
		}
		return result;
	}
	...
	internal XmlSchemaAnnotated FindTypeNode(XmlSchemaAnnotated node)
	{
		...
		string value = (flag ? xmlSchemaAttribute.SchemaTypeName.Name : xmlSchemaElement.SchemaTypeName.Name);
		string text = (flag ? xmlSchemaAttribute.SchemaTypeName.Namespace : xmlSchemaElement.SchemaTypeName.Namespace);
		...
		{
			value = (flag ? xmlSchemaAttribute.RefName.Name : xmlSchemaElement.RefName.Name);
			if (string.IsNullOrEmpty(value))
			{
	...
	internal void HandleSimpleTypeSimpleContentColumn(XmlSchemaSimpleType typeNode, string strType, DataTable table, bool isBase, XmlAttribute[] attrs, bool isNillable)
	{
		...
		Type type;
		if (!string.IsNullOrEmpty(typeNode.QualifiedName.Name) && typeNode.QualifiedName.Namespace != "http://www.w3.org/2001/XMLSchema")
		{
	...
	{
		if ((FromInference && table.XmlText != null) || strType == null)
		{
			return;
		}
		Type type = ParseDataType(strType);
		string text;
		if (FromInference)
		{
			int num = 0;
	...
	internal void HandleAttributeColumn(XmlSchemaAttribute attrib, DataTable table, bool isBase)
	{
		...
		else if (xmlSchemaAnnotated is XmlSchemaSimpleType)
		{
			...
			if (!string.IsNullOrEmpty(xmlSchemaSimpleType.QualifiedName.Name) && xmlSchemaSimpleType.QualifiedName.Namespace != "http://www.w3.org/2001/XMLSchema")
			{
	...
	internal void HandleElementColumn(XmlSchemaElement elem, DataTable table, bool isBase)
	{
		...
		else if (xmlSchemaAnnotated is XmlSchemaSimpleType)
		{
			...
			if (!string.IsNullOrEmpty(((XmlSchemaSimpleType)xmlSchemaAnnotated).Name) && ((XmlSchemaSimpleType)xmlSchemaAnnotated).QualifiedName.Namespace != "http://www.w3.org/2001/XMLSchema")
			{
	...
	private string GetTableNamespace(XmlSchemaIdentityConstraint key)
	{
		...
		if (text.Contains(':'))
		{
			...
			return GetNamespaceFromPrefix(name);
		}
