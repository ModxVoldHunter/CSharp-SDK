...
internal sealed class XmlTreeGen
{
	...
	internal void AddXdoProperties(object instance, XmlElement root)
	{
		...
		if (instance is DataSet || instance is DataTable || instance is DataColumn || instance is DataRelation)
		{
			...
			{
				AddXdoProperty(properties[i], instance, root);
			}
		}
	}
	...
	internal void SchemaTree(XmlDocument xd, XmlWriter xmlWriter, DataSet ds, DataTable dt, bool writeHierarchy)
	{
		...
		if (_ds != null)
		{
			WriteSchemaRoot(xmlElement, _ds.Namespace);
		}
		else
		{
			WriteSchemaRoot(xmlElement, dt.Namespace);
		}
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				AppendChildWithoutRef(array[i].Namespace, xmlElement3);
	...
	{
		FileStream fileStream = ((xw is DataTextWriter dataTextWriter) ? (dataTextWriter.BaseStream as FileStream) : null);
		if (fileStream == null)
		{
	...
	private static XmlElement FindSimpleType(XmlElement schema, string name)
	{
		...
		{
			if (xmlNode is XmlElement xmlElement && xmlElement.GetAttribute("name") == name)
			{
				return xmlElement;
			}
		}
	...
	internal XmlElement GetSchema(string NamespaceURI)
	{
		...
		if (xmlElement == null)
		{
			...
			WriteSchemaRoot(xmlElement, NamespaceURI);
	...
	internal XmlElement HandleColumn(DataColumn col, XmlDocument dc, XmlElement schema, bool fWriteOrdinal)
	{
		...
		{
			AddXdoProperties(col, xmlElement);
		}
		else
		{
	...
	internal XmlElement HandleTable(DataTable table, XmlDocument dc, XmlElement schema, bool genNested)
	{
		...
		AddXdoProperties(table, xmlElement);
		...
		if (!table.TypeName.IsEmpty && _schFormat != SchemaFormat.Remoting)
		{
			...
			{
				xmlElement3 = ((_ds != null) ? (flag ? GetSchema(_ds.Namespace) : GetSchema(table.Namespace)) : GetSchema(table.Namespace));
			}
		...
		if (xmlText != null)
		{
			...
			{
				AddXdoProperties(xmlText, xmlElement4);
			}
			else
			{
				AddColumnProperties(xmlText, xmlElement4);
			}
			AddExtendedProperties(xmlText._extendedProperties, xmlElement4);
			...
			HandleColumnType(xmlText, dc, xmlElement5, schema);
		...
		bool fWriteOrdinal = HasMixedColumns(table);
		for (int l = 0; l < count; l++)
		{
			...
			if (dataColumn3.ColumnMapping != MappingType.SimpleContent && (dataColumn3.ColumnMapping == MappingType.Attribute || dataColumn3.ColumnMapping == MappingType.Element || dataColumn3.ColumnMapping == MappingType.Hidden) && !IsAutoGenerated(dataColumn3))
			{
				...
				XmlElement newChild = HandleColumn(dataColumn3, dc, schema, fWriteOrdinal);
		...
		{
			if (constraints[n] is UniqueConstraint uniqueConstraint)
			{
