...
public class DataViewManager : MarshalByValueComponent, IBindingList, IList, ICollection, IEnumerable, ITypedList
{
		...
		set
		{
			...
			while (xmlTextReader.Read())
			{
				if (xmlTextReader.NodeType == XmlNodeType.Element)
				{
					...
					{
						_dataViewSettingsCollection[tableName].RowStateFilter = Enum.Parse<DataViewRowState>(xmlTextReader.Value);
					}
				}
			}
		}
	...
	string ITypedList.GetListName(PropertyDescriptor[] listAccessors)
	{
		...
		DataTable dataTable = System.Data.DataSet.FindTable(null, listAccessors, 0);
		if (dataTable != null)
		{
	...
	PropertyDescriptorCollection ITypedList.GetItemProperties(PropertyDescriptor[] listAccessors)
	{
		...
		if (dataTable != null)
		{
			return dataTable.GetPropertyDescriptorCollection();
		}
