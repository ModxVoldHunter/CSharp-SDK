...
public sealed class DataTableCollection : InternalDataCollectionBase
{
	...
	{
		string result;
		while (Contains(result = MakeName(_defaultNameIndex)))
		{
			_defaultNameIndex++;
		}
		return result;
	...
	private void BaseAdd([NotNull] DataTable table)
	{
		...
		{
			if (InternalDataCollectionBase.NamesEqual(table.TableName, _dataSet.DataSetName, fCaseSensitive: false, _dataSet.Locale) != 0 && !table._fNestedInDataset)
			{
	...
	internal bool Contains(string name, string tableNamespace, bool checkProperty, bool caseSensitive)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (InternalDataCollectionBase.NamesEqual(dataTable.TableName, name, fCaseSensitive: true, _dataSet.Locale) == 1 && text == tableNamespace)
			{
				return true;
			}
		}
	...
	internal bool Contains(string name, bool caseSensitive)
	{
		...
		{
			DataTable dataTable = (DataTable)_list[i];
			if (InternalDataCollectionBase.NamesEqual(dataTable.TableName, name, fCaseSensitive: true, _dataSet.Locale) == 1)
			{
				return true;
			}
		}
	...
	internal int InternalIndexOf(string tableName)
	{
		...
		if (tableName != null && 0 < tableName.Length)
		{
			...
			{
				DataTable dataTable = (DataTable)_list[i];
				switch (InternalDataCollectionBase.NamesEqual(dataTable.TableName, tableName, fCaseSensitive: false, _dataSet.Locale))
				{
				case 1:
				{
					for (int j = i + 1; j < count; j++)
					{
						...
						if (InternalDataCollectionBase.NamesEqual(dataTable2.TableName, tableName, fCaseSensitive: false, _dataSet.Locale) == 1)
						{
							return -3;
						}
					}
	...
	internal int InternalIndexOf(string tableName, string tableNamespace)
	{
		...
		if (tableName != null && 0 < tableName.Length)
		{
			...
			{
				DataTable dataTable = (DataTable)_list[i];
				int num2 = InternalDataCollectionBase.NamesEqual(dataTable.TableName, tableName, fCaseSensitive: false, _dataSet.Locale);
	...
	internal void RegisterName(string name, string tbNamespace)
	{
		...
		{
			DataTable dataTable = (DataTable)_list[i];
			if (InternalDataCollectionBase.NamesEqual(name, dataTable.TableName, fCaseSensitive: true, locale) != 0 && tbNamespace == dataTable.Namespace)
			{
				throw ExceptionBuilder.DuplicateTableName(((DataTable)_list[i]).TableName);
			}
		}
		if (InternalDataCollectionBase.NamesEqual(name, MakeName(_defaultNameIndex), fCaseSensitive: true, locale) != 0)
		{
			_defaultNameIndex++;
		}
	}
	...
	internal void UnregisterName(string name)
	{
		...
		if (InternalDataCollectionBase.NamesEqual(name, MakeName(_defaultNameIndex - 1), fCaseSensitive: true, _dataSet.Locale) != 0)
		{
			do
			{
				_defaultNameIndex--;
