...
internal sealed class Select
{
	...
	private int Eval(BinaryNode expr, DataRow row, DataRowVersion version)
	{
		...
		if (expr._op != 13 && expr._op != 39)
		{
			...
			StorageType storageType3 = ((!expr._left.IsSqlColumn && !expr._right.IsSqlColumn) ? BinaryNode.ResultType(storageType, storageType2, flag, flag2, expr._op) : BinaryNode.ResultSqlType(storageType, storageType2, expr._op));
			if (storageType3 == StorageType.Empty)
			{
				BinaryNode.SetTypeMismatchError(expr._op, obj.GetType(), obj2.GetType());
			}
			CompareInfo comparer = (((flag && !flag2 && storageType == StorageType.String && storageType2 == StorageType.Guid && expr._right is NameNode nameNode && nameNode._column.DataType == typeof(Guid)) || (flag2 && !flag && storageType2 == StorageType.String && storageType == StorageType.Guid && expr._left is NameNode nameNode2 && nameNode2._column.DataType == typeof(Guid))) ? CultureInfo.InvariantCulture.CompareInfo : null);
	...
	private int CompareRecords(int record1, int record2)
	{
		...
		long num4 = dataRow2?.rowID ?? 0;
		int num5 = ((num3 < num4) ? (-1) : ((num4 < num3) ? 1 : 0));
		if (num5 == 0 && record1 != record2 && dataRow3 != null && dataRow2 != null)
		{
			num3 = (long)dataRow3.GetRecordState(record1);
			num4 = (long)dataRow2.GetRecordState(record2);
