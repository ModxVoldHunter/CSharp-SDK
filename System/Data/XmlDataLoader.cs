...
internal sealed class XmlDataLoader
{
	...
	private void LoadRowData(DataRow row, XmlElement rowElement)
	{
		...
		if (textOnlyColumn != null)
		{
			...
			string valueForTextOnlyColumns = GetValueForTextOnlyColumns(xmlNode);
			if (XMLSchema.GetBooleanAttribute(rowElement, "nil", "http://www.w3.org/2001/XMLSchema-instance", defVal: false) && string.IsNullOrEmpty(valueForTextOnlyColumns))
			{
				row[textOnlyColumn] = DBNull.Value;
			}
			else
			{
				SetRowValueFromXmlText(row, textOnlyColumn, valueForTextOnlyColumns);
			}
		}
				...
				if (obj == null || obj is DataColumn)
				{
					...
					if (obj != null && obj is DataColumn)
					{
						...
						if (dataColumn.Table == row.Table && dataColumn.ColumnMapping != MappingType.Attribute && hashtable[dataColumn] == null)
						{
							...
							string valueForTextOnlyColumns2 = GetValueForTextOnlyColumns(xmlNode);
							if (XMLSchema.GetBooleanAttribute(xmlElement, "nil", "http://www.w3.org/2001/XMLSchema-instance", defVal: false) && string.IsNullOrEmpty(valueForTextOnlyColumns2))
							{
								row[dataColumn] = DBNull.Value;
							}
							else
							{
								SetRowValueFromXmlText(row, dataColumn, valueForTextOnlyColumns2);
							}
						}
					}
	...
	private void LoadTopMostTable(DataTable table)
	{
		...
		{
			DataRow dataRow = _childRowsStack.Pop();
			bool flag3 = dataRow.RowState == DataRowState.Unchanged;
			dataRow.SetNestedParentRow(parentRow, setNonNested: false);
			if (flag3)
			{
	...
	private void LoadTable(DataTable table, bool isNested)
	{
		...
		if (_isDiffgram)
		{
			...
			for (int num2 = array.Length - 1; num2 >= 0; num2--)
			{
				...
				dataColumn2[dataRow._tempRecord] = array[num2] ?? DBNull.Value;
			}
			dataRow.EndEdit();
	...
	private bool ProcessXsdSchema()
	{
		if (_dataReader.LocalName == _XSD_SCHEMA && _dataReader.NamespaceURI == _XSDNS)
		{
			...
			{
				_dataTable.ReadXSDSchema(_dataReader);
				_nodeToSchemaMap = new XmlToDatasetMap(_dataReader.NameTable, _dataTable);
			}
			else
			{
				_dataSet.ReadXSDSchema(_dataReader);
