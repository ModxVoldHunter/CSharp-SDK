...
internal abstract class ReflectionReader
{
	...
	private static MethodInfo CollectionSetItemDelegateMethod => s_getCollectionSetItemDelegateMethod ?? (s_getCollectionSetItemDelegateMethod = typeof(ReflectionReader).GetMethod("GetCollectionSetItemDelegate", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic));
	...
	public void ReflectionReadGetOnlyCollection(XmlReaderDelegator xmlReader, XmlObjectSerializerReadContext context, XmlDictionaryString collectionItemName, XmlDictionaryString collectionItemNamespace, CollectionDataContract collectionContract)
	{
		...
		if (!ReflectionReadSpecialCollection(xmlReader, context, collectionContract, collectionMember) && xmlReader.IsStartElement(collectionItemName, collectionItemNamespace))
		{
			...
			ReadCollectionItems(xmlReader, context, collectionItemName, collectionItemNamespace, collectionContract, collectionMember, isReadOnlyCollection);
		}
	}
	...
	private object ReflectionReadCollectionCore(XmlReaderDelegator xmlReader, XmlObjectSerializerReadContext context, XmlDictionaryString collectionItemName, XmlDictionaryString collectionItemNamespace, CollectionDataContract collectionContract)
	{
		...
		if (flag && ReflectionTryReadPrimitiveArray(xmlReader, context, collectionItemName, collectionItemNamespace, collectionContract.ItemType, arraySize, out var resultArray))
		{
			return resultArray;
		}
	...
	private object ReadCollectionItems(XmlReaderDelegator xmlReader, XmlObjectSerializerReadContext context, XmlDictionaryString collectionItemName, XmlDictionaryString collectionItemNamespace, CollectionDataContract collectionContract, object resultCollection, bool isReadOnlyCollection)
	{
		...
		CollectionSetItemDelegate collectionSetItemDelegate = (CollectionSetItemDelegate)methodInfo.Invoke(null, new object[3] { collectionContract, resultCollection, isReadOnlyCollection });
		int num = 0;
		while (true)
		{
	...
	{
		int num = ((classContract.BaseClassContract != null) ? ReflectionGetMembers(classContract.BaseClassContract, members) : 0);
		int num2 = num;
		int num3 = 0;
	...
	protected void ReflectionReadMember(XmlReaderDelegator xmlReader, XmlObjectSerializerReadContext context, ClassDataContract classContract, ref object obj, int memberIndex, DataMember[] members)
	{
		...
		else
		{
			...
			object memberValue = ReflectionReadValue(xmlReader, context, dataMember, classContract.XmlName.Namespace);
	...
	private static CollectionSetItemDelegate GetCollectionSetItemDelegate<T>(CollectionDataContract collectionContract, object resultCollectionObject, bool isReadOnlyCollection)
	{
		...
		if (addMethod == null)
		{
			throw new InvalidDataContractException(System.SR.Format(System.SR.CollectionMustHaveAddMethod, DataContract.GetClrTypeFullName(collectionContract.UnderlyingType)));
		}
		return delegate(object resultCollection, object collectionItem, int index)
		{
