...
internal sealed class JsonReaderDelegator : XmlReaderDelegator
{
	...
	private DateTimeArrayJsonHelperWithString DateTimeArrayHelper => _dateTimeArrayHelper ?? (_dateTimeArrayHelper = new DateTimeArrayJsonHelperWithString(_dateTimeFormat));
	...
	internal static DateTime ParseJsonDateInDefaultFormat(string originalDateTimeValue)
	{
		...
		ReadOnlySpan<char> s = text.AsSpan(6, text.Length - 8);
		DateTimeKind dateTimeKind = DateTimeKind.Utc;
		int num = s.Slice(1).IndexOf('+');
		if (num == -1)
		{
			num = s.Slice(1).IndexOf('-');
		}
		if (num >= 0)
		{
		...
		try
		{
			num2 = long.Parse(s, CultureInfo.InvariantCulture);
		}
		catch (Exception ex) when (((ex is ArgumentException || ex is FormatException || ex is OverflowException) ? 1 : 0) != 0)
		{
			throw XmlExceptionHelper.CreateConversionException(s.ToString(), "Int64", ex);
		}
		...
		{
			throw XmlExceptionHelper.CreateConversionException(s.ToString(), "DateTime", exception);
		}
	}
