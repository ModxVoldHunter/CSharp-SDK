...
internal sealed class XmlJsonReader : XmlBaseReader, IXmlJsonReaderInitializer
{
	...
	public override decimal ReadContentAsDecimal()
	{
		...
		catch (Exception ex) when (((ex is ArgumentException || ex is FormatException || ex is OverflowException) ? 1 : 0) != 0)
		{
			throw XmlExceptionHelper.CreateConversionException(text, "decimal", ex);
		}
	}
	...
	public override long ReadContentAsLong()
	{
		...
		{
			throw XmlExceptionHelper.CreateConversionException(text, "Int64", ex);
		}
	}
	...
	private static int ParseInt(ReadOnlySpan<char> value, NumberStyles style)
	{
		...
		{
			throw XmlExceptionHelper.CreateConversionException(value.ToString(), "Int32", ex);
		}
	}
	...
	private void ReadServerTypeAttribute(bool consumedObjectChar)
	{
		...
		int offset;
		int offsetMax;
		byte[] buffer = base.BufferReader.GetBuffer(8, out offset, out offsetMax);
		if (offset + 8 <= offsetMax && buffer[offset] == 34 && buffer[offset + 1] == 95 && buffer[offset + 2] == 95 && buffer[offset + 3] == 116 && buffer[offset + 4] == 121 && buffer[offset + 5] == 112 && buffer[offset + 6] == 101 && buffer[offset + 7] == 34)
		{
			...
			SkipWhitespaceInBufferReader();
			SkipExpectedByteInBufferReader(34);
			base.BufferReader.GetBuffer(out offset, out var _);
			do
			{
				if (base.BufferReader.GetByte() == 92)
				{
		...
		for (int i = 0; i < val.Length; i++)
		{
			if (val[i] == '\\')
			{
				...
				case 'u':
					...
					stringBuilder.Append(ParseChar(val.AsSpan(i + 1, 4), NumberStyles.HexNumber));
					i += 4;
					break;
				}
				startIndex = i + 1;
	...
	{
		throw new NotSupportedException(System.SR.Format(System.SR.JsonMethodNotSupported, "CreateSigningNodeWriter"));
	}
}
