...
internal sealed class XmlObjectSerializerWriteContextComplexJson : XmlObjectSerializerWriteContextComplex
{
	...
	internal static XmlDictionaryString CollectionItemName => JsonGlobals.itemDictionaryString;
	...
	internal static void VerifyObjectCompatibilityWithInterface(DataContract contract, object graph, Type declaredType)
	{
		...
		{
			throw XmlObjectSerializer.CreateSerializationException(System.SR.Format(System.SR.XmlObjectAssignedToIncompatibleInterface, graph.GetType(), declaredType));
		}
		if (type == typeof(CollectionDataContract) && !CollectionDataContract.IsCollectionInterface(declaredType))
		{
			throw XmlObjectSerializer.CreateSerializationException(System.SR.Format(System.SR.CollectionAssignedToIncompatibleInterface, graph.GetType(), declaredType));
		}
	}
	...
	internal void WriteJsonISerializable(XmlWriterDelegator xmlWriter, ISerializable obj)
	{
		...
		XmlObjectSerializerWriteContext.GetObjectData(obj, serializationInfo, GetStreamingContext());
		if (DataContract.GetClrTypeFullName(type) != serializationInfo.FullTypeName)
		{
			throw XmlObjectSerializer.CreateSerializationException(System.SR.Format(System.SR.ChangingFullTypeNameNotSupported, serializationInfo.FullTypeName, DataContract.GetClrTypeFullName(type)));
		}
