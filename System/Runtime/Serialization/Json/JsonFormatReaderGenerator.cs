...
internal sealed class JsonFormatReaderGenerator
{
	private sealed class CriticalHelper
	{
		...
		public JsonFormatClassReaderDelegate GenerateClassReader(ClassDataContract classContract)
		{
			...
			try
			{
				BeginMethod(_ilg, "Read" + DataContract.SanitizeTypeName(classContract.XmlName.Name) + "FromJson", typeof(JsonFormatClassReaderDelegate), flag);
			}
		...
		private CodeGenerator GenerateCollectionReaderHelper(CollectionDataContract collectionContract, bool isGetOnlyCollection)
		{
			...
			try
			{
				...
				{
					BeginMethod(_ilg, "Read" + DataContract.SanitizeTypeName(collectionContract.XmlName.Name) + "FromJsonIsGetOnly", typeof(JsonFormatGetOnlyCollectionReaderDelegate), flag);
				}
				else
				{
					BeginMethod(_ilg, "Read" + DataContract.SanitizeTypeName(collectionContract.XmlName.Name) + "FromJson", typeof(JsonFormatCollectionReaderDelegate), flag);
				}
			}
			catch (SecurityException securityException)
			{
		...
		{
			MethodInfo invokeMethod = CodeGenerator.GetInvokeMethod(delegateType);
			...
			ilg.BeginMethod(dynamicMethod, delegateType, array);
		}
		...
		private void CreateObject(ClassDataContract classContract)
		{
			...
			_objectLocal = _ilg.DeclareLocal(objectType);
		...
		private void ReadClass(ClassDataContract classContract)
		{
			if (classContract.HasExtensionData)
			{
				...
				for (ClassDataContract classDataContract = classContract; classDataContract != null; classDataContract = classDataContract.BaseClassContract)
				{
		...
		private void ReadMembers(ClassDataContract classContract, LocalBuilder extensionDataLocal)
		{
			...
			BitFlagsGenerator bitFlagsGenerator = new BitFlagsGenerator(num, _ilg);
			...
			LocalBuilder localBuilder = _ilg.DeclareLocal(Globals.TypeOfInt, -1);
		...
		{
			int num = ((classContract.BaseClassContract != null) ? ReadMembers(classContract.BaseClassContract, expectedElements, memberLabels, throwDuplicateMemberLabel, memberIndexLocal) : 0);
			int num2 = 0;
			while (num2 < classContract.Members.Count)
			{
				DataMember dataMember = classContract.Members[num2];
				Type memberType = dataMember.MemberType;
				_ilg.Case(memberLabels[num]);
				...
				if (dataMember.IsGetOnlyCollection)
				{
					...
					LocalBuilder localBuilder = _ilg.DeclareLocal(memberType);
				...
				else
				{
					...
					LocalBuilder localBuilder = ReadValue(memberType, dataMember.Name);
					_ilg.LoadAddress(_objectLocal);
					_ilg.ConvertAddress(_objectLocal.LocalType, _objectType);
		...
		{
			LocalBuilder localBuilder = _ilg.DeclareLocal(Globals.TypeOfByteArray);
		...
		{
			int num = ((contract.BaseClassContract != null) ? SetRequiredElements(contract.BaseClassContract, requiredElements) : 0);
		...
		private void ReadISerializable(ClassDataContract classContract)
		{
			...
			if (constructor == null)
			{
				throw XmlObjectSerializer.CreateSerializationException(System.SR.Format(System.SR.SerializationInfo_ConstructorNotFound, DataContract.GetClrTypeFullName(classContract.UnderlyingType)));
			}
			_ilg.LoadAddress(_objectLocal);
			_ilg.ConvertAddress(_objectLocal.LocalType, _objectType);
		...
		private LocalBuilder ReadValue(Type type, string name)
		{
			...
			{
				LocalBuilder localBuilder3 = _ilg.DeclareLocal(Globals.TypeOfString);
				...
				if (num != 0)
				{
					...
					localBuilder = _ilg.DeclareLocal(type);
				}
		...
		private void ReadCollection(CollectionDataContract collectionContract)
		{
			...
			LocalBuilder localBuilder = _ilg.DeclareLocal(Globals.TypeOfString);
			_ilg.Call(_contextArg, XmlFormatGeneratorStatics.GetObjectIdMethod);
			_ilg.Stloc(localBuilder);
			bool flag3 = false;
			...
			if (flag)
			{
				localBuilder2 = _ilg.DeclareLocal(type);
		...
		private void ReadSimpleDictionary(CollectionDataContract collectionContract, Type keyValueType)
		{
			...
			LocalBuilder localBuilder = _ilg.DeclareLocal(typeof(XmlNodeType));
			...
			LocalBuilder localBuilder2 = _ilg.DeclareLocal(type);
			_ilg.Stloc(localBuilder2);
			if (num > 0)
			{
				LocalBuilder localBuilder3 = _ilg.DeclareLocal(type3);
		...
		private void ReadGetOnlyCollection(CollectionDataContract collectionContract)
		{
			...
			Type itemType = collectionContract.ItemType;
			bool flag = collectionContract.Kind == CollectionKind.Array;
			LocalBuilder localBuilder = _ilg.DeclareLocal(Globals.TypeOfInt);
			_objectLocal = _ilg.DeclareLocal(underlyingType);
			_ilg.Load(_contextArg);
			...
			LocalBuilder localBuilder2 = _ilg.DeclareLocal(Globals.TypeOfInt);
		...
		private LocalBuilder ReadCollectionItem(CollectionDataContract collectionContract, Type itemType)
		{
			if (collectionContract.Kind == CollectionKind.Dictionary || collectionContract.Kind == CollectionKind.GenericDictionary)
			{
				...
				LocalBuilder localBuilder = _ilg.DeclareLocal(itemType);
		...
		private void StoreCollectionValue(LocalBuilder collection, LocalBuilder value, CollectionDataContract collectionContract)
		{
			if (collectionContract.Kind == CollectionKind.GenericDictionary || collectionContract.Kind == CollectionKind.Dictionary)
			{
				...
				LocalBuilder localBuilder = _ilg.DeclareLocal(dataMember.MemberType);
				LocalBuilder localBuilder2 = _ilg.DeclareLocal(dataMember2.MemberType);
