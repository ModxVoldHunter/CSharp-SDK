...
internal sealed class BinaryParser
{
	...
	private unsafe static DateTime FromBinaryRaw(long dateData)
	{
	...
	private void ReadArray(BinaryHeaderEnum binaryHeaderEnum)
	{
		...
		if (binaryArray._binaryTypeEnum == BinaryTypeEnum.ObjectUser)
		{
			...
			assemblyInfo = (BinaryAssemblyInfo)AssemIdToAssemblyTable[binaryArray._assemId];
		}
		else
		{
		...
		BinaryTypeConverter.TypeFromInfo(binaryArray._binaryTypeEnum, binaryArray._typeInformation, _objectReader, assemblyInfo, out pr._arrayElementTypeCode, out pr._arrayElementTypeString, out pr._arrayElementType, out pr._isArrayVariant);
		pr._dtTypeCode = InternalPrimitiveTypeE.Invalid;
