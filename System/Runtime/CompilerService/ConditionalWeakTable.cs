...
public sealed class ConditionalWeakTable<TKey, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] TValue> : IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable where TKey : class where TValue : class?
{
	...
	private sealed class Container
	{
		...
		{
			int num = RuntimeHelpers.TryGetHashCode(key);
			if (num == 0)
			{
				value = null;
				return -1;
	...
	{
		ArgumentNullException.ThrowIfNull(createValueCallback, "createValueCallback");
	...
	IEnumerator<KeyValuePair<TKey, TValue>> IEnumerable<KeyValuePair<TKey, TValue>>.GetEnumerator()
	{
		lock (_lock)
		{
			...
			{
				IEnumerator<KeyValuePair<TKey, TValue>> enumerator = GenericEmptyEnumerator<KeyValuePair<TKey, TValue>>.Instance;
				result = enumerator;
			}
			return result;
		}
