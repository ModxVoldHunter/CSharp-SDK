...
public ref struct DefaultInterpolatedStringHandler
{
	...
	public void AppendFormatted<T>(T value)
	{
		...
		if (value is IFormattable)
		{
			...
			{
				int charsWritten2;
				while (!((ISpanFormattable)(object)value).TryFormat(_chars.Slice(_pos), out charsWritten2, default(ReadOnlySpan<char>), _provider))
				{
					Grow();
				}
				_pos += charsWritten2;
	...
	public void AppendFormatted<T>(T value, string? format)
	{
		...
		if (value is IFormattable)
		{
			...
			{
				int charsWritten2;
				while (!((ISpanFormattable)(object)value).TryFormat(_chars.Slice(_pos), out charsWritten2, format, _provider))
				{
					Grow();
				}
				_pos += charsWritten2;
