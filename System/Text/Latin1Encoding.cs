...
internal class Latin1Encoding : Encoding
{
	...
	public unsafe override bool TryGetChars(ReadOnlySpan<byte> bytes, Span<char> chars, out int charsWritten)
	{
		if (bytes.Length <= chars.Length)
		{
			fixed (byte* pBytes = &MemoryMarshal.GetReference(bytes))
			{
				fixed (char* pChars = &MemoryMarshal.GetReference(chars))
				{
					charsWritten = GetCharsCommon(pBytes, bytes.Length, pChars, chars.Length);
					return true;
				}
			}
		}
	...
	public unsafe override string GetString(byte[] bytes)
	{
		...
		string text = string.FastAllocateString(bytes.Length);
		fixed (byte* pBytes = bytes)
		{
			fixed (char* pChars = text)
			{
				GetCharsCommon(pBytes, bytes.Length, pChars, text.Length);
			}
		}
		return text;
	}
	...
	public unsafe override string GetString(byte[] bytes, int index, int count)
	{
		...
		fixed (byte* ptr = bytes)
		{
			fixed (char* pChars = text)
			{
				GetCharsCommon(ptr + index, count, pChars, count);
			}
		}
		return text;
	}
