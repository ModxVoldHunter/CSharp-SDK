	...
	protected unsafe override void LoadManagedCodePage()
	{
		fixed (byte* ptr = &m_codePageHeader[0])
		{
			...
			lock (BaseCodePageEncoding.s_streamLock)
			{
				...
				int num2 = BaseCodePageEncoding.s_codePagesEncodingDataStream.Read(array, 0, array.Length);
			}
	...
	protected unsafe override void ReadBestFitTable()
	{
		lock (InternalSyncObject)
		{
			...
			lock (BaseCodePageEncoding.s_streamLock)
			{
				...
				int num = BaseCodePageEncoding.s_codePagesEncodingDataStream.Read(array, 0, array.Length);
			}
