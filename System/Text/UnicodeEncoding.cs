				...
				if (c2 >= '\ud800' && c2 <= '\udfff')
				{
					if (c2 <= '\udbff')
					{
						if (c > '\0')
						{
							...
							{
								encoderFallbackBuffer = ((encoder == null) ? encoderFallback.CreateFallbackBuffer() : encoder.FallbackBuffer);
								encoderFallbackBuffer.InternalInitialize(charStart, ptr, encoder, setEncoder: false);
							}
							chars2 = chars;
			...
			if (c2 >= '\ud800' && c2 <= '\udfff')
			{
				if (c2 <= '\udbff')
				{
					if (c > '\0')
					{
						...
						{
							decoderFallbackBuffer = ((decoder == null) ? decoderFallback.CreateFallbackBuffer() : decoder.FallbackBuffer);
