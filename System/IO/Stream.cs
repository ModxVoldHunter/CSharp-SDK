...
public abstract class Stream : MarshalByRefObject, IDisposable, IAsyncDisposable
{
	...
	[CompilerGenerated]
	private struct _003CReadAtLeastAsyncCore_003Ed__46 : IAsyncStateMachine
	{
	...
	{
		ValueTask<int> valueTask = ReadAtLeastAsyncCore(buffer, buffer.Length, throwOnEndOfStream: true, cancellationToken);
		return ValueTask.DangerousCreateFromTypedValueTask(valueTask);
	}
	...
	{
		ValidateBufferArguments(buffer, offset, count);
		ValueTask<int> valueTask = ReadAtLeastAsyncCore(buffer.AsMemory(offset, count), count, throwOnEndOfStream: true, cancellationToken);
		return ValueTask.DangerousCreateFromTypedValueTask(valueTask);
	}
	...
	[AsyncStateMachine(typeof(_003CReadAtLeastAsyncCore_003Ed__46))]
	...
	{
		Unsafe.SkipInit(out _003CReadAtLeastAsyncCore_003Ed__46 stateMachine);
