...
public struct BrotliEncoder : IDisposable
{
	...
	public static int GetMaxCompressedLength(int inputSize)
	{
		...
		ArgumentOutOfRangeException.ThrowIfGreaterThan(inputSize, 2147483132, "inputSize");
		if (inputSize == 0)
		{
			return 1;
		}
	...
	internal unsafe OperationStatus Compress(ReadOnlySpan<byte> source, Span<byte> destination, out int bytesConsumed, out int bytesWritten, BrotliEncoderOperation operation)
	{
		...
		while ((int)availableOut > 0)
		{
			fixed (byte* ptr = &MemoryMarshal.GetReference(source))
			{
				...
				{
					byte* ptr4 = ptr3;
					if (global::Interop.Brotli.BrotliEncoderCompressStream(_state, operation, ref availableIn, &ptr2, ref availableOut, &ptr4, out var _) == global::Interop.BOOL.FALSE)
					{
