...
public class FileStream : Stream
{
	...
	public FileStream(string path, FileStreamOptions options)
	{
		...
		if (options.UnixCreateMode.HasValue && (options.Mode == FileMode.Truncate || options.Mode == FileMode.Open))
		{
			throw new ArgumentException(SR.Argument_InvalidUnixCreateMode, "options");
		}
		FileStreamHelpers.SerializationGuard(options.Access);
		_strategy = FileStreamHelpers.ChooseStrategy(this, path, options.Mode, options.Access, options.Share, options.BufferSize, options.Options, options.PreallocationSize, options.UnixCreateMode);
	}
	...
	private FileStream(string path, FileMode mode, FileAccess access, FileShare share, int bufferSize, FileOptions options, long preallocationSize)
	{
		...
		_strategy = FileStreamHelpers.ChooseStrategy(this, path, mode, access, share, bufferSize, options, preallocationSize, null);
	}
