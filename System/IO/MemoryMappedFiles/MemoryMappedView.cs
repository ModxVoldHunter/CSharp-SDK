...
internal sealed class MemoryMappedView : IDisposable
{
	...
	public static MemoryMappedView CreateView(SafeMemoryMappedFileHandle memMappedFileHandle, MemoryMappedFileAccess access, long offset, long size)
	{
		...
		global::Interop.Kernel32.VirtualQuery(safeMemoryMappedViewHandle, ref lpBuffer, (nuint)Marshal.SizeOf(lpBuffer));
		ulong num = lpBuffer.RegionSize;
		if ((lpBuffer.State & 0x2000u) != 0 || num < newSize)
		{
			global::Interop.VirtualAlloc(safeMemoryMappedViewHandle, (nuint)((newSize != 0L) ? newSize : num), 4096, MemoryMappedFile.GetPageAccess(access));
	...
	public unsafe void Flush(nuint capacity)
	{
		byte* pointer = null;
		try
		{
			_viewHandle.AcquirePointer(ref pointer);
			if (global::Interop.Kernel32.FlushViewOfFile((nint)pointer, capacity))
			{
				return;
			}
			int lastPInvokeError = Marshal.GetLastPInvokeError();
	...
	{
		Unsafe.SkipInit(out global::Interop.Kernel32.SYSTEM_INFO sYSTEM_INFO);
		global::Interop.Kernel32.GetSystemInfo(&sYSTEM_INFO);
