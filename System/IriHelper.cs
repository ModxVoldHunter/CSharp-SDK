...
internal static class IriHelper
{
	...
	internal static bool CheckIsReserved(char ch, UriComponents component)
	{
		if ((UriComponents.AbsoluteUri & component) == 0)
		{
			...
			{
				return System.UriHelper.IsGenDelim(ch);
			}
			return false;
		}
		return ";/?:@&=+$,#[]!'()*".Contains(ch);
	...
	internal unsafe static string EscapeUnescapeIri(char* pInput, int start, int end, UriComponents component)
	{
		...
		for (int i = start; i < end; i++)
		{
			...
			if (c == '%')
			{
				if (end - i > 2)
				{
					c = System.UriHelper.DecodeHexChars(pInput[i + 1], pInput[i + 2]);
					if (c == '\uffff' || c == '%' || CheckIsReserved(c, component) || System.UriHelper.IsNotSafeForUnescape(c))
					{
						to.Append(pInput[i++]);
						to.Append(pInput[i++]);
						to.Append(pInput[i]);
					...
					{
						int num2 = System.PercentEncodingHelper.UnescapePercentEncodedUTF8Sequence(pInput + i, end - i, ref to, component == UriComponents.Query, iriParsing: true);
						i += num2 - 1;
					}
				}
				else
			...
			else if (c > '\u007f')
			{
				...
				else
				{
					...
					{
						byte b = span2[j];
						System.UriHelper.PercentEncodeByte(b, ref to);
					}
				}
				if (isSurrogatePair)
				{
