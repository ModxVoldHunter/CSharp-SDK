...
public readonly struct ValueTask : IEquatable<ValueTask>
{
	private sealed class ValueTaskSourceAsTask : Task
	{
		private static readonly Action<object> s_completionAction = delegate(object state)
		{
			...
			{
				throw new ArgumentOutOfRangeException("state", (state is ValueTaskSourceAsTask valueTaskSourceAsTask2) ? $"{"ValueTaskSourceAsTask"}.{"_source"} : {valueTaskSourceAsTask2._source}" : $"{"state"} : {state}", SR.Argument_UnexpectedStateForKnownCallback);
			}
		};
