...
internal static class DelegateHelpers
{
	...
	{
		public static Func<Type, Func<object[], object>, Delegate> CreateObjectArrayDelegate { get; } = CreateObjectArrayDelegateInternal();
		...
		private static Func<Type, Func<object[], object>, Delegate> CreateObjectArrayDelegateInternal()
		{
			...
			{
				return Type.GetType("Internal.Runtime.Augments.DynamicDelegateAugments, System.Private.CoreLib", throwOnError: true).GetMethod("CreateObjectArrayDelegate").CreateDelegate<Func<Type, Func<object[], object>, Delegate>>();
			}
			return delegate
			{
	...
	{
		public static Func<IDisposable> ForceAllowDynamicCodeDelegate { get; } = ForceAllowDynamicCodeDelegateInternal();


		private static Func<IDisposable> ForceAllowDynamicCodeDelegateInternal()
		{
			return typeof(AssemblyBuilder).GetMethod("ForceAllowDynamicCode", BindingFlags.Static | BindingFlags.NonPublic)?.CreateDelegate<Func<IDisposable>>();
		}
	}
	...
	{
		return new Func<object[]>(Array.Empty<object>).GetMethodInfo();
	}
	...
	private static Delegate CreateObjectArrayDelegateRefEmit(Type delegateType, Func<object[], object> handler)
	{
		if (!s_thunks.TryGetValue(delegateType, out var value))
		{
			...
			if (value == null)
			{
				using (CreateForceAllowDynamicCodeScope())
				{
