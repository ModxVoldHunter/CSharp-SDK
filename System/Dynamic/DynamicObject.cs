...
public class DynamicObject : IDynamicMetaObjectProvider
{
	private sealed class MetaDynamic : DynamicMetaObject
	{
		...
		private DynamicMetaObject CallMethodNoResult<TBinder>(MethodInfo method, TBinder binder, Expression[] args, Fallback<TBinder> fallback) where TBinder : DynamicMetaObjectBinder
		{
			...
			ParameterExpression parameterExpression = System.Linq.Expressions.Expression.Parameter(typeof(object[]), null);
			ReadOnlyCollection<Expression> convertedArgs = GetConvertedArgs(args);
			DynamicMetaObject errorSuggestion = new DynamicMetaObject(System.Linq.Expressions.Expression.Block(new TrueReadOnlyCollection<ParameterExpression>(parameterExpression), new TrueReadOnlyCollection<Expression>(System.Linq.Expressions.Expression.Assign(parameterExpression, System.Linq.Expressions.Expression.NewObjectArrayInit(convertedArgs)), System.Linq.Expressions.Expression.Condition(System.Linq.Expressions.Expression.Call(GetLimitedSelf(), method, BuildCallArgs(binder, args, parameterExpression, null)), System.Linq.Expressions.Expression.Block(ReferenceArgAssign(parameterExpression, args), System.Linq.Expressions.Utils.Empty), dynamicMetaObject.Expression, typeof(void)))), GetRestrictions().Merge(dynamicMetaObject.Restrictions));
			return fallback(this, binder, errorSuggestion);
		}

		[UnconditionalSuppressMessage("ReflectionAnalysis", "IL2075:UnrecognizedReflectionPattern", Justification = "This is looking if the method is overridden on an instantiated type. An overridden method will never be trimmed if the virtual method exists.")]
