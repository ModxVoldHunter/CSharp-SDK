...
{
	private abstract class DynamicPartitionEnumerator_Abstract<TSource, TSourceReader> : IEnumerator<KeyValuePair<long, TSource>>, IEnumerator, IDisposable
	{
	...
	private sealed class DynamicPartitionerForIEnumerable<TSource> : OrderablePartitioner<TSource>
	{
		...
		{
			ArgumentOutOfRangeException.ThrowIfNegativeOrZero(partitionCount, "partitionCount");
			IEnumerator<KeyValuePair<long, TSource>>[] array = new IEnumerator<KeyValuePair<long, TSource>>[partitionCount];
			InternalPartitionEnumerable internalPartitionEnumerable = new InternalPartitionEnumerable(_source.GetEnumerator(), _useSingleChunking, isStaticPartitioning: true);
			for (int i = 0; i < partitionCount; i++)
			{
				array[i] = internalPartitionEnumerable.GetEnumerator();
			}
			return array;
		}
	...
	private abstract class StaticIndexRangePartitioner<TSource, TCollection> : OrderablePartitioner<TSource>
	{
		...
		{
			ArgumentOutOfRangeException.ThrowIfNegativeOrZero(partitionCount, "partitionCount");
			(int Quotient, int Remainder) tuple = Math.DivRem(SourceCount, partitionCount);
			...
			{
				int num2 = num + 1;
				num = ((i >= item2) ? (num2 + item - 1) : (num2 + item));
				array[i] = CreatePartition(num2, num);
			}
			return array;
		}
	}

	private abstract class StaticIndexRangePartition<TSource> : IEnumerator<KeyValuePair<long, TSource>>, IEnumerator, IDisposable
	{
	...
	{
		ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(toExclusive, fromInclusive, "toExclusive");
	...
	{
		ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(toExclusive, fromInclusive, "toExclusive");
		ArgumentOutOfRangeException.ThrowIfNegativeOrZero(rangeSize, "rangeSize");
