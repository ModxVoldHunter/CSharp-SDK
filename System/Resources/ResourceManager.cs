...
public class ResourceManager
{
	...
	internal sealed class ResourceManagerMediator
	{
		...
		{
			ArgumentNullException.ThrowIfNull(rm, "rm");
			_rm = rm;
		}
	...
	{
		ArgumentNullException.ThrowIfNull(baseName, "baseName");
		ArgumentNullException.ThrowIfNull(resourceDir, "resourceDir");
	...
	{
		ArgumentNullException.ThrowIfNull(resourceSource, "resourceSource");
		...
		BaseNameField = ((@namespace == null) ? resourceSource.Name : string.Concat(@namespace, new ReadOnlySpan<char>(ref reference), resourceSource.Name));
		CommonAssemblyInit();
	}
	...
	public virtual ResourceSet? GetResourceSet(CultureInfo culture, bool createIfNotExists, bool tryParents)
	{
		...
		if (_useManifest && culture.HasInvariantCultureName)
		{
			...
			Stream manifestResourceStream = MainAssembly.GetManifestResourceStream(resourceFileName);
	...
	internal static bool IsDefaultType(string asmTypeName, string defaultTypeName)
	{
		...
		int length = ((num != -1) ? num : asmTypeName.Length);
		if (!MemoryExtensions.Equals(asmTypeName.AsSpan(0, length), defaultTypeName, StringComparison.Ordinal))
		{
			return false;
		}
		if (num == -1)
		...
		int num2 = asmTypeName.IndexOf(',', num + 1);
		int length2 = ((num2 != -1) ? num2 : asmTypeName.Length) - (num + 1);
		return MemoryExtensions.Equals(asmTypeName.AsSpan(num + 1, length2).Trim(), "mscorlib", StringComparison.OrdinalIgnoreCase);
	}
