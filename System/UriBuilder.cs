...
public class UriBuilder
{
	...
	public int Port
	{
		...
		set
		{
			...
			ArgumentOutOfRangeException.ThrowIfGreaterThan(value, 65535, "value");
			_port = value;
			_changed = true;
		}
	}
	...
	private static string EncodeUserInfo(string input)
	{
		...
		return input.Replace("/", "%2F", StringComparison.Ordinal).Replace("\\", "%5C", StringComparison.Ordinal).Replace("?", "%3F", StringComparison.Ordinal)
	...
	public override string ToString()
	{
		...
		if (scheme.Length != 0)
		{
			...
			string s = ((syntax != null) ? ((syntax.InFact(System.UriSyntaxFlags.MustHaveAuthority) || (host.Length != 0 && syntax.NotAny(System.UriSyntaxFlags.MailToLikeUri) && syntax.InFact(System.UriSyntaxFlags.OptionalAuthority))) ? System.Uri.SchemeDelimiter : ":") : ((host.Length == 0) ? ":" : System.Uri.SchemeDelimiter));
			valueStringBuilder.Append(scheme);
			valueStringBuilder.Append(s);
		}
		string text = EncodeUserInfo(UserName);
		if (text.Length != 0)
		{
			valueStringBuilder.Append(text);
			string text2 = EncodeUserInfo(Password);
			if (text2.Length != 0)
			{
