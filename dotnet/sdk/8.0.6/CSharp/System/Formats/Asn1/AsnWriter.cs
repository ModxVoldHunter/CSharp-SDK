...
public sealed class AsnWriter
{
	...
	private void WriteIntegerCore(Asn1Tag tag, ReadOnlySpan<byte> value)
	{
		...
		{
			ushort num = (ushort)(value2 & 0xFF80u);
			if (num == 0 || num == 65408)
			{
				throw new ArgumentException(System.SR.Argument_IntegerRedundantByte, "value");
			}
	...
	private void WriteIntegerCore(Asn1Tag tag, BigInteger value)
	{
		...
		value.TryWriteBytes(_buffer.AsSpan(_offset), out var bytesWritten, isUnsigned: false, isBigEndian: true);
	...
	public void WriteObjectIdentifier(ReadOnlySpan<char> oidValue, Asn1Tag? tag = null)
	{
		...
		ReadOnlySpan<byte> contents = WellKnownOids.GetContents(oidValue);
		if (!contents.IsEmpty)
		{
			WriteTag(tag?.AsPrimitive() ?? Asn1Tag.ObjectIdentifier);
			WriteLength(contents.Length);
			contents.CopyTo(_buffer.AsSpan(_offset));
