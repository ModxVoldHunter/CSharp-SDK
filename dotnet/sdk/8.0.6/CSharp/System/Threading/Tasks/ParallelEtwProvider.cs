	...
	public unsafe void ParallelLoopBegin(int OriginatingTaskSchedulerID, int OriginatingTaskID, int ForkJoinContextID, ForkJoinOperationType OperationType, long InclusiveFrom, long ExclusiveTo)
	{
		if (IsEnabled(EventLevel.Informational, EventKeywords.All))
		{
			...
			*ptr = new EventData
			{
				Size = 4,
				DataPointer = (nint)(&OriginatingTaskSchedulerID)
			};
			ptr[1] = new EventData
			{
				Size = 4,
				DataPointer = (nint)(&OriginatingTaskID)
			};
			ptr[2] = new EventData
			{
				Size = 4,
				DataPointer = (nint)(&ForkJoinContextID)
			};
			ptr[3] = new EventData
			{
				Size = 4,
				DataPointer = (nint)(&OperationType)
			};
			ptr[4] = new EventData
			{
				Size = 8,
				DataPointer = (nint)(&InclusiveFrom)
			};
			ptr[5] = new EventData
			{
				Size = 8,
				DataPointer = (nint)(&ExclusiveTo)
			};
	...
	public unsafe void ParallelLoopEnd(int OriginatingTaskSchedulerID, int OriginatingTaskID, int ForkJoinContextID, long TotalIterations)
	{
		if (IsEnabled(EventLevel.Informational, EventKeywords.All))
		{
			...
			*ptr = new EventData
			...
			{
				Size = 8,
				DataPointer = (nint)(&TotalIterations)
			};
