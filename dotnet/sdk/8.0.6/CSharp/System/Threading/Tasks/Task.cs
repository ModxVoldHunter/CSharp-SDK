...
public class Task<TResult> : Task
{
	...
	public new ConfiguredTaskAwaitable<TResult> ConfigureAwait(ConfigureAwaitOptions options)
	{
		...
		{
			throw ((options & ConfigureAwaitOptions.SuppressThrowing) == 0) ? new ArgumentOutOfRangeException("options") : new ArgumentOutOfRangeException("options", SR.TaskT_ConfigureAwait_InvalidOptions);
		}
	}
...
public class Task : IAsyncResult, IDisposable
{
	...
	private sealed class WhenAllPromise : Task, ITaskCompletionAction
	{
		...
		public void Invoke(Task completedTask)
		{
			...
			if (completedTask != null)
			{
				...
				if (!completedTask.IsCompletedSuccessfully && Interlocked.CompareExchange(ref _failedOrCanceled, completedTask, null) != null)
				{
					object failedOrCanceled;
					do
					{
						failedOrCanceled = _failedOrCanceled;
					...
					while (Interlocked.CompareExchange(ref _failedOrCanceled, new List<Task>
					{
						(Task)failedOrCanceled,
						completedTask
			...
			object failedOrCanceled2 = _failedOrCanceled;
			if (failedOrCanceled2 == null)
			{
				if (TplEventSource.Log.IsEnabled())
				{
					TplEventSource.Log.TraceOperationEnd(base.Id, AsyncCausalityStatus.Completed);
			...
			if (failedOrCanceled2 is List<Task> list2)
			{
			...
			{
				HandleTask((Task)failedOrCanceled2);
			}
			...
			void HandleTask(Task task)
			{
				if (task.IsFaulted)
				{
					(observedExceptions ?? (observedExceptions = new List<ExceptionDispatchInfo>())).AddRange(task.GetExceptionDispatchInfos());
				}
	...
	public static Task WhenAll(IEnumerable<Task> tasks)
	{
		...
		if (tasks is ICollection<Task> collection)
		{
			...
			Task[] array2 = new Task[collection.Count];
			collection.CopyTo(array2, 0);
			return WhenAll((ReadOnlySpan<Task>)array2);
		}
		...
		return WhenAll(CollectionsMarshal.AsSpan(list2));
	}
	...
	internal static Task WhenAll(ReadOnlySpan<Task> tasks)
	{
		if (tasks.Length == 0)
		{
			return CompletedTask;
	...
	public static Task<TResult[]> WhenAll<TResult>(IEnumerable<Task<TResult>> tasks)
	{
		...
		if (tasks is ICollection<Task<TResult>> collection)
		{
			...
			foreach (Task<TResult> task in array2)
			{
				if (task == null)
				{
					ThrowHelper.ThrowArgumentException(ExceptionResource.Task_MultiTaskContinuation_NullTask, ExceptionArgument.tasks);
	...
	public static Task<TResult[]> WhenAll<TResult>(params Task<TResult>[] tasks)
	{
		...
		Task<TResult>[] array = (Task<TResult>[])tasks.Clone();
		Task<TResult>[] array2 = array;
		foreach (Task<TResult> task in array2)
		{
			if (task == null)
	...
	private static Task<TTask> WhenAny<TTask>(IEnumerable<TTask> tasks) where TTask : Task
	{
		...
		{
			if (tasks is List<TTask> list)
			{
				return WhenAny<TTask>(CollectionsMarshal.AsSpan(list));
			}
			if (tasks is TTask[] array)
			{
				return WhenAny((ReadOnlySpan<TTask>)array);
			}
		...
		List<TTask> list2 = new List<TTask>();
		...
		return TaskFactory.CommonCWAnyLogic(list2);
	}
