...
internal sealed class XMLDiffLoader
{
	...
	internal void ProcessDiffs(XmlReader ssync)
	{
		...
		while (depth < ssync.Depth)
		{
			...
			int num = ReadOldRowData(_dataSet, ref table, ref pos, ssync);
			if (num == -1)
			{
				continue;
			}
	...
	private int ReadOldRowData(DataSet ds, ref DataTable table, ref int pos, XmlReader row)
	{
		...
		foreach (DataColumn column2 in table.Columns)
		{
			...
			{
				attribute = ((column2.ColumnMapping != MappingType.Hidden) ? row.GetAttribute(column2.EncodedColumnName, column2.Namespace) : row.GetAttribute("hidden" + column2.EncodedColumnName, "urn:schemas-microsoft-com:xml-msdata"));
				if (attribute != null)
				{
					column2[num] = column2.ConvertXmlToObject(attribute);
				}
			}
		}
		row.Read();
