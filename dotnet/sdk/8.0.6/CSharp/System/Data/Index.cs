...
internal sealed class Index
{
	...
	private int CompareRecords(int record1, int record2)
	{
		...
		DataRow row = _table._recordManager[record2];
		DataRow row2 = dataRow;
		return _table.Rows.IndexOf(row2).CompareTo(_table.Rows.IndexOf(row));
	}
	...
	private int CompareDuplicateRecords(int record1, int record2)
	{
		...
		int num = dataRow3.rowID.CompareTo(dataRow2.rowID);
		if (num == 0 && record1 != record2)
		{
			num = ((int)dataRow3.GetRecordState(record1)).CompareTo((int)dataRow2.GetRecordState(record2));
		}
		return num;
	}
