...
internal sealed class NewDiffgramGen
{
	...
	private void GenerateColumn(DataRow row, DataColumn col, DataRowVersion version)
	{
		...
		case MappingType.Attribute:
			_xmlw.WriteAttributeString(prefix, col.EncodedColumnName, col.Namespace, columnValueAsString);
			break;
		case MappingType.Hidden:
			_xmlw.WriteAttributeString("msdata", "hidden" + col.EncodedColumnName, "urn:schemas-microsoft-com:xml-msdata", columnValueAsString);
			break;
		case MappingType.SimpleContent:
			_xmlw.WriteString(columnValueAsString);
			break;
		case MappingType.Element:
		{
			bool flag = true;
			object obj = row[col, version];
			if (!col.IsCustomType || !DataColumn.IsValueCustomTypeInstance(obj) || typeof(IXmlSerializable).IsAssignableFrom(obj.GetType()))
			{
			...
			{
				if (DataColumn.IsValueCustomTypeInstance(obj))
				{
