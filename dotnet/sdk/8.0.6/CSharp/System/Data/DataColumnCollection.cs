...
public sealed class DataColumnCollection : InternalDataCollectionBase
{
	...
	internal int IndexOfCaseInsensitive(string name)
	{
		...
		for (int i = 0; i < Count; i++)
		{
			DataColumn dataColumn = (DataColumn)_list[i];
			if ((specialHashCode == 0 || dataColumn._hashCode == 0 || dataColumn._hashCode == specialHashCode) && InternalDataCollectionBase.NamesEqual(dataColumn.ColumnName, name, fCaseSensitive: false, _table.Locale) != 0)
			{
				if (num != -1)
				{
					return -2;
	...
	internal void RegisterColumnName(string name, DataColumn column)
	{
		...
		if (column == null && InternalDataCollectionBase.NamesEqual(name, MakeName(_defaultNameIndex), fCaseSensitive: true, _table.Locale) != 0)
		{
			do
			{
				_defaultNameIndex++;
	...
	internal void UnregisterName(string name)
	{
		...
		if (InternalDataCollectionBase.NamesEqual(name, MakeName(_defaultNameIndex - 1), fCaseSensitive: true, _table.Locale) != 0)
		{
			do
			{
				_defaultNameIndex--;
