...
internal sealed class XmlToDatasetMap
{
	...
	private TableSchemaInfo AddTableSchema(XmlNameTable nameTable, DataTable table)
	{
		...
		string localName = (table._encodedTableName = nameTable.Get(encodedTableName) ?? nameTable.Add(encodedTableName));
		string text = nameTable.Get(table.Namespace);
		if (text == null)
		{
			text = nameTable.Add(table.Namespace);
		}
		else if (table._tableNamespace != null)
		{
			table._tableNamespace = text;
		}
		TableSchemaInfo tableSchemaInfo = new TableSchemaInfo(table);
		_tableSchemaMap[new XmlNodeIdentety(localName, text)] = tableSchemaInfo;
		return tableSchemaInfo;
	}
	...
	private static bool AddColumnSchema(XmlNameTable nameTable, DataColumn col, XmlNodeIdHashtable columns)
	{
		...
		string localName = (col._encodedColumnName = nameTable.Get(array) ?? nameTable.Add(array));
		string text = nameTable.Get(col.Namespace);
		if (text == null)
		{
			text = nameTable.Add(col.Namespace);
		}
		else if (col._columnUri != null)
		{
			col._columnUri = text;
		}
		XmlNodeIdentety key = new XmlNodeIdentety(localName, text);
		columns[key] = col;
		if (col.ColumnName.StartsWith("xml", StringComparison.OrdinalIgnoreCase))
		{
			HandleSpecialColumn(col, nameTable, columns);
	...
	{
		_tableSchemaMap = new XmlNodeIdHashtable(dataSet.Tables.Count);
		string namespaceURI = nameTable.Get(dataSet.Namespace) ?? nameTable.Add(dataSet.Namespace);
		dataSet._namespaceURI = namespaceURI;
		foreach (DataTable table in dataSet.Tables)
		{
			...
			foreach (DataRelation childRelation in table.ChildRelations)
			{
				if (childRelation.Nested)
				{
					...
					string localName = nameTable.Get(array) ?? nameTable.Add(array);
					string namespaceURI2 = nameTable.Get(childRelation.ChildTable.Namespace) ?? nameTable.Add(childRelation.ChildTable.Namespace);
					XmlNodeIdentety key = new XmlNodeIdentety(localName, namespaceURI2);
		...
		foreach (DataTable item in selfAndDescendants)
		{
			...
			foreach (DataRelation childRelation in item.ChildRelations)
			{
				if (childRelation.Nested)
				{
					string array = XmlConvert.EncodeLocalName(childRelation.ChildTable.TableName);
					string localName = nameTable.Get(array) ?? nameTable.Add(array);
					string namespaceURI = nameTable.Get(childRelation.ChildTable.Namespace) ?? nameTable.Add(childRelation.ChildTable.Namespace);
					XmlNodeIdentety key = new XmlNodeIdentety(localName, namespaceURI);
					tableSchemaInfo.ColumnsSchemaMap[key] = childRelation.ChildTable;
				}
			}
		}
