...
internal sealed class XDRSchema : XMLSchema
{
	...
	{
		string name = dt;
		Span<System.Range> destination = stackalloc System.Range[3];
		int num = dt.AsSpan().Split(destination, ':');
		if (num <= 2)
		{
			if (num == 2)
			{
				System.Range range = destination[1];
				name = dt[range.Start..range.End];
			}
			NameType nameType = FindNameType(name);
			if (nameType == s_enumerationNameType && string.IsNullOrEmpty(dtValues))
			{
	...
	internal void HandleColumn(XmlElement node, DataTable table)
	{
		...
		else
		{
			...
			{
				attribute2 = string.Empty;
				simpleType = SimpleType.CreateByteArrayType();
			}
		...
		GetMinMax(node, ref minOccurs, ref maxOccurs);
		string attribute4 = node.GetAttribute("default");
		bool flag2 = false;
		...
		string attribute5 = node.GetAttribute("targetNamespace");
		if (attribute5 != null && attribute5.Length > 0)
		{
			dataColumn.Namespace = attribute5;
		}
		...
		if (!string.IsNullOrEmpty(attribute4))
		{
			try
			{
				dataColumn.DefaultValue = SqlConvert.ChangeTypeForXML(attribute4, type);
			}
			catch (FormatException)
			{
				throw ExceptionBuilder.CannotConvert(attribute4, type.FullName);
			}
		}
	}
	...
	internal static void GetMinMax(XmlElement elNode, ref int minOccurs, ref int maxOccurs)
	{
		...
		if (attribute != null && attribute.Length > 0 && !int.TryParse(attribute, CultureInfo.InvariantCulture, out minOccurs))
		{
		...
		if (attribute != null && attribute.Length > 0)
		{
			...
			else if (!int.TryParse(attribute, CultureInfo.InvariantCulture, out maxOccurs) || maxOccurs != 1)
			{
