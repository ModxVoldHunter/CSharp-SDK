...
internal sealed class XmlDataTreeWriter
{
	...
	internal void Save(XmlWriter xw, bool writeSchema)
	{
		...
		if (flag)
		{
			...
			else if (string.IsNullOrEmpty(_ds.DataSetName))
			{
	...
	internal void XmlDataRowWriter(DataRow row, string encodedTableName)
	{
		...
		foreach (DataColumn column2 in row.Table.Columns)
		{
			...
			if (column2._columnMapping != MappingType.SimpleContent && (!column2.IsCustomType || !DataColumn.IsValueCustomTypeInstance(obj) || typeof(IXmlSerializable).IsAssignableFrom(obj.GetType())))
			{
			...
			else if (DataColumn.IsValueCustomTypeInstance(obj))
			{
