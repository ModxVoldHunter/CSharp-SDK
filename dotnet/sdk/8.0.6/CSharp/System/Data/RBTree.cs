...
internal abstract class RBTree<K> : IEnumerable
{
	...
	private sealed class TreePage
	{
		...
		internal int AllocSlot(RBTree<K> tree)
		{
			...
			{
				for (int i = _nextFreeSlotLine; i < _slotMap.Length; i++)
				{
					if ((uint)_slotMap[i] < uint.MaxValue)
					{
						int num2 = ~_slotMap[i] & (_slotMap[i] + 1);
						_slotMap[i] |= num2;
						_inUseCount++;
						if (_inUseCount == _slots.Length)
						{
	...
	private int GetNewNode(K key)
	{
		...
		TreePage treePage = ((indexOfPageWithFreeSlot != -1) ? _pageTable[indexOfPageWithFreeSlot] : ((_inUsePageCount < 4) ? AllocPage(32) : ((_inUsePageCount < 32) ? AllocPage(256) : ((_inUsePageCount < 128) ? AllocPage(1024) : ((_inUsePageCount < 4096) ? AllocPage(4096) : ((_inUsePageCount >= 32768) ? AllocPage(65536) : AllocPage(8192)))))));
	...
	private int RBInsert(int root_id, int x_id, int mainTreeNodeID, int position, bool append)
	{
		...
		if (_accessMethod == TreeAccessMethod.KEY_SEARCH_AND_INDEX && !append)
		{
			while (num2 != 0)
			{
				IncreaseSize(num2);
				num = num2;
				int num3 = ((root_id == 0) ? CompareNode(Key(x_id), Key(num2)) : CompareSatelliteTreeNode(Key(x_id), Key(num2)));
				if (num3 < 0)
				{
					num2 = Left(num2);
					continue;
		...
		else
		{
			...
			{
				num6 = ((root_id == 0) ? CompareNode(Key(x_id), Key(num)) : CompareSatelliteTreeNode(Key(x_id), Key(num)));
			}
			else
			{
	...
	private int SearchSubTree(int root_id, K key)
	{
		...
		while (num != 0)
		{
			int num2 = ((root_id == 0) ? CompareNode(key, Key(num)) : CompareSatelliteTreeNode(key, Key(num)));
			if (num2 == 0)
			{
				break;
			}
