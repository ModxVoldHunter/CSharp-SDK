...
public abstract class DbCommandBuilder : Component
{
	...
	private void BuildInformation(DataTable schemaTable)
	{
		...
		if (!string.IsNullOrEmpty(quotePrefix) && text4.Contains(quotePrefix, StringComparison.Ordinal))
		{
			throw ADP.DynamicSQLNestedQuote(text4, quotePrefix);
		}
		if (!string.IsNullOrEmpty(quoteSuffix) && text4.Contains(quoteSuffix, StringComparison.Ordinal))
		{
	...
	private int BuildWhereClause(DataTableMapping mappings, DataRow dataRow, StringBuilder builder, DbCommand command, int parameterCount, bool isUpdate)
	{
		...
		{
			DbSchemaRow dbSchemaRow = dbSchemaRows[i];
			if (dbSchemaRow != null && dbSchemaRow.BaseColumnName.Length != 0 && IncludeInWhereClause(dbSchemaRow))
			{
	...
	{
		BuildCache(closeConnection: true, useColumnsForParameterNames);
	...
	private void RowUpdatingHandlerBuilder(RowUpdatingEventArgs rowUpdatingEvent)
	{
		...
		BuildCache(closeConnection: false, useColumnsForParameterNames: false);
