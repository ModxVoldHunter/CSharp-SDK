...
internal static class PercentEncodingHelper
{
	public unsafe static int UnescapePercentEncodedUTF8Sequence(char* input, int length, ref System.Text.ValueStringBuilder dest, bool isQuery, bool iriParsing)
	{
		...
		while (true)
		{
			...
			uint num8 = num;
			if (Rune.DecodeFromUtf8(new ReadOnlySpan<byte>(&num8, num2), out var result, out bytesConsumed) == OperationStatus.Done && (!iriParsing || System.IriHelper.CheckIriUnicodeRange((uint)result.Value, isQuery)))
			{
				if (num4 != 0)
				{
