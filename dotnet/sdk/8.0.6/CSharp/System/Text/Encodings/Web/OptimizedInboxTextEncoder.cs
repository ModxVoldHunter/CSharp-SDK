...
internal sealed class OptimizedInboxTextEncoder
{
	...
	public unsafe int GetIndexOfFirstByteToEncode(ReadOnlySpan<byte> data)
	{
		...
		if (Ssse3.IsSupported || (AdvSimd.Arm64.IsSupported && BitConverter.IsLittleEndian))
		{
			...
			{
				nuint num = ((!AdvSimd.Arm64.IsSupported || !BitConverter.IsLittleEndian) ? GetIndexOfFirstByteToEncodeSsse3(pData, (uint)length) : GetIndexOfFirstByteToEncodeAdvSimd64(pData, (uint)length));
				num2 = (int)num;
			}
			if (!SpanUtility.IsValidIndex(data, num2))
			{
				return -1;
			}
			if (System.Text.UnicodeUtility.IsAsciiCodePoint(data[num2]))
			{
				return num2;
			}
