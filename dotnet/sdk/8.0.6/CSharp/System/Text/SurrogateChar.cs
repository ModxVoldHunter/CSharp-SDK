...
internal readonly struct SurrogateChar
{
	...
	public SurrogateChar(int ch)
	{
		...
		{
			throw new ArgumentException(System.SR.Format(System.SR.XmlInvalidSurrogate, ch.ToString("X", CultureInfo.InvariantCulture)), "ch");
		}
	...
	public SurrogateChar(char lowChar, char highChar)
	{
		if (lowChar < '\udc00' || lowChar > '\udfff')
		{
			...
			int num = lowChar;
			throw new ArgumentException(System.SR.Format(xmlInvalidLowSurrogate, num.ToString("X", CultureInfo.InvariantCulture)), "lowChar");
		}
		if (highChar < '\ud800' || highChar > '\udbff')
		{
			string xmlInvalidHighSurrogate = System.SR.XmlInvalidHighSurrogate;
			int num = highChar;
			throw new ArgumentException(System.SR.Format(xmlInvalidHighSurrogate, num.ToString("X", CultureInfo.InvariantCulture)), "highChar");
		}
		_lowChar = lowChar;
		_highChar = highChar;
	}
