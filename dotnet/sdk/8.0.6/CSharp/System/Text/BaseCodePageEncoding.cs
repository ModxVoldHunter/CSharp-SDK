...
internal abstract class BaseCodePageEncoding : EncodingNLS, ISerializable
{
	...
	protected unsafe byte* GetNativeMemory(int iSize)
	{
		...
		{
			byte* handle = (byte*)Marshal.AllocHGlobal(iSize);
			safeNativeMemoryHandle = new SafeAllocHHandle((nint)handle);
		}
	...
	private unsafe static void ReadCodePageIndex(Stream stream, Span<byte> codePageIndex)
	{
		int num = stream.Read(codePageIndex);
		if (BitConverter.IsLittleEndian)
		{
			return;
		}
	...
	internal unsafe static EncodingInfo[] GetEncodings(CodePagesEncodingProvider provider)
	{
		lock (s_streamLock)
		{
			...
			Span<byte> codePageIndex2 = new Span<byte>(&codePageIndex, sizeof(CodePageIndex));
			for (int i = 0; i < codePageCount; i++)
			{
				...
				{
					text2 = EncodingNLS.GetLocalizedEncodingNameResource(codePageIndex.CodePage);
				}
