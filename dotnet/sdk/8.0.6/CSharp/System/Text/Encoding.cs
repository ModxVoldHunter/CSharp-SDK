...
public abstract class Encoding : ICloneable
{
	...
	internal sealed class EncodingCharBuffer
	{
		...
		internal unsafe EncodingCharBuffer(Encoding enc, DecoderNLS decoder, char* charStart, int charCount, byte* byteStart, int byteCount)
		{
			...
			_fallbackBuffer = ((_decoder == null) ? enc.DecoderFallback.CreateFallbackBuffer() : _decoder.FallbackBuffer);
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegative(codePage, "codePage");
		_codePage = codePage;
		SetDefaultFallbacks();
	}
	...
	{
		ArgumentNullException.ThrowIfNull(srcEncoding, "srcEncoding");
		ArgumentNullException.ThrowIfNull(dstEncoding, "dstEncoding");
		ArgumentNullException.ThrowIfNull(bytes, "bytes");
	...
	public unsafe int GetByteCount(string s, int index, int count)
	{
		...
		ArgumentOutOfRangeException.ThrowIfGreaterThan(index, s.Length - count, "index");
		fixed (char* ptr = s)
		{
	...
	{
		int charCount = GetCharCount(bytes);
	...
	{
		ArgumentNullException.ThrowIfNull(innerStream, "innerStream");
		ArgumentNullException.ThrowIfNull(innerStreamEncoding, "innerStreamEncoding");
		ArgumentNullException.ThrowIfNull(outerStreamEncoding, "outerStreamEncoding");
	...
	{
		throw new ArgumentException(SR.Format(SR.Argument_EncodingConversionOverflowBytes, _codePage, EncoderFallback.GetType()), "bytes");
	}
	...
	{
		throw new ArgumentException(SR.Format(SR.Argument_EncodingConversionOverflowChars, _codePage, DecoderFallback.GetType()), "chars");
	}
	...
	{
		return GetBytesWithFallback(new ReadOnlySpan<char>(pOriginalChars, originalCharCount).Slice(charsConsumedSoFar), originalCharCount, new Span<byte>(pOriginalBytes, originalByteCount).Slice(bytesWrittenSoFar), originalByteCount, null, throwForDestinationOverflow);
	}
	...
	private protected unsafe virtual int GetBytesWithFallback(ReadOnlySpan<char> chars, int originalCharsLength, Span<byte> bytes, int originalBytesLength, EncoderNLS encoder, bool throwForDestinationOverflow = true)
	{
	...
	{
		return GetCharsWithFallback(new ReadOnlySpan<byte>(pOriginalBytes, originalByteCount).Slice(bytesConsumedSoFar), originalByteCount, new Span<char>(pOriginalChars, originalCharCount).Slice(charsWrittenSoFar), originalCharCount, null, throwForDestinationOverflow);
	}
	...
	private protected unsafe virtual int GetCharsWithFallback(ReadOnlySpan<byte> bytes, int originalBytesLength, Span<char> chars, int originalCharsLength, DecoderNLS decoder, bool throwForDestinationOverflow = true)
	{
