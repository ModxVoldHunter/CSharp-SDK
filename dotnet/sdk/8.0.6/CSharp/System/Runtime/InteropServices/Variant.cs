...
internal struct Variant
{
	...
	public nint AsByRefVariant => _typeUnion._unionTypes._pvarVal;
	...
	public unsafe void CopyFromIndirect(object value)
	{
		...
		if (value == null)
		{
			...
			{
				*(nint*)_typeUnion._unionTypes._byref = IntPtr.Zero;
			}
			return;
		}
		if ((varEnum & VarEnum.VT_ARRAY) != 0)
		{
			...
			*(nint*)_typeUnion._unionTypes._byref = variant._typeUnion._unionTypes._byref;
			return;
		}
		switch (varEnum)
		{
		...
		case VarEnum.VT_BOOL:
			*(short*)_typeUnion._unionTypes._byref = (short)(((bool)value) ? (-1) : 0);
			break;
		case VarEnum.VT_I4:
		case VarEnum.VT_INT:
		...
		case VarEnum.VT_ERROR:
			*(int*)_typeUnion._unionTypes._byref = ((ErrorWrapper)value).ErrorCode;
			break;
		case VarEnum.VT_I8:
			*(long*)_typeUnion._unionTypes._byref = (long)value;
			break;
		case VarEnum.VT_UI8:
			*(ulong*)_typeUnion._unionTypes._byref = (ulong)value;
			break;
		case VarEnum.VT_R4:
		...
		case VarEnum.VT_UNKNOWN:
			*(nint*)_typeUnion._unionTypes._byref = Marshal.GetIUnknownForObject(value);
			break;
		case VarEnum.VT_DISPATCH:
			*(nint*)_typeUnion._unionTypes._byref = Marshal.GetIDispatchForObject(value);
			break;
		case VarEnum.VT_BSTR:
			*(nint*)_typeUnion._unionTypes._byref = Marshal.StringToBSTR((string)value);
			break;
		case VarEnum.VT_CY:
	...
	public unsafe void Clear()
	{
		...
		else if ((variantType & VarEnum.VT_ARRAY) != 0 || variantType == VarEnum.VT_BSTR || variantType == VarEnum.VT_UNKNOWN || variantType == VarEnum.VT_DISPATCH || variantType == VarEnum.VT_VARIANT || variantType == VarEnum.VT_RECORD)
		{
			...
			{
				void* variant = ptr;
				Interop.OleAut32.VariantClear((nint)variant);
			}
		}
		else
		{
