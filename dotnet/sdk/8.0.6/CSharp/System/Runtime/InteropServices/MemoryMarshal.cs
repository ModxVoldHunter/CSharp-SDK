...
public static class MemoryMarshal
{
	...
	internal unsafe static ref T GetNonNullPinnableReference<T>(Span<T> span)
	{
		...
		return ref Unsafe.AsRef(ref span._reference);
	}
	...
	public static Span<TTo> Cast<TFrom, TTo>(Span<TFrom> span) where TFrom : struct where TTo : struct
	{
		...
		return new Span<TTo>(ref Unsafe.As<TFrom, TTo>(ref span._reference), length2);
	}
	...
	{
		return new Span<T>(ref Unsafe.AsRef(ref reference), length);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static ReadOnlySpan<T> CreateReadOnlySpan<T>([In][RequiresLocation] scoped ref T reference, int length)
	{
		return new ReadOnlySpan<T>(ref Unsafe.AsRef(ref reference), length);
	}
	...
	public static IEnumerable<T> ToEnumerable<T>(ReadOnlyMemory<T> memory)
	{
		...
		if (RuntimeHelpers.ObjectHasComponentSize(objectStartLength))
		{
			T[] array2 = Unsafe.As<T[]>(objectStartLength);
			start &= 0x7FFFFFFF;
			if (start != 0 || length != array2.Length)
			{
				return FromArray(array2, start, length);
			}
			return array2;
		}
