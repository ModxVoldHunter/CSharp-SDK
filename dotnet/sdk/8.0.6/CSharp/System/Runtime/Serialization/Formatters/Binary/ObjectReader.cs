...
internal sealed class ObjectReader
{
	...
	[RequiresDynamicCode("ObjectReader requires unreferenced code")]
	[RequiresUnreferencedCode("Types might be removed")]
	internal object Deserialize(BinaryParser serParser)
	{
		ArgumentNullException.ThrowIfNull(serParser, "serParser");
	...
	[RequiresDynamicCode("ObjectReader requires dynamic code")]
	...
	private void ParseArrayMember(ParseRecord pr)
	{
		...
		else if (pr._memberValueEnum == InternalMemberValueE.InlineValue)
		{
			...
			else if (parseRecord._isArrayVariant)
			{
				...
				{
					obj = pr._varValue ?? Converter.FromString(pr._value, pr._dtTypeCode);
				}
				if (parseRecord._objectA != null)
				{
			...
			{
				object obj2 = pr._varValue ?? Converter.FromString(pr._value, parseRecord._arrayElementTypeCode);
				if (parseRecord._objectA != null)
				{
	...
	private void ParseMember(ParseRecord pr)
	{
		...
		else if (pr._memberValueEnum == InternalMemberValueE.InlineValue)
		{
			...
			{
				object value = pr._varValue ?? Converter.FromString(pr._value, pr._dtTypeCode);
	...
	private void RegisterObject(object obj, ParseRecord pr, ParseRecord objectPr, bool bIsString)
	{
		...
		if (bIsString)
		{
			_objectManager.RegisterString((string)obj, pr._objectId, si, idOfContainingObj, member);
		}
		else
		{
			_objectManager.RegisterObject(obj, pr._objectId, si, idOfContainingObj, member, arrayIndex);
		}
	}
