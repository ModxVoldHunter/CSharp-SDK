...
internal sealed class XmlFormatReaderGenerator
{
	private sealed class CriticalHelper
	{
		...
		public XmlFormatClassReaderDelegate GenerateClassReader(ClassDataContract classContract)
		{
			...
			try
			{
				_ilg.BeginMethod("Read" + classContract.XmlName.Name + "FromXml", Globals.TypeOfXmlFormatClassReaderDelegate, flag);
			}
			...
			{
				localBuilder = _ilg.DeclareLocal(Globals.TypeOfString);
		...
		private CodeGenerator GenerateCollectionReaderHelper(CollectionDataContract collectionContract, bool isGetOnlyCollection)
		{
			...
			try
			{
				...
				{
					_ilg.BeginMethod("Read" + collectionContract.XmlName.Name + "FromXmlIsGetOnly", Globals.TypeOfXmlFormatGetOnlyCollectionReaderDelegate, flag);
				}
				else
				{
					_ilg.BeginMethod("Read" + collectionContract.XmlName.Name + "FromXml" + string.Empty, Globals.TypeOfXmlFormatCollectionReaderDelegate, flag);
				}
			}
			catch (SecurityException securityException)
			{
		...
		private void ReadMembers(ClassDataContract classContract, LocalBuilder extensionDataLocal)
		{
			...
			LocalBuilder localBuilder2 = (flag ? _ilg.DeclareLocal(Globals.TypeOfInt, firstRequiredMember) : null);
		...
		{
			int num = ((classContract.BaseClassContract != null) ? ReadMembers(classContract.BaseClassContract, requiredMembers, memberLabels, memberIndexLocal, requiredIndexLocal) : 0);
			int num2 = 0;
			while (num2 < classContract.Members.Count)
			{
				...
				else
				{
					...
					LocalBuilder localBuilder = ReadValue(memberType, dataMember.Name, classContract.XmlName.Namespace);
					_ilg.LoadAddress(_objectLocal);
					_ilg.ConvertAddress(_objectLocal.LocalType, _objectType);
		...
		private void InternalDeserialize(LocalBuilder value, Type type, string name, string ns)
		{
			...
			_ilg.Load(DataContract.GetId(type2.TypeHandle));
			_ilg.Ldtoken(type2);
			_ilg.Load(name);
			_ilg.Load(ns);
		...
		private void ReadCollection(CollectionDataContract collectionContract)
		{
			...
			string @namespace = collectionContract.XmlName.Namespace;
			_objectLocal = _ilg.DeclareLocal(type);
			if (!flag)
			{
				if (type.IsValueType)
			...
			_ilg.Stloc(localBuilder);
			LocalBuilder localBuilder2 = _ilg.DeclareLocal(Globals.TypeOfString);
			_ilg.Call(_contextArg, XmlFormatGeneratorStatics.GetObjectIdMethod);
			_ilg.Stloc(localBuilder2);
			bool flag2 = false;
			if (flag && TryReadPrimitiveArray(itemType, localBuilder))
			{
				flag2 = true;
				_ilg.IfNot();
			}
			...
			if (flag)
			{
				localBuilder3 = _ilg.DeclareLocal(type);
				_ilg.NewArray(itemType, 32);
				_ilg.Stloc(localBuilder3);
			}
			LocalBuilder localBuilder4 = _ilg.DeclareLocal(Globals.TypeOfInt);
			...
			LocalBuilder localBuilder5 = _ilg.DeclareLocal(Globals.TypeOfInt);
