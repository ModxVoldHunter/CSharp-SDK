...
internal sealed class XmlJsonWriter : XmlDictionaryWriter, IXmlJsonWriterInitializer
{
	...
	public XmlJsonWriter(bool indent, string indentChars)
	{
		...
		if (indent)
		{
			ArgumentNullException.ThrowIfNull(indentChars, "indentChars");
	...
	public override void WriteWhitespace(string ws)
	{
		...
		ArgumentNullException.ThrowIfNull(ws, "ws");
		int num = ws.AsSpan().IndexOfAnyExcept(" \t\r\n");
		if (num >= 0)
		{
			throw new ArgumentException(System.SR.Format(System.SR.JsonOnlyWhitespace, ws[num].ToString(), "WriteWhitespace"), "ws");
		}
		WriteString(ws);
	}
