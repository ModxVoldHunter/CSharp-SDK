...
internal sealed class JsonFormatWriterGenerator
{
	private sealed class CriticalHelper
	{
		...
		internal JsonFormatClassWriterDelegate GenerateClassWriter(ClassDataContract classContract)
		{
			...
			try
			{
				BeginMethod(_ilg, "Write" + DataContract.SanitizeTypeName(classContract.XmlName.Name) + "ToJson", typeof(JsonFormatClassWriterDelegate), flag);
			}
		...
		internal JsonFormatCollectionWriterDelegate GenerateCollectionWriter(CollectionDataContract collectionContract)
		{
			...
			try
			{
				BeginMethod(_ilg, "Write" + DataContract.SanitizeTypeName(collectionContract.XmlName.Name) + "ToJson", typeof(JsonFormatCollectionWriterDelegate), flag);
			}
			catch (SecurityException securityException)
			{
				if (!flag)
		...
		private void WriteCollection(CollectionDataContract collectionContract)
		{
			...
			if (collectionContract.Kind == CollectionKind.Array)
			{
				...
				if (!TryWritePrimitiveArray(collectionContract.UnderlyingType, itemType, _objectLocal, localBuilder))
				{
					WriteArrayAttribute();
					...
					if (!TryWritePrimitive(itemType, null, null, localBuilder2, localBuilder, 0))
					{
						...
						LocalBuilder localBuilder3 = _ilg.DeclareLocal(itemType);
			...
			LocalBuilder localBuilder4 = _ilg.DeclareLocal(returnType);
			LocalBuilder localBuilder5 = _ilg.DeclareLocal(type);
			...
			if (flag3)
			{
				...
				LocalBuilder localBuilder6 = _ilg.DeclareLocal(Globals.TypeOfString);
				LocalBuilder localBuilder7 = _ilg.DeclareLocal(array[1]);
				_ilg.ForEach(localBuilder4, returnType, localBuilder5, methodInfo3);
