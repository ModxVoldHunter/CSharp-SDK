...
public sealed class MemoryFailPoint : CriticalFinalizerObject, IDisposable
{
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegativeOrZero(sizeInMegabytes, "sizeInMegabytes");
		...
		for (int i = 0; i < 3; i++)
		{
			...
			case 1:
				if (flag2)
				{
					nuint numBytes = new UIntPtr(num2);
	...
	private unsafe static ulong MemFreeAfterAddress(void* address, ulong size)
	{
		...
		nuint dwLength = (nuint)sizeof(Interop.Kernel32.MEMORY_BASIC_INFORMATION);
		while ((ulong)((long)address + (long)size) < s_topOfMemory)
		{
			nuint num2 = Interop.Kernel32.VirtualQuery(address, ref lpBuffer, dwLength);
			if (num2 == UIntPtr.Zero)
			{
				throw Win32Marshal.GetExceptionForLastWin32Error();
			}
			ulong num3 = ((UIntPtr)lpBuffer.RegionSize).ToUInt64();
			...
			address = (void*)((ulong)address + num3);
		}
		return num;
	}

	private unsafe static void GrowPageFileIfNecessaryAndPossible(nuint numBytes)
	{
