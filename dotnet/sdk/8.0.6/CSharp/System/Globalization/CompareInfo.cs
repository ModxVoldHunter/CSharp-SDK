...
public sealed class CompareInfo : IDeserializationCallback
{
	...
	private static readonly SearchValues<char> s_nonSpecialAsciiChars = SearchValues.Create("\t\v\f !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~");
	...
	{
		return IsSortable(new ReadOnlySpan<char>(ref ch));
	}
	...
	public int IndexOf(string source, char value, CompareOptions options)
	{
		...
		return IndexOf(source, new ReadOnlySpan<char>(ref value), options);
	}
	...
	public int IndexOf(string source, char value, int startIndex, int count, CompareOptions options)
	{
		...
		int num = IndexOf(slice, new ReadOnlySpan<char>(ref value), options);
		if (num >= 0)
		{
			num += startIndex;
		}
	...
	public int LastIndexOf(string source, char value, CompareOptions options)
	{
		...
		return LastIndexOf(source, new ReadOnlySpan<char>(ref value), options);
	}
	...
	public int LastIndexOf(string source, char value, int startIndex, int count, CompareOptions options)
	{
		...
		int num = LastIndexOf(slice, new ReadOnlySpan<char>(ref value), options);
		if (num >= 0)
		{
			num += startIndex;
		}
						...
						while (true)
						{
							...
							if (num4 != num2)
							{
								...
								while (true)
								{
									...
									if (num6 < source.Length && ptr2[num6] >= '\u0080')
									{
										break;
									}
							...
							return -1;
							IL_00fd:
							if (num6 < source.Length - 1 && (ptr2 + num6)[1] >= '\u0080')
							{
								break;
							}
							num4 += num3;
	...
	internal unsafe static nint NlsGetSortHandle(string cultureName)
	{
		...
		int num2 = Interop.Kernel32.LCMapStringEx(cultureName, 536870912u, null, 0, &num, 8, null, null, IntPtr.Zero);
		if (num2 > 0)
		{
			int num3 = 0;
			char c = 'a';
			num2 = Interop.Kernel32.LCMapStringEx(null, 262144u, &c, 1, &num3, 4, null, null, num);
			if (num2 > 1)
			{
				return num;
			}
	...
	private unsafe bool StartsWithCoreUtf8(ReadOnlySpan<byte> source, ReadOnlySpan<byte> prefix, CompareOptions options)
	{
		...
		if (Utf8.ToUtf16PreservingReplacement(source, span, out var bytesRead, out var charsWritten) != 0)
		{
			return false;
		}
		...
		if (Utf8.ToUtf16PreservingReplacement(prefix, span2, out bytesRead, out var charsWritten2) != 0)
		{
			return false;
		}
