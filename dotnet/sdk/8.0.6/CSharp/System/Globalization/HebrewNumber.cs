...
internal static class HebrewNumber
{
	...
	private static readonly HS[] s_numberPassingState = new HS[170]
	{
		HS.S400,
		HS.X00,
		HS.X00,
	...
	internal static void Append<TChar>(ref ValueListBuilder<TChar> outputBuffer, int Number) where TChar : unmanaged, IUtfChar<TChar>
	{
		...
		if (num > 0)
		{
			...
			{
				DateTimeFormat.AppendChar(ref outputBuffer, 'ת');
			}
			int num2 = num % 4;
			if (num2 > 0)
			{
				DateTimeFormat.AppendChar(ref outputBuffer, (char)(1510 + num2));
			}
		}
		int num3 = Number / 10;
		Number %= 10;
		...
		if (c != 0)
		{
			DateTimeFormat.AppendChar(ref outputBuffer, c);
		}
		if (c2 != 0)
		{
			DateTimeFormat.AppendChar(ref outputBuffer, c2);
		}
		if (outputBuffer.Length - length > 1)
		{
			if (typeof(TChar) == typeof(char))
			{
				TChar item = outputBuffer[outputBuffer.Length - 1];
				outputBuffer.Length--;
			...
			{
				Rune.DecodeLastFromUtf8(MemoryMarshal.AsBytes(outputBuffer.AsSpan()), out var value, out var bytesConsumed);
	...
	internal static HebrewNumberParsingState ParseByChar(char ch, ref HebrewNumberParsingContext context)
	{
		...
		context.state = s_numberPassingState[(int)context.state * 10 + (int)hebrewToken];
