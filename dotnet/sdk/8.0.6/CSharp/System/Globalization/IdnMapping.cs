...
public sealed class IdnMapping
{
	...
	{
		ArgumentNullException.ThrowIfNull(unicode, "unicode");
	...
	{
		ArgumentNullException.ThrowIfNull(ascii, "ascii");
	...
	private static bool ValidateStd3AndAscii(string unicode, bool bUseStd3, bool bCheckAscii)
	{
		...
		{
			throw new ArgumentException(SR.Format(SR.Argument_IdnBadNameSize, 255 - ((!IsDot(unicode[unicode.Length - 1])) ? 1 : 0)), "unicode");
		}
		if (bUseStd3)
		{
			if (!IsDot(unicode[unicode.Length - 1]))
			{
	...
	private static string PunycodeEncode(string unicode)
	{
		...
		{
			num = unicode.AsSpan(num2).IndexOfAny(".。．｡");
			num = ((num < 0) ? unicode.Length : (num + num2));
			if (num == num2)
			{
			...
			{
				if (unicode.AsSpan(num2).StartsWith("xn--", StringComparison.OrdinalIgnoreCase))
				{
				...
				while (num5 < num - num2)
				{
					int num11 = 134217727;
					int num12;
					for (int j = num2; j < num; j += ((!IsSupplementary(num12)) ? 1 : 2))
					{
						num12 = char.ConvertToUtf32(unicode, j);
						if (num12 >= num8 && num12 < num11)
						{
							num11 = num12;
						}
					}
					num9 += (num11 - num8) * (num5 - num7 + 1);
					num8 = num11;
					for (int j = num2; j < num; j += ((!IsSupplementary(num12)) ? 1 : 2))
					{
						num12 = char.ConvertToUtf32(unicode, j);
	...
	private static string PunycodeDecode(string ascii)
	{
		...
		{
			throw new ArgumentException(SR.Format(SR.Argument_IdnBadNameSize, 255 - ((!IsDot(ascii[ascii.Length - 1])) ? 1 : 0)), "ascii");
		}
			...
			else
			{
				...
				else
				{
					...
					for (int i = num2; i < num2 + num5; i++)
					{
						...
						stringBuilder.Append((char)(char.IsAsciiLetterUpper(ascii[i]) ? (ascii[i] - 65 + 97) : ascii[i]));
					}
				}
