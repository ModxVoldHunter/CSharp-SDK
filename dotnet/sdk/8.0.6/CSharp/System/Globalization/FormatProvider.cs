...
internal static class FormatProvider
{
	private static class Number
	{
		...
		private unsafe static bool ParseNumber(ref char* str, char* strEnd, NumberStyles options, scoped ref NumberBuffer number, StringBuilder sb, NumberFormatInfo numfmt, bool parseDecimal)
		{
			number.scale = 0;
			number.sign = false;
			string text = null;
			...
			{
				char* ptr2;
				if (char.IsAsciiDigit(c) || ((options & NumberStyles.AllowHexSpecifier) != 0 && char.IsBetween((char)(c | 0x20u), 'a', 'f')))
				{
					num |= 4;
		...
		{
			return !s.Slice(index).ContainsAnyExcept('\0');
		}

		internal unsafe static bool TryStringToNumber(ReadOnlySpan<char> str, NumberStyles options, scoped ref NumberBuffer number, StringBuilder sb, NumberFormatInfo numfmt, bool parseDecimal)
		{
		...
		internal unsafe static void NumberToString(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, char format, int nMaxDigits, NumberFormatInfo info, bool isDecimal)
		{
			int num = -1;
			switch (format)
			{
		...
		private static void FormatCurrency(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, int nMinDigits, int nMaxDigits, NumberFormatInfo info)
		{
		...
		private unsafe static void FormatFixed(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, int nMinDigits, int nMaxDigits, NumberFormatInfo info, int[] groupDigits, string sDecimal, string sGroup)
		{
				...
				if (groupDigits != null)
				{
					...
					if (num3 != 0)
					{
						while (scale > num2 && groupDigits[num] != 0)
						{
							...
							ArgumentOutOfRangeException.ThrowIfNegative(num2, "groupSizeCount");
							ArgumentOutOfRangeException.ThrowIfNegative(num4, "bufferSize");
						}
		...
		private static void FormatNumber(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, int nMinDigits, int nMaxDigits, NumberFormatInfo info)
		{
		...
		private unsafe static void FormatScientific(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, int nMinDigits, int nMaxDigits, NumberFormatInfo info, char expChar)
		{
			char* digits = number.digits;
		...
		private unsafe static void FormatGeneral(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, int nMinDigits, int nMaxDigits, NumberFormatInfo info, char expChar, bool bSuppressScientific)
		{
			int i = number.scale;
			bool flag = false;
		...
		private static void FormatPercent(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, int nMinDigits, int nMaxDigits, NumberFormatInfo info)
		{
		...
		internal unsafe static void NumberToStringFormat(ref System.Text.ValueStringBuilder sb, scoped ref NumberBuffer number, ReadOnlySpan<char> format, NumberFormatInfo info)
		{
			int num = 0;
			char* digits = number.digits;
