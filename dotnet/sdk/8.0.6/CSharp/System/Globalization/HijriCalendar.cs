...
public class HijriCalendar : Calendar
{
	...
	private static ReadOnlySpan<int> HijriMonthDays => RuntimeHelpers.CreateSpan<int>((RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);
	...
	private static int GetAdvanceHijriDate()
	{
		...
		if (string.Compare(text, 0, "AddHijriDate", 0, "AddHijriDate".Length, StringComparison.OrdinalIgnoreCase) == 0)
		{
			...
			else if (int.TryParse(text.AsSpan("AddHijriDate".Length), CultureInfo.InvariantCulture, out result2) && result2 >= -2 && result2 <= 2)
			{
				result = result2;
			}
		}
