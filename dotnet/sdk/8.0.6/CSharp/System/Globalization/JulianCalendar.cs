...
public class JulianCalendar : Calendar
{
	...
	internal static int GetDatePart(long ticks, int part)
	{
		...
		ReadOnlySpan<int> readOnlySpan = ((num4 == 3) ? GregorianCalendar.DaysToMonth366 : GregorianCalendar.DaysToMonth365);
		int i;
		for (i = (num2 >> 5) + 1; num2 >= readOnlySpan[i]; i++)
		{
		}
		if (part == 2)
		{
		...
		return num2 - readOnlySpan[i - 1] + 1;
	}
	...
	internal static long DateToTicks(int year, int month, int day)
	{
		ReadOnlySpan<int> readOnlySpan = ((year % 4 == 0) ? GregorianCalendar.DaysToMonth366 : GregorianCalendar.DaysToMonth365);
		int num = year - 1;
		int num2 = num * 365 + num / 4 + readOnlySpan[month - 1] + day - 1;
