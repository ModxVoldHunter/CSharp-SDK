...
internal static class DateTimeParse
{
	...
	internal static bool TryParseExactMultiple(ReadOnlySpan<char> s, string[] formats, DateTimeFormatInfo dtfi, DateTimeStyles style, scoped ref DateTimeResult result)
	{
		if (formats == null)
		{
			result.SetFailure(ParseFailureKind.ArgumentNull_String, null, "formats");
			return false;
		}
		if (s.Length == 0)
		{
	...
	private static bool ParseFraction(ref __DTString str, out double result)
	{
		...
		int num2 = 0;
		char current;
		while (str.GetNext() && char.IsAsciiDigit(current = str.m_current))
		{
	...
	private static bool GetDayOfNNY(ref DateTimeResult result, scoped ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
	{
		...
		else if (SetDateYDM(ref result, raw.year, number, number2))
		{
			result.flags |= ParseFlags.HaveDate;
			return true;
		}
	...
	internal unsafe static bool TryParse(ReadOnlySpan<char> s, DateTimeFormatInfo dtfi, DateTimeStyles styles, scoped ref DateTimeResult result)
	{
		if (s.Length == 0)
		{
			result.SetFailure(ParseFailureKind.Format_BadDateTime);
	...
	private static bool MatchAbbreviatedMonthName(ref __DTString str, DateTimeFormatInfo dtfi, scoped ref int result)
	{
		...
		if (str.GetNext())
		{
			if (dtfi == DateTimeFormat.InvariantFormatInfo)
			{
				ReadOnlySpan<char> readOnlySpan = str.Value.Slice(str.Index);
				if (readOnlySpan.Length >= 3)
				{
					uint num = readOnlySpan[0];
					uint num2 = readOnlySpan[1];
					uint num3 = readOnlySpan[2];
					if ((num | num2 | num3) <= 127)
					{
						maxMatchStrLen = 3;
						switch ((num << 16) | (num2 << 8) | num3 | 0x202020)
						{
						case 6971758u:
							result = 1;
							break;
			...
			{
				int num4 = ((dtfi.GetMonthName(13).Length == 0) ? 12 : 13);
			...
			{
				int num5 = str.MatchLongestWords(dtfi.InternalGetGenitiveMonthNames(abbreviated: true), ref maxMatchStrLen);
				if (num5 >= 0)
				{
					result = num5 + 1;
				}
	...
	private static bool MatchMonthName(ref __DTString str, DateTimeFormatInfo dtfi, scoped ref int result)
	{
		...
		if (str.GetNext())
		{
				...
				if (span.Length >= 3)
				{
					...
					if ((num | num2 | num3) <= 127)
					{
						...
						case 6971758u:
							...
							break;
						case 6710626u:
							SetIfStartsWith(span, "February", 2, ref result, ref maxMatchStrLen);
							break;
						case 7168370u:
						...
						case 6387047u:
							...
							break;
						case 7562608u:
							SetIfStartsWith(span, "September", 9, ref result, ref maxMatchStrLen);
							break;
						case 7299956u:
							SetIfStartsWith(span, "October", 10, ref result, ref maxMatchStrLen);
							break;
						case 7237494u:
							SetIfStartsWith(span, "November", 11, ref result, ref maxMatchStrLen);
							break;
						case 6579555u:
							SetIfStartsWith(span, "December", 12, ref result, ref maxMatchStrLen);
							break;
						}
					}
				}
			...
			if ((dtfi.FormatFlags & DateTimeFormatFlags.UseGenitiveMonth) != 0)
			{
				int num5 = str.MatchLongestWords(dtfi.InternalGetGenitiveMonthNames(abbreviated: false), ref maxMatchStrLen);
				if (num5 >= 0)
				{
					result = num5 + 1;
				}
	...
	private static bool MatchAbbreviatedDayName(ref __DTString str, DateTimeFormatInfo dtfi, scoped ref int result)
	{
		...
		if (str.GetNext())
		{
			if (dtfi == DateTimeFormat.InvariantFormatInfo)
			{
				...
				if (readOnlySpan.Length >= 3)
				{
					uint num2 = readOnlySpan[0];
					uint num3 = readOnlySpan[1];
					uint num4 = readOnlySpan[2];
					if ((num2 | num3 | num4) <= 127)
					{
						num = 3;
						switch ((num2 << 16) | (num3 << 8) | num4 | 0x202020)
						{
						case 7566702u:
							result = 0;
							break;
		...
		if (str.GetNext())
		{
			if (dtfi == DateTimeFormat.InvariantFormatInfo)
			{
				...
				if (span.Length >= 3)
				{
					...
					if ((num | num2 | num3) <= 127)
					{
						...
						{
						case 7566702u:
							SetIfStartsWith(span, "Sunday", 0, ref result, ref maxMatchStrLen);
							break;
						case 7171950u:
							SetIfStartsWith(span, "Monday", 1, ref result, ref maxMatchStrLen);
							break;
						case 7632229u:
							SetIfStartsWith(span, "Tuesday", 2, ref result, ref maxMatchStrLen);
							break;
						case 7824740u:
							SetIfStartsWith(span, "Wednesday", 3, ref result, ref maxMatchStrLen);
							break;
						case 7628917u:
							SetIfStartsWith(span, "Thursday", 4, ref result, ref maxMatchStrLen);
							break;
						case 6713961u:
							SetIfStartsWith(span, "Friday", 5, ref result, ref maxMatchStrLen);
							break;
						case 7561588u:
							SetIfStartsWith(span, "Saturday", 6, ref result, ref maxMatchStrLen);
							break;
						}
					}
				}
			...
			else
			{
				for (DayOfWeek dayOfWeek = DayOfWeek.Sunday; dayOfWeek <= DayOfWeek.Saturday; dayOfWeek++)
				{
					...
					if ((dtfi.HasSpacesInDayNames ? str.MatchSpecifiedWords(dayName, checkWordBoundary: false, ref matchLength) : str.MatchSpecifiedWord(dayName)) && matchLength > maxMatchStrLen)
					{
						maxMatchStrLen = matchLength;
						result = (int)dayOfWeek;
					}
	...
	private static bool ParseByFormat(ref __DTString str, ref __DTString format, scoped ref ParsingInfo parseInfo, DateTimeFormatInfo dtfi, scoped ref DateTimeResult result)
	{
		int result2 = 0;
		int result3 = 0;
		int result4 = 0;
		...
		case 'y':
		{
			...
			if (!flag && parseInfo.fUseHebrewNumberParser)
			{
				flag = MatchHebrewDigits(ref str, out result2);
			}
			if (!flag)
			{
				result.SetBadDateTimeFailure();
		...
		case 'M':
			...
			if (returnValue <= 2)
			{
				if (!ParseDigits(ref str, returnValue, out result3) && (!parseInfo.fUseHebrewNumberParser || !MatchHebrewDigits(ref str, out result3)))
				{
					result.SetBadDateTimeFailure();
					return false;
				}
		...
		case 'd':
			...
			if (returnValue <= 2)
			{
				if (!ParseDigits(ref str, returnValue, out result4) && (!parseInfo.fUseHebrewNumberParser || !MatchHebrewDigits(ref str, out result4)))
				{
					result.SetBadDateTimeFailure();
					return false;
				}
		...
		case 'f':
			...
			if (returnValue <= 7)
			{
				...
				{
					result.SetFailure(ParseFailureKind.Format_RepeatDateTimePattern, @char);
					return false;
				}
				break;
			}
		...
		{
			returnValue = format.GetRepeatCount();
			TimeSpan result13 = new TimeSpan(0L);
			if (!ParseTimeZoneOffset(ref str, returnValue, ref result13))
			{
				result.SetBadDateTimeFailure();
				return false;
			}
			if ((result.flags & ParseFlags.TimeZoneUsed) != 0 && result13 != result.timeZoneOffset)
			{
				result.SetFailure(ParseFailureKind.Format_RepeatDateTimePattern, 'z');
				return false;
			}
			result.timeZoneOffset = result13;
			result.flags |= ParseFlags.TimeZoneUsed;
			break;
		}
		case 'Z':
		...
		case '\'':
		{
			...
			ValueStringBuilder result11 = new ValueStringBuilder(initialBuffer);
			if (!TryParseQuoteString(format.Value, format.Index, ref result11, out returnValue))
			{
				result.SetFailure(ParseFailureKind.Format_BadQuote, @char);
				result11.Dispose();
				return false;
			}
			format.Index += returnValue - 1;
			ReadOnlySpan<char> span = result11.AsSpan();
			for (int i = 0; i < span.Length; i++)
			{
				if (parseInfo.fAllowInnerWhite && char.IsWhiteSpace(span[i]))
				{
					str.SkipWhiteSpaces();
				}
				else if (!str.Match(span[i]))
				{
					result.SetBadDateTimeFailure();
					result11.Dispose();
					return false;
			...
			if ((result.flags & ParseFlags.CaptureOffset) != 0 && (((result.flags & ParseFlags.Rfc1123Pattern) != 0 && span.SequenceEqual("GMT")) || ((result.flags & ParseFlags.UtcSortPattern) != 0 && span.SequenceEqual("Z"))))
			{
				result.flags |= ParseFlags.TimeZoneUsed;
				result.timeZoneOffset = TimeSpan.Zero;
			}
		...
		case '.':
			...
			break;
		default:
			if (parseInfo.fAllowInnerWhite && char.IsWhiteSpace(@char))
			{
				break;
			}
			...
			{
				if (parseInfo.fAllowTrailingWhite && char.IsWhiteSpace(@char) && format.GetNext() && ParseByFormat(ref str, ref format, ref parseInfo, dtfi, ref result))
				{
					return true;
				}
				result.SetBadDateTimeFailure();
	...
	private static bool DoStrictParse(ReadOnlySpan<char> s, ReadOnlySpan<char> formatParam, DateTimeStyles styles, DateTimeFormatInfo dtfi, scoped ref DateTimeResult result)
	{
		ParsingInfo parseInfo = new ParsingInfo(dtfi.Calendar);
		...
		if (formatParam.Length == 1)
		{
			...
			formatParam = ExpandPredefinedFormat(c, ref dtfi, ref parseInfo, ref result);
		}
	...
	{
		if (source.Length < 27 || source[4] != '-' || source[7] != '-' || source[10] != 'T' || source[13] != ':' || source[16] != ':' || source[19] != '.')
		{
			result.SetBadDateTimeFailure();
			return false;
		}
	...
	private static Exception GetDateTimeParseException(scoped ref DateTimeResult result)
	{
		...
		{
			ParseFailureKind.ArgumentNull_String => new ArgumentNullException(result.failureArgumentName, SR.ArgumentNull_String), 
			ParseFailureKind.Format_BadDatePattern => new FormatException(SR.Format(SR.Format_BadDatePattern, result.failureMessageFormatArgument)), 
			ParseFailureKind.Format_BadDateTime => new FormatException(SR.Format(SR.Format_BadDateTime, new string(result.originalDateTimeString))), 
			ParseFailureKind.Format_BadDateTimeCalendar => new FormatException(SR.Format(SR.Format_BadDateTimeCalendar, new string(result.originalDateTimeString), result.calendar)), 
			ParseFailureKind.Format_BadDayOfWeek => new FormatException(SR.Format(SR.Format_BadDayOfWeek, new string(result.originalDateTimeString))), 
			ParseFailureKind.Format_BadFormatSpecifier => new FormatException(SR.Format(SR.Format_BadFormatSpecifier, new string(result.failedFormatSpecifier))), 
			ParseFailureKind.Format_BadQuote => new FormatException(SR.Format(SR.Format_BadQuote, result.failureMessageFormatArgument)), 
			ParseFailureKind.Format_DateOutOfRange => new FormatException(SR.Format(SR.Format_DateOutOfRange, new string(result.originalDateTimeString))), 
			ParseFailureKind.Format_MissingIncompleteDate => new FormatException(SR.Format(SR.Format_MissingIncompleteDate, new string(result.originalDateTimeString))), 
			ParseFailureKind.Format_NoFormatSpecifier => new FormatException(SR.Format_NoFormatSpecifier), 
			ParseFailureKind.Format_OffsetOutOfRange => new FormatException(SR.Format(SR.Format_OffsetOutOfRange, new string(result.originalDateTimeString))), 
			ParseFailureKind.Format_RepeatDateTimePattern => new FormatException(SR.Format(SR.Format_RepeatDateTimePattern, result.failureMessageFormatArgument)), 
			ParseFailureKind.Format_UnknownDateTimeWord => new FormatException(SR.Format(SR.Format_UnknownDateTimeWord, new string(result.originalDateTimeString), result.failureMessageFormatArgument)), 
			ParseFailureKind.Format_UTCOutOfRange => new FormatException(SR.Format(SR.Format_UTCOutOfRange, new string(result.originalDateTimeString))), 
			_ => null, 
		};
	}
}
