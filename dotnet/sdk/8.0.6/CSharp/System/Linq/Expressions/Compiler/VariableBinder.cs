...
[RequiresDynamicCode("Creating a StrongBox requires dynamic code generation.")]
internal sealed class VariableBinder : ExpressionVisitor
{
	...
	private ReadOnlyCollection<Expression> MergeScopes(Expression node)
	{
		...
		while (readOnlyCollection.Count == 1 && readOnlyCollection[0].NodeType == ExpressionType.Block)
		{
			...
			if (blockExpression.Variables.Count > 0)
			{
				...
				if (compilerScope2.MergedScopes == null)
				{
					compilerScope2.MergedScopes = new HashSet<BlockExpression>(ReferenceEqualityComparer.Instance);
				}
	...
	protected internal override Expression VisitParameter(ParameterExpression node)
	{
		...
		CompilerScope compilerScope2 = compilerScope;
		if (compilerScope2.ReferenceCount == null)
		{
			compilerScope2.ReferenceCount = new Dictionary<ParameterExpression, int>();
		}
