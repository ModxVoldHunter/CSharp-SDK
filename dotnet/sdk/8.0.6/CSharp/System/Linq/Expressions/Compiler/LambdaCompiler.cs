...
[RequiresDynamicCode("Compiling a lambda expression requires dynamic code generation.")]
internal sealed class LambdaCompiler : ILocalCache
{
	...
	private void EmitBinaryMethod(BinaryExpression b, CompilationFlags flags)
	{
		if (b.IsLifted)
		{
			...
			EmitLift(resultType: (!b.IsLiftedToNull) ? typeof(bool) : methodCallExpression.Type.LiftPrimitiveOrThrow(), nodeType: b.NodeType, mc: methodCallExpression, paramList: new ParameterExpression[2] { parameterExpression, parameterExpression2 }, argList: new Expression[2] { b.Left, b.Right });
		}
		else
		{
	...
	private void EmitSwitchBuckets(SwitchInfo info, List<List<SwitchLabel>> buckets, int first, int last)
	{
		while (first != last)
		{
			...
			else
			{
				...
				List<SwitchLabel> list = buckets[num - 1];
				EmitConstant(list[list.Count - 1].Constant);
