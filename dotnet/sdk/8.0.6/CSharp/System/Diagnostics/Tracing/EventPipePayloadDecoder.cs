...
internal static class EventPipePayloadDecoder
{
	internal static object[] DecodePayload(ref EventSource.EventMetadata metadata, ReadOnlySpan<byte> payload)
	{
		...
		for (int i = 0; i < parameters.Length; i++)
		{
			...
			{
				array[i] = BinaryPrimitives.ReadIntPtrLittleEndian(payload);
			...
			{
				array[i] = BinaryPrimitives.ReadSingleLittleEndian(payload);
				payload = payload.Slice(4);
			}
			else if (parameterType == typeof(double))
			{
				array[i] = BinaryPrimitives.ReadDoubleLittleEndian(payload);
				payload = payload.Slice(8);
			}
