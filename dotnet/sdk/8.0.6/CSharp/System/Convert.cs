...
public static class Convert
{
	...
	public unsafe static string ToBase64String(ReadOnlySpan<byte> bytes, Base64FormattingOptions options = Base64FormattingOptions.None)
	{
		...
		int length = ToBase64_CalculateAndValidateOutputLength(bytes.Length, flag);
		...
		else
		{
			fixed (byte* inData = &MemoryMarshal.GetReference(bytes))
			{
				...
				{
					int num = ConvertToBase64Array(outChars, inData, 0, bytes.Length, flag);
				}
			}
		}
		return text;
	...
	public unsafe static int ToBase64CharArray(byte[] inArray, int offsetIn, int length, char[] outArray, int offsetOut, Base64FormattingOptions options)
	{
		...
		else
		{
			fixed (char* outChars = &outArray[offsetOut])
			{
				...
				{
					int num4 = ConvertToBase64Array(outChars, inData, offsetIn, length, flag);
				}
			}
		}
		return num3;
	...
	public unsafe static bool TryToBase64Chars(ReadOnlySpan<byte> bytes, Span<char> chars, out int charsWritten, Base64FormattingOptions options = Base64FormattingOptions.None)
	{
		...
		else
		{
			fixed (char* outChars = &MemoryMarshal.GetReference(chars))
			{
				fixed (byte* inData = &MemoryMarshal.GetReference(bytes))
				{
					int num2 = ConvertToBase64Array(outChars, inData, 0, bytes.Length, flag);
				}
			}
		}
		charsWritten = num;
	...
	private static void ToBase64CharsLargeNoLineBreaks(ReadOnlySpan<byte> bytes, Span<char> chars, int charLengthRequired)
	{
		int bytesConsumed;
		int bytesWritten;
		OperationStatus operationStatus = Base64.EncodeToUtf8(bytes, MemoryMarshal.AsBytes(chars), out bytesConsumed, out bytesWritten);
		...
		if (Vector256.IsHardwareAccelerated)
		{
			...
			while (Unsafe.IsAddressGreaterThan(ref reference, ref right))
			{
				...
				var (source3, source4) = Vector256.Widen(Vector256.LoadUnsafe(ref reference));
		...
		if (Vector128.IsHardwareAccelerated)
		{
			...
			while (Unsafe.IsAddressGreaterThan(ref reference, ref right2))
			{
				...
				var (source5, source6) = Vector128.Widen(Vector128.LoadUnsafe(ref reference));
				source5.StoreUnsafe(ref source);
				source6.StoreUnsafe(ref source, 8u);
			}
		}
		ref byte right3 = ref Unsafe.Add(ref source2, 3);
		while (Unsafe.IsAddressGreaterThan(ref reference, ref right3))
		{
			source = ref Unsafe.Subtract(ref source, 4);
			reference = ref Unsafe.Subtract(ref reference, 4);
			Ascii.WidenFourAsciiBytesToUtf16AndWriteToBuffer(ref Unsafe.As<ushort, char>(ref source), Unsafe.ReadUnaligned<uint>(ref reference));
		}
	}
