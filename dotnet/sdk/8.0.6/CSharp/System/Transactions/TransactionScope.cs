...
public sealed class TransactionScope : IDisposable
{
	...
	{
		ArgumentNullException.ThrowIfNull(transactionToUse, "transactionToUse");
		ValidateScopeTimeout("scopeTimeout", scopeTimeout);
		CommonInitialize();
	...
	public void Dispose()
	{
		...
		try
		{
			...
			if (!Equals(currentScope))
			{
				...
				{
					Transaction transaction2 = (Transaction)(((object)_committableTransaction) ?? ((object)_dependentTransaction));
	...
	private void InternalDispose()
	{
		...
		try
		{
			...
			if (!_complete)
			{
				...
				Transaction transaction = (Transaction)(((object)_committableTransaction) ?? ((object)_dependentTransaction));
				transaction.Rollback();
			}
	...
	private static void TimerCallback(object state)
	{
		if (!(state is TransactionScope transactionScope))
		{
			...
			if (log.IsEnabled())
			{
				log.InternalError("TransactionScopeTimerObjectInvalid");
			}
			throw TransactionException.Create(System.SR.InternalError + System.SR.TransactionScopeTimerObjectInvalid, null);
		}
