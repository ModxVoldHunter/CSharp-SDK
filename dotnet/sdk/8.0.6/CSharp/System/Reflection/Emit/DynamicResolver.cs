...
internal sealed class DynamicResolver : Resolver
{
	...
	~DynamicResolver()
	{
		...
		if (!(method == null) && method._methodHandle != null)
		{
			...
			destroyScout.m_methodHandle = method._methodHandle.Value;
		}
	}
	...
	internal override RuntimeType GetJitContext(out int securityControlFlags)
	{
		...
		RuntimeType typeOwner = m_method._typeOwner;
	...
	internal override void ResolveToken(int token, out nint typeHandle, out nint methodHandle, out nint fieldHandle)
	{
		...
		else if (obj is VarArgMethod varArgMethod)
		{
			if (varArgMethod.m_dynamicMethod == null)
			{
				...
				typeHandle = varArgMethod.m_method.GetDeclaringTypeInternal().TypeHandle.Value;
			}
			else
			{
