...
internal static class PseudoCustomAttribute
{
	...
	private static MarshalAsAttribute GetMarshalAsCustomAttribute(int token, RuntimeModule scope)
	{
		...
		RuntimeType safeArrayUserDefinedSubType2 = (string.IsNullOrEmpty(safeArrayUserDefinedSubType) ? null : TypeNameParser.GetTypeReferencedByCustomAttribute(safeArrayUserDefinedSubType, scope));
		RuntimeType marshalTypeRef = null;
		try
		{
			marshalTypeRef = ((marshalType == null) ? null : TypeNameParser.GetTypeReferencedByCustomAttribute(marshalType, scope));
		}
