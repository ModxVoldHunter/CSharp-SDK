...
{
	private sealed class ArrayInitializeCache
	{
		...
		public unsafe ArrayInitializeCache(RuntimeType arrayType)
		{
	...
	private unsafe static extern Array InternalCreate(RuntimeType elementType, int rank, int* pLengths, int* pLowerBounds);
	...
	private unsafe static void CopyImpl(Array sourceArray, int sourceIndex, Array destinationArray, int destinationIndex, int length, bool reliable)
	{
		...
		ArgumentOutOfRangeException.ThrowIfNegative(sourceIndex - lowerBound, "sourceIndex");
		...
		ArgumentOutOfRangeException.ThrowIfNegative(destinationIndex - lowerBound2, "destinationIndex");
	...
	private unsafe nint GetFlattenedIndex(int rawIndex)
	{
		...
		{
			rawIndex -= Unsafe.Add(ref RuntimeHelpers.GetMultiDimensionalArrayBounds(this), 1);
		}
	...
	{
		MethodTable* methodTable = RuntimeHelpers.GetMethodTable(this);
		return methodTable->ElementType == RuntimeHelpers.GetMethodTable(value);
	}
	...
	public unsafe void Initialize()
	{
		...
		if (ptr->HasDefaultConstructor && ptr->IsValueType)
		{
			...
			ArrayInitializeCache arrayInitializeCache = runtimeType.GenericCache as ArrayInitializeCache;
			if (arrayInitializeCache == null)
			{
				arrayInitializeCache = (ArrayInitializeCache)(runtimeType.GenericCache = new ArrayInitializeCache(runtimeType));
			}
			delegate*<ref byte, void> constructorEntrypoint = arrayInitializeCache.ConstructorEntrypoint;
	...
	public unsafe static Array CreateInstance(Type elementType, int length)
	{
		...
		return InternalCreate(runtimeType, 1, &length, null);
	}
	...
	public unsafe static void Copy(Array sourceArray, Array destinationArray, int length)
	{
		...
		MethodTable* methodTable = RuntimeHelpers.GetMethodTable(sourceArray);
		if (MethodTable.AreSameType(methodTable, RuntimeHelpers.GetMethodTable(destinationArray)) && !methodTable->IsMultiDimensionalArray && (uint)length <= sourceArray.NativeLength && (uint)length <= destinationArray.NativeLength)
		{
	...
	public unsafe static void Copy(Array sourceArray, int sourceIndex, Array destinationArray, int destinationIndex, int length)
	{
		...
		{
			MethodTable* methodTable = RuntimeHelpers.GetMethodTable(sourceArray);
			if (MethodTable.AreSameType(methodTable, RuntimeHelpers.GetMethodTable(destinationArray)) && !methodTable->IsMultiDimensionalArray && length >= 0 && sourceIndex >= 0 && destinationIndex >= 0 && (uint)(sourceIndex + length) <= sourceArray.NativeLength && (uint)(destinationIndex + length) <= destinationArray.NativeLength)
			{
				nuint num = methodTable->ComponentSize;
				nuint num2 = (uint)length * num;
	...
	public static int IndexOf<T>(T[] array, T value, int startIndex, int count)
	{
		...
		if (RuntimeHelpers.IsBitwiseEquatable<T>())
		{
			...
			{
				int num = SpanHelpers.IndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, byte>(ref MemoryMarshal.GetArrayDataReference(array)), startIndex), Unsafe.As<T, byte>(ref value), count);
				return ((num >= 0) ? startIndex : 0) + num;
			}
			if (Unsafe.SizeOf<T>() == 2)
			{
				int num2 = SpanHelpers.IndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, short>(ref MemoryMarshal.GetArrayDataReference(array)), startIndex), Unsafe.As<T, short>(ref value), count);
				return ((num2 >= 0) ? startIndex : 0) + num2;
			}
			if (Unsafe.SizeOf<T>() == 4)
			{
				int num3 = SpanHelpers.IndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, int>(ref MemoryMarshal.GetArrayDataReference(array)), startIndex), Unsafe.As<T, int>(ref value), count);
				return ((num3 >= 0) ? startIndex : 0) + num3;
			}
			if (Unsafe.SizeOf<T>() == 8)
			{
				int num4 = SpanHelpers.IndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, long>(ref MemoryMarshal.GetArrayDataReference(array)), startIndex), Unsafe.As<T, long>(ref value), count);
	...
	public static int LastIndexOf<T>(T[] array, T value, int startIndex, int count)
	{
		...
		if (RuntimeHelpers.IsBitwiseEquatable<T>())
		{
			...
			{
				int num = startIndex - count + 1;
				int num2 = SpanHelpers.LastIndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, byte>(ref MemoryMarshal.GetArrayDataReference(array)), num), Unsafe.As<T, byte>(ref value), count);
				return ((num2 >= 0) ? num : 0) + num2;
			}
			if (Unsafe.SizeOf<T>() == 2)
			{
				int num3 = startIndex - count + 1;
				int num4 = SpanHelpers.LastIndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, short>(ref MemoryMarshal.GetArrayDataReference(array)), num3), Unsafe.As<T, short>(ref value), count);
				return ((num4 >= 0) ? num3 : 0) + num4;
			}
			if (Unsafe.SizeOf<T>() == 4)
			{
				int num5 = startIndex - count + 1;
				int num6 = SpanHelpers.LastIndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, int>(ref MemoryMarshal.GetArrayDataReference(array)), num5), Unsafe.As<T, int>(ref value), count);
				return ((num6 >= 0) ? num5 : 0) + num6;
			}
			if (Unsafe.SizeOf<T>() == 8)
			{
				int num7 = startIndex - count + 1;
				int num8 = SpanHelpers.LastIndexOfValueType(ref Unsafe.Add(ref Unsafe.As<T, long>(ref MemoryMarshal.GetArrayDataReference(array)), num7), Unsafe.As<T, long>(ref value), count);
