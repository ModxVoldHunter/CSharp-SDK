...
public static class Utf8Parser
{
	...
	private static bool TryCreateDateTime(int year, int month, int day, int hour, int minute, int second, int fraction, DateTimeKind kind, out DateTime value)
	{
		...
		ReadOnlySpan<int> readOnlySpan = (DateTime.IsLeapYear(year) ? GregorianCalendar.DaysToMonth366 : GregorianCalendar.DaysToMonth365);
		int num2 = year - 1;
		int num3 = num2 * 365 + num2 / 4 - num2 / 100 + num2 / 400 + readOnlySpan[month - 1] + day - 1;
	...
	{
		Span<byte> digits = stackalloc byte[114];
	...
	{
		Span<byte> digits = stackalloc byte[769];
		Number.NumberBuffer number = new Number.NumberBuffer(Number.NumberBufferKind.FloatingPoint, digits);
		if (TryParseNormalAsFloatingPoint(source, ref number, out bytesConsumed, standardFormat))
		{
