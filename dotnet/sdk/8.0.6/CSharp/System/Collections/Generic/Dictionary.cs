...
public class Dictionary<TKey, TValue> : IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IDictionary, ICollection, IReadOnlyDictionary<TKey, TValue>, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, ISerializable, IDeserializationCallback where TKey : notnull
{
	...
	public sealed class KeyCollection : ICollection<TKey>, IEnumerable<TKey>, IEnumerable, ICollection, IReadOnlyCollection<TKey>
	{
		...
		IEnumerator<TKey> IEnumerable<TKey>.GetEnumerator()
		{
			...
			return SZGenericArrayEnumerator<TKey>.Empty;
		}
	...
	public sealed class ValueCollection : ICollection<TValue>, IEnumerable<TValue>, IEnumerable, ICollection, IReadOnlyCollection<TValue>
	{
		...
		IEnumerator<TValue> IEnumerable<TValue>.GetEnumerator()
		{
			...
			return SZGenericArrayEnumerator<TValue>.Empty;
		}
	...
	private void AddRange(IEnumerable<KeyValuePair<TKey, TValue>> enumerable)
	{
		...
		{
			Dictionary<TKey, TValue> dictionary = (Dictionary<TKey, TValue>)enumerable;
		...
		ReadOnlySpan<KeyValuePair<TKey, TValue>> readOnlySpan;
		...
		else
		{
			...
			readOnlySpan = CollectionsMarshal.AsSpan((List<KeyValuePair<TKey, TValue>>)enumerable);
		}
	...
	IEnumerator<KeyValuePair<TKey, TValue>> IEnumerable<KeyValuePair<TKey, TValue>>.GetEnumerator()
	{
		...
		return GenericEmptyEnumerator<KeyValuePair<TKey, TValue>>.Instance;
	}
	...
	private void Resize(int newSize, bool forceNewHashCodes)
	{
		...
		{
			IEqualityComparer<TKey> equalityComparer = (_comparer = (IEqualityComparer<TKey>)((NonRandomizedStringEqualityComparer)_comparer).GetRandomizedEqualityComparer());
			for (int i = 0; i < count; i++)
			{
				if (array[i].next >= -1)
				{
	...
	public bool Remove(TKey key)
	{
		...
		if (_buckets != null)
		{
			uint num = 0u;
			IEqualityComparer<TKey> comparer = _comparer;
			uint num2 = (uint)((typeof(TKey).IsValueType && comparer == null) ? key.GetHashCode() : comparer.GetHashCode(key));
			...
			{
				ref Entry reference = ref entries[num4];
				if (reference.hashCode == num2 && ((typeof(TKey).IsValueType && comparer == null) ? EqualityComparer<TKey>.Default.Equals(reference.key, key) : comparer.Equals(reference.key, key)))
				{
					if (num3 < 0)
					{
