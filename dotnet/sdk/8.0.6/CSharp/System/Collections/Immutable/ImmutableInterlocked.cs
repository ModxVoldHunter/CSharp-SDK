...
public static class ImmutableInterlocked
{
	...
	{
		Requires.NotNull(transformer, "transformer");
		T[] array = Volatile.Read(ref Unsafe.AsRef(ref location.array));
		bool flag;
		do
		{
			...
			T[] array2 = Interlocked.CompareExchange(ref Unsafe.AsRef(ref location.array), immutableArray.array, array);
			flag = array == array2;
			array = array2;
		}
		while (!flag);
	...
	{
		return new ImmutableArray<T>(Interlocked.Exchange(ref Unsafe.AsRef(ref location.array), value.array));
	}
	...
	{
		return new ImmutableArray<T>(Interlocked.CompareExchange(ref Unsafe.AsRef(ref location.array), value.array, comparand.array));
	}
