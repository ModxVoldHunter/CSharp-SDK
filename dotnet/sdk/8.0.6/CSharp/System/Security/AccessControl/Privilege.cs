...
internal sealed class Privilege
{
	private sealed class TlsContents : IDisposable
	{
		...
		private Microsoft.Win32.SafeHandles.SafeTokenHandle threadHandle = new Microsoft.Win32.SafeHandles.SafeTokenHandle(IntPtr.Zero);

		private readonly bool isImpersonating;

		private static volatile Microsoft.Win32.SafeHandles.SafeTokenHandle processHandle = new Microsoft.Win32.SafeHandles.SafeTokenHandle(IntPtr.Zero);

		private static readonly object syncRoot = new object();

		public int ReferenceCountValue => referenceCount;

		public Microsoft.Win32.SafeHandles.SafeTokenHandle ThreadHandle => threadHandle;
		...
		public TlsContents()
		{
			...
			if (processHandle.IsInvalid)
			{
				lock (syncRoot)
				{
					if (processHandle.IsInvalid)
					{
						...
						{
							num2 = Marshal.GetLastPInvokeError();
							flag = false;
						}
			...
			try
			{
				Microsoft.Win32.SafeHandles.SafeTokenHandle safeTokenHandle = threadHandle;
	...
	{
		Unsafe.SkipInit(out global::Interop.Advapi32.LUID value);
		value.LowPart = 0;
		value.HighPart = 0;
		try
		{
			privilegeLock.EnterReadLock();
			if (luids.TryGetValue(privilege, out value))
			{
			...
			{
				privilegeLock.ExitReadLock();
				if (!global::Interop.Advapi32.LookupPrivilegeValue(null, privilege, out value))
				{
		...
		finally
		{
			...
			{
				if (luids.TryAdd(privilege, value))
				{
					privileges[value] = privilege;
				}
	...
	{
		ArgumentNullException.ThrowIfNull(privilegeName, "privilegeName");
	...
	private unsafe void ToggleState(bool enable)
	{
		...
		try
		{
			...
			else if (1300 == Marshal.GetLastPInvokeError())
			{
				num = 1300;
			}
			else
