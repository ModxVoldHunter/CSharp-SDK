...
public class CryptoStream : Stream, IDisposable
{
	...
	public CryptoStream(Stream stream, ICryptoTransform transform, CryptoStreamMode mode, bool leaveOpen)
	{
		...
		case CryptoStreamMode.Read:
			if (!_stream.CanRead)
			{
				throw new ArgumentException(System.SR.Argument_StreamNotReadable, "stream");
			}
			_canRead = true;
			break;
		case CryptoStreamMode.Write:
			if (!_stream.CanWrite)
			{
				throw new ArgumentException(System.SR.Argument_StreamNotWritable, "stream");
			}
			_canWrite = true;
			break;
		default:
	...
	public override void Write(ReadOnlySpan<byte> buffer)
	{
		...
		{
			CryptographicOperations.ZeroMemory(array.AsSpan(0, buffer.Length));
			ArrayPool<byte>.Shared.Return(array);
		}
	}
	...
	private async ValueTask WriteAsyncCore(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken, bool useAsync)
	{
		...
		if (_inputBufferIndex == _inputBlockSize)
		{
			...
			if (useAsync)
			{
				await _stream.WriteAsync(new ReadOnlyMemory<byte>(_outputBuffer, 0, numOutputBytes), cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			}
			else
			{
			...
			if (bytesToWrite >= _inputBlockSize)
			{
				...
				if (_transform.CanTransformMultipleBlocks && num > 1)
				{
					...
					try
					{
						...
						if (useAsync)
						{
							await _stream.WriteAsync(new ReadOnlyMemory<byte>(tempOutputBuffer, 0, numOutputBytes), cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
						}
						else
						{
				...
				{
					int numOutputBytes = TransformBlock(_transform, buffer.Slice(currentInputIndex, _inputBlockSize), _outputBuffer, 0);
					if (useAsync)
					{
						await _stream.WriteAsync(new ReadOnlyMemory<byte>(_outputBuffer, 0, numOutputBytes), cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
					}
