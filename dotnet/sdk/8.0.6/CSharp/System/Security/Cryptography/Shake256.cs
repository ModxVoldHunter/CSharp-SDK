...
public sealed class Shake256 : IDisposable
{
	...
	public static bool IsSupported { get; } = HashProviderDispenser.HashSupported("CSHAKE256");


	public Shake256()
	{
		CheckPlatformSupport();
		_hashProvider = LiteHashProvider.CreateXof("CSHAKE256");
	}
	...
	public static byte[] HashData(Stream source, int outputLength)
	{
		...
		CheckPlatformSupport();
		return LiteHashProvider.XofStream("CSHAKE256", outputLength, source);
	}
	...
	public static void HashData(Stream source, Span<byte> destination)
	{
		...
		CheckPlatformSupport();
		LiteHashProvider.XofStream("CSHAKE256", source, destination);
	}
	...
	public static ValueTask HashDataAsync(Stream source, Memory<byte> destination, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		CheckPlatformSupport();
		return LiteHashProvider.XofStreamAsync("CSHAKE256", source, destination, cancellationToken);
	}
	...
	public static ValueTask<byte[]> HashDataAsync(Stream source, int outputLength, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		CheckPlatformSupport();
		return LiteHashProvider.XofStreamAsync("CSHAKE256", outputLength, source, cancellationToken);
	}

	private static void HashDataCore(ReadOnlySpan<byte> source, Span<byte> destination)
	{
		HashProviderDispenser.OneShotHashProvider.HashDataXof("CSHAKE256", source, destination);
	}
