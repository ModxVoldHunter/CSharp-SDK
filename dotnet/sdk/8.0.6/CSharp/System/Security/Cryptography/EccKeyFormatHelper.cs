...
internal static class EccKeyFormatHelper
{
	...
	private static void WriteUncompressedBasePoint(in ECParameters ecParameters, AsnWriter writer)
	{
		...
		span[0] = 4;
		ecParameters.Curve.G.X.CopyTo(span.Slice(1));
		ecParameters.Curve.G.Y.CopyTo(span.Slice(1 + ecParameters.Curve.G.X.Length));
		writer.WriteOctetString(span.Slice(0, num));
		if (array != null)
		{
	...
	private static void WriteUncompressedPublicKey(in ECParameters ecParameters, AsnWriter writer)
	{
		...
		span[0] = 4;
		ecParameters.Q.X.CopyTo(span.Slice(1));
		ecParameters.Q.Y.CopyTo(span.Slice(1 + ecParameters.Q.X.Length));
		writer.WriteBitString(span.Slice(0, num));
		if (array != null)
		{
			System.Security.Cryptography.CryptoPool.Return(array, 0);
		}
