...
public class HttpWebRequest : WebRequest, ISerializable
{
	...
	public string? Connection
	{
		...
		set
		{
			...
			bool flag = value.Contains("keep-alive", StringComparison.OrdinalIgnoreCase);
			bool flag2 = value.Contains("close", StringComparison.OrdinalIgnoreCase);
			if (flag || flag2)
			{
	...
	private async Task<WebResponse> SendRequest(bool async)
	{
		...
		HttpRequestMessage httpRequestMessage = new HttpRequestMessage(HttpMethod.Parse(_originVerb), _requestUri);
		...
		try
		{
			...
			AddCacheControlHeaders(httpRequestMessage);
			foreach (string item in _webHeaderCollection)
			{
				if (IsWellKnownContentHeader(item))
				{
					...
					{
						httpRequestMessage2.Content = new ByteArrayContent(Array.Empty<byte>());
					}
