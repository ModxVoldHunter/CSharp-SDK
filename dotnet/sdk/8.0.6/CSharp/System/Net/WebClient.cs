...
public class WebClient : Component
{
	...
	private void OpenFileInternal(bool needsHeaderAndBoundary, string fileName, out FileStream fs, out byte[] buffer, ref byte[] formHeaderBytes, ref byte[] boundaryBytes)
	{
		...
		if (string.Equals(_method, "POST", StringComparison.Ordinal))
		{
			if (needsHeaderAndBoundary)
			{
				...
				int bytesWritten;
				OperationStatus operationStatus = Ascii.FromUtf16(text2, boundaryBytes.AsSpan("\r\n--".Length), out bytesWritten);
			}
			else
			{
	...
	private byte[] UploadBits(WebRequest request, FileStream readStream, byte[] buffer, int chunkSize, byte[] header, byte[] footer)
	{
		try
		{
	...
	private async void UploadBitsAsync(WebRequest request, FileStream readStream, byte[] buffer, int chunkSize, byte[] header, byte[] footer, AsyncOperation asyncOp, Action<byte[], Exception, AsyncOperation> completionDelegate)
	{
	...
	private string GetStringUsingEncoding(WebRequest request, byte[] data)
	{
		...
		if (encoding == null)
		{
			...
			foreach (Encoding encoding2 in array3)
			{
				ReadOnlySpan<byte> preamble = encoding2.Preamble;
		...
		if (num == -1)
		{
			ReadOnlySpan<byte> preamble2 = encoding.Preamble;
			num = (data.AsSpan().StartsWith(preamble2) ? preamble2.Length : 0);
		}
