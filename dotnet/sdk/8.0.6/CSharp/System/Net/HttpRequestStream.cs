...
internal sealed class HttpRequestStream : Stream
{
	private sealed class HttpRequestStreamAsyncResult : System.Net.LazyAsyncResult
	{
		...
		internal unsafe HttpRequestStreamAsyncResult(ThreadPoolBoundHandle boundHandle, object asyncObject, object userState, AsyncCallback callback, byte[] buffer, int offset, uint dataAlreadyRead)
			: base(asyncObject, userState, callback)
		{
			_dataAlreadyRead = dataAlreadyRead;
		...
		private unsafe static void IOCompleted(HttpRequestStreamAsyncResult asyncResult, uint errorCode, uint numBytes)
		{
			...
			try
			{
				...
				else
				{
					...
					if (System.Net.NetEventSource.Log.IsEnabled())
					{
						System.Net.NetEventSource.DumpBuffer(asyncResult, new ReadOnlySpan<byte>(asyncResult._pPinnedBuffer, (int)numBytes), "IOCompleted");
					}
				}
				if (System.Net.NetEventSource.Log.IsEnabled())
				{
		...
		private unsafe static void Callback(uint errorCode, uint numBytes, NativeOverlapped* nativeOverlapped)
		{
			...
			if (System.Net.NetEventSource.Log.IsEnabled())
			{
				System.Net.NetEventSource.Info(null, $"asyncResult: {httpRequestStreamAsyncResult} errorCode:0x {errorCode:x8} numBytes: {numBytes} nativeOverlapped:0x{(nint)nativeOverlapped:x8}", "Callback");
			}
	...
	private unsafe int ReadCore(byte[] buffer, int offset, int size)
	{
		...
		if (_dataChunkIndex == -1 && num < size)
		{
			...
			if (num2 != 0 && num2 != 38)
			{
				HttpListenerException ex = new HttpListenerException((int)num2);
				if (System.Net.NetEventSource.Log.IsEnabled())
				{
	...
	public unsafe IAsyncResult BeginReadCore(byte[] buffer, int offset, int size, AsyncCallback callback, object state)
	{
		...
		if (_dataChunkIndex != -1)
		{
			...
			if (_dataChunkIndex != -1 && num == size)
			{
				httpRequestStreamAsyncResult2 = new HttpRequestStreamAsyncResult(_httpContext.RequestQueueBoundHandle, this, state, callback, buffer, offset, 0u);
		...
		if (_dataChunkIndex == -1 && num < size)
		{
			...
			httpRequestStreamAsyncResult2 = new HttpRequestStreamAsyncResult(_httpContext.RequestQueueBoundHandle, this, state, callback, buffer, offset, num);
			uint bytesReturned;
			try
			{
				fixed (byte* ptr = buffer)
			...
			if (num2 != 0 && num2 != 997)
			{
				...
				if (num2 != 38)
				{
					HttpListenerException ex2 = new HttpListenerException((int)num2);
					if (System.Net.NetEventSource.Log.IsEnabled())
					{
