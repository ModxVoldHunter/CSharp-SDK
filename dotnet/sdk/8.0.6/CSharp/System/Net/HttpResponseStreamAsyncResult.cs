...
internal sealed class HttpResponseStreamAsyncResult : System.Net.LazyAsyncResult
{
	...
	internal unsafe global::Interop.HttpApi.HTTP_DATA_CHUNK* pDataChunks
	{
		get
		{
			...
			return (global::Interop.HttpApi.HTTP_DATA_CHUNK*)Marshal.UnsafeAddrOfPinnedArrayElement(_dataChunks, 0);
		}
	}
		...
		: base(asyncObject, userState, callback)
	{
		...
		{
			System.Net.NetEventSource.Info(this, $"m_pOverlapped:0x{(nint)_pOverlapped:x8}", ".ctor");
		}
		...
		if (chunked)
		{
			_dataChunks[0].pBuffer = (byte*)Marshal.UnsafeAddrOfPinnedArrayElement(array2, offset2);
	...
	private unsafe static void IOCompleted(HttpResponseStreamAsyncResult asyncResult, uint errorCode, uint numBytes)
	{
		...
		try
		{
			...
			else if (asyncResult._dataChunks == null)
			{
				...
				if (System.Net.NetEventSource.Log.IsEnabled())
				{
					System.Net.NetEventSource.DumpBuffer(null, Array.Empty<byte>(), "IOCompleted");
				}
			}
			else
			{
				result = ((asyncResult._dataChunks.Length == 1) ? asyncResult._dataChunks[0].BufferLength : 0u);
				if (System.Net.NetEventSource.Log.IsEnabled())
				{
					global::Interop.HttpApi.HTTP_DATA_CHUNK[] dataChunks = asyncResult._dataChunks;
					for (int i = 0; i < dataChunks.Length; i++)
					{
						global::Interop.HttpApi.HTTP_DATA_CHUNK hTTP_DATA_CHUNK = dataChunks[i];
						System.Net.NetEventSource.DumpBuffer(null, new ReadOnlySpan<byte>(hTTP_DATA_CHUNK.pBuffer, (int)hTTP_DATA_CHUNK.BufferLength), "IOCompleted");
					}
				}
			}
			if (System.Net.NetEventSource.Log.IsEnabled())
	...
	private unsafe static void Callback(uint errorCode, uint numBytes, NativeOverlapped* nativeOverlapped)
	{
		...
		if (System.Net.NetEventSource.Log.IsEnabled())
		{
			System.Net.NetEventSource.Info(null, $"errorCode:0x{errorCode:x8} numBytes:{numBytes} nativeOverlapped:0x{(nint)nativeOverlapped:x8}", "Callback");
		}
