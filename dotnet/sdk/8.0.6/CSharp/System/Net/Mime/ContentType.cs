...
public class ContentType
{
	...
	{
		ArgumentException.ThrowIfNullOrEmpty(contentType, "contentType");
		_isChanged = true;
		_type = contentType;
		ParseValue();
	}
	...
	private void ParseValue()
	{
		try
		{
			...
			while (MailBnfHelper.SkipCFWS(_type, ref offset))
			{
				...
				string text = MailBnfHelper.ReadParameterAttribute(_type, ref offset);
				...
				string text2 = ((_type[offset] == '"') ? MailBnfHelper.ReadQuotedString(_type, ref offset, null) : MailBnfHelper.ReadToken(_type, ref offset));
				if (text2 == null)
				{
					throw new FormatException(System.SR.ContentTypeInvalid);
				}
