...
internal static class TlsFrameHelper
{
	...
	private static readonly byte[] s_protocolMismatch13 = new byte[7] { 21, 3, 4, 0, 2, 2, 70 };

	private static readonly byte[] s_protocolMismatch12 = new byte[7] { 21, 3, 3, 0, 2, 2, 70 };

	private static readonly byte[] s_protocolMismatch11 = new byte[7] { 21, 3, 2, 0, 2, 2, 70 };

	private static readonly byte[] s_protocolMismatch10 = new byte[7] { 21, 3, 1, 0, 2, 2, 70 };

	private static readonly byte[] s_protocolMismatch30 = new byte[7] { 21, 3, 0, 0, 2, 2, 40 };
	...
	public static bool TryGetFrameHeader(ReadOnlySpan<byte> frame, ref TlsFrameHeader header)
	{
		...
		if (frame[1] == 3)
		{
			header.Length = ((frame[3] << 8) | frame[4]) + 5;
			header.Version = TlsMinorVersionToProtocol(frame[2]);
		}
		else if (frame[2] == 1 && frame[3] == 3)
		{
			int length = (((frame[0] & 0x80) == 0) ? ((((frame[0] & 0x3F) << 8) | frame[1]) + 3) : ((((frame[0] & 0x7F) << 8) | frame[1]) + 2));
	...
	public static bool TryGetFrameInfo(ReadOnlySpan<byte> frame, ref TlsFrameInfo info, ProcessingOptions options = ProcessingOptions.All, HelloExtensionCallback callback = null)
	{
		...
		bool result = frame.Length >= info.Header.Length;
	...
	private static bool TryParseHelloExtensions(ReadOnlySpan<byte> extensions, ref TlsFrameInfo info, ProcessingOptions options, HelloExtensionCallback callback)
	{
		...
		while (extensions.Length >= 4)
		{
			...
			else if (extensionType == ExtensionType.ApplicationProtocols && (options == ProcessingOptions.All || options.HasFlag(ProcessingOptions.ApplicationProtocol) || options.HasFlag(ProcessingOptions.RawApplicationProtocol)))
			{
				...
				{
					info.RawApplicationProtocols = readOnlySpan.Slice(2).ToArray();
				}
			}
