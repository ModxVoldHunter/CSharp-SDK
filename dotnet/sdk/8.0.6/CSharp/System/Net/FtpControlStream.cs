...
internal sealed class FtpControlStream : CommandStream
{
	...
	private static readonly char[] s_whitespaceDot = new char[4] { ' ', '.', '\r', '\n' };

	private static readonly char[] s_spaceCommaBrackets = new char[4] { ' ', '(', ',', ')' };
	...
	internal string BannerMessage => _bannerMessage?.ToString();

	internal string WelcomeMessage => _welcomeMessage?.ToString();

	internal string ExitMessage => _exitMessage?.ToString();
	...
	protected override PipelineInstruction PipelineCallback(PipelineEntry entry, ResponseDescription response, bool timeout, ref Stream stream)
	{
		...
		if (_index == -1)
		{
			...
			default:
				throw CommandStream.GenerateException(status, response.StatusDescription, null);
			}
		}
		...
		if (entry.Command.Contains("USER") && status == FtpStatusCode.LoggedInProceed)
		{
	...
	protected override PipelineEntry[] BuildCommandsList(WebRequest req)
	{
		...
		if (!ftpWebRequest.MethodInfo.IsCommandOnly)
		{
			...
			else
			{
				...
				list.Add(new PipelineEntry(FormatFtpCommand(command2, GetPortCommandLine())));
			}
		...
		if (ftpWebRequest.MethodInfo.Operation == FtpOperation.Rename)
		{
			...
			string parameter = ((ftpWebRequest.RenameTo == null || !ftpWebRequest.RenameTo.StartsWith('/')) ? (text4 + ftpWebRequest.RenameTo) : ftpWebRequest.RenameTo);
	...
	private PipelineInstruction QueueOrCreateDataConection(PipelineEntry entry, ResponseDescription response, out bool isSocketReady)
	{
		...
		if (flag)
		{
			try
			{
				_dataSocket = CreateFtpDataSocket(base.Socket);
			}
			catch (ObjectDisposedException)
			{
	...
	private DateTime GetLastModifiedFrom213Response(string str)
	{
		...
		int num = source.SplitAny(destination, " .");
		if (num < 2)
		{
			return result;
		}
		Range range = destination[1];
		ReadOnlySpan<char> readOnlySpan = source[range.Start..range.End];
		if (readOnlySpan.Length < 14)
		{
			return result;
		}
		int year = int.Parse(readOnlySpan.Slice(0, 4), NumberFormatInfo.InvariantInfo);
		int month = short.Parse(readOnlySpan.Slice(4, 2), NumberFormatInfo.InvariantInfo);
		int day = short.Parse(readOnlySpan.Slice(6, 2), NumberFormatInfo.InvariantInfo);
		int hour = short.Parse(readOnlySpan.Slice(8, 2), NumberFormatInfo.InvariantInfo);
		int minute = short.Parse(readOnlySpan.Slice(10, 2), NumberFormatInfo.InvariantInfo);
		int second = short.Parse(readOnlySpan.Slice(12, 2), NumberFormatInfo.InvariantInfo);
		int millisecond = 0;
		if (num > 2)
		{
			range = destination[2];
			millisecond = short.Parse(source[range.Start..range.End], NumberFormatInfo.InvariantInfo);
		}
		try
		{
	...
	private void TryUpdateResponseUri(string str, FtpWebRequest request)
	{
		...
		if (num2 > num)
		{
			string text = str.AsSpan(num, num2 - num).TrimEnd(s_whitespaceDot).ToString();
			string text2 = text.Replace("%", "%25");
			text2 = text2.Replace("#", "%23");
			string absolutePath = uri.AbsolutePath;
			if (absolutePath.Length > 0 && !absolutePath.EndsWith('/'))
			{
	...
	{
		string[] array = responseString.Split(s_spaceCommaBrackets);
		if (array.Length <= 7)
		{
			throw new FormatException(System.SR.Format(System.SR.net_ftp_response_invalid_format, responseString));
		}
	...
	private string GetPortCommandLine()
	{
		...
		{
			throw CommandStream.GenerateException(System.SR.net_ftp_protocolerror, WebExceptionStatus.ProtocolError, innerException);
		}
	}
	...
	private static string FormatFtpCommand(string command, string parameter)
	{
		...
		{
			throw new FormatException(System.SR.net_ftp_no_newlines);
		}
