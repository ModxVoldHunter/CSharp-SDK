...
internal sealed class ListenerClientCertAsyncResult : System.Net.LazyAsyncResult
{
	...
	internal unsafe void Reset(uint size)
	{
		if (size != _size)
		{
			...
			else
			{
				...
				_memoryBlob = (global::Interop.HttpApi.HTTP_SSL_CLIENT_CERT_INFO*)Marshal.UnsafeAddrOfPinnedArrayElement(_backingBuffer, 0);
			}
		}
	}
		...
		try
		{
			...
			else
			{
				...
				if (memoryBlob != null)
				{
					...
					{
						System.Net.NetEventSource.Info(null, $"pClientCertInfo:{(nint)memoryBlob} pClientCertInfo->CertFlags: {memoryBlob->CertFlags} pClientCertInfo->CertEncodedSize: {memoryBlob->CertEncodedSize} pClientCertInfo->pCertEncoded: {(nint)memoryBlob->pCertEncoded} pClientCertInfo->Token: {(nint)memoryBlob->Token} pClientCertInfo->CertDeniedByMapper: {memoryBlob->CertDeniedByMapper}", "IOCompleted");
					}
					if (memoryBlob->pCertEncoded != null)
					{
						try
						{
							...
							Marshal.Copy((nint)memoryBlob->pCertEncoded, array, 0, array.Length);
