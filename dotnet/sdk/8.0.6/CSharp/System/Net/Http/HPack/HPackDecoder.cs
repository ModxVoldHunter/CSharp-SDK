...
internal sealed class HPackDecoder
{
	...
	private void ProcessHeaderValue(ReadOnlySpan<byte> data, IHttpStreamHeadersHandler handler)
	{
		...
		if (_headerStaticIndex > 0)
		{
			...
			if (_index)
			{
				_dynamicTable.Insert(_headerStaticIndex, H2StaticTable.Get(_headerStaticIndex - 1).Name, value);
			}
		}
		else
		{
	...
	private void OnIndexedHeaderField(int index, IHttpStreamHeadersHandler handler)
	{
		...
		else
		{
			...
			handler.OnDynamicIndexedHeader(dynamicHeader.StaticTableIndex, dynamicHeader.Name, dynamicHeader.Value);
		}
		_state = State.Ready;
	}
