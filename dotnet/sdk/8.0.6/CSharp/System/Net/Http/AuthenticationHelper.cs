...
internal static class AuthenticationHelper
{
	...
	public static async Task<string> GetDigestTokenForCredential(NetworkCredential credential, HttpRequestMessage request, DigestResponse digestResponse)
	{
		...
		else if (!Ascii.IsValid(credential.UserName))
		{
	...
	{
		return RandomNumberGenerator.GetString("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", 16);
	}
	...
	private static string ComputeHash(string data, string algorithm)
	{
		...
		return System.HexConverter.ToString(destination[..((!algorithm.StartsWith("SHA-256", StringComparison.OrdinalIgnoreCase)) ? MD5.HashData(bytes, destination) : SHA256.HashData(bytes, destination))], System.HexConverter.Casing.Lower);
	}
	...
	private static bool ProxySupportsConnectionAuth(HttpResponseMessage response)
	{
		...
		{
			if (item.Equals("Session-Based-Authentication", StringComparison.OrdinalIgnoreCase))
			{
				return true;
			}
		}
	...
	private static async Task<HttpResponseMessage> SendWithNtAuthAsync(HttpRequestMessage request, Uri authUri, bool async, ICredentials credentials, bool isProxyAuth, HttpConnection connection, HttpConnectionPool connectionPool, CancellationToken cancellationToken)
	{
		...
		if (TryGetAuthenticationChallenge(response, isProxyAuth, authUri, credentials, out var challenge) && (challenge.AuthenticationType == AuthenticationType.Negotiate || challenge.AuthenticationType == AuthenticationType.Ntlm))
		{
			...
			try
			{
				...
				using NegotiateAuthentication authContext = new NegotiateAuthentication(clientOptions);
				...
				while (true)
				{
					...
					if (!IsAuthenticationChallenge(response, isProxyAuth))
					{
						...
						if (statusCode > NegotiateAuthenticationStatusCode.ContinueNeeded)
						{
							...
							connection.Dispose();
							throw new HttpRequestException(HttpRequestError.UserAuthenticationError, System.SR.Format(System.SR.net_http_authvalidationfailure, statusCode), null, HttpStatusCode.Unauthorized);
						}
						break;
					}
					needDrain = true;
