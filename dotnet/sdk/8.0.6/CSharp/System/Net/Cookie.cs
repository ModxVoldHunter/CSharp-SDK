...
public sealed class Cookie
{
	...
	private static readonly SearchValues<char> s_domainChars = SearchValues.Create("-.0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz");
	...
	public string Name
	{
		...
		set
		{
			...
			{
				throw new CookieException(System.SR.Format(System.SR.net_cookie_attribute, "Name", value ?? "<null>"));
			}
		}
	}
	...
	public string Port
	{
		...
		set
		{
			...
			string[] array = value.Split(PortSplitDelimiters, StringSplitOptions.RemoveEmptyEntries);
	...
	{
		if (string.IsNullOrEmpty(value) || value.StartsWith('$') || value.StartsWith(' ') || value.EndsWith(' ') || value.AsSpan().ContainsAny(s_reservedToNameChars))
		{
			m_name = string.Empty;
			return false;
		}
	...
	internal bool VerifySetDefaults(CookieVariant variant, Uri uri, bool isLocalDomain, string localDomain, bool setDefault, bool shouldThrow)
	{
		...
		if (string.IsNullOrEmpty(m_name) || m_name.StartsWith('$') || m_name.StartsWith(' ') || m_name.EndsWith(' ') || m_name.AsSpan().ContainsAny(s_reservedToNameChars))
		{
			if (shouldThrow)
			{
				throw new CookieException(System.SR.Format(System.SR.net_cookie_attribute, "Name", m_name ?? "<null>"));
			}
			return false;
		}
		if (m_value == null || ((m_value.Length <= 2 || !m_value.StartsWith('"') || !m_value.EndsWith('"')) && m_value.AsSpan().ContainsAny(';', ',')))
		{
			if (shouldThrow)
			{
				throw new CookieException(System.SR.Format(System.SR.net_cookie_attribute, "Value", m_value ?? "<null>"));
			}
			return false;
		}
		if (Comment != null && (Comment.Length <= 2 || !Comment.StartsWith('"') || !Comment.EndsWith('"')) && Comment.AsSpan().ContainsAny(';', ','))
		{
			if (shouldThrow)
			{
		...
		if (Path != null && (Path.Length <= 2 || !Path.StartsWith('"') || !Path.EndsWith('"')) && Path.AsSpan().ContainsAny(';', ','))
		{
			if (shouldThrow)
			{
		...
		else
		{
			if (!m_domain_implicit)
			{
				...
				if (!DomainCharsTest(text))
				{
					if (shouldThrow)
					{
						throw new CookieException(System.SR.Format(System.SR.net_cookie_attribute, "Domain", text ?? "<null>"));
					}
					return false;
				}
				if (text[0] != '.')
		...
		if (setDefault && m_path_implicit)
		{
			...
			{
				int length;
				if (!absolutePath.StartsWith('/') || (length = absolutePath.LastIndexOf('/')) == 0)
				{
					m_path = "/";
				}
				else
	...
	private static bool DomainCharsTest(string name)
	{
		...
		{
			return !name.AsSpan().ContainsAnyExcept(s_domainChars);
		}
		return false;
	}
	...
	{
		if (comparand is Cookie cookie && string.Equals(Name, cookie.Name, StringComparison.OrdinalIgnoreCase) && string.Equals(Value, cookie.Value, StringComparison.Ordinal) && string.Equals(Path, cookie.Path, StringComparison.Ordinal) && CookieComparer.EqualDomains(Domain, cookie.Domain))
		{
	...
	{
		return HashCode.Combine(StringComparer.OrdinalIgnoreCase.GetHashCode(Name), StringComparer.Ordinal.GetHashCode(Value), StringComparer.Ordinal.GetHashCode(Path), StringComparer.OrdinalIgnoreCase.GetHashCode(Domain), Version);
	}
