...
public sealed class FtpWebRequest : WebRequest
{
	...
	private TimerThread.Queue TimerQueue => _timerQueue ?? (_timerQueue = TimerThread.GetOrCreateQueue(RemainingTimeout));
	...
	public override WebHeaderCollection Headers
	{
		get
		{
			return _ftpRequestHeaders ?? (_ftpRequestHeaders = new WebHeaderCollection());
		}
		set
		{
	...
	internal FtpWebRequest(Uri uri)
	{
		...
		if (!string.IsNullOrEmpty(_uri.UserInfo))
		{
			...
			if (num != -1)
			{
				...
				num++;
				password = Uri.UnescapeDataString(userInfo.Substring(num));
			}
	...
	private async void CreateConnectionAsync()
	{
		...
		try
		{
			...
			obj = new FtpControlStream(new NetworkStream(client, ownsSocket: true));
		}
		catch (Exception e)
		{
	...
	private FtpControlStream CreateConnection()
	{
		...
		return new FtpControlStream(new NetworkStream(socket, ownsSocket: true));
	}
