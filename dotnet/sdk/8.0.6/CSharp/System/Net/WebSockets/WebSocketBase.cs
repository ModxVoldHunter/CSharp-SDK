...
internal abstract class WebSocketBase : WebSocket, IDisposable
{
	private abstract class WebSocketOperation
	{
		public sealed class ReceiveOperation : WebSocketOperation
		{
			...
			protected override void ProcessAction_IndicateReceiveComplete(ArraySegment<byte>? buffer, WebSocketProtocolComponent.BufferType bufferType, WebSocketProtocolComponent.Action action, global::Interop.WebSocket.Buffer[] dataBuffers, uint dataBufferCount, nint actionContext)
			{
				int num = 0;
				_pongReceived = false;
				...
				try
				{
					...
					if (bufferType == WebSocketProtocolComponent.BufferType.Close)
					{
						...
						_webSocket._internalBuffer.ConvertCloseBuffer(dataBuffers[0], out var closeStatus, out var reason);
						receiveResult = new WebSocketReceiveResult(num, messageType, endOfMessage: true, closeStatus, reason);
					}
					else
					{
						ArraySegment<byte> empty = _webSocket._internalBuffer.ConvertNativeBuffer(dataBuffers[0], bufferType);
		...
		protected virtual void ProcessAction_IndicateReceiveComplete(ArraySegment<byte>? buffer, WebSocketProtocolComponent.BufferType bufferType, WebSocketProtocolComponent.Action action, global::Interop.WebSocket.Buffer[] dataBuffers, uint dataBufferCount, nint actionContext)
		{
				...
				while (ShouldContinue(cancellationToken))
				{
					...
					while (!completed)
					{
							...
							if (ProcessAction_NoAction())
							{
								...
								try
								{
									if (_webSocket.StartOnCloseReceived(ref thisLockTaken))
									{
										...
										try
										{
											flag = await _webSocket.StartOnCloseCompleted(thisLockTaken, sessionHandleLockTaken, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
										}
										catch (Exception)
										{
						...
						case WebSocketProtocolComponent.Action.ReceiveFromNetwork:
						{
							...
							try
							{
								ArraySegment<byte> arraySegment = _webSocket._internalBuffer.ConvertNativeBuffer(array[0], bufferType);
								ReleaseLock(_webSocket.SessionHandle, ref sessionHandleLockTaken);
								...
								try
								{
									...
									count = await task.ConfigureAwait(continueOnCapturedContext: false);
						...
						case WebSocketProtocolComponent.Action.IndicateSendComplete:
							...
							ReleaseLock(_webSocket.SessionHandle, ref sessionHandleLockTaken);
							await _webSocket._innerStream.FlushAsync(cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
							Monitor.Enter(_webSocket.SessionHandle, ref sessionHandleLockTaken);
							break;
						case WebSocketProtocolComponent.Action.SendToNetwork:
						{
							...
							try
							{
								if ((_webSocket.State != WebSocketState.CloseSent || (bufferType != WebSocketProtocolComponent.BufferType.PingPong && bufferType != WebSocketProtocolComponent.BufferType.UnsolicitedPong)) && dataBufferCount != 0)
								{
									...
									ArraySegment<byte> item = _webSocket._internalBuffer.ConvertNativeBuffer(array[0], bufferType);
									list.Add(item);
									sendBufferSize += item.Count;
									if (dataBufferCount == 2)
									{
										ArraySegment<byte> item2 = ((!_webSocket._internalBuffer.IsPinnedSendPayloadBuffer(array[1], bufferType)) ? _webSocket._internalBuffer.ConvertNativeBuffer(array[1], bufferType) : _webSocket._internalBuffer.ConvertPinnedSendPayloadFromNative(array[1], bufferType));
										list.Add(item2);
										sendBufferSize += item2.Count;
									}
									ReleaseLock(_webSocket.SessionHandle, ref sessionHandleLockTaken);
									HttpWebSocket.ThrowIfConnectionAborted(_webSocket._innerStream, read: false);
									await _webSocket.SendFrameAsync(list, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
									Monitor.Enter(_webSocket.SessionHandle, ref sessionHandleLockTaken);
									_webSocket.ThrowIfPendingException();
	...
	private abstract class KeepAliveTracker : IDisposable
	{
		...
		private sealed class DefaultKeepAliveTracker : KeepAliveTracker
		{
			...
			{
				TimeSpan timeElapsed = GetTimeElapsed(_lastSendActivityTimestamp);
				TimeSpan timeElapsed2 = GetTimeElapsed(_lastReceiveActivityTimestamp);
	...
	private async Task<WebSocketReceiveResult> ReceiveAsyncCore(ArraySegment<byte> buffer, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			webSocketReceiveResult = await _receiveOperation.Process(buffer, linkedCancellationToken).ConfigureAwait(continueOnCapturedContext: false);
	...
	private async Task SendAsyncCore(ArraySegment<byte> buffer, WebSocketMessageType messageType, bool endOfMessage, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			await _sendOperation.Process(buffer, linkedCancellationToken).ConfigureAwait(continueOnCapturedContext: false);
		}
		catch (Exception exception)
		{
			bool isCancellationRequested = linkedCancellationToken.IsCancellationRequested;
	...
	private async Task SendFrameAsync(IList<ArraySegment<byte>> sendBuffers, CancellationToken cancellationToken)
	{
		...
		try
		{
			await _sendFrameThrottle.WaitAsync(cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			...
			{
				await _innerStreamAsWebSocketStream.MultipleWriteAsync(sendBuffers, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				return;
			}
			foreach (ArraySegment<byte> sendBuffer in sendBuffers)
			{
				await _innerStream.WriteAsync(sendBuffer.Array.AsMemory(sendBuffer.Offset, sendBuffer.Count), cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			}
		}
		catch (ObjectDisposedException innerException)
		{
	...
	private async Task CloseOutputAsyncCore(WebSocketCloseStatus closeStatus, string statusDescription, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			if (!ownsCloseOutputCancellationTokenSource)
			{
				...
				{
					ReleaseLocks(ref thisLockTaken, ref sessionHandleLockTaken);
					await closeOutputTask.ConfigureAwait(continueOnCapturedContext: false);
					TakeLocks(ref thisLockTaken, ref sessionHandleLockTaken);
				}
				return;
			}
			...
			ReleaseLocks(ref thisLockTaken, ref sessionHandleLockTaken);
			await _closeOutputTask.ConfigureAwait(continueOnCapturedContext: false);
			TakeLocks(ref thisLockTaken, ref sessionHandleLockTaken);
			if (OnCloseOutputCompleted())
			{
				...
				try
				{
					flag2 = await StartOnCloseCompleted(thisLockTaken, sessionHandleLockTaken, linkedCancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				}
				catch (Exception)
				{
	...
	private async Task<bool> StartOnCloseCompleted(bool thisLockTakenSnapshot, bool sessionHandleLockTakenSnapshot, CancellationToken cancellationToken)
	{
		...
		if (_innerStreamAsWebSocketStream != null)
		{
			...
			try
			{
				...
				await _closeNetworkConnectionTask.ConfigureAwait(continueOnCapturedContext: false);
			}
			catch (Exception ex)
			{
	...
	private async Task CloseAsyncCore(WebSocketCloseStatus closeStatus, string statusDescription, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			if (OnCloseOutputCompleted())
			{
				...
				try
				{
					flag = await StartOnCloseCompleted(lockTaken, sessionHandleLockTakenSnapshot: false, linkedCancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				}
				catch (Exception)
				{
			...
			if (flag2)
			{
				...
				try
				{
					receiveResult = await task2.ConfigureAwait(continueOnCapturedContext: false);
				}
				catch (Exception ex3)
				{
					Monitor.Enter(_thisLock, ref lockTaken);
			...
			else
			{
				...
				ReleaseLock(_thisLock, ref lockTaken);
				await _closeReceivedTaskCompletionSource.Task.ConfigureAwait(continueOnCapturedContext: false);
			}
			if (!lockTaken)
			{
				Monitor.Enter(_thisLock, ref lockTaken);
			...
			try
			{
				...
				try
				{
					flag3 = await StartOnCloseCompleted(lockTaken, sessionHandleLockTakenSnapshot: false, linkedCancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				}
				catch (Exception)
				{
					ResetFlagAndTakeLock(_thisLock, ref lockTaken);
		...
		try
		{
			...
			try
			{
				...
				if (ownsCancellationTokenSource)
				{
					...
					await thisPtr._keepAliveTask.ConfigureAwait(continueOnCapturedContext: false);
				}
			}
			finally
			{
