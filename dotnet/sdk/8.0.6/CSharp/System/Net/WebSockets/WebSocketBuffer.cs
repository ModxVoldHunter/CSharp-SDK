...
internal sealed class WebSocketBuffer : IDisposable
{
	...
	private WebSocketBuffer(ArraySegment<byte> internalBuffer, int receiveBufferSize, int sendBufferSize)
	{
		...
		_startAddress = ((IntPtr)Marshal.UnsafeAddrOfPinnedArrayElement(internalBuffer.Array, internalBuffer.Offset)).ToInt64();
	...
	{
		ThrowIfDisposed();
		nint num = _gcHandle.AddrOfPinnedObject();
		int offset = _propertyBuffer.Offset;
		Marshal.WriteInt32(num, offset, _receiveBufferSize);
		offset += 4;
		Marshal.WriteInt32(num, offset, _sendBufferSize);
		offset += 4;
		Marshal.WriteIntPtr(num, offset, num + _internalBuffer.Offset);
		offset += IntPtr.Size;
		Marshal.WriteInt32(num, offset, useZeroMaskingKey ? 1 : 0);
		int num2 = (useZeroMaskingKey ? 4 : 3);
		global::Interop.WebSocket.Property[] array = new global::Interop.WebSocket.Property[num2];
		...
		array[0] = new global::Interop.WebSocket.Property
		{
			...
			PropertySize = 4u,
			PropertyData = IntPtr.Add(num, offset)
		};
		offset += 4;
	...
	internal void PinSendBuffer(ArraySegment<byte> payload, out bool bufferHasBeenPinned)
	{
		...
		_pinnedSendBufferStartAddress = ((IntPtr)Marshal.UnsafeAddrOfPinnedArrayElement(_pinnedSendBuffer.Array, _pinnedSendBuffer.Offset)).ToInt64();
		_pinnedSendBufferEndAddress = _pinnedSendBufferStartAddress + _pinnedSendBuffer.Count;
	}

	internal nint ConvertPinnedSendPayloadToNative(ArraySegment<byte> payload)
	{
		return ConvertPinnedSendPayloadToNative(payload.Array, payload.Offset, payload.Count);
	}

	internal nint ConvertPinnedSendPayloadToNative(byte[] buffer, int offset, int count)
	{
	...
	internal ArraySegment<byte> ConvertPinnedSendPayloadFromNative(global::Interop.WebSocket.Buffer buffer, WebSocketProtocolComponent.BufferType bufferType)
	{
		...
		int num = (int)(((IntPtr)bufferData).ToInt64() - _pinnedSendBufferStartAddress);
	...
	internal bool IsPinnedSendPayloadBuffer(global::Interop.WebSocket.Buffer buffer, WebSocketProtocolComponent.BufferType bufferType)
	{
		...
		UnwrapWebSocketBuffer(buffer, bufferType, out var bufferData, out var bufferLength);
		long num = ((IntPtr)bufferData).ToInt64();
	...
	internal ArraySegment<byte> ConvertNativeBuffer(global::Interop.WebSocket.Buffer buffer, WebSocketProtocolComponent.BufferType bufferType)
	{
		ThrowIfDisposed();
		UnwrapWebSocketBuffer(buffer, bufferType, out var bufferData, out var bufferLength);
	...
	internal void ConvertCloseBuffer(global::Interop.WebSocket.Buffer buffer, out WebSocketCloseStatus closeStatus, out string reason)
	{
		ThrowIfDisposed();
	...
	internal static void UnwrapWebSocketBuffer(global::Interop.WebSocket.Buffer buffer, WebSocketProtocolComponent.BufferType bufferType, out nint bufferData, out uint bufferLength)
	{
	...
	private int GetOffset(nint pBuffer)
	{
		return (int)(((IntPtr)pBuffer).ToInt64() - _startAddress + _internalBuffer.Offset);
	}
	...
	internal nint ToIntPtr(int offset)
	{
		return new IntPtr(_startAddress + offset - _internalBuffer.Offset);
	}

	private bool IsNativeBuffer(nint pBuffer, uint bufferSize)
	{
		long num = ((IntPtr)pBuffer).ToInt64();
