...
public class BinaryReader : IDisposable
{
	...
	public virtual int Read()
	{
		...
		while (num == 0)
		{
			...
			try
			{
				num = _decoder.GetChars(new ReadOnlySpan<byte>(_charBytes, 0, num3), new Span<char>(ref reference), flush: false);
			}
			catch
			{
				if (_stream.CanSeek)
	...
	{
		return BinaryPrimitives.ReadHalfLittleEndian(InternalRead(2));
	}

	public virtual float ReadSingle()
	{
		return BinaryPrimitives.ReadSingleLittleEndian(InternalRead(4));
	}

	public virtual double ReadDouble()
	{
		return BinaryPrimitives.ReadDoubleLittleEndian(InternalRead(8));
	}
	...
	protected virtual void FillBuffer(int numBytes)
	{
		...
		else if (_stream.Read(_buffer, 0, 0) == 0)
		{
			ThrowHelper.ThrowEndOfFileException();
		}
	}
