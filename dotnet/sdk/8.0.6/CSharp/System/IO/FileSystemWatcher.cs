	...
	private unsafe void Monitor(AsyncReadState state)
	{
		...
		try
		{
			if (_enabled && !IsHandleInvalid(state.DirectoryHandle))
			{
				...
				flag = global::Interop.Kernel32.ReadDirectoryChangesW(state.DirectoryHandle, state.Buffer, (uint)state.Buffer.Length, _includeSubdirectories, (uint)_notifyFilters, null, ptr, null);
			}
		}
