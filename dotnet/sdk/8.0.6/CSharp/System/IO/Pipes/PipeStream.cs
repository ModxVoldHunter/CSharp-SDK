...
public abstract class PipeStream : Stream
{
		...
		internal unsafe void RegisterForCancellation(CancellationToken cancellationToken)
		{
			...
			try
			{
				_cancellationRegistration = cancellationToken.UnsafeRegister(delegate(object s, CancellationToken token)
				{
					...
					if (internalHandle != null && !internalHandle.IsClosed)
					{
						try
						{
							global::Interop.Kernel32.CancelIoEx(internalHandle, pipeValueTaskSource._overlapped);
						}
	...
	protected PipeStream(PipeDirection direction, int bufferSize)
	{
		...
		ArgumentOutOfRangeException.ThrowIfNegative(bufferSize, "bufferSize");
	...
	protected PipeStream(PipeDirection direction, PipeTransmissionMode transmissionMode, int outBufferSize)
	{
		...
		ArgumentOutOfRangeException.ThrowIfNegative(outBufferSize, "outBufferSize");
	...
	{
		return System.Threading.AsyncOverSyncWithIoCancellation.InvokeAsync(((PipeStream Stream, Memory<byte> Buffer) s) => s.Stream.ReadCore(s.Buffer.Span), (this, buffer), cancellationToken);
	}
	...
	{
		return System.Threading.AsyncOverSyncWithIoCancellation.InvokeAsync(delegate((PipeStream Stream, ReadOnlyMemory<byte> Buffer) s)
		{
			s.Stream.WriteCore(s.Buffer.Span);
		}, (this, buffer), cancellationToken);
	}
	...
	internal unsafe static global::Interop.Kernel32.SECURITY_ATTRIBUTES GetSecAttrs(HandleInheritability inheritability, PipeSecurity pipeSecurity, ref GCHandle pinningHandle)
	{
		...
		if (pipeSecurity != null)
		{
			...
			fixed (byte* lpSecurityDescriptor = securityDescriptorBinaryForm)
			{
				secAttrs.lpSecurityDescriptor = lpSecurityDescriptor;
			}
		}
		return secAttrs;
	}
