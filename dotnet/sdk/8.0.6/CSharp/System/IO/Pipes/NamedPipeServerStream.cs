...
public sealed class NamedPipeServerStream : PipeStream
{
	...
	private void ValidateParameters(string pipeName, PipeDirection direction, int maxNumberOfServerInstances, PipeTransmissionMode transmissionMode, PipeOptions options, int inBufferSize, int outBufferSize, HandleInheritability inheritability)
	{
		...
		if ((options & (PipeOptions)536346623) != 0)
		{
			throw new ArgumentOutOfRangeException("options", System.SR.ArgumentOutOfRange_OptionsInvalid);
		}
		ArgumentOutOfRangeException.ThrowIfNegative(inBufferSize, "inBufferSize");
	...
	{
		return TaskToAsyncResult.Begin(WaitForConnectionAsync(), callback, state);
	}
	...
	public Task WaitForConnectionAsync(CancellationToken cancellationToken)
	{
		if (!cancellationToken.IsCancellationRequested)
		{
			if (!base.IsAsync)
			{
				return System.Threading.AsyncOverSyncWithIoCancellation.InvokeAsync(delegate(NamedPipeServerStream s)
				{
					s.WaitForConnection();
				}, this, cancellationToken).AsTask();
			}
	...
	public void RunAsClient(PipeStreamImpersonationWorker impersonationWorker)
	{
		...
		{
			RevertImpersonationOnBackout(executeHelper);
		}
