...
public abstract class TextReader : MarshalByRefObject, IDisposable
{
	...
	{
		return ReadLineCoreAsync(default(CancellationToken));
	}

	public virtual ValueTask<string?> ReadLineAsync(CancellationToken cancellationToken)
	{
		return new ValueTask<string>(ReadLineCoreAsync(cancellationToken));
	}

	private Task<string> ReadLineCoreAsync(CancellationToken cancellationToken)
	{
		return Task<string>.Factory.StartNew((object state) => ((TextReader)state).ReadLine(), this, cancellationToken, TaskCreationOptions.DenyChildAttach, TaskScheduler.Default);
	}
	...
	public virtual async Task<string> ReadToEndAsync(CancellationToken cancellationToken)
	{
		...
		{
			int charCount;
			while ((charCount = await ReadAsyncInternal(chars, cancellationToken).ConfigureAwait(continueOnCapturedContext: false)) != 0)
			{
