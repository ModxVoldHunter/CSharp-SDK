...
internal static class Helper
{
	...
	internal static void GetDefaultIdentityAndHash(out object identity, out string hash, char separator)
	{
		...
		if (entryAssembly != null)
		{
			...
			if (hash != null)
			{
				hash = string.Concat("StrongName", new ReadOnlySpan<char>(ref separator), hash);
				identity = name;
				return;
			}
		...
		hash = string.Concat("Url", new ReadOnlySpan<char>(ref separator), IdentityHelper.GetNormalizedUriHash(uri));
		identity = uri;
	}
