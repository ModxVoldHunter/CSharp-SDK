	...
	public Span<T> Span
	{
		...
		get
		{
			...
			if (@object != null)
			{
				...
				{
					reference = ref Unsafe.As<char, T>(ref Unsafe.As<string>(@object).GetRawStringData());
					length = Unsafe.As<string>(@object).Length;
				}
				else if (RuntimeHelpers.ObjectHasComponentSize(@object))
				{
					reference = ref MemoryMarshal.GetArrayDataReference(Unsafe.As<T[]>(@object));
				...
				reference = ref Unsafe.Add(ref reference, (nint)num);
				length = length2;
			}
