...
internal sealed class XsdValidatingReader : XmlReader, IXmlSchemaInfo, IXmlLineInfo, IXmlNamespaceResolver
{
	...
	public override string Name
	{
		get
		{
			if (_validationState == ValidatingReaderState.OnDefaultAttribute)
			{
				...
				if (!string.IsNullOrEmpty(defaultAttributePrefix))
				{
	...
	public override string GetAttribute(int i)
	{
		...
		ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(i, _attributeCount, "i");
	...
	private AttributePSVIInfo GetAttributePSVI(string localName, string ns)
	{
		...
		{
			AttributePSVIInfo attributePSVIInfo = _attributePSVINodes[i];
	...
	private ValidatingReaderNodeData GetDefaultAttribute(string attrLocalName, string ns, bool updatePosition)
	{
		...
		{
			ValidatingReaderNodeData validatingReaderNodeData = (ValidatingReaderNodeData)_defaultAttributes[i];
	...
	private AttributePSVIInfo AddAttributePSVI(int attIndex)
	{
		...
		ref AttributePSVIInfo reference = ref _attributePSVINodes[attIndex];
		return reference ?? (reference = new AttributePSVIInfo());
	}
		...
		case XmlNodeType.Attribute:
			...
			if (_attributePSVI != null && _attributePSVI.typedAttributeValue != null)
			{
				if (_validationState == ValidatingReaderState.OnDefaultAttribute)
				{
					...
					originalStringValue = schemaAttribute.DefaultValue ?? schemaAttribute.FixedValue;
				}
	...
	private object InternalReadElementContentAsObject(out XmlSchemaType xmlType, bool unwrapTypedValue, out string originalString)
	{
		...
		if (IsEmptyElement)
		{
			result = ((_xmlSchemaInfo.ContentType != 0) ? _atomicValue : ReturnBoxedValue(_atomicValue, _xmlSchemaInfo.XmlType, unwrapTypedValue));
		...
		else
		{
			...
			result = InternalReadContentAsObject(unwrapTypedValue, out originalString);
			if (NodeType != XmlNodeType.EndElement)
			{
				throw new XmlException(System.SR.Xml_MixedReadElementContentAs, string.Empty, this);
			}
	...
	private string GetOriginalAtomicValueStringOfElement()
	{
		if (_xmlSchemaInfo.IsDefault)
		{
			...
			{
				return schemaElement.DefaultValue ?? schemaElement.FixedValue;
			}
			return string.Empty;
		}
	...
	private Task ProcessReaderEventAsync()
	{
		...
		{
			validator.ValidateText(await t.ConfigureAwait(continueOnCapturedContext: false));
		}
		static async Task ValidateWhitespace(Task<string> t, XmlSchemaValidator validator)
		{
			validator.ValidateWhitespace(await t.ConfigureAwait(continueOnCapturedContext: false));
		}
	}
	...
	private async Task<(string, object)> InternalReadContentAsObjectTupleAsync(bool unwrapTypedValue)
	{
		...
		case XmlNodeType.Attribute:
		{
			...
			if (_attributePSVI != null && _attributePSVI.typedAttributeValue != null)
			{
				...
				{
					XmlSchemaAttribute schemaAttribute = _attributePSVI.attributeSchemaInfo.SchemaAttribute;
					originalAtomicValueString = schemaAttribute.DefaultValue ?? schemaAttribute.FixedValue;
				}
	...
	private async Task<object> ReadTillEndElementAsync()
	{
		if (_atomicValue == null)
		{
			while (await _coreReader.ReadAsync().ConfigureAwait(continueOnCapturedContext: false))
			{
				...
				case XmlNodeType.CDATA:
				{
					XmlSchemaValidator validator = _validator;
					validator.ValidateText(await GetValueAsync().ConfigureAwait(continueOnCapturedContext: false));
					continue;
				}
				case XmlNodeType.Whitespace:
				case XmlNodeType.SignificantWhitespace:
				{
					XmlSchemaValidator validator = _validator;
					validator.ValidateWhitespace(await GetValueAsync().ConfigureAwait(continueOnCapturedContext: false));
					continue;
				}
				case XmlNodeType.EndElement:
					_atomicValue = _validator.ValidateEndElement(_xmlSchemaInfo);
