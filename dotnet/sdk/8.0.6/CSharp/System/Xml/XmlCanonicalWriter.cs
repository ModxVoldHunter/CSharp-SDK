...
internal sealed class XmlCanonicalWriter
{
	...
	public void SetOutput(Stream stream, bool includeComments, string[] inclusivePrefixes)
	{
		...
		for (int j = 0; j < inclusivePrefixes.Length; j++)
		{
			...
			{
				throw new ArgumentException(System.SR.InvalidInclusivePrefixListCollection);
			}
	...
	{
		ArgumentNullException.ThrowIfNull(prefixBuffer, "prefixBuffer");
		ArgumentOutOfRangeException.ThrowIfNegative(prefixOffset, "prefixOffset");
		if (prefixOffset > prefixBuffer.Length)
		{
			throw new ArgumentOutOfRangeException("prefixOffset", System.SR.Format(System.SR.OffsetExceedsBufferSize, prefixBuffer.Length));
		}
		ArgumentOutOfRangeException.ThrowIfNegative(prefixLength, "prefixLength");
		if (prefixLength > prefixBuffer.Length - prefixOffset)
		{
			throw new ArgumentOutOfRangeException("prefixLength", System.SR.Format(System.SR.SizeExceedsRemainingBufferSpace, prefixBuffer.Length - prefixOffset));
		}
		ArgumentNullException.ThrowIfNull(localNameBuffer, "localNameBuffer");
		ArgumentOutOfRangeException.ThrowIfNegative(localNameOffset, "localNameOffset");
		if (localNameOffset > localNameBuffer.Length)
		{
			throw new ArgumentOutOfRangeException("localNameOffset", System.SR.Format(System.SR.OffsetExceedsBufferSize, localNameBuffer.Length));
		}
		ArgumentOutOfRangeException.ThrowIfNegative(localNameLength, "localNameLength");
		if (localNameLength > localNameBuffer.Length - localNameOffset)
		{
			throw new ArgumentOutOfRangeException("localNameLength", System.SR.Format(System.SR.SizeExceedsRemainingBufferSpace, localNameBuffer.Length - localNameOffset));
		}
		ThrowIfClosed();
		bool flag = _depth == 0;
		StartElement();
	...
	public void WriteXmlnsAttribute(string prefix, string ns)
	{
		...
		{
			throw new ArgumentOutOfRangeException("ns", System.SR.Format(System.SR.CombinedPrefixNSLength, 715827882));
		}
	...
	public void WriteXmlnsAttribute(byte[] prefixBuffer, int prefixOffset, int prefixLength, byte[] nsBuffer, int nsOffset, int nsLength)
	{
		...
		ArgumentNullException.ThrowIfNull(nsBuffer, "nsBuffer");
		ArgumentOutOfRangeException.ThrowIfNegative(nsOffset, "nsOffset");
		if (nsOffset > nsBuffer.Length)
		{
			throw new ArgumentOutOfRangeException("nsOffset", System.SR.Format(System.SR.OffsetExceedsBufferSize, nsBuffer.Length));
		}
		ArgumentOutOfRangeException.ThrowIfNegative(nsLength, "nsLength");
		if (nsLength > nsBuffer.Length - nsOffset)
		{
			throw new ArgumentOutOfRangeException("nsLength", System.SR.Format(System.SR.SizeExceedsRemainingBufferSpace, nsBuffer.Length - nsOffset));
		}
		ThrowIfClosed();
		if (prefixLength > int.MaxValue - nsLength)
		{
			throw new ArgumentOutOfRangeException("nsLength", System.SR.Format(System.SR.CombinedPrefixNSLength, int.MaxValue));
		}
