...
public class XmlTextWriter : XmlWriter
{
	...
	public override void WriteStartElement(string? prefix, string localName, string? ns)
	{
		try
		{
			...
			else if (!string.IsNullOrEmpty(ns) || !string.IsNullOrEmpty(prefix))
			{
	...
	{
		ArgumentException.ThrowIfNullOrEmpty(ns, "ns");
		string text = FindPrefix(ns);
	...
	{
		int value = _stack[_top].prefixCount++ + 1;
		...
		handler.AppendFormatted(_top, "d");
	...
	private void HandleSpecialAttribute()
	{
		...
		{
			ReadOnlySpan<char> span = attributeValue.AsSpan().Trim(XmlConvert.WhitespaceChars);
	...
	{
		if (prefix != null && prefix.Equals("xml", StringComparison.OrdinalIgnoreCase) && "http://www.w3.org/XML/1998/namespace" != ns)
		{
