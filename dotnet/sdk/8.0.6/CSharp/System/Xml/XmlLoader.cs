...
internal sealed class XmlLoader
{
	...
	{
		XmlNode newChild;
		while ((newChild = LoadNode(skipOverWhitespace: true)) != null)
		{
	...
	private void LoadDocumentType(IDtdInfo dtdInfo, XmlDocumentType dtNode)
	{
		...
		if (schemaInfo.GeneralEntities != null)
		{
			...
			{
				XmlEntity xmlEntity = new XmlEntity(value4.Name.Name, value4.Pubid, value4.Url, value4.NData.IsEmpty ? null : value4.NData.Name, _doc);
		...
		if (schemaInfo.ParameterEntities != null)
		{
			...
			{
				XmlEntity xmlEntity2 = new XmlEntity(value5.Name.Name, value5.Pubid, value5.Url, value5.NData.IsEmpty ? null : value5.NData.Name, _doc);
	...
	internal XmlNamespaceManager ParsePartialContent(XmlNode parentNode, string innerxmltext, XmlNodeType nt)
	{
		...
		try
		{
			...
			{
				XmlNode newChild;
				while (_reader.Read() && (newChild = LoadNodeDirect()) != null)
				{
					parentNode.AppendChildForLoad(newChild, _doc);
				}
			}
			else
			{
				XmlNode newChild2;
				while (_reader.Read() && (newChild2 = LoadNode(skipOverWhitespace: true)) != null)
				{
					parentNode.AppendChildForLoad(newChild2, _doc);
				}
			}
