...
internal sealed class Compiler : BaseProcessor
{
	...
	private void Output(SchemaInfo schemaInfo)
	{
		...
		{
			string key = value.TargetNamespace ?? string.Empty;
			schemaInfo.TargetNamespaces[key] = true;
		}
		...
		foreach (XmlSchemaNotation value5 in _notations.Values)
		{
			...
			schemaInfo.Notations.TryAdd(schemaNotation.Name.Name, schemaNotation);
		}
	}
	...
	private void RecursivelyCheckRedefinedGroups(XmlSchemaGroup redefinedGroup, XmlSchemaGroup baseGroup)
	{
		...
		{
			XmlSchemaGroup xmlSchemaGroup = baseGroup;
			if (xmlSchemaGroup.CanonicalParticle == null)
			{
				XmlSchemaParticle xmlSchemaParticle2 = (xmlSchemaGroup.CanonicalParticle = CanonicalizeParticle(baseGroup.Particle, root: true));
			}
			xmlSchemaGroup = redefinedGroup;
			if (xmlSchemaGroup.CanonicalParticle == null)
			{
				XmlSchemaParticle xmlSchemaParticle2 = (xmlSchemaGroup.CanonicalParticle = CanonicalizeParticle(redefinedGroup.Particle, root: true));
			}
	...
	private void CompileGroup(XmlSchemaGroup group)
	{
		...
		{
			XmlSchemaParticle xmlSchemaParticle2 = (group.CanonicalParticle = CanonicalizeParticle(group.Particle, root: true));
		}
	...
	private void CompileSimpleType(XmlSchemaSimpleType simpleType)
	{
		...
		try
		{
			if (simpleType.Content is XmlSchemaSimpleTypeList xmlSchemaSimpleTypeList)
			{
			...
			else if (simpleType.Content is XmlSchemaSimpleTypeRestriction xmlSchemaSimpleTypeRestriction)
			{
		...
		else
		{
			...
			if (xmlSchemaComplexType.ContentType == XmlSchemaContentType.TextOnly)
			{
				...
				else
				{
					...
					if (!XmlSchemaType.IsDerivedFromDatatype(simpleRestriction.BaseType.Datatype, xmlSchemaComplexType.Datatype))
					{
	...
	private void CompileComplexContentExtension(XmlSchemaComplexType complexType, XmlSchemaComplexContent complexContent, XmlSchemaComplexContentExtension complexExtension)
	{
		...
		XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle(complexExtension.Particle, root: true);
	...
	private void CheckParticleDerivation(XmlSchemaComplexType complexType)
	{
		...
		if (xmlSchemaComplexType != null && xmlSchemaComplexType != XmlSchemaComplexType.AnyType && complexType.DerivedBy == XmlSchemaDerivationMethod.Restriction)
		{
			...
			XmlSchemaParticle baseParticle = CanonicalizePointlessRoot(xmlSchemaComplexType.ContentTypeParticle);
	...
	{
		XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle(particle, root: true);
	...
	private XmlSchemaParticle CanonicalizeAll(XmlSchemaAll all, bool root)
	{
		if (all.Items.Count > 0)
		{
			...
			{
				XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle((XmlSchemaElement)all.Items[i], root: false);
				if (xmlSchemaParticle != XmlSchemaParticle.Empty)
				{
	...
	private XmlSchemaParticle CanonicalizeChoice(XmlSchemaChoice choice, bool root)
	{
		...
		if (choice.Items.Count > 0)
		{
			...
			{
				XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle((XmlSchemaParticle)choice.Items[i], root: false);
				if (xmlSchemaParticle == XmlSchemaParticle.Empty)
				{
					continue;
				}
	...
	private XmlSchemaParticle CanonicalizeSequence(XmlSchemaSequence sequence, bool root)
	{
		if (sequence.Items.Count > 0)
		{
			...
			{
				XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle((XmlSchemaParticle)sequence.Items[i], root: false);
				if (xmlSchemaParticle == XmlSchemaParticle.Empty)
				{
					continue;
				}
	...
	private bool IsValidRestriction(XmlSchemaParticle derivedParticle, XmlSchemaParticle baseParticle)
	{
		...
		if (derivedParticle is XmlSchemaElement element)
		{
			derivedParticle = CanonicalizeElement(element);
		}
		if (baseParticle is XmlSchemaElement xmlSchemaElement)
		{
			XmlSchemaParticle xmlSchemaParticle = CanonicalizeElement(xmlSchemaElement);
		...
		else
		{
			...
			try
			{
				...
				else
				{
					...
					{
						if (xe.ElementSchemaType is XmlSchemaComplexType xmlSchemaComplexType)
						{
							CompileComplexType(xmlSchemaComplexType);
						...
						else if (xe.ElementSchemaType is XmlSchemaSimpleType xmlSchemaSimpleType)
						{
							CompileSimpleType(xmlSchemaSimpleType);
	...
	private ContentValidator CompileComplexContent(XmlSchemaComplexType complexType)
	{
		...
		if (contentTypeParticle is XmlSchemaAll xmlSchemaAll)
		{
	...
	{
		if (particle is XmlSchemaElement xe)
		{
			CompileElement(xe);
		}
		else if (particle is XmlSchemaGroupBase)
