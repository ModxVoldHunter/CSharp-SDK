...
internal sealed class SchemaCollectionCompiler : BaseProcessor
{
	...
	private void CompileGroup(XmlSchemaGroup group)
	{
		...
		{
			XmlSchemaParticle xmlSchemaParticle2 = (group.CanonicalParticle = CanonicalizeParticle(group.Particle, root: true, substitution: true));
		}
	...
	private void CompileComplexContentExtension(XmlSchemaComplexType complexType, XmlSchemaComplexContent complexContent, XmlSchemaComplexContentExtension complexExtension)
	{
		...
		XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle(complexExtension.Particle, root: true, substitution: true);
	...
	{
		XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle(particle, root: true, substitution);
	...
	private XmlSchemaParticle CanonicalizeAll(XmlSchemaAll all, bool root, bool substitution)
	{
		if (all.Items.Count > 0)
		{
			...
			{
				XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle((XmlSchemaElement)all.Items[i], root: false, substitution);
				if (xmlSchemaParticle != XmlSchemaParticle.Empty)
				{
	...
	private XmlSchemaParticle CanonicalizeChoice(XmlSchemaChoice choice, bool root, bool substitution)
	{
		...
		if (choice.Items.Count > 0)
		{
			...
			{
				XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle((XmlSchemaParticle)choice.Items[i], root: false, substitution);
				if (xmlSchemaParticle == XmlSchemaParticle.Empty)
				{
					continue;
				}
	...
	private XmlSchemaParticle CanonicalizeSequence(XmlSchemaSequence sequence, bool root, bool substitution)
	{
		if (sequence.Items.Count > 0)
		{
			...
			{
				XmlSchemaParticle xmlSchemaParticle = CanonicalizeParticle((XmlSchemaParticle)sequence.Items[i], root: false, substitution);
				if (xmlSchemaParticle == XmlSchemaParticle.Empty)
				{
					continue;
				}
