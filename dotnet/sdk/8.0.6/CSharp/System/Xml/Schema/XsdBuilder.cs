...
internal sealed class XsdBuilder : SchemaBuilder
{
	...
	private sealed class BuilderNamespaceManager : XmlNamespaceManager
	{
		...
		{
			return _nsMgr.LookupNamespace(prefix) ?? _reader.LookupNamespace(prefix);
		}
	}
	...
	private static ReadOnlySpan<int> DerivationMethodValues => RuntimeHelpers.CreateSpan<int>((RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);
	...
	internal override bool ProcessElement(string prefix, string name, string ns)
	{
		...
		{
			SendValidationEvent(System.SR.Sch_UnsupportedElement, XmlQualifiedName.ToString(name, ns));
		}
		return false;
	}
	...
	internal override void ProcessAttribute(string prefix, string name, string ns, string value)
	{
		...
		if (_currentEntry.Attributes != null)
		{
			for (int i = 0; i < _currentEntry.Attributes.Length; i++)
			{
				...
				if (_schemaNames.TokenToQName[(int)xsdAttributeEntry.Attribute].Equals(name, ns))
				{
					try
					{
		...
		{
			SendValidationEvent(System.SR.Sch_UnsupportedAttribute, XmlQualifiedName.ToString(name, ns));
		}
	}
	...
	private bool GetNextState(string name, string ns)
	{
		if (_currentEntry.NextStates != null)
		{
			for (int i = 0; i < _currentEntry.NextStates.Length; i++)
			{
				...
				if (_schemaNames.TokenToQName[(int)s_schemaEntries[num].Name].Equals(name, ns))
				{
