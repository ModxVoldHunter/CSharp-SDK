...
internal sealed class SchemaCollectionPreprocessor : BaseProcessor
{
	...
	private void LoadExternals(XmlSchema schema, XmlSchemaCollection xsc)
	{
		...
		for (int i = 0; i < schema.Includes.Count; i++)
		{
			...
			if (xsc != null && xmlSchemaExternal is XmlSchemaImport)
			{
				...
				string ns = xmlSchemaImport.Namespace ?? string.Empty;
				...
				if (xmlSchemaExternal.Schema != null)
				{
					...
					for (int j = 0; j < xmlSchemaExternal.Schema.Includes.Count; j++)
					{
						...
						if (!(xmlSchemaExternal2 is XmlSchemaImport xmlSchemaImport2))
						{
							continue;
						}
						Uri uri2 = xmlSchemaImport2.BaseUri ?? xmlSchemaImport2.Schema?.BaseUri;
						if (uri2 != null)
						{
	...
	private void Preprocess(XmlSchema schema, string targetNamespace, Compositor compositor)
	{
		...
		for (int j = 0; j < schema.Includes.Count; j++)
		{
			...
			if (xmlSchemaExternal2 is XmlSchemaRedefine xmlSchemaRedefine)
			{
		...
		for (int l = 0; l < schema.Items.Count; l++)
		{
			...
			else if (schema.Items[l] is XmlSchemaAttributeGroup xmlSchemaAttributeGroup2)
			{
			...
			else if (schema.Items[l] is XmlSchemaComplexType xmlSchemaComplexType)
			{
			...
			else if (schema.Items[l] is XmlSchemaSimpleType xmlSchemaSimpleType)
			{
	...
	private void PreprocessRedefine(XmlSchemaRedefine redefine)
	{
		for (int i = 0; i < redefine.Items.Count; i++)
		{
			...
			else if (redefine.Items[i] is XmlSchemaAttributeGroup xmlSchemaAttributeGroup)
			{
			...
			else if (redefine.Items[i] is XmlSchemaComplexType xmlSchemaComplexType)
			{
				PreprocessComplexType(xmlSchemaComplexType, local: false);
			...
			{
				if (!(redefine.Items[i] is XmlSchemaSimpleType xmlSchemaSimpleType))
				{
					continue;
				}
				PreprocessSimpleType(xmlSchemaSimpleType, local: false);
