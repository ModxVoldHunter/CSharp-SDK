		...
		if (numberOfRunningPos > 0)
		{
			...
			for (i = numberOfRunningPos - 1; i >= 0; i--)
			{
				...
				while (numberOfRunningPos < 10000 && curpos2.Intersects(_positionsWithRangeTerminals))
				{
					...
					ref decimal[] rangeCounters = ref value3.rangeCounters;
					if (rangeCounters == null)
					{
						rangeCounters = new decimal[_minMaxNodesCount];
					}
					...
					else
					{
						...
						rangeCounters = ref value3.rangeCounters;
						if (rangeCounters == null)
						{
							rangeCounters = new decimal[_minMaxNodesCount];
						}
