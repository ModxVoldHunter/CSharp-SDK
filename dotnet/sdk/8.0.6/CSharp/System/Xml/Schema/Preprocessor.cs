...
internal sealed class Preprocessor : BaseProcessor
{
	...
	private void LoadExternals(XmlSchema schema)
	{
		...
		for (int i = 0; i < schema.Includes.Count; i++)
		{
			...
			if (xmlSchemaExternal.Compositor == Compositor.Import)
			{
				...
				string text = xmlSchemaImport.Namespace ?? string.Empty;
	...
	private void BuildRefNamespaces(XmlSchema schema)
	{
		...
		string text2 = schema.TargetNamespace ?? string.Empty;
		...
		for (int i = 0; i < schema.Includes.Count; i++)
		{
			...
			case Compositor.Import:
			{
				...
				if (xmlSchema != null)
				{
					...
					{
						SendValidationEvent(System.SR.Sch_MismatchTargetNamespaceImport, @namespace, xmlSchema.TargetNamespace, xmlSchemaImport);
					}
					XmlSchema rootSchemaForRedefine = _rootSchemaForRedefine;
					_rootSchemaForRedefine = xmlSchema;
					Preprocess(xmlSchema, @namespace, imports);
					_rootSchemaForRedefine = rootSchemaForRedefine;
				}
			...
			if (xmlSchema.TargetNamespace != null)
			{
				...
				{
					SendValidationEvent(System.SR.Sch_MismatchTargetNamespaceInclude, xmlSchema.TargetNamespace, schema.TargetNamespace, xmlSchemaExternal);
				}
			}
			else if (!string.IsNullOrEmpty(targetNamespace))
			{
				xmlSchema = (xmlSchemaExternal.Schema = GetChameleonSchema(targetNamespace, xmlSchema));
			}
			Preprocess(xmlSchema, schema.TargetNamespace, imports);
		}
			...
			if (schema3 != null)
			{
				...
				case Compositor.Import:
					if (schema3 != _rootSchema)
					{
						...
						string text2 = xmlSchemaImport2.Namespace ?? string.Empty;
	...
	private void PreprocessRedefine(RedefineEntry redefineEntry)
	{
		...
		string @namespace = schemaToUpdate.TargetNamespace ?? string.Empty;
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			else if (items[i] is XmlSchemaAttributeGroup xmlSchemaAttributeGroup)
			{
				PreprocessAttributeGroup(xmlSchemaAttributeGroup);
			...
			else if (items[i] is XmlSchemaComplexType xmlSchemaComplexType)
			{
				PreprocessComplexType(xmlSchemaComplexType, local: false);
			...
			{
				if (!(items[i] is XmlSchemaSimpleType xmlSchemaSimpleType))
				{
					continue;
				}
				PreprocessSimpleType(xmlSchemaSimpleType, local: false);
	...
	private void PreprocessSimpleType(XmlSchemaSimpleType simpleType, bool local)
	{
		...
		else if (simpleType.Content is XmlSchemaSimpleTypeList xmlSchemaSimpleTypeList)
		{
