...
internal sealed class XdrValidator : BaseValidator
{
	...
	private void ValidateChildElement()
	{
		...
		{
			context.ElementDecl.ContentValidator.ValidateElement(elementName, context, out var errorCode);
			if (errorCode < 0)
			{
	...
	private void LoadSchemaFromLocation(string uri)
	{
		...
		try
		{
			...
			xmlTextReader = new XmlTextReader(uri2.ToString(), input, base.NameTable);
			xmlTextReader.XmlResolver = base.XmlResolver;
			...
			parser.Parse(xmlTextReader, uri);
	...
	private void LoadSchema(string uri)
	{
		...
		if (schemaInfo != null)
		{
			...
			{
				throw new XmlException(System.SR.Xml_MultipleValidationTypes, string.Empty, base.PositionInfo.LineNumber, base.PositionInfo.LinePosition);
			}
			base.schemaInfo.Add(schemaInfo, base.EventHandler);
		}
		else
