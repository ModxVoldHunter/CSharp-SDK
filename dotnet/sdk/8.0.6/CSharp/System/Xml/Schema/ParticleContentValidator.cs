...
internal sealed class ParticleContentValidator : ContentValidator
{
	...
	public ContentValidator Finish(bool useDFA)
	{
		...
		sequenceNode.LeftChild = _contentNode;
		LeafNode leafNode = (LeafNode)(sequenceNode.RightChild = new LeafNode(_positions.Add(_symbols.AddName(XmlQualifiedName.Empty, null), null)));
		_contentNode.ExpandTree(sequenceNode, _symbols, _positions);
		...
		sequenceNode.ConstructPos(bitSet, lastpos, array);
		if (_minMaxNodesCount > 0)
		{
			BitSet posWithRangeTerminals;
			BitSet[] minmaxFollowPos = CalculateTotalFollowposForRangeNodes(array, out posWithRangeTerminals);
			if (_enableUpaCheck)
			{
			...
			return new RangeContentValidator(bitSet, array, _symbols, _positions, leafNode.Pos, base.ContentType, sequenceNode.LeftChild.IsNullable, posWithRangeTerminals, _minMaxNodesCount);
		}
		int[][] array2 = null;
		...
		if (array2 != null)
		{
			return new DfaContentValidator(array2, _symbols, base.ContentType, base.IsOpen, sequenceNode.LeftChild.IsNullable);
		}
		return new NfaContentValidator(bitSet, array, _symbols, _positions, leafNode.Pos, base.ContentType, base.IsOpen, sequenceNode.LeftChild.IsNullable);
	}
	...
	private int[][] BuildTransitionTable(BitSet firstpos, BitSet[] followpos, int endMarkerPos)
	{
		...
		dictionary.Add(firstpos, 0);
		list.Add(new int[count2 + 1]);
		while (queue.Count > 0)
		{
			...
			for (int i = 0; i < count2; i++)
			{
				...
				if (dictionary.TryGetValue(bitSet2, out var value))
				{
					array[i] = value;
					continue;
				}
