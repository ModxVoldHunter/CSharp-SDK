...
internal sealed class XmlWellFormedWriter : XmlWriter
{
	...
	private sealed class AttributeValueCache
	{
		...
		internal void Trim()
		{
			...
			ReadOnlySpan<char> readOnlySpan = text.AsSpan().Trim(XmlConvert.WhitespaceChars);
			if (text != readOnlySpan)
			{
				_stringValue = new StringBuilder();
				_stringValue.Append(readOnlySpan);
			}
			int i;
	...
	internal XmlWellFormedWriter(XmlWriter writer, XmlWriterSettings settings)
	{
		...
		else
		{
			...
			_nsStack[2].Set(string.Empty, text ?? string.Empty, NamespaceKind.Implied);
		}
		_nsTop = 2;
	...
	{
		string arg = ((prefix.Length > 0) ? (prefix + ":" + localName) : localName);
		return new XmlException(System.SR.Xml_DupAttributeName, arg);
	}
	...
	private string GeneratePrefix()
	{
		...
		do
		{
			...
			handler = new DefaultInterpolatedStringHandler(0, 2, invariantCulture);
			handler.AppendFormatted(text);
			handler.AppendFormatted(num);
			text2 = string.Create(provider2, ref handler);
			num++;
		}
