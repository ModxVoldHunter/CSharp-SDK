...
internal sealed class XmlNodeReaderNavigator
{
		...
		get
		{
			...
			case XmlNodeType.XmlDeclaration:
			{
				...
				for (int i = 0; i < _nDeclarationAttrCount; i++)
				{
					...
					handler.AppendFormatted(decNodeAttributes[i].name);
					handler.AppendLiteral("=\"");
					handler.AppendFormatted(decNodeAttributes[i].value);
					handler.AppendLiteral("\"");
	...
	{
		ArgumentOutOfRangeException.ThrowIfNegative(attributeIndex, "attributeIndex");
		ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(attributeIndex, AttributeCount, "attributeIndex");
	}
	...
	{
		XmlAttribute xmlAttribute = ((ns.Length != 0) ? elem.GetAttributeNode(name, ns) : elem.GetAttributeNode(name));
	...
	public void MoveToAttribute(int attributeIndex)
	{
		...
		{
			CheckIndexCondition(attributeIndex);
			XmlAttribute xmlAttribute = ((XmlElement)_curNode).Attributes[attributeIndex];
			if (xmlAttribute != null)
			{
				_elemNode = _curNode;
				_curNode = xmlAttribute;
		...
		{
			CheckIndexCondition(attributeIndex);
			XmlAttribute xmlAttribute = ((XmlElement)_elemNode).Attributes[attributeIndex];
			if (xmlAttribute != null)
			{
				_curNode = xmlAttribute;
				_attrIndex = attributeIndex;
	...
	internal IDictionary<string, string> GetNamespacesInScope(XmlNamespaceScope scope)
	{
		...
		if (scope != XmlNamespaceScope.Local)
		{
			if (dictionary.TryGetValue(string.Empty, out var value) && value == string.Empty)
			{
	...
	public bool ReadAttributeValue(ref int level, ref bool bResolveEntity, ref XmlNodeType nt)
	{
		...
		else if (_bOnAttrVal)
		{
			...
			XmlNode nextSibling = _curNode.NextSibling;
			if (nextSibling == null)
			{
				XmlNode parentNode = _curNode.ParentNode;
