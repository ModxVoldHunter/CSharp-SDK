...
internal sealed class ReflectionXmlSerializationWriter : XmlSerializationWriter
{
	...
	private void WriteArrayItems(ElementAccessor[] elements, TextAccessor text, ChoiceIdentifierAccessor choice, object o)
	{
		if (o is IList list)
		{
			...
			{
				object o2 = list[i];
				WriteElements(o2, elements, text, choice, writeAccessors: true, isNullable: true);
			}
			return;
		}
		...
		if (enumerator != null)
		{
			while (enumerator.MoveNext())
			{
				...
				WriteElements(current, elements, text, choice, writeAccessors: true, isNullable: true);
			}
		}
	}
	...
	private void WriteText(object o, TextAccessor text)
	{
		if (text.Mapping is PrimitiveMapping primitiveMapping)
		{
			...
			{
				WritePrimitiveValue(primitiveMapping.TypeDesc, o, out stringValue);
			}
			if (o is byte[] value)
			{
				WriteValue(value);
	...
	private void WriteElement(object o, ElementAccessor element, bool writeAccessor)
	{
		...
		if (element.Mapping is ArrayMapping)
		{
			...
			if (o != null)
			{
				...
				WriteArrayItems(arrayMapping.ElementsSortedByDerivation, null, null, o);
				WriteEndElement();
			}
			return;
		}
		if (element.Mapping is EnumMapping)
		{
			...
			{
				WritePrimitive(WritePrimitiveMethodRequirement.WriteElementString, text, ns, element.Default, o, element.Mapping, writeXsiType: false);
			}
			return;
		}
	...
	{
		if (defaultValue != null && defaultValue != DBNull.Value && mapping.TypeDesc.HasDefaultSupport && IsDefaultValue(o, defaultValue))
		{
			return;
		}
		if (isSoap)
		...
		if (memberTypeDesc.IsArrayLike)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (attribute.IsList)
				{
					...
					{
						WritePrimitiveValue(arrayElementTypeDesc, current, out stringValue);
					}
					if (flag)
					{
						if (flag2)
	...
	private void WriteAttribute(object memberValue, AttributeAccessor attribute, object container)
	{
		...
		{
			string ns = ((attribute.Form == XmlSchemaForm.Qualified) ? attribute.Namespace : string.Empty);
			WritePrimitive(WritePrimitiveMethodRequirement.WriteAttribute, attribute.Name, ns, attribute.Default, memberValue, attribute.Mapping, writeXsiType: false);
		}
	}
	...
	private void WritePrimitive(WritePrimitiveMethodRequirement method, string name, string ns, object defaultValue, object o, TypeMapping mapping, bool writeXsiType)
	{
		...
		if (defaultValue != null && defaultValue != DBNull.Value && mapping.TypeDesc.HasDefaultSupport)
		{
			...
			else if (IsDefaultValue(o, defaultValue))
			{
				return;
			}
		}
		...
		{
			flag = WritePrimitiveValue(typeDesc, o, out stringValue);
		}
		if (flag)
		{
