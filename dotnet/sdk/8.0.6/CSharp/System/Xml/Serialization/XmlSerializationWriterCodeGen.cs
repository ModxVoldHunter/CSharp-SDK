...
internal sealed class XmlSerializationWriterCodeGen : XmlSerializationCodeGen
{
	...
	private void WritePrimitive(string method, string name, string ns, object defaultValue, string source, TypeMapping mapping, bool writeXsiType, bool isElement, bool isNullable)
	{
		...
		if (flag)
		{
			if (mapping is EnumMapping)
			{
				...
				{
					base.Writer.Write(ReflectionAwareCodeGen.GetStringForEnumLongValue(source, mapping.TypeDesc.UseReflection));
				}
				else
				{
					base.Writer.Write(source);
	...
	private void WriteEnumMethod(EnumMapping mapping)
	{
		...
		if (constants.Length != 0)
		{
			...
			if (mapping.TypeDesc.UseReflection)
			{
				base.Writer.WriteLine("switch (" + ReflectionAwareCodeGen.GetStringForEnumLongValue("v", mapping.TypeDesc.UseReflection) + " ){");
			}
			else
			{
			...
			if (mapping.IsFlags)
			{
				...
				base.Writer.Write(ReflectionAwareCodeGen.GetStringForEnumLongValue("v", mapping.TypeDesc.UseReflection));
	...
	private void WriteElements(string source, string enumSource, ElementAccessor[] elements, TextAccessor text, ChoiceIdentifierAccessor choice, string arrayName, bool writeAccessors, bool isNullable)
	{
		...
		foreach (ElementAccessor elementAccessor2 in elements)
		{
			...
			else if (choice != null)
			{
				...
				base.Writer.Write("if (");
				base.Writer.Write(useReflection2 ? ReflectionAwareCodeGen.GetStringForEnumLongValue(enumSource, useReflection2) : enumSource);
				base.Writer.Write(" == ");
				base.Writer.Write(text3);
		...
		if (num > 0)
		{
			...
			foreach (ElementAccessor item in arrayList)
			{
				...
				if (choice != null)
				{
					...
					base.Writer.Write("if (");
					base.Writer.Write(useReflection4 ? ReflectionAwareCodeGen.GetStringForEnumLongValue(enumSource, useReflection4) : enumSource);
					base.Writer.Write(" == ");
	...
	private void WriteText(string source, TextAccessor text)
	{
		if (text.Mapping is PrimitiveMapping primitiveMapping)
		{
			...
			{
				WriteEnumValue(mapping, source);
			}
			else
			{
				WritePrimitiveValue(primitiveMapping.TypeDesc, source);
			}
			base.Writer.WriteLine(");");
		}
