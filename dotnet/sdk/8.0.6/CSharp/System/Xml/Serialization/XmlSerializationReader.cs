...
public abstract class XmlSerializationReader : XmlSerializationGeneratedCode
{
	...
	{
		string text = _r.GetAttribute(_nilID, _instanceNsID) ?? _r.GetAttribute(_nullID, _instanceNsID) ?? _r.GetAttribute(_nullID, _instanceNs2000ID) ?? _r.GetAttribute(_nullID, _instanceNs1999ID);
	...
	private SoapArrayInfo ParseArrayType(string value)
	{
		if (value == null)
		{
			throw new ArgumentNullException("value", System.SR.Format(System.SR.XmlMissingArrayType, CurrentTag()));
		}
		if (value.Length == 0)
		{
		...
		if (num2 > 0)
		{
			ReadOnlySpan<char> readOnlySpan = value.AsSpan(num + 1, num2);
			if (!int.TryParse(readOnlySpan, CultureInfo.InvariantCulture, out result.length))
			{
				throw new ArgumentException(System.SR.Format(System.SR.XmlInvalidArrayLength, new string(readOnlySpan)), "value");
			}
		}
		else
		{
		...
		result.dimensions = 1;
		result.qname = new string(value.AsSpan(0, num + 1));
		return result;
	}
	...
	private object ReadXmlNodes(bool elementCanBeType)
	{
		...
		GetCurrentPosition(out var _, out var _);
