...
public class XmlSerializer
{
	...
	{
		ArgumentNullException.ThrowIfNull(xmlTypeMapping, "xmlTypeMapping");
		if (Mode != SerializationMode.ReflectionOnly)
		{
	...
	{
		ArgumentNullException.ThrowIfNull(xmlMapping, "xmlMapping");
	...
	public void Serialize(XmlWriter xmlWriter, object? o, XmlSerializerNamespaces? namespaces, string? encodingStyle, string? id)
	{
		try
		{
			...
			else if (_tempAssembly == null || _typedSerializer)
			{
				...
				xmlSerializationWriter.Init(xmlWriter, (namespaces == null || namespaces.Count == 0) ? DefaultNamespaces : namespaces, encodingStyle, id);
	...
	public object? Deserialize(XmlReader xmlReader, string? encodingStyle, XmlDeserializationEvents events)
	{
		...
		try
		{
			...
			if (_tempAssembly == null || _typedSerializer)
			{
				...
				xmlSerializationReader.Init(xmlReader, events, encodingStyle);
	...
	public static XmlSerializer?[] FromMappings(XmlMapping[]? mappings, Type? type)
	{
		...
		if (assembly == null)
		{
			...
			if (type == null)
			{
				TempAssembly tempAssembly = new TempAssembly(mappings, new Type[1] { type }, null, null);
		...
		{
			array2[k] = contract.TypedSerializers[mappings[k].Key] as XmlSerializer;
			if (array2[k] != null)
			{
				TempAssembly.VerifyLoadContext(array2[k]._rootType, type.Assembly);
			}
		}
		return array2;
	}
	...
	internal static bool GenerateSerializer(Type[] types, XmlMapping[] mappings, Stream stream)
	{
		...
		ArgumentNullException.ThrowIfNull(mappings, "mappings");
	...
	{
		XmlSerializer[] array = new XmlSerializer[mappings.Length];
		Dictionary<XmlSerializerMappingKey, XmlSerializer> dictionary = null;
		AssemblyLoadContext loadContext = AssemblyLoadContext.GetLoadContext(type.Assembly);
		dictionary = s_xmlSerializerTable.GetOrCreateValue(type, (Type _) => new Dictionary<XmlSerializerMappingKey, XmlSerializer>());
		lock (dictionary)
		{
			Dictionary<XmlSerializerMappingKey, int> dictionary2 = new Dictionary<XmlSerializerMappingKey, int>();
			for (int i = 0; i < mappings.Length; i++)
			{
				XmlSerializerMappingKey key = new XmlSerializerMappingKey(mappings[i]);
				if (!dictionary.TryGetValue(key, out array[i]))
				{
					dictionary2.Add(key, i);
				}
			}
			if (dictionary2.Count > 0)
			{
				XmlMapping[] array2 = new XmlMapping[dictionary2.Count];
				int num = 0;
				foreach (XmlSerializerMappingKey key2 in dictionary2.Keys)
				{
				...
				foreach (XmlSerializerMappingKey key3 in dictionary2.Keys)
				{
	...
	private void SerializePrimitive(XmlWriter xmlWriter, object o, XmlSerializerNamespaces namespaces)
	{
		...
		xmlSerializationPrimitiveWriter.Init(xmlWriter, namespaces, null, null);
	...
	private object DeserializePrimitive(XmlReader xmlReader, XmlDeserializationEvents events)
	{
		...
		xmlSerializationPrimitiveReader.Init(xmlReader, events, null);
		switch (Type.GetTypeCode(_primitiveType))
		{
		case TypeCode.String:
