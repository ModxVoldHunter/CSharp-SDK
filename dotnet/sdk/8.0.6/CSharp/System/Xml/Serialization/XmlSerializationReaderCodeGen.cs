...
internal sealed class XmlSerializationReaderCodeGen : XmlSerializationCodeGen
{
		...
		internal Member(XmlSerializationReaderCodeGen outerClass, string source, string arraySource, string arrayName, int i, MemberMapping mapping, bool multiRef, string choiceSource)
		{
			...
			if (mapping.TypeDesc.IsArrayLike)
			{
				...
				if (mapping.ChoiceIdentifier != null)
				{
					...
					string value = (useReflection ? "" : ("(" + cSharpName + "[])"));
					...
					handler.AppendLiteral(", ");
					handler.AppendFormatted(outerClass.RaCodeGen.GetStringForTypeof(cSharpName, useReflection));
					handler.AppendLiteral(");");
					string text2 = handler.ToStringAndClear();
					_choiceArraySource = text2 + outerClass.RaCodeGen.GetStringForArrayMember(choiceArrayName, text + "++", mapping.ChoiceIdentifier.Mapping.TypeDesc);
				}
				else
				{
	...
	internal Hashtable Enums => _enums ?? (_enums = new Hashtable());
	...
	private void WriteEnumAndArrayTypes()
	{
		...
		foreach (TypeScope typeScope in scopes)
		{
			foreach (Mapping typeMapping in typeScope.TypeMappings)
			{
				...
				{
					if (!(typeMapping is ArrayMapping arrayMapping) || !arrayMapping.TypeDesc.HasDefaultConstructor)
					{
						continue;
					}
					base.Writer.Write("if (");
	...
	private void WriteMemberElementsIf(Member[] members, Member anyElement, string elementElseString, string checkTypeSource)
	{
		...
		foreach (Member member in members)
		{
			...
			for (int j = 0; j < elements.Length; j++)
			{
				...
				if (!flag2 && elementAccessor.Any && string.IsNullOrEmpty(elementAccessor.Name))
				{
					continue;
				}
				if (!flag2)
	...
	private string GetArraySource(TypeDesc typeDesc, string arrayName, bool multiRef)
	{
		...
		if (typeDesc.IsArray)
		{
			...
			string value2 = (useReflection ? "" : ("(" + cSharpName + "[])"));
			value = $"{value}{arrayName} = {value2}EnsureArrayIndex({arrayName}, {text}, {base.RaCodeGen.GetStringForTypeof(cSharpName, useReflection2)});";
	...
	private void WriteElement(string source, string arrayName, string choiceSource, ElementAccessor element, ChoiceIdentifierAccessor choice, string checkSpecified, bool checkForNull, bool readOnly, int fixupIndex, int elementIndex)
	{
		...
		else
		{
			...
			case TypeKind.Serializable:
			{
				...
				base.Writer.Write(")");
				if (serializableMapping.TypeDesc.CSharpName == "global::System.Xml.Linq.XElement")
				{
					base.Writer.Write(base.RaCodeGen.GetStringForCreateInstance(serializableMapping.TypeDesc.CSharpName, useReflection: false, ctorInaccessible: false, cast: false, "\"default\""));
				}
				else
				{
	...
	{
		base.Writer.Write((length <= 32) ? "System.Span<bool> paramsRead = stackalloc bool[" : "System.Span<bool> paramsRead = new bool[");
