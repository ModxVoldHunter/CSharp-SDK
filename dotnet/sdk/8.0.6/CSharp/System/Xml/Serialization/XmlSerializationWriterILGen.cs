...
{
	private static readonly string[] s_argNamesP = new string[1] { "p" };

	private static readonly string[] s_argNamesO = new string[1] { "o" };

	private static readonly Type[] s_objectArrayType = new Type[1] { typeof(object[]) };

	private static readonly Type[] s_objectType = new Type[1] { typeof(object) };
	...
	private void WriteQualifiedNameElement(string name, string ns, object defaultValue, SourceInfo source, bool nullable, TypeMapping mapping)
	{
		...
		ilg.Ldarg(0);
		ilg.Ldstr(XmlSerializationILGen.GetCSharpString(name));
		list.Add(typeof(string));
		if (ns != null)
		{
			ilg.Ldstr(XmlSerializationILGen.GetCSharpString(ns));
			list.Add(typeof(string));
		}
	...
	private void WritePrimitive(string method, string name, string ns, object defaultValue, SourceInfo source, TypeMapping mapping, bool writeXsiType, bool isElement, bool isNullable)
	{
		...
		if (writeXsiType)
		{
			...
			ilg.Ldstr(XmlSerializationILGen.GetCSharpString(mapping.TypeName));
			ilg.Ldstr(XmlSerializationILGen.GetCSharpString(mapping.Namespace));
			ilg.New(constructor);
		}
	...
	{
		MethodInfo method = typeof(XmlSerializationWriter).GetMethod("WriteEndElement", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, s_objectType);
	...
	private string GenerateMembersElement(XmlMembersMapping xmlMembersMapping)
	{
		...
		ilg = new CodeGenerator(typeBuilder);
		ilg.BeginMethod(typeof(void), text, s_objectArrayType, s_argNamesP, MethodAttributes.Public | MethodAttributes.HideBySig);
		...
		if (hasWrapperElement)
		{
			...
			if (num >= 0)
			{
				...
				handler = new DefaultInterpolatedStringHandler(7, 2, invariantCulture);
			...
			for (int i = 0; i < membersMapping.Members.Length; i++)
			{
				...
				handler = new DefaultInterpolatedStringHandler(3, 1);
				handler.AppendLiteral("p[");
				handler.AppendFormatted(i);
				handler.AppendLiteral("]");
				SourceInfo source2 = new SourceInfo(handler.ToStringAndClear(), null, null, localBuilder.LocalType.GetElementType(), ilg);
				...
				if (memberMapping.CheckSpecified != 0)
				{
					...
					for (int j = 0; j < membersMapping.Members.Length; j++)
					{
						...
						{
							handler = new DefaultInterpolatedStringHandler(11, 1);
							handler.AppendLiteral("((bool)p[");
							handler.AppendFormatted(j);
							handler.AppendLiteral("])");
							sourceInfo = new SourceInfo(handler.ToStringAndClear(), null, null, typeof(bool), ilg);
							intVar = j;
							break;
						}
					}
		...
		for (int k = 0; k < membersMapping.Members.Length; k++)
		{
			...
			if (memberMapping2.CheckSpecified != 0)
			{
				...
				for (int l = 0; l < membersMapping.Members.Length; l++)
				{
					if (membersMapping.Members[l].Name == text3)
					{
						...
						handler.AppendLiteral("])");
						sourceInfo2 = new SourceInfo(handler.ToStringAndClear(), null, null, typeof(bool), ilg);
						intVar2 = l;
						break;
					}
				}
			...
			handler.AppendLiteral("]");
			string text4 = handler.ToStringAndClear();
			...
			if (memberMapping2.ChoiceIdentifier != null)
			{
				for (int m = 0; m < membersMapping.Members.Length; m++)
				{
					...
					{
						handler = new DefaultInterpolatedStringHandler(7, 2);
						handler.AppendLiteral("((");
						handler.AppendFormatted(membersMapping.Members[m].TypeDesc.CSharpName);
						handler.AppendLiteral(")p[");
						handler.AppendFormatted(m);
						handler.AppendLiteral("])");
						choiceSource = handler.ToStringAndClear();
						break;
					}
				}
			}
	...
	private string GenerateTypeElement(XmlTypeMapping xmlTypeMapping)
	{
		...
		string text = NextMethodName(accessor.Name);
		ilg = new CodeGenerator(typeBuilder);
		ilg.BeginMethod(typeof(void), text, s_objectType, s_argNamesO, MethodAttributes.Public | MethodAttributes.HideBySig);
		MethodInfo method = typeof(XmlSerializationWriter).GetMethod("WriteStartDocument", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, Type.EmptyTypes);
		ilg.Ldarg(0);
		ilg.Call(method);
		...
		if (constants.Length != 0)
		{
			...
			foreach (ConstantMapping constantMapping in constants)
			{
				if (hashSet.Add(constantMapping.Value))
				{
					...
					ilg.Beq(label3);
					list3.Add(label3);
					list4.Add(XmlSerializationILGen.GetCSharpString(constantMapping.XmlName));
				}
			}
			if (mapping.IsFlags)
			{
				...
				for (int k = 0; k < constants.Length; k++)
				{
					...
					ilg.Ldc(k);
					ilg.Ldstr(XmlSerializationILGen.GetCSharpString(constantMapping2.XmlName));
				...
				ilg.Ldloc(localBuilder4);
				ilg.Ldstr(XmlSerializationILGen.GetCSharpString(mapping.TypeDesc.FullName));
		...
		foreach (TypeScope typeScope in scopes)
		{
			foreach (Mapping typeMapping in typeScope.TypeMappings)
			{
				if (typeMapping is EnumMapping)
				{
					...
					ilg.Ldarg(0);
					ilg.Ldstr(XmlSerializationILGen.GetCSharpString(enumMapping.TypeName));
					ilg.Ldstr(XmlSerializationILGen.GetCSharpString(enumMapping.Namespace));
					ilg.Call(method3);
					...
					ilg.Ldarg(0);
					ilg.Load(variable);
					ilg.ConvertValue(CodeGenerator.GetVariableType(variable), enumMapping.TypeDesc.Type);
					ilg.Call(methodInfo);
					ilg.Call(method4);
				...
				else if (typeMapping is ArrayMapping && typeMapping is ArrayMapping arrayMapping)
				{
					...
					ilg.Ldarg(0);
					ilg.Ldstr(XmlSerializationILGen.GetCSharpString(arrayMapping.TypeName));
					ilg.Ldstr(XmlSerializationILGen.GetCSharpString(arrayMapping.Namespace));
					ilg.Call(method8);
	...
	private void WriteStructMethod(StructMapping mapping)
	{
		...
		{
			MethodInfo method4 = typeof(XmlSerializationWriter).GetMethod("CreateUnknownTypeException", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, s_objectType);
			ilg.Ldarg(0);
			ilg.Ldarg(arg);
		...
		if (!mapping.TypeDesc.IsAbstract)
		{
			...
			if (num >= 0)
			{
				...
				text = ReflectionAwareILGen.GetStringForMember("o", memberMapping.Name);
			}
			ilg.Ldarg(0);
			ilg.Ldarg("n");
			ilg.Ldarg("ns");
			...
			foreach (MemberMapping memberMapping2 in allMembers)
			{
				if (memberMapping2.Attribute != null)
				{
					...
					if (memberMapping2.CheckSpecified != 0)
					{
						string stringForMember = ReflectionAwareILGen.GetStringForMember("o", memberMapping2.Name + "Specified");
						ILGenLoad(stringForMember);
						ilg.If();
					}
					WriteMember(ReflectionAwareILGen.GetSourceForMember("o", memberMapping2, ilg), memberMapping2.Attribute, memberMapping2.TypeDesc, "o");
					if (memberMapping2.CheckSpecified != 0)
					{
						ilg.EndIf();
					}
			...
			foreach (MemberMapping memberMapping3 in allMembers)
			{
				if (memberMapping3.Xmlns == null)
				{
					...
					{
						string stringForMember2 = ReflectionAwareILGen.GetStringForMember("o", memberMapping3.Name + "Specified");
					...
					if (memberMapping3.ChoiceIdentifier != null)
					{
						...
						choiceSource = ReflectionAwareILGen.GetStringForMember("o", memberMapping3.ChoiceIdentifier.MemberName);
					}
					WriteMember(ReflectionAwareILGen.GetSourceForMember("o", memberMapping3, memberMapping3.MemberInfo, ilg), choiceSource, memberMapping3.ElementsSortedByDerivation, memberMapping3.Text, memberMapping3.ChoiceIdentifier, memberMapping3.TypeDesc, writeAccessors: true);
					if (memberMapping3.CheckSpecified != 0)
					{
						ilg.EndIf();
					}
		...
		if (memberTypeDesc.IsArrayLike)
		{
			...
			if (attribute.IsList)
			{
				if (CanOptimizeWriteListSequence(memberTypeDesc.ArrayElementTypeDesc))
				{
					...
					ilg.Call(method);
					ilg.Load(null);
					ilg.Ldstr(XmlSerializationILGen.GetCSharpString(attribute.Name));
					ilg.Ldstr(XmlSerializationILGen.GetCSharpString(value));
					ilg.Call(method2);
				}
				else
				{
			...
			WriteLocalDecl(text2, ReflectionAwareILGen.GetStringForArrayMember(text, text3), arrayElementTypeDesc.Type);
			if (attribute.IsList)
			{
				string name;
				Type typeFromHandle;
			...
			if (attribute.IsList)
			{
				...
				else
				{
					...
					if (text4 != null)
					{
						ilg.Ldstr(XmlSerializationILGen.GetCSharpString(text4));
						list.Add(typeof(string));
					}
	...
	private void WriteArray(SourceInfo source, string choiceSource, ElementAccessor[] elements, TextAccessor text, ChoiceIdentifierAccessor choice, TypeDesc arrayTypeDesc)
	{
		if (elements.Length != 0 || text != null)
		{
			...
			if (choice != null)
			{
				...
				ilg.Ldarg(0);
				ilg.Ldstr(XmlSerializationILGen.GetCSharpString(choice.Mapping.TypeDesc.FullName));
				ilg.Ldstr(XmlSerializationILGen.GetCSharpString(choice.MemberName));
				ilg.Call(method);
				ilg.Throw();
				ilg.EndIf();
			}
	...
	private void WriteArrayItems(ElementAccessor[] elements, TextAccessor text, ChoiceIdentifierAccessor choice, TypeDesc arrayTypeDesc, string arrayName, string choiceName)
	{
		...
		if (num > 1)
		{
			WriteLocalDecl(text4, ReflectionAwareILGen.GetStringForArrayMember(arrayName, text3), arrayElementTypeDesc.Type);
			if (choice != null)
			{
				WriteLocalDecl(choiceName + "i", ReflectionAwareILGen.GetStringForArrayMember(choiceName, text3), choice.Mapping.TypeDesc.Type);
			}
		...
		{
			WriteElements(new SourceInfo(ReflectionAwareILGen.GetStringForArrayMember(arrayName, text3), null, null, arrayElementTypeDesc.Type, ilg), null, elements, text, choice, arrayName3, writeAccessors: true, arrayElementTypeDesc.IsNullable);
		}
		ilg.EndFor();
	}
	...
	private void WriteElements(SourceInfo source, string enumSource, ElementAccessor[] elements, TextAccessor text, ChoiceIdentifierAccessor choice, string arrayName, bool writeAccessors, bool isNullable)
	{
		...
		foreach (ElementAccessor elementAccessor2 in elements)
		{
			...
			else if (choice != null)
			{
				...
				ilg.AndIf();
				WriteChoiceTypeCheck(source, choice, enumName, elementAccessor2.Mapping.TypeDesc);
		...
		if (num > 0)
		{
			...
			foreach (ElementAccessor item in list)
			{
				...
				ilg.Ldloc(localBuilder);
				ilg.Call(method);
				ilg.Ldstr(XmlSerializationILGen.GetCSharpString(item.Name));
				...
				ilg.Ldloc(localBuilder);
				ilg.Call(method2);
				ilg.Ldstr(XmlSerializationILGen.GetCSharpString(item.Namespace));
				ilg.Call(method3);
				ilg.Br(label4);
				ilg.MarkLabel(label3);
				ilg.Ldc(boolVar: false);
		...
		if (elements.Length != 0)
		{
			...
			MethodInfo method8 = typeof(XmlSerializationWriter).GetMethod("CreateUnknownTypeException", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, s_objectType);
			ilg.Ldarg(0);
			source.Load(typeof(object));
			ilg.Call(method8);
			ilg.Throw();
	...
	private void WriteElement(SourceInfo source, ElementAccessor element, string arrayName, bool writeAccessor)
	{
		...
		if (element.Mapping is PrimitiveMapping primitiveMapping)
		{
		...
		if (element.Mapping is StructMapping structMapping)
		{
			...
			List<Type> list = new List<Type>();
			ilg.Ldarg(0);
			ilg.Ldstr(XmlSerializationILGen.GetCSharpString(text));
			list.Add(typeof(string));
			ilg.Ldstr(XmlSerializationILGen.GetCSharpString(text2));
			list.Add(typeof(string));
		...
		if (element.Mapping is SpecialMapping)
		{
			...
			ilg.Else();
			MethodInfo method2 = typeof(XmlSerializationWriter).GetMethod("CreateInvalidAnyTypeException", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, s_objectType);
			ilg.Ldarg(0);
			source.Load(null);
			ilg.Call(method2);
			ilg.Throw();
	...
	private void WriteCheckDefault(SourceInfo source, object value, bool isNullable)
	{
		...
		else
		{
			...
			source.Load(type);
			ilg.Ldc((value is string) ? XmlSerializationILGen.GetCSharpString((string)value) : value);
	...
	private void WriteChoiceTypeCheck(SourceInfo source, ChoiceIdentifierAccessor choice, string enumName, TypeDesc typeDesc)
	{
		...
		ilg.Ldarg(0);
		ilg.Ldstr(XmlSerializationILGen.GetCSharpString(typeDesc.FullName));
		ilg.Ldstr(XmlSerializationILGen.GetCSharpString(choice.MemberName));
		ilg.Ldstr(XmlSerializationILGen.GetCSharpString(enumName));
		ilg.Call(method);
		ilg.Throw();
		ilg.EndIf();
	}
