		...
		if (obj == null)
		{
			lock (orCreateValue)
			{
				if ((obj = orCreateValue[memberName]) == null)
				{
					...
					MethodInfo methodInfo = method.MakeGenericMethod(o.GetType(), type2);
					...
					obj = (orCreateValue[memberName] = func(effectiveSetInfo));
				}
			}
		}
	...
	private object WriteLiteralStructMethod(StructMapping structMapping, bool isNullable, bool checkType, string defaultNamespace)
	{
		...
		if (checkType)
		{
			...
			if (!(xmlQualifiedName == null) && (structMapping.TypeDesc.IsRoot || !QNameEqual(xmlQualifiedName, structMapping.TypeName, defaultNamespace)))
			{
				...
				if (structMapping.TypeDesc.IsRoot && WriteEnumAndArrayTypes(out o2, xmlQualifiedName, defaultNamespace))
				{
					return o2;
				}
				if (structMapping.TypeDesc.IsRoot)
		...
		foreach (MemberMapping memberMapping3 in settableMembers)
		{
			...
			if (!flag2)
			{
				...
				{
					if (memberMapping3.Elements[j].Any && string.IsNullOrEmpty(memberMapping3.Elements[j].Name))
					{
			...
			if (memberMapping3.TypeDesc.IsArrayLike && member.Source == null && memberMapping3.TypeDesc.IsArrayLike && (memberMapping3.Elements.Length != 1 || !(memberMapping3.Elements[0].Mapping is ArrayMapping)))
			{
			...
			if (member.Source == null)
			{
				...
				if (pi != null && typeof(IList).IsAssignableFrom(pi.PropertyType) && (pi.SetMethod == null || !pi.SetMethod.IsPublic))
				{
					member.Source = delegate(object value)
					{
						IList list2 = (IList)pi.GetValue(o);
						if (value is IList list3)
						{
							{
			...
			void Wrapper(object _)
			{
				...
				o.GetType().GetMethod("set_" + text3)?.Invoke(o, new object[1] { true });
			}
			[RequiresUnreferencedCode("Calls SetOrAddValueToMember")]
