...
internal sealed class QilGenerator : IErrorHelper, IXPathEnvironment, IFocus
{
	...
	private void CompileDataTypeAttribute(string attValue, bool fwdCompat, ref QilNode select, out QilNode select2)
	{
		...
		if (qilNode != null)
		{
			...
			{
				QilIterator qilIterator;
				qilNode = _f.Loop(qilIterator = _f.Let(qilNode), _f.Conditional(_f.Eq(qilIterator, _f.String("number")), _f.False(), _f.Conditional(_f.Eq(qilIterator, _f.String("text")), _f.True(), fwdCompat ? _f.True() : _f.Loop(_f.Let(ResolveQNameDynamic(ignoreDefaultNs: true, qilIterator)), _f.Error(_lastScope.SourceLine, System.SR.Xslt_BistateAttribute, "data-type", "text", "number")))));
				QilIterator qilIterator2 = _f.Let(qilNode);
				_varHelper.AddVariable(qilIterator2);
				select2 = select.DeepClone(_f.BaseFactory);
				select = _f.Conditional(qilIterator2, _f.ConvertToString(select), _f.String(string.Empty));
				select2 = _f.Conditional(qilIterator2, _f.Double(0.0), _f.ConvertToNumber(select2));
				return;
			}
			...
			{
				if (_compiler.ParseQName(text, out var prefix, out var _, this))
				{
	...
	private QilNode CompileSystemProperty(QilNode name)
	{
		if (name.NodeType == QilNodeType.LiteralString)
		{
			...
			if (EvaluateFuncCalls)
			{
				...
				if (xPathItem.ValueType == typeof(string))
				{
