...
internal sealed class XslAstAnalyzer : XslVisitor<XslFlags>
{
	internal sealed class Graph<V> : Dictionary<V, List<V>> where V : XslNode
	{
		...
		public void AddEdge(V v1, V v2)
		{
			if (v1 != v2)
			{
				...
				{
					value2 = (base[v1] = new List<V>());
					value = value2;
				}
				value.Add(v2);
				if (!TryGetValue(v2, out value2))
				{
					base[v2] = null;
				}
			}
	...
	internal sealed class XPathAnalyzer : IXPathBuilder<XslFlags>
	{
		...
		[UnconditionalSuppressMessage("ReflectionAnalysis", "IL2026:RequiresUnreferencedCode", Justification = "Suppressing warning about not having the RequiresUnreferencedCode attribute since xsl Scripts are not supported in .NET Core")]
