...
internal sealed class XsltLoader : IErrorHelper
{
	...
	public void Load(Compiler compiler, object stylesheet, XmlResolver xmlResolver, XmlResolver origResolver)
	{
		...
		{
			if (origResolver == null)
			{
				origResolver = XmlReaderSettings.GetDefaultPermissiveResolver();
			}
			Uri uri = origResolver.ResolveUri(null, text);
			if (uri == null)
			{
			...
			XmlReader reader2;
			using (reader2 = CreateReader(uri, origResolver))
			{
				Load(reader2);
			}
		}
		...
		do
		{
			...
			case XmlNodeType.Element:
				if (_input.IsXsltNamespace())
				{
					...
					{
						LoadGlobalVariableOrParameter(attributes.nsList);
					}
	...
	private void LoadDecimalFormat(NsDecl stylesheetNsList)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			span[i] = ParseCharAttribute(3 + i, _decimalFormatAttributes[3 + i].name, characters[i]);
		}
		for (int j = 0; j < 7; j++)
		{
			for (int k = j + 1; k < 7; k++)
		...
		if (_compiler.DecimalFormats.Contains(xmlQualifiedName))
		{
			...
			for (int l = 0; l < 8; l++)
			{
				...
				CheckError(span[l] != decimalFormatDecl.Characters[l], System.SR.Xslt_DecimalFormatRedefined, _decimalFormatAttributes[3 + l].name, char.ToString(span[l]));
			}
		}
		else
		{
			DecimalFormatDecl item = new DecimalFormatDecl(xmlQualifiedName, text, text2, new string(span));
	...
	{
		if (++_loadInstructionsDepth > 1024 && LocalAppContextSwitches.LimitXPathComplexity)
		{
	...
	private XslNode XslApplyTemplates()
	{
		...
		attributes.SaveExtendedLineInfo(_input);
		return SetInfo(AstFactory.ApplyTemplates(mode, select, attributes, _input.XslVersion), content, attributes);
	}
	...
	private void ParseWhitespaceRules(string elements, bool preserveSpace)
	{
		if (!string.IsNullOrEmpty(elements))
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				string prefix;
				string localName;
				string text = (_compiler.ParseNameTest(array[i], out prefix, out localName, this) ? ((!string.IsNullOrEmpty(prefix)) ? (_input.LookupXmlNamespace(prefix) ?? _compiler.CreatePhantomNamespace()) : prefix) : _compiler.CreatePhantomNamespace());
