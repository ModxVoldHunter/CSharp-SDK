...
internal sealed class XPathParser<Node>
{
	...
	{
		if (++_parseRelativePath > 1024 && LocalAppContextSwitches.LimitXPathComplexity)
		{
	...
	{
		if (++_parseSubExprDepth > 1024 && LocalAppContextSwitches.LimitXPathComplexity)
		{
			throw _scanner.CreateException(System.SR.Xslt_InputTooComplex);
		}
		...
		if (_scanner.Kind == LexKind.Minus)
		{
			...
			byte callerPrec2 = readOnlySpan[(int)xPathOperator];
			_scanner.NextLex();
		...
		while (true)
		{
			...
			byte b = readOnlySpan[(int)xPathOperator];
			if (b <= callerPrec)
			{
				break;
			}
			_scanner.NextLex();
			val = _builder.Operator(xPathOperator, val, ParseSubExpr(b));
		}
		_parseSubExprDepth--;
		return val;
	}
