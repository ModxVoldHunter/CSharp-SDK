...
internal static class XmlQueryTypeFactory
{
	...
	private sealed class ChoiceType : XmlQueryType
	{
		...
		{
			uint num = (uint)nodeKinds;
			if (BitOperations.IsPow2(num))
			{
				return ItemType.Create(s_nodeKindToTypeCode[BitOperations.TrailingZeroCount(num) + 1], isStrict: false);
			}
			List<XmlQueryType> list = new List<XmlQueryType>();
			while (num != 0)
			{
				list.Add(ItemType.Create(s_nodeKindToTypeCode[BitOperations.TrailingZeroCount(num) + 1], isStrict: false));
				num &= num - 1;
			}
			return Create(list);
		}
