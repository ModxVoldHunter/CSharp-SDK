...
public sealed class XmlQueryContext
{
	...
	public XmlNameTable? DefaultNameTable => _defaultDataSource?.NameTable;
	...
	internal XmlQueryContext(XmlQueryRuntime runtime, object defaultDataSource, XmlResolver dataSources, XsltArgumentList argList, WhitespaceRuleLookup wsRules)
	{
		...
		if (defaultDataSource is string text)
		{
			_defaultDataSource = GetDataSource(text, null);
			if (_defaultDataSource == null)
			{
				throw new XslTransformException(System.SR.XmlIl_UnknownDocument, text);
			}
		}
	...
	private XPathNavigator ConstructDocument(object dataSource, string uriRelative, Uri uriResolved)
	{
		...
		{
			XmlReader xmlReader = _readerSettings.CreateReader(stream, uriResolved?.ToString());
			try
			{
		...
		if (dataSource is XmlReader baseReader)
		{
			return new XPathDocument(WhitespaceRuleReader.CreateReader(baseReader, _wsRules), XmlSpace.Preserve).CreateNavigator();
		}
		if (dataSource is IXPathNavigable iXPathNavigable)
		{
			if (_wsRules != null)
			{
	...
	{
		object obj = _argList?.GetExtensionObject(namespaceUri);
		if (obj == null)
		{
		...
		for (int i = 0; i < args.Length; i++)
		{
			...
			{
				array[i] = XmlQueryRuntime.ChangeTypeXsltArgument(xmlArgumentType, array[i], clrArgumentType);
			}
		}
		object obj2 = xmlExtensionFunction.Invoke(obj, array);
		if (obj2 == null && xmlExtensionFunction.ClrReturnType == typeof(void))
		{
