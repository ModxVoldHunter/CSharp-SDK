...
public sealed class XmlQueryRuntime
{
	...
	public XsltLibrary XsltFunctions => _xsltLib ?? (_xsltLib = new XsltLibrary(this));
	...
	internal object ChangeTypeXsltResult(XmlQueryType xmlType, object value)
	{
		...
		case XmlTypeCode.Node:
			...
			if (value is XPathArrayIterator { AsList: XmlQueryNodeSequence asList })
			{
				value = asList;
			}
			else
		...
		case XmlTypeCode.Item:
		{
			...
			case XmlTypeCode.Double:
				...
				break;
			case XmlTypeCode.String:
				value = ((!(type == typeof(DateTime))) ? new XmlQueryItemSequence(new XmlAtomicValue(XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.String), value)) : new XmlQueryItemSequence(new XmlAtomicValue(XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.String), XsltConvert.ToString((DateTime)value))));
				break;
			case XmlTypeCode.Node:
			...
			case XmlTypeCode.Item:
				...
				value = ((!(value is XPathNavigator navigator)) ? new XmlQueryNodeSequence(iXPathNavigable.CreateNavigator()) : new XmlQueryNodeSequence(navigator));
				break;
			}
			break;
		}
	...
	public bool MatchesXmlType(IList<XPathItem> seq, int indexType)
	{
		...
		{
			if (!CreateXmlType(seq[i]).IsSubtypeOf(xmlType))
			{
				return false;
			}
		}
