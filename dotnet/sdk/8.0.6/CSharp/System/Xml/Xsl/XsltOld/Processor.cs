...
internal sealed class Processor : IXsltProcessor
{
	...
	internal ArrayList NumberList => _numberList ?? (_numberList = new ArrayList());
	...
	internal XPathNavigator GetNavigator(Uri ruri)
	{
		...
		{
			if (_documentCache[ruri] is XPathNavigator xPathNavigator)
			{
		...
		if (entity is Stream)
		{
			...
			xPathNavigator2 = ((IXPathNavigable)Compiler.LoadDocument(xmlTextReaderImpl)).CreateNavigator();
		}
		else
		{
			...
			xPathNavigator2 = (XPathNavigator)entity;
		}
		_documentCache[ruri] = xPathNavigator2.Clone();
	...
	{
		return _valueOfContext ?? (_valueOfContext = new XsltCompileContext());
	}

	private XsltCompileContext GetMatchesContext()
	{
		return _matchesContext ?? (_matchesContext = new XsltCompileContext());
	}
