...
internal static class XPathConvert
{
	...
	private sealed class FloatingDecimal
	{
		...
		public int MantissaSize => _mantissaSize;
	...
	public static ReadOnlySpan<double> C10toN => RuntimeHelpers.CreateSpan<double>((RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);
	...
	{
		if (double.TryParse(s, NumberStyles.Float, CultureInfo.InvariantCulture, out var result) && double.IsFinite(result))
		{
			return result;
		}
		return double.NaN;
