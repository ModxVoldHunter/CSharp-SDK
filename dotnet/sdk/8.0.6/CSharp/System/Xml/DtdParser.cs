...
internal sealed class DtdParser : IDtdParser
{
	...
	{
		Initialize(adapter);
		ArgumentException.ThrowIfNullOrEmpty(docTypeName, "docTypeName");
	...
	private void ParseElementDecl()
	{
		...
		{
			XmlQualifiedName nameQualified = GetNameQualified(canHavePrefix: true);
			if (_schemaInfo.ElementDecls.TryGetValue(nameQualified, out var value))
			{
				if (_validate)
				{
	...
	private void ParseEntityDecl()
	{
		...
		if (flag)
		{
			_schemaInfo.ParameterEntities.TryAdd(nameQualified, schemaEntity);
		}
		else
		{
			_schemaInfo.GeneralEntities.TryAdd(nameQualified, schemaEntity);
		}
	...
	private void Throw(int curPos, string res, string arg)
	{
		_curPos = curPos;
		Uri baseUri = _readerAdapter.BaseUri;
		_readerAdapter.Throw(new XmlException(res, arg, LineNo, LinePos, baseUri?.ToString()));
	}
	...
	private void Throw(int curPos, string res, string[] args)
	{
		_curPos = curPos;
		Uri baseUri = _readerAdapter.BaseUri;
		_readerAdapter.Throw(new XmlException(res, args, LineNo, LinePos, baseUri?.ToString()));
	}
	...
	private async Task ParseElementDeclAsync()
	{
		...
		{
			XmlQualifiedName nameQualified = GetNameQualified(canHavePrefix: true);
			if (_schemaInfo.ElementDecls.TryGetValue(nameQualified, out var elementDecl))
			{
				if (_validate)
				{
					SendValidationEvent(_curPos - nameQualified.Name.Length, XmlSeverityType.Error, System.SR.Sch_DupElementDecl, GetNameString());
	...
	private async Task ParseEntityDeclAsync()
	{
		...
		if (isParamEntity)
		{
			_schemaInfo.ParameterEntities.TryAdd(nameQualified, entity);
		}
		else
		{
			_schemaInfo.GeneralEntities.TryAdd(nameQualified, entity);
		}
