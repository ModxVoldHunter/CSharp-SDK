...
public class XmlEntityReference : XmlLinkedNode
{
	...
	protected internal XmlEntityReference(string name, XmlDocument doc)
		: base(doc)
	{
		if (!doc.IsLoading && name.StartsWith('#'))
		{
	...
	private static string ConstructBaseURI(string baseURI, string systemId)
	{
		...
		return string.Create(text.Length + systemId.Length, (text, systemId), delegate(Span<char> dest, (string buf, string systemId) state)
		{
