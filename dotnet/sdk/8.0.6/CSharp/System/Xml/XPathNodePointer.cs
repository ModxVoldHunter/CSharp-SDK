...
internal sealed class XPathNodePointer : IXmlDataVirtualNode
{
	...
	internal static ReadOnlySpan<int> XmlNodeTypeToXpathNodeTypeMap => RuntimeHelpers.CreateSpan<int>((RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);
	...
	internal string XmlLang
	{
		...
		{
			RealFoliate();
			for (XmlNode xmlNode = _node; xmlNode != null; xmlNode = ((xmlNode.NodeType != XmlNodeType.Attribute) ? xmlNode.ParentNode : ((XmlAttribute)xmlNode).OwnerElement))
			{
				if (xmlNode is XmlBoundElement xmlBoundElement)
				{
					...
					else if (xmlBoundElement.HasAttribute("xml:lang"))
					{
	...
	private XPathNodeType ConvertNodeType(XmlNode node)
	{
		...
		int num = XmlNodeTypeToXpathNodeTypeMap[(int)node.NodeType];
		if (num == 2)
		{
	...
	private XmlBoundElement GetRowElement()
	{
		...
		DataSetMapper.GetRegion(_node, out var rowElem);
		return rowElem;
	}
	...
	{
		int num = XmlNodeTypeToXpathNodeTypeMap[(int)child.NodeType];
		if (num == -1)
		{
			return false;
		}
		return XmlNodeTypeToXpathNodeTypeMap[(int)parent.NodeType] switch
		{
			0 => num == 1 || num == 8 || num == 7, 
	...
	internal bool MoveToParent()
	{
		...
		if (_node != null)
		{
			...
			XmlNode xmlNode = ((_node.NodeType != XmlNodeType.Attribute) ? _node.ParentNode : ((XmlAttribute)_node).OwnerElement);
			if (xmlNode != null)
			{
				MoveTo(xmlNode);
				return true;
	...
	internal string GetNamespace(string name)
	{
		...
		while (xmlNode != null)
		{
			...
			if (xmlNode != null)
			{
				string @namespace = GetNamespace((XmlBoundElement)xmlNode, name);
				if (@namespace != null)
				{
					return @namespace;
				}
