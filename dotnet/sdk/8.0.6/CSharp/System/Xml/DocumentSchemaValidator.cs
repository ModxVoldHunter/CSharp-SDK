...
internal sealed class DocumentSchemaValidator : IXmlNamespaceResolver
{
	...
	{
		return _nsManager.LookupNamespace(prefix) ?? _startNode.GetNamespaceOfPrefixStrict(prefix);
	}

	public string LookupPrefix(string namespaceName)
	{
		return _nsManager.LookupPrefix(namespaceName) ?? _startNode.GetPrefixOfNamespaceStrict(namespaceName);
	}
	...
	private void ValidateAttributes(XmlElement elementNode)
	{
		...
		for (int i = 0; i < attributes.Count; i++)
		{
			XmlAttribute xmlAttribute = (XmlAttribute)(_currentNode = attributes[i]);
		...
		{
			XmlSchemaAttribute xmlSchemaAttribute = (XmlSchemaAttribute)_defaultAttributes[j];
			XmlQualifiedName qualifiedName = xmlSchemaAttribute.QualifiedName;
			XmlAttribute xmlAttribute = _document.CreateDefaultAttribute(GetDefaultPrefix(qualifiedName.Namespace), qualifiedName.Name, qualifiedName.Namespace);
	...
	private XmlSchemaObject FindSchemaInfo(XmlElement elementToValidate)
	{
		...
		CheckNodeSequenceCapacity(num);
		_nodeSequenceToValidate[num++] = parentNode;
		XmlSchemaObject xmlSchemaObject = schemaInfo.SchemaElement;
		if (xmlSchemaObject == null)
		{
			xmlSchemaObject = schemaInfo.SchemaType;
		}
	...
	private XmlSchemaObject GetTypeFromAncestors(XmlElement elementToValidate, XmlSchemaObject ancestorType, int ancestorsCount)
	{
		...
		XmlSchemaObject xmlSchemaObject = ((_schemaInfo.SchemaElement == null) ? ((XmlSchemaAnnotated)_schemaInfo.SchemaType) : ((XmlSchemaAnnotated)_schemaInfo.SchemaElement));
		if (xmlSchemaObject == null)
		{
	...
	private static XmlSchemaComplexType GetComplexType(XmlSchemaObject schemaObject)
	{
		...
		if (schemaObject is XmlSchemaElement xmlSchemaElement)
		{
