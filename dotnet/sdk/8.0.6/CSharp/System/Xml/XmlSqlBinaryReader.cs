...
internal sealed class XmlSqlBinaryReader : XmlReader, IXmlNamespaceResolver
{
	...
	internal struct QName : IEquatable<QName>
	{
		...
		{
			if (prefix == other.prefix && localname == other.localname)
			{
			...
			if (NodeType != XmlNodeType.Element && NodeType != XmlNodeType.EndElement)
			{
				while (Read())
				{
					...
					if ((uint)(nodeType - 7) > 1u)
					{
						...
						throw CreateNotSupportedException(System.SR.XmlBinary_ListsOfValuesNotSupported);
					}
				}
			}
			return _pos;
	...
	private void Fill_(int require)
	{
		...
		{
			throw CreateXmlException(System.SR.Xml_UnexpectedEOF1);
		}
	}
		...
		if (b > 127)
		{
			...
			if (b > 127)
			{
				...
				if (b > 127)
				{
					...
					if (b > 7)
					{
						throw CreateXmlException(System.SR.XmlBinary_ValueTooBig);
					}
					num += num2 << 28;
				}
			}
	...
	private bool ReadInit(bool skipXmlDecl)
	{
		...
		if (!_sniffed)
		{
			...
			if (num != 65503)
			{
				res = System.SR.XmlBinary_InvalidSignature;
				goto IL_01e4;
			}
		}
		_version = ReadByte();
		...
		{
			res = System.SR.XmlBinary_InvalidProtocolVersion;
		}
		else
		{
			if (1200 == ReadUShort())
			{
				...
				if (BinXmlToken.XmlDecl == PeekToken())
				{
					...
					if (b != 0)
					{
						if ((uint)(b - 1) > 1u)
						{
							res = System.SR.XmlBinary_InvalidStandalone;
							goto IL_01e4;
						}
			...
			res = System.SR.XmlBinary_UnsupportedCodePage;
		}
		goto IL_01e4;
		IL_01e4:
		_state = ScanState.Error;
		throw new XmlException(res, (string[])null);
	}
	...
	private void ScanAttributes()
	{
		...
		while (BinXmlToken.EndAttrs != (binXmlToken = NextToken()))
		{
			if (BinXmlToken.Attr == binXmlToken)
			{
				...
				{
					throw CreateXmlException(System.SR.XmlBinary_AttrWithNsNoPrefix, n.localname, n.namespaceUri);
				}
				_attrCount++;
				flag = false;
			}
	...
	private string CDATAValue()
	{
		...
		while (PeekToken() == BinXmlToken.CData)
		{
			...
			{
				stringBuilder = new StringBuilder(text.Length + text.Length / 2).Append(text);
			}
	...
	private void ImplReadEndElement()
	{
		if (_elemDepth == 0)
		{
			throw CreateXmlException(System.SR.Xml_UnexpectedEndTag);
		}
		int elemDepth = _elemDepth;
	...
	private void ImplReadDoctype()
	{
		...
		{
			throw CreateXmlException(System.SR.Xml_DtdIsProhibited);
		}
		switch (_docState)
		{
		case 9:
			throw CreateXmlException(System.SR.Xml_DtdNotAllowedInFragment);
		default:
			throw CreateXmlException(System.SR.Xml_BadDTDLocation);
		case 0:
		case 1:
			_docState = 2;
	...
	private void CheckAllowContent()
	{
		...
		case 0:
			...
			break;
		default:
			throw CreateXmlException(System.SR.Xml_InvalidRootData);
		case 3:
		case 9:
			break;
		}
	...
	private static Type[] GenerateTokenTypeMap()
	{
		...
		array[9] = (array[24] = (array[17] = (array[14] = (array[22] = (array[16] = (array[13] = typeof(string)))))));
		return array;
	}
	...
	{
		ReadOnlySpan<byte> source = _data.AsSpan(_tokDataPos, _pos - _tokDataPos);
		if (!attr)
		{
			while (true)
			{
		...
		while (true)
		{
			...
			if (!XmlCharType.IsCharData(c))
			{
				...
				{
					throw CreateXmlException(System.SR.Xml_InvalidSurrogateMissingLowChar);
				}
	...
	{
		return CreateXmlException(System.SR.XmlBinary_UnexpectedToken);
	}
