...
internal sealed class ManifestBasedResourceGroveler : IResourceGroveler
{
		...
		if (store.CanSeek && store.Length > 4)
		{
			...
			if (num == ResourceManager.MagicNumber)
			{
				...
				if (num2 == ResourceManager.HeaderVersionNumber)
				{
					...
					readerTypeName = binaryReader.ReadString();
					resSetTypeName = binaryReader.ReadString();
				}
				else
				{
					if (num2 <= ResourceManager.HeaderVersionNumber)
	...
	{
		return satellite.GetManifestResourceStream(fileName) ?? CaseInsensitiveManifestResourceStreamLookup(satellite, fileName);
	}
	...
	private static Stream CaseInsensitiveManifestResourceStreamLookup(Assembly satellite, string name)
	{
		...
		{
			if (string.Equals(text2, name, StringComparison.InvariantCultureIgnoreCase))
			{
				if (text != null)
				{
		...
		return satellite.GetManifestResourceStream(text);
	}
	...
	private void HandleResourceStreamMissing(string fileName)
	{
		...
		throw new MissingManifestResourceException(SR.Format(SR.MissingManifestResource_NoNeutralAsm, fileName, _mediator.MainAssembly.GetName().Name, GetManifestResourceNamesList(_mediator.MainAssembly)));
	}
}
